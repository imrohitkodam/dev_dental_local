...
class modLatestBloggerHelper extends EasyBlog
{
	public $lib = null;
	...
	public function getBloggers($sort = 'latest', $limit = 5, $filter='showallblogger', $featuredOnly = '')
	{
		$db = EB::db();
		...
		$results = $this->formatBlogger($results);

		return $results;
	}

	public function formatBlogger($bloggers)
	{
		if (!empty($bloggers)) {
			foreach ($bloggers as $row) {
				...
				$bioLength = (int) $this->params->get('bio_length', 50);
