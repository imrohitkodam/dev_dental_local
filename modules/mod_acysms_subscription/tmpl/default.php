						...
						if($params->get('dropdown', 0)){
							...
							<select name="subscription[1]">
								<?php foreach($visibleGroupsArray as $myGroupId){ ?>
									<option value="<?php echo $myGroupId ?>"><?php echo $allGroups[$myGroupId]->group_name; ?></option>
								<?php } ?>
							</select>
						<?php }else{
							?>
							<table class="acysms_groups">
								<?php foreach($visibleGroupsArray as $myGroupId){
						...
						<tr>
							...
							foreach($fieldsToDisplay as $oneField){
								if(!empty($extraFields[$oneField])){
									...
									if(empty($extraFields[$oneField]->fields_options['size'])) $extraFields[$oneField]->fields_options['size'] = '150';
									?>
							...
							<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
								<?php if($params->get('showsubscribe', true)){ ?>
									...
									if(empty($identifiedUser->user_id) OR empty($subtext)){
								...
								<?php }
								if($params->get('showunsubscribe', false) AND (!$params->get('showsubscribe', true) OR empty($identifiedUser->user_id) OR !empty($countUnsub))){ ?>
