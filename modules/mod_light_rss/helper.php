...
class ModLightRSSHelper
{
    ...
    public static function getFeed(&$params)
    {
        ...
        $xtpSimplePie = new XTP_SimplePie();

        $use_sp_cache = EParameter::getComponentParam(CAUTOTWEETNG, 'use_sp_cache', true);

        if (($use_sp_cache) && (is_writable(JPATH_CACHE))) {
            $xtpSimplePie->set_cache_location(JPATH_CACHE);
            $xtpSimplePie->enable_cache(true);

            $cache_time = ((int) $rsscache);
            $xtpSimplePie->set_cache_duration($cache_time);
        } else {
            $xtpSimplePie->enable_cache(false);
        }

        $xtpSimplePie->set_feed_url($rssurl);

        // Process the loaded feed
        $xtpSimplePie->init();
        $xtpSimplePie->handle_content_type();

        // Store any error message
        if (isset($xtpSimplePie->error)) {
            $light_rss['error'][] = $xtpSimplePie->error;
        }

        // Start building the feed meta-info (title, desc and image)
        // Feed title
        if ($xtpSimplePie->get_title() && $rsstitle) {
            $light_rss['title']['link'] = $xtpSimplePie->get_link();
            $light_rss['title']['title'] = $xtpSimplePie->get_title();
        }

        // Feed description
        if ($rssdesc) {
            $light_rss['description'] = $xtpSimplePie->get_description();
        }

        // Feed image
        if ($rssimage && $xtpSimplePie->get_image_url()) {
            $light_rss['image']['url'] = $xtpSimplePie->get_image_url();
            $light_rss['image']['title'] = $xtpSimplePie->get_image_title();
        }
        ...
        if ($xtpSimplePie->get_item_quantity()) {
            // Start looping through the feed items
            $light_rss_item = 0;

            // Item counter for array indexing in the loop
            foreach ($xtpSimplePie->get_items(0, $rssitems) as $currItem) {
                // Item title
                ...
                if ('yes' === $enable_tooltip) {
                    ...
                    $tooltip_title = htmlspecialchars(html_entity_decode($tooltip_title), \ENT_QUOTES | \ENT_SUBSTITUTE);
                    ...
                    // Format text for tooltip
                    $text = htmlspecialchars(html_entity_decode($text), \ENT_QUOTES | \ENT_SUBSTITUTE);

                    // Tooltip Body
                    $light_rss['items'][$light_rss_item]['tooltip']['description'] = $text;
                } else {
                    // Blank
                    $light_rss['items'][$light_rss_item]['tooltip'] = [];
                }
