...
class FsssModelFaq extends JModelLegacy
{
	...
	function store($data)
	{
		...
		foreach ($lines as $tag)
		{
			...
			$query = "REPLACE INTO #__fss_faq_tags (faq_id, tag, language) VALUES (" . FSSJ3Helper::getEscaped($db, $row->id) . ",'" . FSSJ3Helper::getEscaped($db, $tag) . "','" . FSSJ3Helper::getEscaped($db, $row->language) . "')";
			$db->setQuery($query);$db->Query();					
		}
