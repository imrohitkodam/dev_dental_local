...
class FsssModelFusers extends JModelLegacy
{
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT u.*, CONCAT(m.username," (",m.name,")") as name FROM #__fss_users as u LEFT JOIN #__users as m ON u.user_id = m.id';

		$where = array();

		if ($this->lists['search']) {
			$where[] = '(LOWER( name ) LIKE '.$db->Quote( '%'.FSSJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}
