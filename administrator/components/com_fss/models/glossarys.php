...
class FsssModelGlossarys extends JModelLegacy
{
     var $_data;
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT  id, word, description, published, access, language FROM #__fss_glossary ';

		$where = array();

        if ($this->lists['search']) {
			$where[] = '(LOWER( word ) LIKE '.$db->Quote( '%'.FSSJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}
