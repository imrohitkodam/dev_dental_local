...
class FSSUpdater
{
	...
	function ConvertComments()
	{
		...
		if (array_key_exists('#__fss_kb_comment',$existingtables))
		{
			...
			$count = substr("af85ecd757b858421985045128310c5f", 0, 8);
	...
	function ForceMyISAM($path = "")
	{
		...
		if ($path)
		{
			$updatefile = $path . DS . 'admin' . DS . 'database_fss.json';
		} else {
			$updatefile = JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_fss'.DS.'database_fss.json';
		}
