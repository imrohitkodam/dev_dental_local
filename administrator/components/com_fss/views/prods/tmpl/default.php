...
?>
<form action="<?php echo FSSRoute::_( 'index.php?option=com_fss&view=prods' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
        ...
        $link = FSSRoute::_( 'index.php?option=com_fss&controller=prod&task=edit&cid[]='. $row->id );
...
jQuery(document).ready(function () {
	jQuery('.toggle_me').click(function (ev) {
		...
		var url = '<?php echo FSSRoute::_('index.php?option=com_fss&view=prods&what=togglefield',false); ?>&id=' + rowid + '&val=' + val + '&field=' + field;
		jQuery(t).html('--');
		jQuery.ajax({
			url: url,
