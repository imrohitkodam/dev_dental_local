...
?>
<form action="<?php echo FSSRoute::_( 'index.php?option=com_fss&view=ticketpris' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    <thead>
        ...
        <tr>
            ...
            <th>
                <?php echo JHTML::_('grid.sort',   'Priority', 'title', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
            </th>
            <th width="8%">
				<?php echo JHTML::_('grid.sort',   'Color', 'color', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
			</th>
            <th width="8%">
				<?php echo JHTML::_('grid.sort',   'BACK_COLOR', 'backcolor', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
			</th>
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
        ...
        $link = FSSRoute::_( 'index.php?option=com_fss&controller=ticketpri&task=edit&cid[]='. $row->id );
        ...
        <tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php if ($row->color): ?>
					<span style='color: <?php echo $row->color; ?>'>
				<?php else: ?>
					<span>
				<?php endif; ?>
				<?php echo $row->color; ?>
			...
			<?php if ($row->backcolor): ?>
				<td style="background-color: <?php echo $row->backcolor; ?>">	
			<?php else: ?>
				<td>
			<?php endif; ?>
			    <?php echo $row->backcolor; ?>
			</td>
