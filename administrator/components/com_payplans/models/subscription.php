...
class PayplansModelSubscription extends PayPlansModel
{
	...
	public function getItems()
	{
		...
		if ($dateRange) {
			...
			$wheres[] = $db->qn('subscription_date') . '>' . $db->Quote($startDate);
		...
		if ($search) {
			...
			$searchQuery[] = 'LOWER(c.' . $db->qn('title') . ') LIKE ' . $db->Quote('%' . PPJString::strtolower($search) . '%');

			// Search by username, email or name
			$searchQuery[] = 'LOWER(b.' . $db->qn('name') . ') LIKE ' . $db->Quote('%' . PPJString::strtolower($search) . '%');
			$searchQuery[] = 'LOWER(b.' . $db->qn('username') . ') LIKE ' . $db->Quote('%' . PPJString::strtolower($search) . '%');
			$searchQuery[] = 'LOWER(b.' . $db->qn('email') . ') LIKE ' . $db->Quote('%' . PPJString::strtolower($search) . '%');

			// Search text could be subscription key
			$searchQuery[] = 'LOWER(a.' . $db->qn('subscription_id') . '=' . $db->Quote(PP::getIdFromKey($search, true)) . ')';
	...
	public function getItemsWithoutState($options = [])
	{
		...
		$excludeUserIds = PP::normalize($options, 'excludeUserIds', []);
		$limit = PP::normalize($options, 'limit', null);
		$duration = PP::normalize($options, 'duration', null);
		...
		if (!empty($excludeUserIds)) {
			$excludeUserIds = is_array($excludeUserIds) ? implode(',', $excludeUserIds) : $excludeUserIds;

			$wheres[] = 'a.' . $db->qn('user_id') . ' NOT IN (' . $excludeUserIds . ')';
		}
		...
		if ($duration) {
			...
			$wheres[] = 'a.`subscription_date` BETWEEN DATE_SUB(' . $db->Quote($now) . ', INTERVAL ' . $duration . ' HOUR) AND ' . $db->Quote($now);
		}
	...
	/**
	 * Get subscriptions where the trial is about to expire from the given pre expiry time
	 *
	 * @since	4.0.9
	 * @access	public
	 */
	public function getTrialPreExpirySubscriptions(Array $plans, $preExpiryTime, $onAllPlans = false)
	{
		$e1 = PP::date(PP::config()->get('cronAcessTime'));
		$e2	= PP::date('now');
		...
		if (!$onAllPlans) {
			...
			$query .= ' AND ' . $db->qn('params') . ' LIKE ' . $db->Quote('%"expirationtype":"recurring_trial_%');
		}
	...
	public function getPostActivationSubscriptions(Array $plans, $postActivationTime, $onAllPlans = false)
	{
		...
		$query = 'SELECT * FROM ' . $db->qn('#__payplans_subscription');
		$query .= ' WHERE ' . $db->qn('subscription_date') . ' < ' . $db->Quote($e1->toSql());
		$query .= ' AND ' . $db->qn('subscription_date') . ' < ' . $db->Quote($e2->toSql());
		$query .= ' AND ' . $db->qn('status') . ' = ' . $db->Quote(PP_SUBSCRIPTION_ACTIVE);
	 ...
	 * sales including fixed and forever type plans only
	 * Recurring calculated seperatly 
	*/
  	public function getSalesOfPlans(PPDate $firstDate, PPDate $endDate)
	{
		...
		$query .= ' WHERE subscription_date >= '."'".$firstDate->toMySQL()."'";
		$query .= ' AND subscription_date <= '."'".$endDate->toMySQL()."'";
		$query .= ' AND `params` NOT LIKE '."'".'%"expirationtype":"recurring%'."'";
		$query .= ' GROUP BY date(`subscription_date`), `plan_id`';
	...
	public function getUpgradesOfPlans(PPDate $firstDate, PPDate $endDate)
	{
		...
		$query->select('count(distinct orders.order_id) as upgCount, sub.plan_id as plan_id , date(sub.subscription_date) as sub_date, invoice.`total` AS amount')
		...
		if ($results) {
			foreach ($results as $key => $value) {
				...
				$sales[$value->plan_id]['amount'] = isset($sales[$value->plan_id]['amount']) ? $sales[$value->plan_id]['amount'] + $value->amount : $value->amount;
			}
		}
	...
	public function getDataToExport($options = [])
	{
		...
		$query = [];
		$query[] = "SELECT " . $db->nameQuote('a.subscription_id') . ", ". $db->nameQuote('a.plan_id'). " AS plan, ". $db->nameQuote('b.username') . ", " . $db->nameQuote('a.user_id') . ", " . $db->nameQuote('a.status') . ", " . $db->nameQuote('a.total') . ", " . $db->nameQuote('a.subscription_date') . ", " . $db->nameQuote('a.expiration_date') . ", " . $db->nameQuote('a.params');
		$query[] = "from `#__payplans_subscription` as a";
		...
		// Get the subscription custom details
		$customDetailsmodel = PP::model('CustomDetails');
		$customDetails = $customDetailsmodel->getCustomDetailsFields('subscription');

		// Set subscription custom details on data
		if ($customDetails) {
	...
	public function getExpireSubscription(PPDate $firstDate, PPDate $lastDate)
	{
		...
		$query .= ' AND `status` = ' . $db->Quote(PP_SUBSCRIPTION_EXPIRED);
	 ...
	 * @access	public
	 */
	public function getUserSubscription($options = [])
	{
		$db = PP::db();

		$orderColumn = $this->normalize($options, 'ordering', 'subscription_id');
		$orderDirection = $this->normalize($options, 'orderingDirection', 'DESC');
