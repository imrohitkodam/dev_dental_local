...
class PayplansModelStatistics extends PayPlansModel
{
	...
	public function insertStatisticsData($value)
	{
		...
		$query .= ' (`statistics_type`, `purpose_id_1`, `purpose_id_2`, `count_1`, `count_2`, `count_3`, `count_4`, `count_5`, `count_6`, `details_1`, `details_2`, `message`, `statistics_date`, `modified_date`)';
		$query .= ' VALUES ' . $str;
	...
	public function updateStatiscticsData($values)
	{
		...
		$tmpDate = $statistics_date->toMySQL(false, '%Y-%m-%d');
		$startDate = $tmpDate . ' 00:00:00';
		$endDate = $tmpDate . ' 23:59:59';

		$query .= ' AND (`statistics_date` >= ' . $db->Quote($startDate);
		$query .= ' AND `statistics_date` <= ' . $db->Quote($endDate) . ')';
	 ...
	 * @access	public
	 */
	public function getSumOfRecords($options = [])
	{
		$db = $this->db;
	...
	public function getAllActiveExpiredSubscription($firstDate, $lastDate)
	{
		...
		$status = [PP_SUBSCRIPTION_EXPIRED, PP_SUBSCRIPTION_ACTIVE];
	...
	public function getPlanDataWithinDates($firstDate, $lastDate, $type = PP_STATISTICS_TYPE_ALL)
	{
		...
		// count_5 = total renewal revenue
		// count_6 = total upgrade revenue
		$columnType = [
			'count_1' => 'sales', 
			'count_2' => 'revenue', 
			'count_3' => 'renewals', 
			'count_4' => 'upgrades', 
			'count_5' => 'renewal_revenue', 
			'count_6' => 'upgrade_revenue'
		];
		...
		if ($type == 'all') {
			...
			foreach ($columnType as $column => $value) {
				if ($type == $value) {
					...
					if ($type == 'renewals') {
						$query .= ', sum(' . $db->nameQuote('count_5') .') as renewals_revenue';
					}

					if ($type == 'upgrades') {
						$query .= ', sum(' . $db->nameQuote('count_6') .') as upgrades_revenue';
					}
	...
	public function getTotalMembersWithinDates($firstDate, $lastDate)
	{
		$db = $this->db;

		$query = 'SELECT date(`statistics_date`) as statistics_date';

		// count_1 = total registration
		$columnType = [
			'count_1' => 'total_registration'
		];
		...
		$query .= ' WHERE `statistics_type` = ' . $db->Quote('member');
		$query .= ' AND date(statistics_date) >= ' . 'date(' . $db->Quote($firstDate) . ')';
		$query .= ' AND date(statistics_date) <= ' . 'date(' . $db->Quote($lastDate) . ')';
