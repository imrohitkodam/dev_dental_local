...
class PayplansModelTransaction extends PayPlansModel
{
	...
	public function getItems()
	{
		...
		if ($search) {
			...
			$searchQuery[] = 'LOWER(c.' . $db->qn('invoice_id') . '=' . $db->Quote(PP::getIdFromKey($search, true)) . ')';
	...
	public function getItemsWithoutState($options = array())
	{
		...
		$paymentId = PP::normalize($options, 'payment_id', '');
		...
		if ($paymentId) {
			$wheres[] = $db->nameQuote('a.payment_id') . ' = ' . $db->Quote($paymentId);
		}
	 ...
	 * Fixed and forver plans revenue only
	*/
	public function getRevenuesOfPlans(PPDate $firstDate, PPDate $lastDate)
	{
		$q = [];
		$q[] = 'select subscription.`plan_id` as plan_id, sum(transaction.`amount`) as amount, transaction.`created_date` as sub_date';
		$q[] = 'from (';
		$q[] = '	select distinct a.`invoice_id`, a.`amount`, date(a.`created_date`) as `created_date` FROM `#__payplans_transaction` as a';
		$q[] = '	where a.`created_date` >= ' . $this->db->Quote($firstDate->toMySQL());
		$q[] = '	and a.`created_date` <= ' . $this->db->Quote($lastDate->toMySQL());
		$q[] = ') as transaction';
		$q[] = 'INNER JOIN `#__payplans_invoice` as invoice on transaction.`invoice_id` = invoice.`invoice_id`';
		$q[] = 'LEFT JOIN `#__payplans_subscription` as subscription ON invoice.`object_id` = subscription.`order_id`';
		$q[] = 'WHERE invoice.`params` NOT LIKE '."'".'%"expirationtype":"recurring%'."'";
		$q[] = 'GROUP BY subscription.`plan_id`, transaction.`created_date`';
...
class PayplansModelformTransaction extends PayPlansModelform {}
