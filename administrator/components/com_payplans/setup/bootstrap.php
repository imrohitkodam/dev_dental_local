...
define('PP_KEY', 'db6281be0ebde305533a147335688aad');
define('PP_INSTALLER', 'full');
define('PP_PACKAGE', 'com_payplans_5.0.2_component_pro.zip');
...
$contents = file_get_contents(JPATH_ROOT. '/administrator/components/com_payplans/payplans.xml');
...
//Initialize steps
$contents = file_get_contents(PP_CONFIG . '/installation.json');
	...
	if ($stepIndex == 0) {
		...
		foreach ($files as $file) {
			...
			if ($posixExists) {
				...
				$file->owner = ($owner !== false) ? $owner['name'] : 'N/A';
				$file->group = ($group !== false) ? $group['name'] : 'N/A';
