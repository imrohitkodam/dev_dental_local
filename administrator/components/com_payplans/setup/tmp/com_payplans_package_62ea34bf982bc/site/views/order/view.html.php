...
class PayPlansViewOrder extends PayPlansSiteView
{
	...
	{ 
		...
		if (!$subscriptionKey) {
			...
			$message = JText::_('COM_PAYPLANS_RENEWAL_INVALID_SUBSCRIPTION');
		...
		if ( in_array($expirationType, ['PP_RECURRING_TRIAL_1', 'PP_RECURRING_TRIAL_2']) ) {
		...
		$args = [$subscription, $newSubscription, $order, $newInvoice];
		$results = PPEvent::trigger('onPayplansSubscriptionAfterRenewalInvoiceCreation', $args, '', $newSubscription);

		// redirect to checkout page
		$url = PPR::_('index.php?option=com_payplans&view=checkout&invoice_key=' . $newInvoice->getKey() . PP::getExcludeTplQuery('checkout'), false);
