...
class PayplansControllerGroup extends PayPlansController
{
	...
	public function togglePublish()
	{
		...
		$actionString = $task == 'publish' ? 'COM_PP_ACTIONLOGS_GROUP_PUBLISHED' : 'COM_PP_ACTIONLOGS_GROUP_UNPUBLISHED';
		...
		foreach ($ids as $id) {
			...
			$actionlog->log($actionString, 'group', array(
					'groupTitle' => $table->title,
					'groupLink' => 'index.php?option=com_payplans&view=group&layout=form&id=' . $table->group_id
			));
		}
	...
	public function toggleVisible()
	{
		...
		$actionlog = PP::actionlog();
		$actionString = 'COM_PP_ACTIONLOGS_GROUP_VISIBLE';

		$message = 'COM_PP_GROUP_VISIBLED_SUCCESSFULLY';
		$state = 1;

		if ($task == 'invisible') {
			$actionString = 'COM_PP_ACTIONLOGS_GROUP_INVISIBLE';
	...
	public function delete()
	{
		...
		foreach ($ids as $id) {
			...
			$actionlog->log('COM_PP_ACTIONLOGS_GROUP_DELETED', 'group', array(
					'groupTitle' => $title
			));
		}

		$this->info->set(JText::_('COM_PP_GROUP_DELETE_SUCCESSFULLY'), 'success');
		return $this->redirectToView('group');
	}
	...
	public function copy()
	{
		...
		foreach ($ids as $id) {
			$group = PP::group($id);
			$selectedGroupTitle = $group->getTitle();
			$group->setId(0);
			$group->title = JText::_("COM_PAYPLANS_COPY_OF") . $group->getTitle();
			$state = $group->save();

			if ($state === false) {
				$this->info->set($group->getError(), 'danger');
				return $this->redirectToView('group');
			}

			$actionlog->log('COM_PP_ACTIONLOGS_GROUP_COPIED', 'group', array(
					'groupTitle' => $selectedGroupTitle,
					'groupLink' => 'index.php?option=com_payplans&view=group&layout=form&id=' . $id,
					'newGroupTitle' => $group->getTitle(),
					'newGroupLink' => 'index.php?option=com_payplans&view=group&layout=form&id=' . $group->getId()
			));
		}

		$this->info->set('COM_PAYPLANS_ITEMS_COPIED', 'success');
	...
	public function store()
	{
		...
		$actionlog = PP::actionlog();
		$actionString = $id ? 'COM_PP_ACTIONLOGS_GROUP_UPDATED' : 'COM_PP_ACTIONLOGS_GROUP_CREATED';

		$actionlog->log($actionString, 'group', array(
				'groupTitle' => $group->getTitle(),
				'groupLink' => 'index.php?option=com_payplans&view=group&layout=form&id=' . $group->getId(),
		));
	...
	public function saveorder()
	{
		...
		$this->info->set(JText::_('COM_PP_GROUP_ORDERED_SUCCESSFULLY'), 'success');
		return $this->redirectToView('group');
	}
