...
class PayplansControllerOrder extends PayplansController
{
	...
	public function createInvoice()
	{
		...
		// Log here instead of at $order->createInvoice() because it will log for the frontend as well and might confuse the site admin
		$actionlog = PP::actionlog();
		$actionlog->log('COM_PP_ACTIONLOGS_INVOICE_CREATED', 'order', [
		...
		$this->info->set('COM_PP_INVOICE_CREATED', 'success');
	...
	public function terminate()
	{
		...
		JError::raiseError(500, JText::_('COM_PAYPLANS_ERROR_INVALID_ORDER_ID'));
		...
		JError::raiseError(500, JText::_('COM_PAYPLANS_ERROR_INVALID_PAYMENT'));

		// if not confirm then set confirm = false on its view
		if($this->input->get('confirm', false) == false){
