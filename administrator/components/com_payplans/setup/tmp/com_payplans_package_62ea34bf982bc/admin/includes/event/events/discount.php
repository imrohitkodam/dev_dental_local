...
class PPEventDiscount extends PayPlans
{
	...
	public function onPayplansSubscriptionBeforeSave($prevSubs, $newSubs)
	{
		...
		if ($prevSubs->getStatus() != PP_SUBSCRIPTION_ACTIVE && $newSubs->getStatus() == PP_SUBSCRIPTION_ACTIVE) {
			...
			$modifiers = $invoice->getModifiers([
				'type'=> PP_PRODISCOUNT_EXTEND_TIME_DISCOUNT
			]);
	...
	/**
	 * For autoDiscount on invoice Creation, Renewal and Url discount 
	 *
	 * @since	4.0
	 * @access	public
	 */
	public function onPayplansInvoiceAfterSave($prev, $newInvoice)
	{
		...
		if (floatval(0) != floatval($newInvoice->getTotal() && !isset($prev) && in_array($newInvoice->getStatus(),$status))) {
			...
			$order = $newInvoice->getOrder();
			$isRenewal = $order->getParam('isRenewal', false);

			// If it's renewal from frontend dashboard then first apply renewal discount
			if ($isRenewal && $layout === 'processRenew') {
	...
	public function applyAutoDiscount($newInvoice, $discountType, $discountCodePrefix)
	{
		...
		$applicableDiscounts = PPHelperDiscount::getApplicableDiscounts($newInvoice, [
		...
		if ($applicableDiscounts) {
			foreach ($applicableDiscounts as $discount) {
				...
				$isInvoiceApplicable = $discount->isInvoiceApplicable($newInvoice);

				if ($isInvoiceApplicable) {
