...
class PPCustomDetails extends PayPlans
{
	...
	public function isApplicable(PPUser $user)
	{
		...
		$plans = $user->getPlans([PP_SUBSCRIPTION_ACTIVE, PP_SUBSCRIPTION_EXPIRED]);
	...
	public function getFieldsOutput($userParams)
	{
		...
		foreach ($fields->fieldset as $fieldset) {
			foreach ($fieldset->children() as $child) {
				...
				$item->type = (string) $child->attributes()['type'];
				$item->name = $name;
	 ...
	 * @access	public
	 */
	public function renderForm($obj, $site = false, $type = 'userparams')
	{
		$xml = @simplexml_load_string(base64_decode($this->data));
		...
		$output = $renderer->render($site, $this->title, $type, $obj);
		return $output;
	}
}
