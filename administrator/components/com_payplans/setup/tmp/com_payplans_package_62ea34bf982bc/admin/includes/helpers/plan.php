...
class PPHelperPlan
{
	...
	/**
	 * Convert time array into days for Advance pricing
	 *
	 * @since   4.0.0
	 * @access  public
	 */
	public static function convertAdvancePricingTimeArrayToDays($timeArray)
	{
		if (is_string($timeArray)) {
			$timeArray = PPHelperPlan::convertIntoTimeArray($timeArray);
		}
		...
		$days = (!empty($timeArray['day'])) ? intval($timeArray['day']) : 0;
		$days += (!empty($timeArray['month'])) ? intval($timeArray['month']) * 30 : 0;
		$days += (!empty($timeArray['year'])) ? intval($timeArray['year']) * 360 : 0;
