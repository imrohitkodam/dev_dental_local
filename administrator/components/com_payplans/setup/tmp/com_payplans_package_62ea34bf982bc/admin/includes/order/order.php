...
class PPOrder extends PPAbstract implements PPAppTriggerableInterface, PPMaskableInterface
{
	...
	/** 
	* Retrieve payment attached to current order
	*
	* @since 4.2.0
	* @access public 
	*/
	...
	{
		$invoiceId = $this->getParam('last_master_invoice_id', 0);
		$this->loadPayment($invoiceId);
	...
	/**
	 * Loads payment records for the order
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	...
	private function createChildInvoice($invoiceCount)
	{
		...
		if ($recurring) {
			...
			$invoiceParams = $masterInvoice->getParams();
			$paramProperties = array('title', 'expirationtype', 'expiration', 'recurrence_count', 'price', 'trial_price_1', 'trial_time_1', 'trial_price_2', 'trial_time_2');

			$params = new JRegistry();

			foreach ($paramProperties as $property) {
				$params->set($property, $masterInvoice->getParam($property));
			}
		
			if ($recurring == PP_RECURRING_TRIAL_2) {
				$expirationType = 'recurring_trial_1';

				$params->set('trial_price_1', $masterInvoice->getParam('trial_price_2', '0.00'));
				$params->set('trial_time_1', $masterInvoice->getParam('trial_time_2', '000000000000'));
			}

			$params->set('expirationtype', $expirationType);
		}

		$params->set('title', $this->getTitle());
		$invoice->table->params = $params->toString();

		$invoice->save()->refresh();
