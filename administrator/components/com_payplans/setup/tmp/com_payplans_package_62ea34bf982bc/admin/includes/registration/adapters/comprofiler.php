...
class PPRegistrationComprofiler extends PPRegistrationAbstract
{
	...
	public function __construct()
	{
		...
		$this->url = $this->getRegistrationLink();
	}
	...
	public function getRegistrationLink()
	{
		...
		if (is_null($url)) {
			...
			// check the CB is it got created any registration menu item
			$menuItem = $menu->getItems('link', 'index.php?option=com_comprofiler&view=registers', true);

			$url = JRoute::_('index.php?option=com_comprofiler&view=registers&fromPayplans=1', false);

			if (!empty($menuItem->id)) {
				$url = JRoute::_('index.php?option=com_comprofiler&view=registers&Itemid=' . $menuItem->id . '&fromPayplans=1', false);
			}
		}
	...
	public function onPayplansAccessCheck()
	{
		$option = $this->input->get('option', '', 'default');
		$view = $this->input->get('view', '', 'default');
		$fromPayplans = $this->input->get('fromPayplans', 0);
