...
class PPRegistrationAbstract extends PayPlans
{
	...
	public function onAfterRoute()
	{
		...
		// Determine whether site onwer has activate this setting to force all the registration integration type registration link redirect to plan listing page
		$isForceRegistrationRedirection = $this->isForceRegistrationRedirection();

		// if its a direct registration without plan id
		if (($isRegistrationUrl || $isForceRegistrationRedirection) && !$planId && $isPluginEnabled) {
	...
	public function processData()
	{
		...
		if ($data) {
			$mapping = ['name', 'username', 'email', 'password', 'password2'];
	...
	/**
	 * Determine all the registration integration type registration link force to redirect to the plan listing page
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	public function isForceRegistrationRedirection()
	{
		$isEnabledForceRegistrationRedirection = $this->config->get('registration_force_redirection');

		if (!$isEnabledForceRegistrationRedirection) {
			return false;
		}
