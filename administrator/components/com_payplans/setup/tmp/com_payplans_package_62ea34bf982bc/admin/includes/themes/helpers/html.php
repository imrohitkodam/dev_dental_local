...
class PPThemesHelperHtml extends PPThemesHelperAbstract
{
	 ...
	 * @access	public
	 */
	public function plantime($timer, $options = [])
	{
		$isRecurring = FH::normalize($options, 'isRecurring', false);
		$lifetime = true;
		$count = 0;
		...
		foreach ($timer as $key => $value) {
			...
			// It does not make sense to say e.g. "per 1 year" only
			if ($isRecurring && $count === 1 && $value === 1) {
				$value = '';
			}
