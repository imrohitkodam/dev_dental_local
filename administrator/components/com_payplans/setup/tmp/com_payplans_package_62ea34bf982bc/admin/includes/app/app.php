...
class PPApp extends PPAbstract
{
	...
	public function load($id = 0)
	{
		...
		$model = PP::model('App');
		$apps = $model->loadRecords(['id' => $id]);

		// if app not exist then no need to bind
		if (empty($apps)) {
			return $this;
		}
	...
	private function _saveAppPlans()
	{
		...
		$model->deleteMany(['app_id' => $this->getId()]);
	...
	public function hasPurpose($purpose = '')
	{
		...
		$type = PPJString::ucfirst(PPJString::strtolower($purpose));
	...
	protected function _getResource($userId, $groupId, $title)
	{
		...
		$resource->subscription_ids = PPJString::trim($resource->subscription_ids, ',');
