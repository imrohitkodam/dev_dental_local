...
class PayplansGdprSubscription extends PayplansGdprAbstract
{
	...
	public function onEasySocialGdprExport(SocialGdprSection &$section, SocialGdprItem $adapter)
	{
		...
		$subscriptions = $model->loadRecords(['user_id' => $this->userId]);
		...
		foreach ($subscriptions as $value) {
			...
			$item = $adapter->getTemplate($subscription->getId(), $adapter->type);
			$item->view = false;
			$item->title = '';
	...
	public function getIntro($subscription)
	{
		...
		<table class="gdpr-table" style="width:520px;">
			...
			<tbody>
				...
				<?php
				$subscriptionParams = $subscription->getParams()->toArray();
				$subArray = ['expirationtype', 'trial_price_1', 'trial_time_1', 'trial_price_2', 'trial_time_2', 'price', 'expiration', 'recurrence_count', 'currency', 'fixed_date_expiration_applicable'];
				$contains = ['preExpiry', 'postExpiry', 'postActivation', 'trialPreExpiry', 'oncartabondonment'];
