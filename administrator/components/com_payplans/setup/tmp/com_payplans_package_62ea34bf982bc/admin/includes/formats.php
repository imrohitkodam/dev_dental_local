...
class PPFormats
{
	 ...
	 * @access	public
	 */
	public static function country($item, $config = [])
	{
		if (!is_object($item)) {
			...
			throw new Exception(JText::_('INVALID_COUNTRY_CODE'));
		}
	 ...
	 * @access	public
	 */
	static function currency($item, $config = [], $format = null)
	{
		$config = PP::config();
	...
	public static function amount($amount, $currency, $config = [])
	{
		$config = PP::config();
	 ...
	 * @access	public
	 */
	public static function displayAmount($amount, $config = [])
	{
		$config = PP::config();
	...
	static function user($item, $config = [])
	{
		if (!empty($config)) {
	...
	static function app($item, $config = [])
	{
		return $item->title;
	}
	 ...
	 * @deprecated	4.0.0
	 */
	public static function planTime($time, $config = [])
	{
	}

	static function order($item, $config = ['prefix' => false, 'link' => false, 'admin' => false, 'attr' => ''])
	{
		return self::_ops('order', $item, $config);
	}

	static function subscription($item, $config = ['prefix' => false, 'link' => false, 'admin' => false, 'attr' => ''])
	{
		return self::_ops('subscription', $item, $config);
	}

	static function payment($item, $config = ['prefix' => false, 'link' => false, 'admin' => false, 'attr' => ''])
	{
		return self::_ops('payment', $item, $config);
	}

	static function invoice($item, $config = ['prefix' => false, 'link' => false, 'admin' => false, 'attr' => ''])
	{
	...
	{
		$str = $config['prefix'] ? JText::_('COM_PAYPLANS_SEARCH_'.PPJString::strtoupper($entity)).' # ' : '' ;
		...
		if ($config['link']) {
			$link = JRoute::_('index.php?option=com_payplans&'."view={$entity}&task=edit&{$id['var']}={$id['value']}");
	...
	static function plan($item, $config = [])
	{
		if (!empty($config)) {
	...
	static function group($item, $config = [])
	{
		if (!empty($config)) {
