	...
	public static function onPayplansInvoiceAfterSave($previousInvoice, $newInvoice)
	{
		...
		if ($previousInvoice->getStatus() != PP_INVOICE_PAID && $newInvoice->getStatus() == PP_INVOICE_PAID) {
			...
			if ($services) {
				...
				// Update plan addon's consumption 
				foreach ($services as $service) {
					...
					$invoiceAddon = PP::table('AddonStat');
					$invoiceAddon->load($service->planaddons_stats_id);

					if ($invoiceAddon->planaddons_stats_id) {
						$invoiceAddon->consumed = 1;
