...
class PPSubscription extends PPAbstract implements PPAppTriggerableInterface, PayplansIfaceApiSubscription, PPMaskableInterface
{
	...
	public function activate($order = null)
	{
		...
		$actionlog->log('COM_PP_ACTIONLOGS_SUBSCRIPTION_UPDATE_STATUS_TO_ACTIVE', 'subscription', array(
				'link' => 'index.php?option=com_payplans&view=subscription&layout=form&id=' . $this->getId(), 
				'key' => $this->getKey()
		));
		...
		// #1421
		// if the invoice id paid and its not expired and the plan enabled the moderation.
		// we just need to add  transaction and logs records
		if (($recentInvoice->getId() != 0) && $recentInvoice->getStatus() == PP_INVOICE_PAID) {


			$plan = $recentInvoice->getPlan();

			// we can safely assumed this activation is because of plan's moderation.
			// $this->isNotActive() happen when admin update subscription by setting the status to 'active' from subscription edit page.
			if ($plan->requireModeration() && ($this->isOnHold() || $this->isNotActive())) {

				// previous content
				$prev = $this->toArray();

				// activate subscription.
				$expiration = $recentInvoice->getCurrentExpiration(true);

				if ($expiration == '000000000000' && $recentInvoice->isRecurring()) {
					$invoiceParams = $recentInvoice->table->getParams();
				...
				$this->renew($expiration);

				// current value
				$content = array('previous' => $prev, 'current' => $this->toArray());

				PPLog::log(PPLogger::LEVEL_INFO, JText::_('Subscription Activated'), $this, $content, 'PayplansSubscriptionFormatter', '', false);

				// get the last transaction that was added.
				$transaction = $recentInvoice->getCurrentPaymentTransaction();
				return $transaction;
			}
		}
	...
	public function extend($timeframe)
	{
		...
		if ($timeframe == '000000000000') {
			...
			$actionString = 'COM_PP_ACTIONLOGS_SUBSCRIPTION_EXTENDED_TO_LIFETIME';
		}
		...
		if ($timeframe != '000000000000') {
			...
			$actionString = 'COM_PP_ACTIONLOGS_SUBSCRIPTION_EXTENDED_TO_NEW_EXPDATE';
		}
		...
		$actionlog->log($actionString, 'subscription', array(
				'key' => $this->getKey(),
				'link' => 'index.php?option=com_payplans&view=subscription&layout=form&id=' . $this->getId(),
				'newExpirationDate' => PP::date($this->expiration_date)->format(JText::_('DATE_FORMAT_LC2'))
		));
	...
	{
		$token = $this->table->toArray();

		// Remove unwanted data
		unset($token['checked_out']);
		unset($token['checked_out_time']);
		unset($token['modified_date']);
		unset($token['params']);

		$token['key'] = $this->getKey();

		// If no 'units' param, means this subscription is for 1 unit only
		$token['advancedpricing_units'] = $this->getParams()->get('units', '1');

		// Get custom details
		$customDetails = $this->getCustomDetails(true);

		// Process custom details found.
		foreach ($customDetails as $details) {
			...
			$stored = $details->getFieldsOutput($this->getParams());
			$fieldset = $xml->fields->fieldset;
			$key = strtoupper(str_replace(array(' ','&nbsp;', "\n", "\t", "\r","\r\n"), '_', (string) $fieldset->attributes()->name));
			$fields = (object) $fieldset->field;

			$i = 0;
			foreach ($fields as $index => $field) {
				$name = strtoupper(str_replace(array(' ','&nbsp;', "\n", "\t", "\r","\r\n"), '_', (string) $field->attributes()->name));
				$token['CUSTOM_DETAILS_' . $key . '_' . $name] = $stored[$i]->value ? $stored[$i]->value : '';
				$i++;
			}
		}
	 ...
	 * @access	public
	 */
	public function getCancellationDate($locale = null)
	{
		if ($this->table->cancel_date == '0000-00-00 00:00:00') {
			return false;
		}

		$date = PP::date($this->table->cancel_date, $locale);
	 ...
	 * @access	public
	 */
	public function getSubscriptionDate($locale = null)
	{
		if ($this->table->subscription_date == '0000-00-00 00:00:00') {
			return false;
		}

		$date = PP::date($this->table->subscription_date, $locale);
	 ...
	 * @access	public
	 */
	public function getExpirationDate($locale = null)
	{
		if ($this->table->expiration_date == '0000-00-00 00:00:00') {
			return false;
		}

		$date = PP::date($this->table->expiration_date, $locale);
	...
	public function isRequiredToProcessByCron()
	{
		...
		$recurrenceCount = (int)$this->getRecurrenceCount();
	...
	{ 
		...
		$this->table->subscription_date = $subscriptionDate;

		// setup the expiration date if needed.
		$testExpiration = (int) $expiration;
		...
		if ($testExpiration) {
			...
			$expirationDate = $from->addExpiration($expiration);
		}

		if (!$this->getParams()->get('fixed_date_expiration_applicable', 0)) {
			$this->table->expiration_date = $expirationDate;
		}
	...
	public function refund()
	{
		...
		$actionlog = PP::actionlog();
		$actionlog->log('COM_PP_ACTIONLOGS_SUBSCRIPTION_UPDATE_STATUS_TO_INACTIVE', 'subscription', array(
				'link' => 'index.php?option=com_payplans&view=subscription&layout=form&id=' . $this->getId(), 
				'key' => $this->getKey()
		));
	 ...
	 * @access	public
	 */
	public function expired($order = null)
	{
		// Get the order
		if (is_null($order)) {
			$order = $this->getOrder();
		...
		$actionlog = PP::actionlog();
		$actionlog->log('COM_PP_ACTIONLOGS_SUBSCRIPTION_UPDATE_STATUS_TO_EXPIRED', 'subscriptions', array(
				'link' => 'index.php?option=com_payplans&view=subscription&layout=form&id=' . $this->getId(), 
				'key' => $this->getKey()
		));
	...
	public function setPlan($plan)
	{
		...
		if ($plan->getPlanPriceVariation()) {

			list($title, $price, $time) = explode('_', $plan->getPlanPriceVariation());
	 ...
	 * @access	public
	 */
	public function getCustomDetails($ignoreApplicablePlan = false)
	{
		static $items = array();

		if (!isset($items[$this->getId()])) {
			$model = PP::model('Customdetails');
			$customDetails = $model->getSubscriptionCustomDetails($this, $ignoreApplicablePlan);
