...
class PPEventLog extends PayPlans
{
	...
	public function delete($itemId, $type='PLAN', $formatter='PayplansFormatter')
	{
		...
		$content['previous'] = $object ? $object->toArray() : [];
	...
	public function isEqual($previous, $current, $prevObject)
	{
		...
		$params = ['_appplans', '_planapps', '_groups','_plans'];
	...
	static function onPayplansStartMigration()
	{
		...
		$conditions = ['class'=>'"PayplansSubscription"'];
		$model->deleteMany($conditions, 'OR');

		$conditions = ['class'=>'"PayplansPayment"'];
		$model->deleteMany($conditions, 'OR');	
		
		$conditions = ['class'=>'"PayplansOrder"'];
		$model->deleteMany($conditions, 'OR');
		
		$conditions = ['class'=>'"PayplansPlan"'];
		$model->deleteMany($conditions, 'OR');
		
		$conditions = ['class'=> '"PayplansApp%"'];
	...
	{	
		$message = JText::_('COM_PAYPLANS_MIGRATION_SUCCESS_'.PPJString::strtoupper($pluginKey));
