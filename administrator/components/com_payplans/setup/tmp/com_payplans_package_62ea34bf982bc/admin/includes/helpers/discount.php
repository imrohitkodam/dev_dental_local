...
class PPHelperDiscount
{
	 ...
	 * @access	public
	 */
	public static function getApplicableDiscountsByPlan($plan, $filters)
	{
		$model = PP::model('Discount');
		$discounts = $model->loadRecords($filters);
		...
		foreach ($discounts as $discountId => $value) {
			...
			if (!$discount->isCoreDiscount()) {
				...
				$discountPlans = $discount->getPlans();

				if (!in_array($plan->getId(), $discountPlans)) {
					continue;
				}
			}
	...
	public static function checkForApplicableDates(PPDiscount $discount)
	{
		...
		if ((!$publish_start && $publish_end) || ($publish_start == '0000-00-00 00:00:00' && $publish_end == '0000-00-00 00:00:00')) {
			return true;
		}
