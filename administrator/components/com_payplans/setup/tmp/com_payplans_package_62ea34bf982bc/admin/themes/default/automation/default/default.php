...
<form method="post" name="adminForm" id="adminForm" data-fd-grid>
			...
			<tbody>
				<?php if ($apps) { ?>
					...
					<tr>
						...
						<td>
							<a href="<?php echo JRoute::_('index.php?option=com_payplans&view=automation&layout=form&id=' . $app->app_id);?>"><?php echo $app->title;?></a>
						</td>
						<td class="center">
							<?php echo $this->html('grid.published', $app, 'app', 'published'); ?>
						</td>
						<td class="center"> 
							<?php if ($app->type == 'mysqlquery') { ?>
								<?php echo JText::_('MySQL Queries'); ?>
							<?php } ?>

							<?php if ($app->type == 'httpquery') { ?>
								<?php echo JText::_('HTTP Query'); ?>
							<?php } ?>

							<?php if ($app->type == 'assignplan') { ?>
								<?php echo JText::_('Plan Assignment'); ?>
							<?php } ?>
						</td>
						<td>
							<?php echo $app->app_id;?>
				...
				<?php } else { ?>
					<?php echo $this->html('grid.emptyBlock', 'COM_PP_AUTOMATION_EMPTY', 5); ?>
				<?php } ?>
			</tbody>
	...
	<?php echo $this->html('form.hidden', 'plan_id', '', array('data-apply-plan-id' => '')); ?>
