	...
	<div data-fd-tab-wrapper>
		...
		<div class="tab-content">
			<div id="details" class="t-hidden <?php echo !$activeTab || $activeTab === 'details' ? 't-block' : '';?>">
				<?php echo $this->output('admin/notifications/form/details'); ?>
			</div>
		</div>
	</div>

	<?php if (!$app->getId() && isset($element)) { ?>
		<?php echo $this->html('form.hidden', 'type', $element); ?>
	<?php } ?>

	<?php echo $this->html('form.hidden', 'app_params[when_to_email]', $when); ?>
	<?php echo $this->html('form.action', isset($controller) ? $controller : 'app', isset($task) ? $task : 'store'); ?>
