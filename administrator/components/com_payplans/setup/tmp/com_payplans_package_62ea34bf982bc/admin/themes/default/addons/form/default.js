PayPlans.ready(function($) {
	...
	$('[data-usage-type]').on('change', function(event) {

		var val = $(this).val();

		if (val == '1') {
			// this is limited
			$('[data-usage-type-container]').removeClass('t-hidden');
		} else {
			//unlimited
			$('[data-usage-type-container]').addClass('t-hidden');
			$('[data-usage-type-input]').val('');
		}
	});
	...
	function addonValidation() {
		...
		if ($('input[name="apply_on"]').not('[type=checkbox]').val() == '0' && $('[data-addons-plans]').val() == null) {
