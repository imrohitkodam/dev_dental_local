...
class PayplansModelPlan extends PayPlansModel
{
	...
	public function getAvailableUpgrades($planId)
	{
		...
		if (!isset($result[$planId])) {
			...
			foreach ($upgradeAppInstances as $app) {
				...
				$upgradableTo = $appParams->get('upgrade_to', []);
	...
	public function getItems()
	{
		...
		// Support backwards compatible value
		if ($visible == 0) {
			$visible = 'invisible';
		}
		...
		if ($visible !== 'all') {
			...
			$wheres[] = $db->nameQuote('a.visible') . " = " . $db->Quote($visibility);
		}
	...
	public function delete($pk = null)
	{
		...
		$plans = PP::model('planapp')->loadRecords([
			'plan_id' => $pk
		]);
		...
		return PP::model('plangroup')->deleteMany([
			'plan_id' => $pk
		]);
	}
	...
	public function getUngrouppedPlans($queryFilters = [])
	{
		$db = PP::db();
	...
	public function getTotalRenewalPerPlan(PPDate $firstDate, PPDate $lastDate)
	{
		...
		$query = 'SELECT group_concat(invoice.`invoice_id`) AS invoice_group, invoice.`object_id` AS order_id, invoice.`total` AS amount';
		...
		foreach ($invoiceRecords as $key => $record) {
			...
			$count[$planId.'_amount'] = isset($count[$planId.'_amount']) ? $count[$planId.'_amount'] + $record->amount : $record->amount;
		}
	...
	/**
	 * Get total recurrring sales per plan from given date range
	 * included first recurring as well subsequent recurring payments
	 * @since	4.0
	 * @access	public
	 */
	public function getTotalRecurringPerPlan(PPDate $firstDate, PPDate $lastDate)
	{
		$db = $this->db;

		$query = 'SELECT invoice.`invoice_id`, `counter`, `object_id` AS order_id, `total` AS amount';
		$query .= ' FROM `#__payplans_invoice` AS invoice';
		$query .= ' WHERE invoice.`params` LIKE '."'".'%"expirationtype":"recurring%'."'";
		$query .= ' AND invoice.`status` =  '. $db->Quote(PP_INVOICE_PAID);
		$query .= ' AND invoice.`paid_date` >= ' . $db->Quote($firstDate->toMySQL());
		$query .= ' AND invoice.`paid_date` <= ' . $db->Quote($lastDate->toMySQL());
		...
		// Get order Ids
		$orderIds = [];
		foreach ($invoiceRecords as $value) {
			$orderIds[] = $value->order_id;
		}

		$query = 'SELECT `order_id`, `plan_id` FROM `#__payplans_subscription` AS subs';
		$query .= ' WHERE subs.`order_id` in (' . implode(',', $orderIds) . ')';
		...
		foreach ($invoiceRecords as $key => $record) {
			$planId = $subRecord[$record->order_id]->plan_id;

			// We consider total as 1 here  because we are getting indiviual recurring invoice
			// All transaction attached with master id and invoice attached with same order
			// We can not group_concat invoice as recurring amount can be diffrent for each recurring like recurring_ trial
			// so in count we always add 1 as we get single invoive so amount calculation will not wrong in case diffrent recurring amount
			
			$count[$planId] = isset($count[$planId]) ? $count[$planId] + 1 : 1;
			$count[$planId.'_amount'] = isset($count[$planId.'_amount']) ? $count[$planId.'_amount'] + $record->amount : $record->amount;
		}
	...
	/**
	 * Get plan's group id if exists
	 *
	 * @since	4.2.4
	 * @access	public
	 */
	public function getPlanGroup($planId)
	{
		$db = PP::db();

		$query = "select `group_id` from `#__payplans_plangroup`";
		$query .= " where `plan_id` = " . $db->Quote($planId);
		$query .= " limit 1";
