...
class PayplansModelSubscription extends PayPlansModel
{
	...
	*/
	...
	public function getDataToExport($options = [])
	{
		...
		$query = [];
		$query[] = "SELECT " . $db->nameQuote('a.subscription_id') . ", ". $db->nameQuote('b.username') . ", " . $db->nameQuote('a.user_id') . ", " . $db->nameQuote('a.status') . ", " . $db->nameQuote('a.total') . ", " . $db->nameQuote('a.subscription_date') . ", " . $db->nameQuote('a.expiration_date') . ", " . $db->nameQuote('a.params');
		$query[] = "from `#__payplans_subscription` as a";
