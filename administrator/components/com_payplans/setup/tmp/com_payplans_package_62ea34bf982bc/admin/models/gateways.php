...
class PayplansModelGateways extends PayPlansModel
{
	...
	public function updateOrdering($id, $order)
	{
		...
		$query = "update `#__payplans_app` set ordering = " . $db->Quote($order);
		$query .= " where app_id = " . $db->Quote($id);
		$query .= " and `group` = " . $db->Quote('payment');
	...
	/**
	 * Rebuild ordering for payments method
	 *
	 * @since	4.1.0
	 * @access	public
	 */
	public function rebuildOrdering($group = 'payment')
	{
		$db = PP::db();
		...
		$query = "UPDATE `#__payplans_app` SET `ordering` = (@new_ordering := @new_ordering + @ordering_interval)";
