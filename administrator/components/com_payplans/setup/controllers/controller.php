...
class PayPlansSetupController
{
	...
	public function getVersion()
	{
		...
		if (is_null($version)) {
			$contents = file_get_contents(PP_LAUNCHER_MANIFEST);
	...
	/**
	 * Extract zip file in installation part
	 *
	 * @since	4.2.0
	 * @access	public
	 */
	public function ppExtract($destination, $extracted)
	{
		if ($this->isJoomla4()) {
			...
			return $archive->extract($destination, $extracted);
		}

		return JArchive::extract($destination, $extracted);
	}
	...
	public static function isJoomla4()
	{
		...
		if (is_null($isJoomla4)) {
			$isJoomla4 = version_compare(JVERSION, '4.0') !== -1;
	...
	/**
	 * Abstract method to execute query for compatibility purposes
	 *
	 * @since	4.2.0
	 * @access	public
	 */
