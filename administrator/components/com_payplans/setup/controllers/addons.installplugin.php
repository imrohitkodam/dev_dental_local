...
class PayplansControllerAddonsInstallPlugin extends PayplansSetupController
{
	...
	public function installPlugin($element, $group, $absolutePath)
	{
		...
		$isPluginInstallBefore = false;
		$pluginStatus = false;

		// Retrieve the current plugin whether got enable it or not
		if ($result) {
			$isPluginInstallBefore = true;
			$pluginStatus = $result->enabled;
		}
