...
class PayplansControllerInstallCopy extends PayplansSetupController
{
	/**
	...
	public function execute()
	{
		...
		$state = $this->ppExtract($archivePath, $path);

		if (!$state) {
			$this->setInfo(JText::sprintf('COM_PP_INSTALLATION_COPY_ERROR_UNABLE_EXTRACT', $type), false);
			return $this->output();
		}
		...
		if ($type == 'admin') {
			...
			$target = JPATH_ADMINISTRATOR . '/components/com_payplans';
		}
		...
		if ($type == 'site') {
			...
			$target = JPATH_ROOT . '/components/com_payplans';
		}
		...
		if ($type == 'languages') {
			...
			$adminFile = $path . '/admin/en-GB.com_payplans.ini';
			JFile::copy($adminFile, JPATH_ADMINISTRATOR . '/language/en-GB/en-GB.com_payplans.ini');

			// Copy the admin system language file
			$adminFileSys = $path . '/admin/en-GB.com_payplans.sys.ini';
			JFile::copy($adminFileSys, JPATH_ADMINISTRATOR . '/language/en-GB/en-GB.com_payplans.sys.ini');

			// Copy the site language file
			$siteFile = $path . '/site/en-GB.com_payplans.ini';
			JFile::copy($siteFile, JPATH_ROOT . '/language/en-GB/en-GB.com_payplans.ini');


			$this->setInfo('COM_PP_INSTALLATION_LANGUAGES_UPDATED', true);
			return $this->output();
		}
		...
		if ($type == 'media') {
			...
			$target = JPATH_ROOT . '/media/com_payplans';
		}
		...
		$result = $this->getResultObj(JText::sprintf('COM_PP_INSTALLATION_COPY_FILES_SUCCESS', $totalFiles, $totalFolders), true);
	...
	{
		$excludeFolders = array('log', 'downloads', 'emails');

		$path = JPATH_ROOT . '/media/com_payplans';
	...
	{
		$path = JPATH_ROOT . '/components/com_payplans';
		$exists = JFolder::exists($path);
	...
	public function cleanupAdmin()
	{
		$folders = [
			...
			'/components/com_payplans/models',
			'/components/com_payplans/tables',
			'/components/com_payplans/themes',
