...
class PPTransaction extends PPAbstract implements PPAppTriggerableInterface, PayplansIfaceApiTransaction
{
	 ...
	 * @access  public
	 */
	public function getCreatedDate($locale = null)
	{
		if ($this->created_date == '0000-00-00 00:00:00') {
			return false;
		}

		$date = PP::date($this->created_date, $locale);
	...
	public function getRewriterTokens()
	{
		...
		// added payment method token
		$payment = $this->getPayment();
		$paymentMethod = JText::_('COM_PAYPLANS_TRANSACTION_PAYMENT_GATEWAY_NONE');

		if ($payment->getId()) {
			$paymentMethod = $payment->getAppName();
		}
