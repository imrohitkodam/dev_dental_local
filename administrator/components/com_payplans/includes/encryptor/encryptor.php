...
class PPEncryptor extends PayPlans
{
	...
	public function __construct($key)
	{
		...
		if (strlen($this->scramble1) <> strlen($this->scramble2)) {
			throw new Exception(JText::_('COM_PAYPLANS_ERROR_ENCRYPTOR_SCRAMBLE'));
		}
	 ...
	 * @access	public
	 */
	public function decrypt($source, $fromSearch = false)
	{
		$this->errors = array();
		...
		// if this request from the search then make sure that the key showing upper case
		if ($fromSearch) {
			$source = PPJString::strtoupper($source);
		}
	...
	// NOTE: $fudgefactor is passed by reference so that it can be modified
	{
		...
		if (!empty($this->mod)) {               // if modifier has been supplied
			if ((int) $fudge % $this->mod === 0) {     // if it is divisible by modifier
