...
class PPThemes
{
	...
	public function html($namespace)
	{
		...
		if (!isset($objects[$namespace])) {
			...
			if ($exists) {
				...
				$className = 'PPThemesHelper' . ucfirst($className);

				$objects[$namespace] = new $className();
			}
		}
		...
		return call_user_func_array(array($objects[$namespace], $method), $args);
	}
		...
		if (JFile::exists($template->script)) {
			...
			if (!self::$_inlineScript || !$this->inlineScript) {
				...
				if ($doc->getType() == 'html') {
					...
					PP::scripts()->add($script->parse($args));
				}			
			}
		}
	...
	public function parse($file, $vars = null)
	{
		...
		// Helper method to generate css classes
		$_ = function() {
			return call_user_func_array([$this->fd, 'css'], func_get_args());
		};
	 ...
	 * @access	public
	 */
	public function renderModule($position, $wrapper = null, $attributes = [], $content = null, $force = false)
	{
		$doc = JFactory::getDocument();
		$tmp = $doc->getType();
		...
		foreach ($modules as $module) {
			...
			$moduleOutput = PPJString::trim($moduleOutput);
