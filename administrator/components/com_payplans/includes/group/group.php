...
class PPGroup extends PPAbstract
{
	...
	{
		if ($descriptionFormat == true && $this->description) {

			$groupDescription = new stdClass();
			$groupDescription->text = $this->description;
			JPluginHelper::importPlugin('content');

			$param = null;
			$args = ['com_payplans.groupDescription', &$groupDescription, &$param, 0];
			PP::dispatcher()->triggerPlugin('onContentPrepare', $args);

			// make sure that plan description text is not null
			if ($groupDescription->text != NULL) {
