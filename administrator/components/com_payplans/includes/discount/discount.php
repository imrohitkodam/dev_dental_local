...
class PPDiscount extends PPAbstract
{
	...
	*/
	...
	public function isInvoiceApplicable(PPInvoice $invoice, $discountCode = '')
	{
		...
		// For non core discounts or gift coupon, need to check if the plan matches the allowed plans
		if (!$this->isCoreDiscount() || $couponType === PP_DISCOUNTS_TYPE_GIFT) {
			$plans = $this->getPlans();
			$plan = $invoice->getPlan();

			// Check if applicable plan array or not
			if (!is_array($plans)){
				$plans = explode(",", $plans);
			}
		...
		$filters = [
			'serial' => [
				['IN', '(' . PP_MODIFIER_FIXED_DISCOUNT . ',' . PP_MODIFIER_PERCENT_DISCOUNT . ')']
			]
		];

		// check for the current new invoice whether got any modifier record added above serial
		$discountModifiers = $invoice->getModifiers($filters);

		// Retrieve those discount rules is not allow to combine which already logged into the modifier table
		$model = PP::model('Discount');
		$nonCombinableModifiers = $model->getNonCombinableDiscounts($discountModifiers);
		$hasNonCombinableModifiers = count($nonCombinableModifiers) > 0 ? true : false;

		// Additional check if the case like this:
		// need to check the existing modifier record if it check for the "second" discount rule, check the `Allow combine discount` got enable or not
		// if yes, and the applying discount rule allows combination then apply the discount
		// if no, and the applying discount rule disallows combination then show error msg

		if (($allowClubbing && !$this->isCombinable() && ($hasNonCombinableModifiers || ($discountModifiers && !$hasNonCombinableModifiers))) || (!$allowClubbing && $invoice->getDiscount() != 0)) {
		...
		$hasUsedDiscountUsage = $model->hasUsed($discountCode, $user->getId(), $this->getCouponType());

		if (!$user->isPlaceholderAccount() && !$this->isReusable() && $hasUsedDiscountUsage) {
	...
	{
		if ($nonAsEmpty && stristr($this->start_date, '0000-00-00') !== false) {
			return '';
		}
	...
	{
		if ($nonAsEmpty && stristr($this->end_date, '0000-00-00') !== false) {
			return '';
		}
	...
	{
		$unique = [
			PP_PRODISCOUNT_AUTOONRENEWAL, 
			PP_PRODISCOUNT_AUTOONUPGRADE, 
	...
	/**
	 * Determines if the coupon type is global coupon
	 *
	 * @since	4.1.0
	 * @access	public
	 */
	...
	{
		return $this->getCouponType() == PP_DISCOUNTS_TYPE_GLOBAL;
	}

	/**
	 * Determines if the coupon type is for each new invoice
	 *
	 * @since	4.1.0
	 * @access	public
	 */
	public function isEachNewInvoiceDiscount()
	{
		return $this->getCouponType() == PP_PRODISCOUNT_AUTO_ON_INVOICE_CREATION;
	}
	...
	public function toModifier(PPInvoice $invoice)
	{
		...
		// If discount is global then in case recurring it should apply on each recurring
		// If Each new invoice discount, set frequency to each time because some recurring payment run on cron
		// in case cron first payment came then invoive generate , to apply discout correctly it should be each time

		if (($this->isGlobalDiscount() || $this->isEachNewInvoiceDiscount() || $this->isForRecurring()) && $invoice->isRecurring()) {
			$modifier->frequency = PP_MODIFIER_FREQUENCY_EACH_TIME;
		} else {
	...
	public function checkForUpperLimit(&$newModifier, $invoice)
	{
		...
		$modifiers = array_merge($invoice->getModifiers([
			'serial' => $serialsFilter
		]), [$modifier]);
		...
		if ($resultAmount > $allowedAmount && $applicableAmount > 0) {
			...
			if ($newModifier->percentage) {
				$newModifier->amount = ($newModifier->amount*100)/($actualTotal-$existingDiscounts);
			}
