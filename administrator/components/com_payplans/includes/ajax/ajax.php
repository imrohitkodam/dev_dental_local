...
class PPAjax extends PayPlans
{
	...
	public function getUrl()
	{
		...
		if (!PP::isFromAdmin() && PPRouter::getMode() == PP_JROUTER_MODE_SEF && $languageFilterEnabled) {
			...
			if ($removeLangCode === 1) {
				...
				if ($rewrite) {
					...
					$path = PPJString::substr($base, 10);
				}

				// Remove trailing / from the url
				$path = PPJString::trim($path, '/');
				$parts = explode('/', $path);
