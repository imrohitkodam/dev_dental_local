...
class PPPlan extends PPAbstract implements PPAppTriggerableInterface, PayplansIfaceApiPlan
{
	...
	public  $_blacklist_tokens = ['published','visible', 'params'];
	...
	public function canSubscribe($userId = null)
	{
		...
		$user = PP::user($userId);
		$permission = $this->getPlanPermission();
		$userGroups = $user->getUserGroups();

		if (!array_intersect($userGroups, $permission)) {
			$message = JText::sprintf('COM_PP_PLANPERMISSION_NOT_ALLOW');
	...
	public function delete()
	{
		...
		$subscription = PP::model('subscription')->loadRecords([
			'plan_id'=>$this->getId()
		]);
	...
	public function getSelectPermalink($xhtml = true, $raw = false)
	{
		...
		$url = 'index.php?option=com_payplans&task=plan.subscribe&plan_id=' . $this->getId() . $excludeTemplate;
	...
	public function getCurrency($format = null)
	{
		...
		$currency = PPFormats::currency(PP::getCurrency($currency), [], $format);
	...
	public function getDescription($descriptionFormat = false)
	{
		if ($descriptionFormat == true && $this->table->description) {
			...
			$param = null;
			$args = ['com_payplans.planDescription', &$planDescription, &$param, 0];
			PP::dispatcher()->triggerPlugin('onContentPrepare', $args);

			// make sure that plan description text is not null
			if ($planDescription->text != NULL) {
	...
	{
		$free = PP::isFree($this->getPrice());

		return $free;
	}

	/**
	 * Determines if billing details can be shown for the plan during checkout
	 * 
	 * NOTE: Only adjustable for free plan
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function canShowBillingDetails()
	{
		if (!$this->isFree()) {
			return true;
		}
		...
		return $params->get('show_billing', true);
	}


	/**
	 * Determines if this plan is a Forever plan
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	...
	{
		return $this->getDetails()->get('recurrence_count', 0);
	}
	...
	public function getRewriterTokens()
	{
		$tokens = [
			...
			'PLAN_DETAILS_PRICE' => $this->getPrice(),
			'PLAN_DETAILS_CURRENCY' => $this->getCurrency(),
			'PLAN_DETAILS_EXPIRATIONTYPE' => $this->getExpirationType(),
			'PLAN_DETAILS_EXPIRATION' => $this->getExpiration(),
			'PLAN_DETAILS_RECURRENCE_COUNT' => $this->getRecurrenceCount(),
			'PLAN_DETAILS_TRIAL_PRICE_1' => $this->getPrice(PP_PRICE_RECURRING_TRIAL_1),
			'PLAN_DETAILS_TRIAL_TIME_1' => $this->getExpiration(PP_PRICE_RECURRING_TRIAL_1),
			'PLAN_DETAILS_TRIAL_PRICE_2' => $this->getPrice(PP_PRICE_RECURRING_TRIAL_2),
			'PLAN_DETAILS_TRIAL_TIME_2' => $this->getExpiration(PP_PRICE_RECURRING_TRIAL_2)
		];
	...
	/**
	 * Set plan price variation
	 *
	 * @since   4.0
	 * @access  public
	 */
	public function setPlanPriceVariation($planPriceVariation)
	{
		$this->_planpricevariation = $planPriceVariation;
	}

	/**
	 * Determine if this plan has a price variation or not
	 *
	 * @since   4.0
	 * @access  public
	 */
	public function getPlanPriceVariation()
	{
		return $this->_planpricevariation;
	}

	/**
	 * Retrieves a list of price variations that can be associated with this plan
	 *
	 * @since	4.1.0
	 * @access	public
	 */
	public function getPlanPriceVariations()
	{
		$planVariations = [];
		...
		$priceVariations = $model->getAppInstances($options);

		if (!$priceVariations) {
			return $planVariations;
		}				

		foreach ($priceVariations as $priceVariation) {
			$app = PP::app($priceVariation->app_id);

			if ($app->getCoreParams()->get('applyAll') == '1' || $model->isPlanRelated($app->getId(), $this->getId())) {
	 ...
	 * @access  public
	 */
	public function getPricePerDay($round = true)
	{
		...
		if ($round) {
			...
			$pricePerDay = $this->getPrice() / $planDuration;
		}
	...
	/**
	 * Retrieve the permission value for this plan
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	public function getPlanPermission()
	{
		$params = $this->getParams();
		$permission = $params->get('planPermission');

		if (!$permission) {
			$permission = [];
			$userGroups = \FH::getUserGroupsTree();
	...
	public function savePlanPermission()
	{
		// Clear the existsing data
		$model = PP::model('planAcl');
		$model->clearAcl($this->getId());

		$params = $this->getParams();
		$permission = $params->get('planPermission', []);

		// Insert permission by default
		if (!$permission) {
			$userGroup = FH::getUserGroupsTree();
		...
		foreach ($permission as $aclId) {

			if (!$aclId) {
				continue;
			}

			$table = PP::table('planAcl');
			$table->plan_id = $this->getId();
			$table->acl_id = $aclId;
			$table->acl_type = 'view';
...
class PayplansPlanxFormatter extends PayPlansFormatter
{
	...
	{
		$ignore = ['_trigger', '_component', '_name', '_errors','_blacklist_tokens'];
		return $ignore;
	}
	...
	function getVarFormatter()
	{
		$rules = [
			...
			'_groups' => [
				'formatter'=> 'PayplansGroupFormatter',
				'function' => 'getPlanGroups'
			],
			'params' => [
				'formatter'=> 'PayplansFormatter',
				'function' => 'getFormattedParams'
			],
			'plan_id' => [
				'formatter' => 'PayplansPlanFormatter',
