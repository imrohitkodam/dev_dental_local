...
class PPRegistrationEasySocial extends PPRegistrationAbstract
{
	...
	public function __construct()
	{
		...
		$this->url = ESR::registration(['layout' => 'steps', 'step' => 1], false);
	}
	 ...
	 * @access	public
	 */
	public function updateNewUserPublishState($invoice, $userId)
	{
		if (!$this->exists()) {
			return;
		}

		// Skip this if the current subscription is free
		if ($invoice->isFree()) {
			return;
		}

		// Check for the profile registration type whether set to auto login type
		if ($this->requireUserAutoLogin()) {

			// update the user state after they complete the registration form.
			$user = ES::user($userId);
			$user->block = 1;
			$user->state = 0;
			$user->save();
	...
	public function onPayplansAccessCheck()
	{
		...
		if ($option == 'com_payplans' && $view == 'thanks') {
			...
			// Make user auto login base on their profile registration type
			if ($this->requireUserAutoLogin()) {
				$this->login();				
			}
    	...
    	// Redirect user to plan listing page if user decide to click switch profile type or registration link on the page.
    	if (empty($isAjaxCall) && $this->isStopProceedRegistrationProcess()) {
	...
	private function checkRegistrationAccess($registerFromPayplans, $oauthRegisterFromEasysocial)
	{
		...
		if ($view == 'registration') {
			...
			// User is trying to activate their registration or confirm email
			if ($task == 'activate' || $task == 'confirmationUserEmail') {
				return true;
			}
		...
		if ($view == 'dashboard') {
			...
			if ($this->session->get('PP_EASYSOCIAL_REGISTRATION_TYPE_AUTOLOGIN') == 1 && $task != 'oauthLinkAccount') {
				...
				if ($this->getInvoiceId()) {
					...
					// Retrieve the new register user id
					$newRegisterId = $this->session->get('REGISTRATION_USER_ID');
					$invoice = PP::invoice($this->getInvoiceId());

					// update the user state if necessary
					$this->updateNewUserPublishState($invoice, $newRegisterId);
	...
	/**
	 * Determines if the registration require user auto login
	 *
	 * @since	4.1.2
	 * @access	public
	 */
	public function requireUserAutoLogin()
	{
		// Retrieve the current subscribed user profile type id
		$profileId = $this->session->get('profile_id', '', SOCIAL_SESSION_NAMESPACE);
	...
	/**
	* Determines if user require to auto login after done the payment
	*
	* @since	4.1.2
	* @access	public
	*/
