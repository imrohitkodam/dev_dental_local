...
class PayPlansModel extends JModelLegacy
{
	 ...
	 * @access	public
	 */
	public function getState($key = null, $default = '')
	{
		$key = $this->key . '.' .$key;
	...
	public function getPrefix()
	{
		...
		$this->_prefix = PPJString::strtolower($r[1]);
		return $this->_prefix;
	}
	...
	public function loadRecords($queryFilters = array(), $queryClean = array(), $emptyRecord = false, $orderby = null)
	{
		...
		$this->db->setQuery($tmpQuery, $limitstart, $limit);
	...
	protected function _buildQueryFilter(&$query, $key, $value, &$temp)
	{
		...
		if (is_array($value)) {
			throw new Exception('COM_PAYPLANS_VALUE_FOR_FILTERS_MUST_BE_AN_ARRAY');
		}
