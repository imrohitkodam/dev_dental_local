...
class PPInvoice extends PPAbstract implements PPAppTriggerableInterface, PPDiscountableInterface, PPMaskableInterface, PPApiInvoiceInterface
{
	...
	public function reset($config = [])
	{
		...
		$this->table->checked_out = NULL;
		$this->table->checked_out_time	= NULL;
	...
	protected function loadPayment($invoiceId = null)
	{
		...
		$rows = $model->loadRecords($options, ['limit']);
	...
	protected function loadModifiers($invoiceId = null)
	{
		...
		$this->_modifiers = PPHelperModifier::get(['invoice_id' => $invoiceId], PP_INSTANCE_REQUIRE);
	...
	/**
	 * Determines if the recurring has a trial
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function hasRecurringWithTrials()
	{
		if (!$this->isRecurring()) {
			return false;
		}
		...
		$trials = [PP_PRICE_RECURRING_TRIAL_1, PP_PRICE_RECURRING_TRIAL_2];
	...
	public function getRewriterTokens()
	{
		...
		$addons = $addonModel->getAvailableServices();

		if ($addons) {
			$addonName = '';
			foreach ($addons as $addon) {
				$addonName = str_replace(' ', '_', $addon->getTitle());
				$data[$addonName."_TITLE"] = '';
				$data[$addonName."_VALUE"] = '';
			}
		}
	...
	public function getReferenceObject()
	{
		...
		return call_user_func(['PP', $objectType], $this->table->object_id);
	}
	...
	public function getTotal($number = 0)
	{
		...
		if ($number != 0 && $number != $this->getCounter()) {
			...
			$total = PPHelperModifier::getTotalByFrequencyOnInvoiceNumber($this->getModifiers(['frequency' => PP_MODIFIER_FREQUENCY_EACH_TIME]), $subtotal, $number);
		}
	...
	{
		$currency = PPFormats::currency(PP::getCurrency($this->currency), [], $format);
	...
	{
		$tax = PPHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), [PP_MODIFIER_FIXED_TAX, PP_MODIFIER_PERCENT_TAX]);
	...
	{
		$discount = PPHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), [PP_MODIFIER_FIXED_DISCOUNT, PP_MODIFIER_PERCENT_DISCOUNT, PP_MODIFIER_FIXED_NON_TAXABLE]);
	...
	{
		$discount = PPHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), [PP_MODIFIER_FIXED_DISCOUNTABLE, PP_MODIFIER_PERCENT_DISCOUNTABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_DISCOUNTABLE]);
		return PPFormats::price($discount);
	}
	...
	public function getTaxableAmount()
		{
			$taxable = PPHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), [PP_MODIFIER_PERCENT_TAXABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_TAXABLE]);
	...
	{
		$tax = PPHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), [PP_MODIFIER_FIXED_NON_TAXABLE, PP_MODIFIER_PERCENT_NON_TAXABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_NON_TAXABLE]);
		return PPFormats::price($tax);
	}
	 ...
	 * @access	public
	 */
	public function getModifiers($filters = [])
	{
		static $_cache = [];

		
		$key = $this->getId();
		$options = ['invoice_id' => $this->getId()];
	...
	public function getTransactions($reload = false)
	{
		...
		if (!isset($cache[$key]) || $reload) {
			...
			$data = $model->getItemsWithoutState(['invoice_id' => $this->getId()]);
	...
	public function getCurrentPaymentTransaction()
	{
		...
		$model = PP::model('Transaction');
		$data = $model->getItemsWithoutState(array('invoice_id' => $this->getId(), 'payment_id' => $payment->getId()));

		$transaction = false;

		if ($data) {
			$transaction = PP::transaction($data[0]);
		}
	...
	public function getPrice($number = 1)
	{
		...
		// No need to reduce counter number as we already sending number as expected counter
		...
		if (in_array($type, [PP_RECURRING_TRIAL_1, PP_RECURRING_TRIAL_2]) && $number == 1){
	...
	public function isRecurring()
	{
		...
		$recurringTypes = [PP_RECURRING, PP_RECURRING_TRIAL_1, PP_RECURRING_TRIAL_2];
	...
	public function getSerial()
	{
		if (is_null($this->serial)) {
			return JText::_('COM_PP_SERIAL_NA');
		}
	 ...
	 * @access	public
	 */
	public function getPaidDate($emptyAsNever = true)
	{
		if ($this->paid_date == '0000-00-00 00:00:00' && $emptyAsNever) {
	...
	public function getPdfContent($invoices)
	{
		...
		foreach ($invoices as $invId) {
			...
			$discountablesSerials = [PP_MODIFIER_FIXED_DISCOUNTABLE, PP_MODIFIER_PERCENT_DISCOUNTABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_DISCOUNTABLE];

			$discountables = $invoice->getModifiers([
				'serial' => $discountablesSerials, 
				'invoice_id' => $invoice->getId()
			]);
			// $discountables = $modifier->rearrange($discountables);

			$nonTaxesSerials = [PP_MODIFIER_FIXED_NON_TAXABLE, PP_MODIFIER_PERCENT_NON_TAXABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_NON_TAXABLE];
			
			$nonTaxables = $invoice->getModifiers([
				'serial' => $nonTaxesSerials, 
				'invoice_id'=>$invoice->getId()
			]);
			// $nonTaxables = $modifier->rearrange($nonTaxables);

			$taxableSerials = [PP_MODIFIER_PERCENT_TAXABLE, PP_MODIFIER_PERCENT_OF_SUBTOTAL_TAXABLE];
			$taxables = $invoice->getModifiers([
				'serial'=> $taxableSerials, 
				'invoice_id'=>$invoice->getId()
			]);
			...
			$discountablesSerials = array_merge($discountablesSerials, [PP_MODIFIER_FIXED_DISCOUNT,PP_MODIFIER_PERCENT_DISCOUNT]);
			$taxableSerials = array_merge($taxableSerials, [PP_MODIFIER_FIXED_TAX,PP_MODIFIER_PERCENT_TAX]);
	...
	public function setNewSerial()
	{
		...
		$search  = ['[[number]]', '[[date]]', '[[month]]', '[[year]]','[[day]]'];
		$replace = [$lastCounter, $paidDate, $paidMonth, $paidYear, $paidDay];
		...
		if (strstr($serialFormate , '[[number]]')) {
			...
			$model->save([
				'expert_invoice_last_serial' => $lastCounter
			]);
	...
	{
		$order = $this->getReferenceObject();
		if (($order instanceof PPOrder) == false) {
			return false;
		}
		...
		if ($order->getFirstInvoice()->getId() == $this->getId()) {
			return false;
		}
		
		$payment = $this->getPayment();
		if (($payment instanceof PPPayment) == false) {
	...
	/**
	 * Check invoice paid for recurring payment
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function isRecurringRenewal()
	{
		$order = $this->getReferenceObject();
		if (($order instanceof PPOrder) == false) {
			return false;
		...
		// do nothing is it's not recurring plan
		$recurring = $this->isRecurring();
		if (!$recurring) {
			return false;
		}
		...
		// do nothing if it's manaul renewal
		$params = $this->getParams();
		$isRenewal = $params->get('isRenewal', false);
		if ($isRenewal) {
			return false;
		}
	...
	*/
	public function isReferralApplicable()
	{
		...
		$options = ['type' => 'referral', 'invoice_id' => $this->getId()];
	...
	/**
	 * Retrieves the discount applied on first payment for recurring payment.
	 *
	 * i.e: To determine whether we need to decrease the recurrence count
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function isRecurringOnetimeDiscountApplied()
	{
		$params = $this->getParams();

		return $params->get('recurring_onetime_discount', false);
	}
