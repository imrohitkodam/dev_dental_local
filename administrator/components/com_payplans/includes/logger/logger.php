...
class PPLogger extends PayPlans
{
	 ...
	 * @access	public
	 */
	public function log($level, $message, $object_id, $class, $content = null, $type = null, $token = null, $sendemail= false, $userEmail = '')
	{
		if ($level < $this->level) {
			return false;
		}
		...
		if ($level == self::LEVEL_ERROR && $sendemail) {
			...
			// Send email to user when payment failed
			if ($userEmail) {

				$jconfig = PP::jconfig();
				$siteName = $jconfig->get('sitename');
		
				$subject = JText::sprintf('COM_PAYPLANS_USER_PAYMENT_FAILURE_EMAIL_SUBJECT', $siteName);
				...
				$mailer->send($userEmail, $subject, $namespace, $params);
			}
		}
	 ...
	 * @access	public
	 */
	protected function save($level, $message, $objectId, $class, $content = null, $type = null, $token='')
	{
		...
		$model->save(['position' => $position, 'legacy' => 0], $id);		
