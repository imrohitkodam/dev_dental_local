...
class PPEventOrder extends PayPlans
{
	...
	protected function processNewTransaction($invoice, PPOrder $order, $transaction, $amount)
	{
		...
		if ($order->isHold() || $order->isCancelled()) {
			return true;
		}
	 ...
	 * @access	public
	 */
	public static function onPayplansPaymentAfterSave($previousPayment = null, PPPayment $newPayment = null)
	{
	...
	public static function onPayplansNewPaymentRequest($subscription)
	{
		...
		if (!isset($subscription->inprocess)) {
			...
			$allPaidInvoices = $order->getInvoices(PP_INVOICE_PAID, true);
			$totalPaidInvoices = count($allPaidInvoices);
			...
			// check recurring payment process by cron
			...
			if (!in_array($order->getStatus(), [PP_ORDER_CANCEL, PP_ORDER_EXPIRED])
	...
	public static function onPayplansInvoiceBeforeSave($prev, $new)
	{
		...
		if (isset($prev) && in_array($prev->getStatus(), [PP_INVOICE_CONFIRMED, PP_NONE]) && $new->getStatus() == PP_INVOICE_PAID) {
			$new->setNewSerial();
		}
	...
	protected static function checkInvoiceCounter($order, $invoice)
	{
		...
		$invoices = $order->getInvoices([PP_INVOICE_PAID, PP_INVOICE_REFUNDED], true);
