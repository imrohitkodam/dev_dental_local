...
class PPEventUpgrade extends PayPlans
{
	...
	public function onPayplansSubscriptionBeforeSave($previous = null, $current = null)
	{
		...
		if (!$oldSub->isRecurring()) {
			return true;
		}

		$oldInvoices = $oldOrder->getInvoices(PP_INVOICE_PAID, true);
		$oldInvoice = array_shift($oldInvoices);

		if ($oldInvoice && $oldInvoice != null) {
			$oldPayment = $oldInvoice->getPayment();
		} else {
			// Get payment from order, if invoice is not exist 
			$oldPayment = $oldOrder->getPayment();
		}

		$supportPaymentCancel = 1;
		if (!empty($oldPayment) && $oldPayment instanceof PPPayment) {
		...
		if ($supportPaymentCancel) {
			...
			$content = [
				'user' => $oldInvoice->getBuyer()->getId(), 
