...
class PPHelperApp
{
	...
	static public function getTags($merged=false, $what = 'tags')
	{
		if(self::$tags === null){
			...
			if ($xml !== null){
				foreach($xml as $key => $array){
					self::$tags[$key] = isset($array[$what])? $array[$what] : [];
				}
			}
	...
	public static function getResource($userid, $groupid, $resource)
	{
		...
		$record->subscription_ids = PPJString::trim($record->subscription_ids, ',');
	...
	public static function addToResource($subId, $userid, $groupid, $resource, $count = 0)
	{
		...
		if($record){
			...
			$record->subscription_ids 	= empty($record->subscription_ids) ? [] : explode(',', $record->subscription_ids);
...
dump($allApps);
		...
		$purposeApps[$purpose] = [];
		$purposeClass = 'PayplansApp'.PPJString::ucfirst($purpose);

		foreach ($allApps as $app) {
			$appClass = 'PayplansApp'.PPJString::ucfirst($app);
