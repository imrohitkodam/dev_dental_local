...
class PayplansControllerUpgrades extends PayPlansController
{
	...
	public function delete()
	{
		...
		foreach ($ids as $id) {
			...
			$actionlog->log('COM_PP_ACTIONLOGS_UPGRADES_DELETED', 'upgrades', [
				'upgradeTitle' => $title,
			]);
		}

		$this->info->set('COM_PP_UPGRADE_DELETED_SUCCESS', 'success');
	...
	public function save()
	{
		...
		if ($state === false) {
			$this->info->set('COM_PP_UPGRADE_SAVED_FAILED', 'danger');

			return $this->redirectToView('upgrades', 'form');
		}

		$actionString = 'COM_PP_ACTIONLOGS_UPGRADES_CREATED';

		if ($id) {
			$message = 'COM_PP_UPGRADE_SAVED_SUCCESS';
			$actionString = 'COM_PP_ACTIONLOGS_UPGRADES_UPDATED';
		}

		$actionlog = PP::actionlog();
		$actionlog->log($actionString, 'upgrades', [
			'upgradeTitle' => $app->getTitle(),
			'upgradeLink' => 'index.php?option=com_payplans&view=upgrades&layout=form&id=' . $app->getId()
		]);
	...
	// 	if (!$invoice) {
	// 		$this->info->set('There was an error when trying to create the invoice for the upgraded plan. Please try upgrading the subscription again.', 'danger');
	...
	public function togglePublish()
	{
		...
		$actionlog = PP::actionlog();
		$actionString = $task == 'publish' ? 'COM_PP_ACTIONLOGS_UPGRADES_PUBLISHED' : 'COM_PP_ACTIONLOGS_UPGRADES_UNPUBLISHED';
		...
		foreach ($ids as $id) {
			...
			$actionlog->log($actionString, 'upgrades', [
				'upgradeTitle' => $table->title,
				'upgradeLink' => 'index.php?option=com_payplans&view=upgrades&layout=form&id=' . $table->app_id
			]);
		}
