...
class PayplansControllerConfig extends PayPlansController
{
	...
	public function save()
	{
		...
		$companyLogo = $this->input->files->get('companyLogo');
		...
		$actionlog->log('COM_PP_ACTIONLOGS_SETTINGS_UPDATE', 'config', [
			'section' => ucfirst($page),
			'sectionLink' => 'index.php?option=com_payplans&view=config&layout=' . $page
		]);
	...
	public function removeLogo()
	{
		...
		$message = JText::_('COM_PP_COMPANY_LOGO_DELETED_SUCCESSFULLY');

		if (!$state) {
			$message = JText::_('COM_PP_COMPANY_LOGO_DELETION_ERROR');
		}
