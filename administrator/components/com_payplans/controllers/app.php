...
class PayplansControllerApp extends PayPlansController
{
	...
	public function delete()
	{
		...
		foreach ($ids as $id) {
			...
			if (!$app->canDelete()) {
				$this->info->set('COM_PP_UNABLE_TO_DELETE_APP', 'danger');
				return $this->redirectToView('app');
			}

			$app->delete();

			$actionlog->log('COM_PP_ACTIONLOGS_APP_DELETED', 'app', array(
	...
	public function togglePublish()
	{
		...
		$task = $this->getTask();
		$actionlog = PP::actionlog();
		$actionString = 'COM_PP_ACTIONLOGS_APP_' . strtoupper($task) . '_';
		...
		foreach ($ids as $id) {
			...
			$actionlog->log($actionString . strtoupper($view), $view, array(
					'appTitle' => $app->getTitle(),
					'appLink' => 'index.php?option=com_payplans&view=' . $view . '&layout=form&id=' . $app->getId()
			));
		}
	...
	public function store()
	{
		...
		$actionString = 'COM_PP_ACTIONLOGS_APP_UPDATED';
		...
		if (!$id) {
			...
			$actionString = 'COM_PP_ACTIONLOGS_APP_CREATED';
		}
		...
		$actionlog = PP::actionlog();
		$actionlog->log($actionString, 'app', array(
				'appTitle' => $app->getTitle(),
				'appLink' => 'index.php?option=com_payplans&view=app&layout=form&id=' . $app->getId()
		));	
