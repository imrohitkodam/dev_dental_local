...
<form name="adminForm" id="adminForm" method="post" data-fd-grid>
	...
	<div class="app-filter-bar">
		...
		<?php echo $this->html('filter.group', 'group_id', $states->group_id, [], [
				'none' => JText::_('COM_PP_SELECT_A_GROUP')
		]); ?>
		...
		<table class="app-table table" data-table>
			<thead>
				<tr>
					...
					<th>
						<?php echo $this->fd->html('table.sort', 'COM_PAYPLANS_PLAN_GRID_PLAN_TITLE', 'title', $states->ordering, $states->direction); ?>
					</th>
					...
					<th width="10%" class="center">
						<?php echo $this->fd->html('table.sort', 'COM_PAYPLANS_PLAN_GRID_PLAN_PUBLISHED', 'published', $states->ordering, $states->direction); ?>
					</th>

					<?php if ($this->tmpl != 'component') { ?>
					<th width="5%" class="center">
						<?php echo $this->fd->html('table.sort', 'COM_PAYPLANS_PLAN_GRID_PLAN_VISIBLE', 'visible', $states->ordering, $states->direction); ?>
					</th>
					<?php } ?>
					...
					<th width="5%" class="hidden-phone center">
						<?php echo $this->fd->html('table.sort', 'COM_PAYPLANS_PLAN_GRID_PLAN_ORDERING', 'ordering', $states->ordering, $states->direction); ?>
				...
				<?php if ($plans) { ?>
					...
					<tr>
						...
						<td class="pp-word-wrap">
							<a 
								href="<?php echo $this->tmpl == 'component' ? 'javascript:void(0);' : 'index.php?option=com_payplans&view=plan&layout=form&id=' . $row->plan_id;?>"
								data-pp-plan-item
								data-title="<?php echo $this->html('string.escape', $row->title);?>"
								data-id="<?php echo $row->plan_id;?>"
								data-permalink="<?php echo $row->getSelectPermalink(false, true) . '&Itemid=' . PP::getMenuIdForView('plan');?>"
						...
						<td class="center order">
							...
							<?php echo $this->fd->html('table.order', $pagination, 'order', $i, $saveOrder, [
									...
									'showOrderDownIcon' => $current !== count($plans),
									'orderUpTask' => 'plan.moveUp',
									'orderDownTask' => 'plan.moveDown',
