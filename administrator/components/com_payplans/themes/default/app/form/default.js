PayPlans.ready(function($) {
	...
	function appValidation() {
		...
		// determine if any field is required
		$(wrapperAppForm).find('input, textarea, select').each(function(){
			if($(this).prop('required')){

				var value = $(this).closest('.o-form-group').find('.o-form-control').val();
				if (value == '') {
					$(this).closest('.o-form-group').addClass('has-error');
					labelText = $(this).closest('.o-form-group').find('.o-control-label').text().trim();
				
					msg.push(labelText + " is required");
					isValid = false;
				}
			}
		});
