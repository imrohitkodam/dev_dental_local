...
.done(function($) {
	...
	$.Joomla('submitbutton', function(task) {
		...
		if (task == 'addTransaction') {
			window.location = "<?php echo JURI::base();?>index.php?option=com_payplans&view=transaction&layout=form&invoice_id=<?php echo $invoice->getId();?>&from=<?php echo base64_encode(PP::getURI());?>";
			return;
		}

		if (task == 'sendInvoiceLink') {
			window.location = "<?php echo JURI::base();?>index.php?option=com_payplans&view=invoice&layout=emailform&id=<?php echo $invoice->getId();?>&from=<?php echo base64_encode(PP::getURI());?>";
			return;
		}
	...
	$('[data-form-select-dropdown]').on('change', function() {
		...
		$('.expirationtype').addClass('t-hidden');

		$.each(items, function(key, value) {
			$('.expirationtype.' + value).removeClass('t-hidden');
		});
	});

	$('[data-form-select-dropdown]').trigger('change');

	$('[data-pp-discount-apply]').on('click.discount.apply', function() {
