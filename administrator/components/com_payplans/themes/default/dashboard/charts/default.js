...
.done(function($) {
	...
	function renderSalesChart(salesFigure) {
		$('[data-dashboard-content-tab]').find('[data-fd-loader-wrapper]').addClass('t-hidden');
		...
		new Chart('chart-revenue', {
			...
			data: {
				labels: dateLabels,
				datasets: [{
					backgroundColor: FD.getStyleToken('--fd-primary-300', {method: 'rgb', opacity: '.05'}),
	...
	function renderPlansChart(plansFigure) {
		...
		wrapper.find('[data-fd-loader-wrapper]').addClass('t-hidden');
	...
	PayPlans.ajax('admin/controllers/dashboard/getStatistic')
