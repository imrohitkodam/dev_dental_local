...
class PayPlansViewUpgrades extends PayPlansAdminView
{
	...
	public function getUpgradeDetails()
	{
		...
		$result = PPUpgrade::calculateUnutilizedValue($old->subscription, $old->plan, $newPlan);
		...
		$taxes = PPUpgrade::calculateTaxes($old->invoices, $planPrice);
		$payableAmount += $taxes;

		// Once we calculate the discount, we need to update the paybale amount
		$discounts = PPUpgrade::calculateDiscounts($newPlan, $payableAmount, $old->invoices);
