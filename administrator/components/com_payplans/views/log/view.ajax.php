...
class PayplansViewLog extends PayPlansAdminView
{
	...
	public function exportLogs()
	{
		...
		// Get total record to process
		$logs = $model->getLogsToExport();

		if (count($logs) == 0) {
			return $this->ajax->resolve('nodata');
		}
		...
		foreach ($logs as $log) {
			...
			$logTable = PP::table('Log');
			$logTable->load($log->log_id);

			list($type, $content) = $model->readBaseEncodeLog($log);
			...
			$ownerId = $model->getOwnerId($content);
			$tokenExist = $model->getPreviousLogPosition($token, $log->class);
			...
			if ($tokenExist) {
				...
				$logTable->current_token = $currentToken;
				$logTable->owner_id = $ownerId;
				$logTable->position = $position;
				$logTable->content = $content;
				$state = $logTable->store();
				$logExported++;

				continue;
			}

			$previousToken = $model->getPreviousLogToken($log->object_id,$log->class, $content);

			if ($previousToken || (isset($content['previous']) && empty($content['previous']))) {
				$content = $content['current'];
			}

			$content = serialize(array('type' => $type, 'content' => $content));
			$content = json_encode($content);
			$contentToDump = '<' . $token . '>' . $content . '<' . $token . '>' . "\n";

			$position = $model->dumpDataIntoFile($log->log_id, $contentToDump);
			$content = '';
			...
			$logExported++;
			$logIds[] = $logTable->log_id;
		}

		$totalLeft = count($logs) - $logExported;

		if ($totalLeft > 0) {
			return $this->ajax->resolve('next', $logIds);
		}

		$actionlog = PP::actionlog();
		$actionlog->log('COM_PP_ACTIONLOGS_LOGS_EXPORT', 'log');

		return $this->ajax->resolve('done', $logIds);
	}
}
