...
class fsj_fssaddViewcanned extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$text = JText::_(($checkedOut ? 'FSJ_VIEW' : ($isNew ? 'FSJ_ADD' : 'FSJ_EDIT'))) . " " . JText::_('COM_fsj_fssadd_ITEMS_fssadd_canned');
		...
		if ($isNew && (count($user->getAuthorisedCategories('com_fsj_fssadd', 'core.create')) > 0)) {
			if ($can_save)
			{
				JToolBarHelper::apply('canned.apply');
				JToolBarHelper::save('canned.save');
				JToolBarHelper::save2new('canned.save2new');
			}
			JToolBarHelper::cancel('canned.cancel');
		}
		else {
			...
			if ($canDo->get('core.create')) {
				JToolBarHelper::save2copy('canned.save2copy');
			}

			JToolBarHelper::cancel('canned.cancel', 'JTOOLBAR_CLOSE');
		}
