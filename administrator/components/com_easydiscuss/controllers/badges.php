...
class EasyDiscussControllerBadges extends EasyDiscussController
{
	...
	public function save()
	{
		...
		if (!$badge->title || !$badge->description) {
			ED::setMessage(JText::_('COM_ED_BADGES_SAVED_ERROR_MESSAGE'), DISCUSS_QUEUE_ERROR);

			return ED::redirect($redirect . '&layout=form');
		}

		if (!$badge->avatar) {
			ED::setMessage(JText::_('COM_ED_BADGES_MISSING_AVATAR_ERROR_MESSAGE'), DISCUSS_QUEUE_ERROR);
		...
		if (!$state) {
			ED::setMessage($badge->getError(), DISCUSS_QUEUE_ERROR);
