...
class EasyDiscussControllerSubscription extends EasyDiscussController
{
	...
	public function save()
	{
		...
		if (!$subscription->fullname || !$subscription->email) {
			ED::setMessage('COM_ED_SUBSCRIPTION_NAME_EMAIL_REQUIRED', ED_MSG_ERROR);
			$redirect = 'index.php?option=com_easydiscuss&view=subscription&layout=form&id=' . $subscription->id;
			return ED::redirect($redirect);
		}

		// check if the email is a valid email address or not. #1445
		if(!ED::string()->isValidEmail($subscription->email)) {
			ED::setMessage('COM_ED_SUBSCRIPTION_EMAIL_INVALID', ED_MSG_ERROR);
			$redirect = 'index.php?option=com_easydiscuss&view=subscription&layout=form&id=' . $subscription->id;
			return ED::redirect($redirect);
		}
		...
		// if this is new subscriptions, lets check if the email has an user account or not.
		if ($isNew && !$subscription->userid) {
			$tmpUser = ED::getUserByEmail($subscription->email);

			if ($tmpUser && $tmpUser->id) {
				$subscription->userid = $tmpUser->id;
			}
		}
		...
		// if this is a new subscription, check if user already subscribed or not.
		if ($isNew) {
			$table = ED::table('Subscribe');

			$options = ['type' => $subscription->type, 'email' => $subscription->email];
			$table->load($options);

			if ($table && $table->id) {

				$errorMsg = JText::sprintf('COM_ED_SUBSCRIPTION_EXISTS', $subscription->type, $subscription->email);

				ED::setMessage($errorMsg, ED_MSG_ERROR);
				$redirect = 'index.php?option=com_easydiscuss&view=subscription&layout=form&id=' . $subscription->id;
				return ED::redirect($redirect);
			}
		}
