...
class EasyDiscussComment extends EasyDiscuss
{
	...
	/**
	 * Retrieves the date source of the comment
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	public function getDate()
	{
		...
		if (!isset($cache[$this->id])) {
			$dateSource = $this->config->get('main_comment_date_source');
			$date = '';
			...
			if ($dateSource !== 'default') {
				...
				$date = ED::date($this->table->$dateSource)->display($dateFormat);
			}
	...
	public function postSave()
	{
		...
		ED::history()->log('easydiscuss.new.comment', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_NEW_COMMENT', $post->id);
