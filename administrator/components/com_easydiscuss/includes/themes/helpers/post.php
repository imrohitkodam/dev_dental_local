...
class EasyDiscussThemesHelperPost extends EasyDiscussHelperAbstract
{
	 ...
	 * @access	public
	 */
	public function creationDate(EasyDiscussPost $post, $force = false, $format = null)
	{
		static $cache = [];

		$config = ED::config();

		if (!$force && !$config->get('layout_post_creationdate', false)) {
			return;
		}
		...
		if (!isset($cache[$key])) {
			...
			$dateDisplayFormat = $this->config->get('layout_post_creationdate_format', 'default');

			$dateString = ED::date()->toLapsed($post->created);

			$dateString = ED::date()->toLapsed($post->lastupdate);
			if ($dateDisplayFormat !== 'default') {

				$dateFormat = $format ? $format : ED::date()->getDateFormat(JText::_('DATE_FORMAT_LC2'));

				$dateString = ED::date($post->created)->display($dateFormat);
				$displayLabel = false;
			}

			$theme = ED::themes();
			$theme->set('dateString', $dateString);
			$cache[$key] = $theme->output('site/helpers/post/creation.date');
		}
	...
	public function lastReplied(EasyDiscussPost $post)
	{
		...
		if (!isset($cache[$key])) {
			...
			$replyDateString = ED::date()->toLapsed($post->lastupdate);
			if ($dateDisplayFormat !== 'default') {
				$replyDateString = ED::date($post->lastupdate)->display(ED::date()->getDateFormat(JText::_('DATE_FORMAT_LC2')));
			}

			// Retrieve the guest user name if the last reply created by guest user
			$lastReplyPosterName = isset($replier->name) && $replier->name ? $replier->name : '';
			$options = ['popbox' => true, 'posterName' => $lastReplyPosterName];

			$theme = ED::themes();
			$theme->set('replier', $replier);
			$theme->set('post', $post);
			$theme->set('replyDateString', $replyDateString);
