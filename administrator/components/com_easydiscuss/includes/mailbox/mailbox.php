...
class EasyDiscussMailbox extends EasyDiscuss
{
	...
	public function __construct()
	{
		...
		// if the POP3 mail server doesn't activate all of these then use this /notls
		if ($this->service === 'pop3' && !$this->ssl && !$this->validate_cert) {
			$this->flags = '/' . $this->service . '/notls'; 
		}
	...
	public function testConnect($server, $port, $service, $ssl, $mailbox, $user, $pass, $validate)
	{
		$flags = '';
	...
	public function searchMessages($criteria = '')
	{
		...
		return imap_search($this->stream, $searchCriteria);
	}
