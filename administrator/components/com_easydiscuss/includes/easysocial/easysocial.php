...
class EasyDiscussEasySocial
{
	...
	public function notify($action, $post, $question = null, $comment = null, $actor = null)
	{
		...
		if ($action === 'accepted.answer') {
			...
			$options = [
				'actor_id' => $actor,
				'uid' => $post->id,
				'title' => JText::sprintf('COM_EASYDISCUSS_EASYSOCIAL_NOTIFICATION_ACCEPTED_ANSWER', $question->title),
				'type' => 'discuss',
				'url' => $permalink
			];
		...
		if ($action === 'accepted.answer.owner') {
			...
			$options = [
				'actor_id' => $actor,
				'uid' => $post->id,
				'title' => JText::sprintf('COM_EASYDISCUSS_EASYSOCIAL_NOTIFICATION_ACCEPTED_ANSWER_OWNER', $question->title),
				'type' => 'discuss',
				'url' => $permalink
			];
	...
	{
		return $this->loadCluster($id, $type);
	}

	public function getPostsCluster($options = array())
	{
		$model = ED::model('clusters');

		$posts = $model->getPostsCluster($options);

		return $posts;
	}

	public function formatPostsCluster($posts, $clusterId, $clusterType)
	{
		$threads = array();
		...
		foreach ($posts as $post) {
			...
			if (!isset($threads[$post->cluster_id])) {
				$thread = new stdClass();
				$thread->cluster = ES::$clusterType($post->cluster_id);
				$thread->posts = array();

				$threads[$post->cluster_id] = $thread;
			}

			$threads[$post->cluster_id]->posts[] = $post;
		}

		return $threads;
	}

	public function renderMiniHeader($clusterId, $clusterType = 'group', $showCreateButton = true)
	{
		if (!$this->exists()) {
			return;
		}
		...
		// load the group
		$cluster = $this->loadCluster($clusterId, $clusterType);
		...
		echo $themes->output('site/clusters/header.easysocial', array('cluster' => $cluster, 'clusterType' => $clusterType, 'returnUrl' => $returnUrl, 'showCreateButton' => $showCreateButton));
		echo '</div>';
	...
	{
		return $this->loadCluster($groupId, 'group');
	}

	public function loadCluster($clusterId, $clusterType = '')
	{
		if (!$this->exists()) {
			return false;
		}
		...
		if (isset($cluster->id) && $cluster->id) {
			return $cluster;
		}
	...
	public function isClusterAppExists($type)
	{
		if (!$this->exists()) {
			return false;
		}

		$groups = ['group', 'page', 'event'];
	...
	public function isClusterPost(EasyDiscussPost $post)
	{
		if (!$post->id) {
			return false;
		}

		if ($post->cluster_id && $post->cluster_type) {
			return true;
		}
