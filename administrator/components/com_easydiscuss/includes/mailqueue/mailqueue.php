...
class EasyDiscussMailQueue extends EasyDiscuss
{
	...
	private function processEmails($mailer = '', $category = '')
	{
		...
		// Provide the email parser a criteria on the search
		$searchCriteria = $this->config->get('main_email_parser_prefix');
	...
	{
		// if the user is not in the whitelist, block him
	...
	private function isWhitelisted($email)
	{
		...
		// if the sender is not in the list, return false
		if (!in_array($email, $emails)) {
