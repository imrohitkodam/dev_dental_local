...
class ED
{
	...
	/**
	 * Method to get user id based on the name
	 *
	 * @since	5.0.0
	 * @access	public
	 */
	public static function getMentionUserId($name, $hash = '')
	{
		$config = ED::config();
		...
		$query = 'SELECT `id` FROM `#__users` WHERE `name` = ' . $db->quote($name);

		if ($nameFormat == 'nickname') {
			$collation = ED::getUsersTableCollation('ed', 'discuss_users');

			$query = 'select a.`id` from `#__discuss_users` as a';
			$query .= ' inner join `#__users` as u on a.`id` = u.`id`';
			$query .= ' where if(a.`nickname` = ' . $db->Quote('') . ', u.`name`, a.`nickname` ' . $collation . ') = ' . $db->quote($name);
		}

		if ($nameFormat == 'username') {
		...
		$userId = $results ? $results[0] : 0;

		if (count($results) > 1 && $hash) {
			// we need to filter the correct user id based on the hash passed in.
			foreach ($results as $id) {
				$hashed = ED::string()->getUserIdHash($id);
	...
	public static function getUserId($username, $isAlias = false)
	{
		...
		if (!isset($userids[$type][$username]) || empty($userids[$type][$username])) {
			...
			if (!$isAlias && $nameFormat == 'nickname') {
				...
				$query = 'select a.`id` from `#__discuss_users` as a';
				$query .= ' inner join `#__users` as u on a.`id` = u.`id`';
				$query .= ' where if(a.`nickname` = ' . $db->Quote('') . ', u.`name`, a.`nickname`) = ' . $db->quote($username);
				$query .= ' LIMIT 1';
	 ...
	 * @access	public
	 */
	public static function getUsersTableCollation($from, $fromtbl = 'discuss_subscription')
	{
					...
					if (version_compare($dbversion, '5.1', '>=')) {
						...
						if (strpos($collation, 'mb4_') !== false) {
							...
							if ($from == 'ed') {
								...
								// make sure there is a table being specified.
								$fromtbl = $fromtbl ? $fromtbl : 'discuss_subscription';

								$tableName2 = $prefix . $fromtbl;
								// dump($tableName2);
	...
	public static function populateCategories($parentId, $userId, $outType, $eleName, $default = false, $isWrite = false, $isPublishedOnly = false, $showPrivateCat = true, $disableContainers = false, $customClass = 'form-control', $exclusion = array(), $aclType = DISCUSS_CATEGORY_ACL_ACTION_VIEW, $sorting = false, $multiple = false, $containerOnly = false, $attributes = array())
	{
		...
		// allow 3rd party plugin to manipulate the params before getting the categories
		$arguments = array(&$exclusion, &$isPublishedOnly, &$showPrivateCat, &$sorting);
		EDDispatcher::trigger('onEasyDiscussBeforePopulateCategories', $arguments);
	...
	public static function getUserByEmail($email)
	{
		...
		$query .= ' WHERE LOWER(' . $db->nameQuote('email') . ') = ' . $db->Quote($email);
	...
	public static function user($ids = null, $resetCache = false, $debug = false)
	{
		...
		return EasyDiscussUser::factory($ids, $resetCache, $debug);
	}
...
<?php
	...
	/**
	 * Determine if the curent JApplication is on CLI mode or not.
	 *
	 * @since	5.0.11
	 * @access	public
	 */
