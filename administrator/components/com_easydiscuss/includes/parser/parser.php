...
class EasyDiscussParser extends EasyDiscuss
{
		...
		if (!empty($matches) && isset($matches[0]) && !empty($matches[0])) {
			...
			foreach ($links as $link) {
				...
				if ($oembed && isset($oembed->html)) {
					...
					if (!ED::string()->isAscii($text)) {
						$text = ED::string()->cleanInvalidUTF8Characters($text);
					}
	...
	public function replaceMentions($text, $debug = false)
	{
		...
		foreach ($users as $user) {
			...
			// form the name for search replace
			$hash = ED::string()->getUserIdHash($user->id); 
			$nameWithHash = $name . ':' . $hash; 

			$search = array('@' . $nameWithHash . '#', '@' . $name . '#');
