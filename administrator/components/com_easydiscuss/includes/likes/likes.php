	...
	public function unlike(EasyDiscussPost $post)
	{
		...
		if ($post->user_id != $this->my->id) {
			if ($post->isQuestion()) {
				...
				ED::history()->log('easydiscuss.discussion.unlike', $post->user_id, 'COM_ED_BADGES_HISTORY_DISCUSSION_UNLIKE', $post->id);
			...
			if ($post->isReply()) {
				...
				ED::history()->log('easydiscuss.reply.unlike', $post->user_id, 'COM_ED_BADGES_HISTORY_REPLY_UNLIKE', $post->id);
	...
	public function like(EasyDiscussPost $post)
	{
		...
		if ($post->user_id != $this->my->id) {
			...
			if ($post->isQuestion()) {
				...
				ED::history()->log('easydiscuss.like.discussion', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_LIKE_DISCUSSION', $post->id);
				...
				ED::history()->log('easydiscuss.discussion.like', $post->user_id, 'COM_ED_BADGES_HISTORY_DISCUSSION_LIKE', $post->id);
			...
			if ($post->isReply()) {
				// Add logging for user.
				ED::history()->log('easydiscuss.like.reply', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_LIKE_REPLY', $post->id);

				ED::badges()->assign('easydiscuss.like.reply', $this->my->id);
				ED::points()->assign('easydiscuss.like.reply', $this->my->id);

				ED::easysocial()->assignBadge('like.reply', $this->my->id, JText::sprintf('COM_EASYDISCUSS_BADGES_HISTORY_LIKE_DISCUSSION', $post->title));

				// Add points to reply author
				ED::history()->log('easydiscuss.reply.like', $post->user_id, 'COM_ED_BADGES_HISTORY_REPLY_LIKE', $post->id);

				ED::badges()->assign( 'easydiscuss.reply.like' , $post->user_id);
				ED::points()->assign( 'easydiscuss.reply.like' , $post->user_id);

				ED::easysocial()->assignBadge('reply.like', $post->user_id, 'COM_ED_BADGES_HISTORY_REPLY_LIKE');
			}
