...
class EasyDiscussString 
{
	...
	public function detectNames($text, $exclude = [])
	{
		...
		foreach ($result as $name) {
			...
			$id = ED::getMentionUserId($name, $idHash);
	...
	/**
	 * Generate user id hash used in mention
	 *
	 * @since	5.0
	 * @access	public
	 */
	public function getUserIdHash($userId)
	{
		$data = hash('crc32', $userId, false);
		return $data;
	}
	...
	public static function replaceUrl($tmp, $text)
	{
		...
		$domainPatterns = ED::string()->getSupportedTld();

		foreach ($linkArrays as $link) {
			$text = str_ireplace($link->customcode, $link->newlink, $text);

			$patternReplace = '/' . $skipPattern . '|((?<!href=")((http|https):\/{2})+(([0-9a-z_-]+\.)+(' . $domainPatterns . ')(:[0-9]+)?((\/([~0-9a-zA-Z\#\!\=\+\%:@\.\/_-]+))?(\?[0-9a-zA-Z\+\#\%@\/&\[\]\.=_-]+)?)?))(\/|\b)/i';
	...
	public function getSupportedTld($separator = '|')
	{
		...
		if (is_null($tlds)) {
			$tlds = ED::config()->get('main_links_parsertld');
			$tlds = EDJString::trim($tlds, ',');

			// sort the value so that value with longer length will
			// stay at top.
			if ($tlds) {
				$tmpArr = explode(',', $tlds);
	...
	/**
	 * Clean up possible invalid utf8 characters
	 *
	 * @since	5.0.7
	 * @access	public
	 */
	public function cleanInvalidUTF8Characters($text)
	{
		$invalidChars = ['İ'];
		$replacements = ['Í'];

		$text = EDJString::str_ireplace($invalidChars, $replacements, $text);
		return $text;
	}
