...
class EDR
{
	 ...
	 * @access	public
	 */
	public static function getClusterRoute($id = 0, $type = '', $layout = '', $xhtml = true, $ssl = null)
	{
		$tmp = 'index.php?option=com_easydiscuss&view=clusters&cluster_type=' . $type;

		$url = self::_($tmp, $xhtml, $ssl);

		if ($id) {
			$url = self::_($tmp . '&cluster_id=' . $id, $xhtml, $ssl);
		}

		if ($id && $layout) {
			$url = self::_($tmp . '&cluster_id=' . $id . '&layout=listings', $xhtml, $ssl);
		}
	...
	public static function decodeAlias($alias, $tablename, $forceAlias = false)
	{
		...
		if ($config->get('main_sef_unicode') && !$forceAlias) {
			...
			$permalinkArr = explode(':', $permalinkSegment, 2);
