...
class EasyDiscussNotifications extends EasyDiscuss
{
	...
	public function format(&$items, $group = false)
	{
		...
		foreach ($items as $item) {
			...
			// format for the user achieved badge notification
			if ($item->type == 'badge' && count($title) == 1) {

				$pattern = "/\<b\>(.*?)\<\/b\>/ims";				
				preg_match($pattern, $title[0], $matches);

				if ($matches) {
					$postTitle = $matches[0];
					$action = str_ireplace($matches[0], '', $title[0]);
