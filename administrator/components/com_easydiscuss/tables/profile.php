...
class DiscussProfile extends EasyDiscussTable
{
	...
	*/
	...
	public function bindAvatar($file, $acl = array()) 
	{
		...
		ED::history()->log('easydiscuss.new.avatar', $this->id, 'COM_EASYDISCUSS_BADGES_HISTORY_UPDATED_AVATAR');
	...
	public function getName($default = '')
	{
		...
		if (!isset($cache[$this->id])) {
			...
			if (!$this->id) {
				...
				// need to retrieve the guest name
	...
	public function getLastOnline($front = false, $timelapse = true)
	{
		...
		if (!isset($cache[$key])) {
			...
			// User never logged in before, so there is no point checking against the date
			if ($user->lastvisitDate === $db->getNullDate()) {
				$cache[$key] = JText::_('COM_EASYDISCUSS_NEVER');
				return $cache[$key];
			}
