...
class EasyDiscussModelPoints extends EasyDiscussAdminModel
{
	...
	public function getPointsHistory($userId)
	{
		...
		// easydiscuss.update.custompoint rules is admin adjust the user current point from backend
		$defaultRules = [
							'easydiscuss.new.discussion', 
							...
							'easydiscuss.update.custompoint',
							'easydiscuss.unvote.question.reversed',
							'easydiscuss.vote.question.reversed',
							'easydiscuss.unvote.reply.reversed',
							'easydiscuss.vote.reply.reversed'
						];

		// Retrieve a list of published point rules
		$rulesPublished = $this->getPublishedPointRules();

		$rules = array_unique(array_merge($defaultRules, $rulesPublished));
		$inclusionsRules = [];
		...
		$query[] = 'IN (' . $db->Quote('easydiscuss.new.avatar') . ' , ' . $db->Quote('easydiscuss.update.profile') . ' , ' . $db->Quote('easydiscuss.update.custompoint') . ')';
