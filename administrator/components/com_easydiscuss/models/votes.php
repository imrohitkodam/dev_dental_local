...
class EasyDiscussModelVotes extends EasyDiscussAdminModel
{
		...
		if ($state) {
			...
			if ($post->isReply()) {
				...
				if ($votes->value == '1') {
					...
					$history->log('easydiscuss.vote.reply.reversed', $userId, 'COM_ED_POINTS_HISTORY_UPVOTE_REPLY_UNDO', $post->id, false);
					...
					$history->log('easydiscuss.unvote.reply.reversed', $userId, 'COM_ED_POINTS_HISTORY_DOWNVOTE_REPLY_UNDO', $post->id, false);
				...
				if ($votes->value == '1') {
					...
					$history->log('easydiscuss.vote.question.reversed', $userId, 'COM_ED_POINTS_HISTORY_UPVOTE_QUESTION_UNDO', $post->id, false);
				} else {
					// Voted -1
					// retrieve back how many point that user gain it just now
					$points = ED::points()->getPoints('easydiscuss.unvote.question');

					$history->log('easydiscuss.unvote.question.reversed', $userId, 'COM_ED_POINTS_HISTORY_DOWNVOTE_QUESTION_UNDO', $post->id, false);
				}
			}
	...
	/**
	 * Delete the votes that associated to the post id given
	 *
	 * @since   5.0.9
	 * @access  public
	 */
	public function deleteVotes($postId)
	{
		$db = ED::db();
		$query = 'DELETE FROM `#__discuss_votes` WHERE `post_id` = ' . $db->Quote($postId);
