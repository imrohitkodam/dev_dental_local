...
class EasyDiscussModelSubscribe extends EasyDiscussAdminModel
{
	...
	public function _buildQuery()
	{
		...
		$filter	= $this->app->getUserStateFromRequest('com_easydiscuss.subscription.filter', 'filter', 'site', 'word');
	...
	{
		$filter_order = $this->app->getUserStateFromRequest('com_easydiscuss.subscription.filter_order', 'filter_order', 'fullname', 'cmd');
		$filter_order_Dir = $this->app->getUserStateFromRequest('com_easydiscuss.subscription.filter_order_Dir', 'filter_order_Dir', '', 'word');
	...
	public function _buildQueryWhere()
	{
		...
		$search = $this->app->getUserStateFromRequest('com_easydiscuss.subscription.search', 'search', '', 'string');
	...
	{
		// If there is no email provided, means it is always unsubscribed
	...
	/**
	 * Remove all the post subscriptions for specific user id
	 *
	 * @since   5.0.9
	 * @access  public
	 */
	public function removeAllPostSubscription($userId)
	{
		$query = [];
		$query[] = 'DELETE FROM ' . $this->db->nameQuote('#__discuss_subscription');
		$query[] = 'WHERE ' . $this->db->nameQuote('userid') . ' = ' . $this->db->quote($userId);
		$query[] = 'AND ' . $this->db->nameQuote('type') . ' = ' . $this->db->quote('post');
