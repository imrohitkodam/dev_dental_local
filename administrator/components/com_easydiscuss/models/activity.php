...
class EasyDiscussModelActivity extends EasyDiscussAdminModel
{
	 ...
	 * @access  public
	 */
	public function getLogs($utype, $uid, $options = [])
	{
		$db = ED::db();
	...
	/**
	 * Deletes all the activities that associated to the post id given
	 *
	 * @since   5.0.9
	 * @access  public
	 */
	...
	{
		$db = ED::db();
		$query = 'DELETE FROM `#__discuss_activities` WHERE `utype` = ' . $db->Quote('post') . ' AND `uid` = ' . $db->Quote($postId);
