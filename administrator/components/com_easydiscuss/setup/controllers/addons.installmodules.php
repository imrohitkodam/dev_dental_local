...
class EasyDiscussControllerAddonsInstallModules extends EasyDiscussSetupController
{
	...
	public function processToolbarModule()
	{
		...
		// Get all published Easydiscuss Toolbar modules
		$query = array();
		$query[] = 'SELECT a.*, b.' . $db->qn('moduleid') . ' FROM ' . $db->qn('#__modules') . ' AS a';
		$query[] = 'LEFT JOIN ' . $db->qn('#__modules_menu') . ' AS b ON a.' . $db->qn('id') . ' = b.' . $db->qn('moduleid');
		$query[] = 'WHERE a.' . $db->qn('module') . ' = ' . $db->Quote('mod_easydiscuss_toolbar');
		...
		// if nothing means the user never use easydiscuss toolbar module
		if (!$results) {
			return;
		}
		...
		foreach ($results as $module) {
			...
			// lastly, unpublish the easydiscuss Toolbar module
			$query = array();
