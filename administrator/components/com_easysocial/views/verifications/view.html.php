<?php
/**
* @package		EasySocial
* @copyright	Copyright (C) Stack Ideas Sdn Bhd. All rights reserved.
* @license		GNU/GPL, see LICENSE.php
* EasySocial is free software. This version may have been modified pursuant
* to the GNU General Public License, and as distributed it includes or
* is derivative of works licensed under the GNU General Public License or
* other free or open source software licenses.
* See COPYRIGHT.php for copyright notices and details.
*/
defined('_JEXEC') or die('Unauthorized Access');

class EasySocialViewVerifications extends EasySocialAdminView
{
	/**
	 * Post processing after verification is approved
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function approve($type)
	{
		$redirect = 'index.php?option=com_easysocial&view=' . $type . '&layout=verifications';

		$this->info->set($this->getMessage());

		$this->redirect($redirect);
	}

	/**
	 * Post process after setting objects as verified
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function enable($type)
	{
		// Enqueue the message
		$this->info->set($this->getMessage());

		$redirect = 'index.php?option=com_easysocial&view=' . $type . 's';

		return $this->redirect($redirect);
	}

	/**
	 * Post process after removing verified
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function disable($type)
	{
		// Enqueue the message
		$this->info->set($this->getMessage());

		$redirect = 'index.php?option=com_easysocial&view=' . $type . 's';

		return $this->redirect($redirect);
	}

	/**
	 * Post processing after verification is rejected
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function reject($type)
	{
		$redirect = 'index.php?option=com_easysocial&view=' . $type . '&layout=verifications';

		$this->info->set($this->getMessage());

		$this->redirect($redirect);
	}
}
