...
class SocialGdrpTab
{
	...
	public function __construct($adapter, $title = '', $rootPath = '')
	{
		...
		$this->title = $title ? $title : JText::_('COM_ES_GDPR_TAB_' . strtoupper($adapter->type));
		$this->root = $rootPath;
		$this->items = array();
		$this->adapter = $adapter;

		$this->path = SocialGdpr::getUserTempPath($adapter->user) . '/' . $this->key;
	}
	 ...
	 * @access public
	 */
	public function addItem(SocialGdprTemplate $item)
	{
		$this->items[] = $item;
	}
	...
	{
		$baseUrl = '';
		$sectionTitle = 'COM_ES_GDPR_TAB_' . strtoupper($this->key);
	 ...
	 * @access	public
	 */
	public function markItemProcessed(SocialGdprTemplate $template)
	{
