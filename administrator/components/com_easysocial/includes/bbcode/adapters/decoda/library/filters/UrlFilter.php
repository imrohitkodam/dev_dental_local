...
class UrlFilter extends DecodaFilter {
	...
	public function parse(array $tag, $content) {
		...
		if (empty($tag['attributes']['href']) && empty($tag['attributes']['default'])) {
			...
			// By default we will use content as the source url
			$url = $content;

			// Check the url from the text section
			if (isset($tag['text'])) {
				$text = $tag['text'];

				// Determine if this text is formatted as [url=url.com] or just a plain text
				preg_match_all('/\[url\=(.*?)\]/ims', $text, $matches);
		...
		// hacks to replace '&amp;' to proper '&' ticket: #180
		$tmpContent = parent::parse($tag, $content);
