...
class FFmpeg
{
	...
	*/
	 ...
	 * @access  public
	 */
	public function resize($input, $output, $bitrate, $size, $logFile, $framerate, $duration = null, $debug = false)
	{
		$command = $this->ffmpeg . ' ';

		// -async 1 -vsync 1 is to fix the error Past Duration too large when the video has the frame rate of 60fps.
		$command .= '-y -i ' . $input . ' -async 1 -vsync 1 -strict -2 ';

		$config = ES::config();

		if ($config->get('video.cpu.limiting')) {
			$command .= '-threads ' . $config->get('video.cpu.limit') . ' ';
		}
		...
		// Limit duration
		if ($duration) {
			$command .= '-ss 00:00:00 -t ' . $duration . ' ';
		}
	 ...
	 * @access  public
	 */
	public function audioMetadata($input, $output)
	{
		$command = $this->ffmpeg . ' ';
		$command .= ' -i ' . $input . ' -f ffmetadata ';
		$command .= $output;
	...
	/**
	 * Generate waveform png file
	 *
	 * @since   2.1
	 * @access  public
	 */
	public function generateWaveform($input, $output, $debug = false)
	{
		$command = $this->ffmpeg . ' ';
		$command .= '-i ' . $input . ' -filter_complex ';
		$command .= '"[0:a]aformat=channel_layouts=mono,showwavespic=s=700x128:colors=#ee82ee[fg];color=s=700x128,colorkey=0x000000:0.1:0.5[bg];[bg][fg]overlay=format=rgb" ';
		$command .= '-frames:v 1 ' . $output;
	 ...
	 * @access  public
	 */
	public function encode($input, $output, $logFile, $bitrate, $debug = false)
	{
		$command = $this->ffmpeg . ' ';
		...
		// This is needed to fix issue with wrong duration after encode
	 ...
	 * @access  public
	 */
	public function getAlbumArt($input, $output, $debug = false)
	{
		$command = $this->ffmpeg . ' ';
		$command .= '-i ' . $input . ' -an -vcodec copy ';
		$command .= $output;
