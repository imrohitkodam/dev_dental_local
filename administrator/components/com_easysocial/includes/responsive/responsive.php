...
class SocialResponsive extends EasySocial
{
	public $mobile = array(
		...
		'Android'       => 'Linux; Android.*|Android; Opera Mini.*|Linux; U; Android.*|Linux;(.*;)? Android.*',
	...
	public $os = array(
		...
		'MacintoshOS'         => '\bMacintosh;|\bIntel Mac OS',
	...
	public function isWebView()
	{
		...
		preg_match('/easysocial-webview/is', $agent, $matches);
	...
	/**
	 * Determines if the current request was made from a machintosh device
	 *
	 * @since	2.1.0
	 * @access	public
	 */
	...
	{
		static $macintosh = null;

		$agent = $this->getAgent();

		if (!is_null($macintosh)) {
			return $macintosh;
		}

		preg_match('/' . $this->os['MacintoshOS'] . '/is', $agent, $matches);
	...
	/**
	 * Determines if this is a Safari browser
	 *
	 * @since	3.2
	 * @access	public
	 */
	public function isSafari($agent = null)
	{
		static $safari = null;
		...
		if (is_null($safari)) {
			...
			preg_match('/AppleWebKit\/[.0-9]+.*Gecko\)\sVersion\/[.0-9].*Safari\/[.0-9A-Za-z]/is', $agent, $matches);
	...
	public function isTablet($agent = null)
	{
		...
		if (is_null($isTablet)) {
			...
			if (!$isTablet) {
				// if agent not match, lets try again with the session token. #5051
				$session = JFactory::getSession();
				$isTablet = $session->get('establet.view', false, SOCIAL_SESSION_NAMESPACE);
			}
		}
