...
class SocialAdvancedSearchHelperMarketplace extends SocialAdvancedSearchHelperAbstract
{
	public $type = 'marketplace';

	public function getLink()
	{
		$link = ESR::search(array('layout' => 'advanced', 'type' => SOCIAL_TYPE_MARKETPLACE));
	...
	public function search($options = array())
	{
		...
		$model = ES::model('SearchMarketplace');
	...
	public function loadFields($key, $element)
	{
		...
		$query .= ' inner join `#__social_fields_steps` as fs on a.`step_id` = fs.`id` and fs.`type` = ' . $db->Quote('marketplaces');
		$query .= ' inner join `#__social_workflows` as w on fs.`workflow_id` = w.`id` and w.`type` = ' . $db->Quote('marketplace');
		...
		$query .= ' and b.`group` = ' . $db->Quote('marketplace');
	...
	/**
	 * Retrieves all fields available for marketplaces
	 *
	 * @since	2.0
	 * @access	public
	 */
	public function getFields($elements = array())
	{
		static $fields = null;
		...
		if (! $fields) {
			...
			$query .= ' inner join `#__social_fields_steps` as fs on a.`step_id` = fs.`id` and fs.`type` = ' . $db->Quote('marketplaces');
			$query .= ' inner join `#__social_workflows` as w on fs.`workflow_id` = w.`id` and w.`type` = ' . $db->Quote('marketplace');
			$query .= ' inner join `#__social_apps` as b on a.`app_id` = b.`id` and b.`group` = ' . $db->Quote('marketplace');
			$query .= ' where a.`searchable` = ' . $db->Quote('1');
			$query .= ' and a.`state` = ' . $db->Quote('1');
			...
			if ($elements) {
				...
				$query .= " AND b.`element` IN ('" . implode("','", $elements) . "')";
			}
