...
class Registry implements \JsonSerializable, \ArrayAccess, \IteratorAggregate, \Countable
{
	...
	public function loadString($data, $format = 'JSON', array $options = [])
	{
		...
		$obj = ESRegistryFactory::getFormat($format, $options)->stringToObject($data, $options);
	...
	{
		return ESRegistryFactory::getFormat($format, $options)->objectToString($this->data, $options);
	}
...
class ESRegistryFactory
{
     ...
     *
     * @since   1.5.0
     * @since   2.0.0  Object caching is no longer supported. `ESRegistryFactory::getFormat()` returns a new `FormatInterface`
     *          instance on each call.
     */
    public static function getFormat($type, array $options = [])
    {
    	require_once(__DIR__ . '/format/json.php');
