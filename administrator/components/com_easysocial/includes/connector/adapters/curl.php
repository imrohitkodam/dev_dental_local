...
class SocialConnectorCurl
{
	...
	public function __construct()
	{
		...
		$this->proxyEnable = $jConfig->getValue('proxy_enable');
		$this->proxyHost = $jConfig->getValue('proxy_host');
		$this->proxyPort = $jConfig->getValue('proxy_port');
		$this->proxyUser = $jConfig->getValue('proxy_user');
		$this->proxyPass = $jConfig->getValue('proxy_pass');
	...
	public function execute()
	{
		...
		foreach ($this->urls as $url) {
			...
			if (isset($this->options[$url][CURLOPT_POST]) && $this->options[$url][CURLOPT_POST] === true) {
				// to prevent some PHP using &amp; as the default value, we need to pass in the default value manually. #1703
	...
	public function executeRedirects($handle, $key, $code = '', $headers = '')
	{
		...
		if ($code == 301 || $code == 302 || $code == 303) {
			...
			$finalRedirection = $this->getFinalRedirectionUrl($newUrl);

			if ($finalRedirection) {
				$newUrl = $finalRedirection;
			}
			...
			curl_setopt($newHandle, CURLOPT_REFERER, 'https://www.google.com');
	...
	{
		$this->addOption(CURLOPT_REFERER, $url);
		return true;
	}
