...
class SocialGroup extends SocialCluster
{
	...
	public static function loadGroups( $ids = null , $debug = false )
	{
		...
		foreach( $groups as $group )
		{
			...
			// to avoid page load performance issue due to looping each users if users more than 10k, the isMember will not check again db directly.
			$group->admins = $model->getMembers( $group->id , array( 'users' => false, 'state' => SOCIAL_GROUPS_MEMBER_PUBLISHED, 'admin' => SOCIAL_STATE_PUBLISHED));
			$group->pending = $model->getMembers( $group->id , array( 'users' => false, 'state' => SOCIAL_GROUPS_MEMBER_PENDING));
	...
	public function getTotalMembers()
	{
		...
		if (! isset($_cache[$this->id])) {
			$model 	= FD::model('Groups');
			$_cache[$this->id] = $model->getTotalMembers($this->id);
		}
	...
	public function notifyMembers($action, $data = array())
	{
		...
		if ($action == 'task.uncompleted') {
			...
			FD::notify('groups.task.uncompleted', $targets, $options, $system);
		}
