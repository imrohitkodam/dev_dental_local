...
<ul class="g-list-unstyled browser-list" data-fields-browser-list>
    <?php foreach ($defaultFields as $defaultField) { ?>
        <?php if ($core == $defaultField->core && (!isset($unique) || $unique == $defaultField->unique)) { ?>
            <?php echo $this->includeTemplate('admin/fields/editor/browser.item', array('defaultField' => $defaultField)); ?>
        <?php } ?>
    <?php } ?>
</ul>
<?php } else { ?>
