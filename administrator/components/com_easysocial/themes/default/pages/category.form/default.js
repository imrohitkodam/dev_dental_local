EasySocial.ready(function($){
	...
	$.Joomla('submitbutton', function(task) {
		if (task == 'cancel') {
			window.location	= 'index.php?option=com_easysocial&view=pages&layout=categories';
		...
		if (task == 'saveCategoryCopy') {
			...
			EasySocial.ajax('admin/controllers/pages/createBlankCategory')
				.done(function(id) {
	...
	$('[data-category-avatar-remove-button]').on('click', function()
	{
		...
		EasySocial.dialog(
		{
			content 	: EasySocial.ajax('admin/views/pages/confirmRemoveCategoryAvatar', { "id" : id }),
			bindings 	:
			{
				"{deleteButton} click" : function()
				{
					EasySocial.ajax('admin/controllers/pages/removeCategoryAvatar',
					{
						"id" : id
					})
					.done(function()
