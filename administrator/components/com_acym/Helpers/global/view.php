...
function acym_getView(string $ctrl, string $view, bool $forceBackend = false)
{
    ...
    if (file_exists($override)) {
        ...
        return $viewsFolder.ucfirst($ctrl).DS.'tmpl'.DS.$view.'.php';
    }
}
...
function acym_loadAssets($ctrl, $task)
{
    ...
    acym_addScript(
        ...
        const FOUNDATION_FOR_EMAIL = "'.ACYM_CSS.'libraries/foundation_email.min.css?v='.filemtime(ACYM_MEDIA.'css'.DS.'libraries'.DS.'foundation_email.min.css').'";
        const ACYM_FIXES_FOR_EMAIL = "'.str_replace('"', '\"', acym_getEmailCssFixes()).'";
        const ACYM_REGEX_EMAIL = /^'.acym_getEmailRegex(true).'$/i;
        const ACYM_JS_TXT = '.acym_getJSMessages().';
        const ACYM_CORE_DYNAMICS_URL = "'.ACYM_CORE_DYNAMICS_URL.'";
        const ACYM_PLUGINS_URL = "'.addslashes(ACYM_PLUGINS_URL).'";
