...
trait DefaultData
{
    ...
    public function installNotifications(): bool
    {
        ...
        if (empty($notifications['acy_notification_auto_campaign_admin'])) {
            $addNotif[] = [
                'name' => 'acy_notification_auto_campaign_admin',
                'subject' => acym_translation('ACYM_NOTIFICATION_AUTO_CAMPAIGN_SUBJECT'),
                'content' => $this->getFormatedNotification(
                    '<h1 style="font-size: 24px;">'.acym_translation('ACYM_HELLO').' '.$this->getDTextDisplay('{subscriber:name|ucfirst}', 'Marc').',</h1>
                    <p>'.acym_translation('ACYM_NOTIFICATION_AUTO_CAMPAIGN_BODY').'</p>'
                ),
            ];
        }
