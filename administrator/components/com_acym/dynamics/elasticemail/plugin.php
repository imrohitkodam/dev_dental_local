...
class plgAcymElasticemail extends AcymPlugin
{
    ...
    {
        if (!isset($credentials[self::SENDING_METHOD_ID.'_password']) || (!empty($credentials[self::SENDING_METHOD_ID.'_password']) && empty(
                trim(
                    $credentials[self::SENDING_METHOD_ID.'_password'],
                    '*'
                )
                ))) {
            return [
