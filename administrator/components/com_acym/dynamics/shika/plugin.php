...
class plgAcymShika extends acymPlugin
{
    ...
    public function insertionOptions($defaultValues = null)
    {
        ...
        $catOptions = [
            ...
            [
                ...
                'tooltip' => 'Only the elements created/published since the specified date will be inserted in your email.',
                'type' => 'date',
                'name' => 'min_startdate',
                'default' => '',
                'relativeDate' => '-',
    ...
    {
        if (!ACYM_J40) {
            require_once JPATH_SITE.DS.'components'.DS.'com_tjlms'.DS.'helpers'.DS.'route.php';
        }
    ...
    public function replaceIndividualContent($tag)
    {
        ...
        $varFields['{intro}'] = $this->cleanDivTags($element->short_desc);
        if (in_array('intro', $tag->display)) {
            $contentText .= $varFields['{intro}'];
        }

        $varFields['{full}'] = $this->cleanDivTags($element->description);
    ...
    private function cleanDivTags(string $text): string
    {
