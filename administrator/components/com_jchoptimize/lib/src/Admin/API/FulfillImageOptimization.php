...
class FulfillImageOptimization
{
    ...
    public function handle(Response $response, array $index): void
    {
        ...
        if (!isset($response)) {
            ...
            $this->logger->error('Unknown response from server: ' . print_r($response, true));

            return;
        }

        if (!$response['success']) {
            $this->messageEvent->send($response['message'] . '. Aborting...', 'apiError');

            return;
        }

        foreach ($response['data'] as $i => $fileEntry) {
