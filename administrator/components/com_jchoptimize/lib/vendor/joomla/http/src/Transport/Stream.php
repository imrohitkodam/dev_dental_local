 ...
 */
namespace _JchOptimizeVendor\Joomla\Http\Transport;

use Composer\CaBundle\CaBundle;
use _JchOptimizeVendor\Joomla\Http\AbstractTransport;
use _JchOptimizeVendor\Joomla\Http\Exception\InvalidResponseCodeException;
use _JchOptimizeVendor\Joomla\Http\Response;
use Joomla\Uri\Uri;
use Joomla\Uri\UriInterface;
use _JchOptimizeVendor\Laminas\Diactoros\Stream as StreamResponse;
/**
...
class Stream extends AbstractTransport
{
    ...
    public function request($method, UriInterface $uri, $data = null, array $headers = [], $timeout = null, $userAgent = null)
    {
        ...
        if (!empty($headers)) {
            ...
            $options['header'] = \trim($headerString, "\r\n");
        }
        ...
        $streamOptions = ['http' => $options, 'ssl' => ['verify_peer' => \true, 'verify_depth' => 5, 'verify_peer_name' => \true]];
        ...
        if (!$stream) {
            ...
            if ($error === null || $error['message'] === '') {
                ...
                $error = array('message' => \sprintf('Could not connect to resource %s', $uri));
            }
