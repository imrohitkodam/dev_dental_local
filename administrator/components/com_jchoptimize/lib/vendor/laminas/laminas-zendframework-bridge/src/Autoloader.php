...
class Autoloader
{
    ...
    public static function load()
    {
        ...
        spl_autoload_register(self::createPrependAutoloader(RewriteRules::namespaceReverse(), $classLoader, $loaded), \true, \true);
        spl_autoload_register(self::createAppendAutoloader(RewriteRules::namespaceRewrite(), $loaded));
    }
    private static function getClassLoader() : ?ClassLoader
    {
        ...
        return self::getClassLoaderFromVendorDirectory(self::UPSTREAM_COMPOSER_VENDOR_DIRECTORY) ?? self::getClassLoaderFromVendorDirectory(self::LOCAL_COMPOSER_VENDOR_DIRECTORY);
    }
    /**
     * @return callable
     */
    private static function createPrependAutoloader(array $namespaces, ClassLoader $classLoader, ArrayObject $loaded)
    {
        ...
        return static function ($class) use($namespaces, $classLoader, $loaded) {
            ...
            if ($classLoader->loadClass($class)) {
                $legacy = $namespaces[$check] . strtr(substr($class, strlen($check)), ['ApiTools' => 'Apigility', 'Mezzio' => 'Expressive', 'Laminas' => 'Zend']);
    ...
    private static function createAppendAutoloader(array $namespaces, ArrayObject $loaded)
    {
        ...
        return static function ($class) use($namespaces, $loaded) {
            ...
            $alias = $namespaces[$check] . strtr(substr($class, strlen($check)), ['Apigility' => 'ApiTools', 'Expressive' => 'Mezzio', 'Zend' => 'Laminas', 'AbstractZendServer' => 'AbstractZendServer', 'ZendServerDisk' => 'ZendServerDisk', 'ZendServerShm' => 'ZendServerShm', 'ZendMonitor' => 'ZendMonitor']);
            $loaded[$alias] = \true;
