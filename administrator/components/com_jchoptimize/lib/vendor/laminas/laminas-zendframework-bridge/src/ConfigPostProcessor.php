...
{
    /** @internal */
    const SERVICE_MANAGER_KEYS_OF_INTEREST = ['aliases' => \true, 'factories' => \true, 'invokables' => \true, 'services' => \true];
    /** @var array String keys => string values */
    private $exactReplacements = ['zend-expressive' => 'mezzio', 'zf-apigility' => 'api-tools'];
    /** @var Replacements */
    ...
    public function __construct()
    {
        ...
        $this->rulesets = [
            // Exact values
            function ($value) {
                return is_string($value) && isset($this->exactReplacements[$value]) ? [$this, 'replaceExactValue'] : null;
            },
            ...
            function ($value, array $keys) {
                ...
                return $key === 'router' && $keys === [] && is_array($value) ? [$this, 'noopReplacement'] : null;
            },
            // service- and pluginmanager handling
            function ($value) {
                return is_array($value) && array_intersect_key(self::SERVICE_MANAGER_KEYS_OF_INTEREST, $value) !== [] ? [$this, 'replaceDependencyConfiguration'] : null;
            },
            // Array values
            function ($value, array $keys) {
                return $keys !== [] && is_array($value) ? [$this, '__invoke'] : null;
            },
        ];
    }
    /**
    ...
    public function __invoke(array $config, array $keys = [])
    {
        ...
        foreach ($config as $key => $value) {
            ...
            if (!array_key_exists($newKey, $rewritten) || !is_array($rewritten[$newKey])) {
                ...
                $rewritten[$newKey] = array_key_exists($newKey, $rewritten) && null === $newValue ? $rewritten[$newKey] : $newValue;
                continue;
            }
    ...
    {
        return is_string($value) ? $this->replacements->replace($value) : $value;
    }
    /**
    ...
    {
        $aliases = isset($config['aliases']) && is_array($config['aliases']) ? $this->replaceDependencyAliases($config['aliases']) : [];
        if ($aliases) {
    ...
    private function replaceDependencyAliases(array $aliases)
    {
        foreach ($aliases as $alias => $target) {
            ...
            if ($newAlias === $alias && !in_array($alias, $notIn, \true)) {
            ...
            if (!in_array($newAlias, $notIn, \true)) {
