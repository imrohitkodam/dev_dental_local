...
class Uri implements UriInterface
{
    ...
    {
        if (!is_string($scheme)) {
            throw new Exception\InvalidArgumentException(sprintf('%s expects a string argument; received %s', __METHOD__, is_object($scheme) ? get_class($scheme) : gettype($scheme)));
        }
    ...
    {
        if (!is_string($user)) {
            throw new Exception\InvalidArgumentException(sprintf('%s expects a string user argument; received %s', __METHOD__, is_object($user) ? get_class($user) : gettype($user)));
        }
        if (null !== $password && !is_string($password)) {
            throw new Exception\InvalidArgumentException(sprintf('%s expects a string or null password argument; received %s', __METHOD__, is_object($password) ? get_class($password) : gettype($password)));
        }
    ...
    {
        if (!is_string($host)) {
            throw new Exception\InvalidArgumentException(sprintf('%s expects a string argument; received %s', __METHOD__, is_object($host) ? get_class($host) : gettype($host)));
        }
        if ($host === $this->host) {
            // Do nothing if no change was made.
            return $this;
    ...
    public function withPort($port) : UriInterface
    {
        if ($port !== null) {
            if (!is_numeric($port) || \is_float($port)) {
                throw new Exception\InvalidArgumentException(sprintf('Invalid port "%s" specified; must be an integer, an integer string, or null', is_object($port) ? get_class($port) : gettype($port)));
            }
            $port = (int) $port;
        }
        if ($port === $this->port) {
    ...
    {
        if (!is_string($path)) {
            throw new Exception\InvalidArgumentException('Invalid path provided; must be a string');
        }
    ...
    public function withFragment($fragment) : UriInterface
    {
        if (!is_string($fragment)) {
            throw new Exception\InvalidArgumentException(sprintf('%s expects a string argument; received %s', __METHOD__, is_object($fragment) ? get_class($fragment) : gettype($fragment)));
        }
    ...
    private function filterScheme(string $scheme) : string
    {
        ...
        if (!isset($this->allowedSchemes[$scheme])) {
            throw new Exception\InvalidArgumentException(sprintf('Unsupported scheme "%s"; must be any empty string or in the set (%s)', $scheme, implode(', ', array_keys($this->allowedSchemes))));
        }
        return $scheme;
    }
    /**
    ...
    private function filterUserInfoPart(string $part) : string
    {
        ...
        return preg_replace_callback('/(?:[^%' . self::CHAR_UNRESERVED . self::CHAR_SUB_DELIMS . ']+|%(?![A-Fa-f0-9]{2}))/u', [$this, 'urlEncodeChar'], $part);
    }
    /**
    ...
    private function filterQueryOrFragment(string $value) : string
    {
        ...
        return preg_replace_callback('/(?:[^' . self::CHAR_UNRESERVED . self::CHAR_SUB_DELIMS . '%:@\\/\\?]+|%(?![A-Fa-f0-9]{2}))/u', [$this, 'urlEncodeChar'], $value);
    }
    /**
