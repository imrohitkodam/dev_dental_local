...
trait MessageTrait
{
    ...
    private function validateProtocolVersion($version) : void
    {
        ...
        if (!is_string($version)) {
            throw new Exception\InvalidArgumentException(sprintf('Unsupported HTTP protocol version; must be a string, received %s', is_object($version) ? get_class($version) : gettype($version)));
        }
