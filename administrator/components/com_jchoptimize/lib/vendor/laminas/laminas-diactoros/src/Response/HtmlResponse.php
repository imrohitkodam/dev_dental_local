...
use _JchOptimizeVendor\Laminas\Diactoros\Stream;
...
class HtmlResponse extends Response
{
    ...
    private function createBody($html) : StreamInterface
    {
        ...
        if (!is_string($html)) {
            throw new Exception\InvalidArgumentException(sprintf('Invalid content (%s) provided to %s', is_object($html) ? get_class($html) : gettype($html), __CLASS__));
        }
