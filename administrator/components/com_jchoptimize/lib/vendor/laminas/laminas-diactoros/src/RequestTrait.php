...
trait RequestTrait
{
    ...
    private function createUri($uri) : UriInterface
    {
        ...
        throw new Exception\InvalidArgumentException('Invalid URI provided; must be null, a string, or a Psr\\Http\\Message\\UriInterface instance');
    }
    /**
    ...
    {
        if (!is_string($method)) {
            throw new Exception\InvalidArgumentException(sprintf('Unsupported HTTP method; must be a string, received %s', is_object($method) ? get_class($method) : gettype($method)));
        }
