...
<?php if ($this->handlers !== null):?>
<div class="alert alert-info mb-3">
    <h3 class="alert-heading">
        <?= Text::_('COM_ADMINTOOLS_' . $viewName . '_LBL_PHPHANDLERS'); ?>
    </h3>
    <p>
        <?= Text::_('COM_ADMINTOOLS_' . $viewName . '_LBL_PHPHANDLERS_DESC'); ?>
    </p>
	<pre><?= $this->escape($this->handlers) ?></pre>
    <a href="<?= Route::_('index.php?option=com_admintools&view=Htaccessmaker&task=addphphandler&'.$app->getFormToken().'=1')?>" class="btn btn-success"><?= Text::_('COM_ADMINTOOLS_' . $viewName . '_LBL_PHPHANDLERS_APPLY')?></a>
</div>
<?php endif;?>
