...
class TjCertificateCertificate extends CMSObject
{
	...
	public function save()
	{
		...
		try
		{
			...
			// Set issued_on date - always update when set via setIssuedDate()
			// For new records without issued_on, use current date
			// For updates, use the value set via setIssuedDate() (which is current date for regeneration)
			if (!empty($this->issued_on))
			{
				$table->issued_on = $this->issued_on;
			}
			elseif ($isNew)
			{
