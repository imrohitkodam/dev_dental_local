...
class TjlmsModelAttemptreport extends ListModel
{
	...
	public function updateAttemptData($attempt_id, $score = '', $lesson_status = '')
	{
		...
		if (is_numeric($score))
		{
			...
			$dispatcher->trigger('onAfterLessonAttemptScoreUpdate', array($data));
		}

		if ($lesson_status)
		{
			// Trigger on after attempt status change
			$dispatcher->trigger('onAfterLessonAttemptStatusChange', array($data));
		}
	...
	public function delete($cid)
	{
		...
		if (!empty($cid))
		{
			...
			foreach ($cid as $lessonTrackId)
			{
				...
				{
						$this->setError(JText::_('COM_TJLMS_CANNOT_DELETE_LESSON_TRACK'));
				...
				$dispatcher = JDispatcher::getInstance();
				$dispatcher->trigger('onAfterLessonAttemptDelete', array($attemptData));
			}
		}
		...
		if ($data)
		{
			...
			if ($courseProgress)
			{
				...
				if ($courseProgress['status'] == 'C')
				{
					...
					$dispatcher->trigger('onAfterCourseCompletion',
								array(
									$data->user_id,
									$data->course_id
								)
