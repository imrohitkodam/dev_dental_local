...
function checkForUnlimited(time_measure,time_measure_id)
{
	...
	if(time_measure=='unlimited')
	{
		...
		techjoomla.jQuery('#subs_plan_duration_'+split_id[4]).hide();
	...
	{
		techjoomla.jQuery('#subs_plan_duration_'+split_id[4]).show();
		var textbox = document.getElementById('subs_plan_duration_'+split_id[4]);
...
function getReportdata(page, colToShow, limit, sortCol, sortOrder, action)
{
	...
	var filterValue;
	var filterName;
	techjoomla.jQuery('#report-containing-div .alert').remove();
	...
	if (action == 'datesearch')
	{
		...
		{
			techjoomla.jQuery('#report-containing-div').prepend("<div class='alert alert-error'><p>"+invalidDate+"</p></div>");
			return false;
		}
...
function getPaginationBar(action, totalRows)
	...
	{
		var inputHidden = techjoomla.jQuery('#queryName').is(":hidden");

		if (inputHidden == 1)
		{
			techjoomla.jQuery('#queryName').show();
			techjoomla.jQuery('#saveQuery').val('Save Query');
		}
		else
		{
			var queryName = techjoomla.jQuery('input[name="queryName"]').val();
...
function formTrackinvalidFields(form_to_be_validated, form_container)
{
	...
	techjoomla.jQuery("label.invalid", form_to_be_validated).each(function() {
			msg.push(Joomla.JText._("COM_TJLMS_FORM_INVALID_FIELD") + jQuery(this).text());
	});

	techjoomla.jQuery(".tjlms_form_errors .msg", form_container).html('<div>'+msg.join('<br />')+'</div>');
	techjoomla.jQuery(".tjlms_form_errors", form_container).show();
    return false;
}
