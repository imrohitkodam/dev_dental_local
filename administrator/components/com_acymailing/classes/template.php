...
class templateClass extends acymailingClass{
	...
	function saveForm(){
		...
		$formData = acymailing_getVar('array', 'data', array(), '', ACY_ALLOWHTML);
	...
	function doupload(){
		...
		if(!extension_loaded('zlib')){
			acymailing_raiseError(500, acymailing_translation('ACY_MISSING_ZLIB'));
			return false;
		}
	...
	function export($tempid){
		if(!extension_loaded('zlib')){
			acymailing_raiseError(500, acymailing_translation('WARNINSTALLZLIB'));
			return false;
		}
...
<style type="text/css">
';
        if (file_exists(ACYMAILING_TEMPLATE.'css'.DS.'template_'.$tempid.'.css')) {
		    $css .= file_get_contents(ACYMAILING_TEMPLATE.'css'.DS.'template_'.$tempid.'.css');
        }
		$css .= '
</style>';
