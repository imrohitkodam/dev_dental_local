...
class subscriberClass extends acymailingClass{
	...
	var $geolocData = null;
    public $confirmationSentSuccess;
    public $confirmationSentError;
    ...
    function save($subscriber){
		...
		if(empty($subscriber->subid)){
            ...
            $acycheckerHelper = acymailing_get('helper.acychecker');
            if (!$acycheckerHelper->checkSubscriber($subscriber)) {
                echo "<script>alert('".acymailing_translation('ACY_INVALID_EMAIL_ADDRESS', true)."'); window.history.go(-1);</script>";
                exit;
            }
			...
			if($this->allowModif || !in_array($column, $this->restrictedFields)){
				...
				if(!is_numeric($subscriber->$column)){
					if(function_exists('mb_detect_encoding') && mb_detect_encoding($subscriber->$column, 'UTF-8', true) != 'UTF-8'){
						$subscriber->$column = acy_utf8Encode($subscriber->$column);
	...
	function saveForm(){
		...
		$this->recordHistory = true;
		$this->newUser = empty($subscriber->subid);
