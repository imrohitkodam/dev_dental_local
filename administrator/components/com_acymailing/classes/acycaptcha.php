...
class acycaptchaClass{
	...
	function display($formName = '', $isModule = false){
		...
		if($captchaPluginName == 'acycaptcha'){
			...
			if('WordPress' === ACYMAILING_CMS){
                $captchaURL = acymailing_frontendLink('captcha&acyformname='.$formName.'&val='.rand(0, 10000));
            }elseif(ACYMAILING_J16){
			    $captchaURL = acymailing_completeLink('captcha&acyformname='.$formName.'&val='.rand(0, 10000));
			}else{
			    $captchaURL = rtrim(acymailing_rootURI(), '/').'/index.php?option=com_acymailing&amp;ctrl=captcha&amp;acyformname='.$formName.'&amp;val='.rand(0, 10000);
			}
            $image = '<img id="captcha_picture_'.$formName.'" title="'.acymailing_translation('ERROR_CAPTCHA').'" width="'.$config->get('captcha_width'.$configSuffix).'" height="'.$config->get('captcha_height'.$configSuffix).'" class="captchaimagemodule" src="'.$captchaURL.'" alt="captcha" />';
