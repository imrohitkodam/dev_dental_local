...
class mailClass extends acymailingClass{
	...
	function saveForm(){
		...
		if($result){
			...
			if($config->get('emojiwarning', 1)){
				...
				acymailing_enqueueMessage(acymailing_translation_sprintf('ACY_EMOJI_CONFIRMATION', '<a target="_blank" href="https://docs.acymailing.com/v/old/main-pages/newsletters">', '</a>').' '.$notremind, 'warning');
			}
		}
		...
		if('joomla' == 'wordpress' && $mail->type == 'autonews') {
	...
	function save($mail){
		...
		if(empty($mail->mailid)){
			if(empty($mail->summary)) $mail->summary = '';
		...
		if(!empty($mail->senddate) && !is_numeric($mail->senddate)){
            ...
            if (ACYMAILING_CMS_SIMPLE === 'joomla') {
                $timeoffset = acymailing_getCMSConfig('offset');
                if (ACYMAILING_J16) {
                    $dateC = JFactory::getDate(acymailing_getDate($mail->senddate, '%Y-%m-%d %H:%M'), $timeoffset);
                    $timeoffset = $dateC->getOffsetFromGMT(true);
                }
                $mail->senddate = $mail->senddate - $timeoffset * 60 * 60 + date('Z');
            }
		}
