...
class fieldsClass extends acymailingClass{
	...
	private function _generateCountryArray(){
		...
		$this->country['1 '] = 'United States of America';
	...
	private function _displayBirthday($field, $value, $map, $inside){
		...
		if(empty($field->options['format'])) $field->options['format'] = "%d %m %Y";
		$vals = explode('-', $value);
		if (!empty($vals[2]) && strlen($vals[2]) === 1) {
			$vals[2] = '0'.$vals[2];
		}
		if (!empty($vals[1]) && strlen($vals[1]) === 1) {
			$vals[1] = '0'.$vals[1];
		}
		$days = array();
		...
		foreach($fieldsDisplayed as $fieldName => $oneField){
				...
				if(in_array($condFieldName, array('email', 'name')) && in_array($page, array('userProfile', 'modifyForm'))){
					...
					if(tmpField != undefined){
						if(tmpField.length != undefined){
							...
							precond['tmpField_" . $fieldName . "'] = tmpField.value;
						}
					}";
			...
			if(!empty($currentFieldCond)){
				...
				if($page == 'joomlaProfile' && ACYMAILING_J25 && !ACYMAILING_J30){
                    ...
                    if($oneField->required === '1') {
                        $currentFieldCond .= "
						if(document.getElementById('field_".$oneField->namekey."') !== null){
							document.getElementById('field_".$oneField->namekey."').className += ' required';
							document.getElementById('field_".$oneField->namekey."').required = 'required';
						}
						";
                    }
                    ...
                    if($oneField->required === '1') {
                        $currentFieldCond .= "
						if(document.getElementById('field_".$oneField->namekey."') !== null){
							document.getElementById('field_".$oneField->namekey."').className = document.getElementById('field_".$oneField->namekey."').className.replace(/required/g, '');
							document.getElementById('field_".$oneField->namekey."').removeAttribute('required');
						}
						";
                    }
