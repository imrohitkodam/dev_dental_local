...
class geolocationClass extends acymailingClass{
	...
	function saveGeolocation($geoloc_action, $subid){
        ...
        if($geo_element === false) return false;
	...
	function getIpLocation($element){
		...
		$config = acymailing_config();
		$ipInfoDbApiKey = trim($config->get('geoloc_api_key', ''));
		$ip2LocationApiKey = trim($config->get('ip2location_api_key', ''));

        if(empty($ipInfoDbApiKey) && empty($ip2LocationApiKey)){
            return false;
        }

        if(!empty($ipInfoDbApiKey)){
            $geoClass = acymailing_get('inc.ipinfodb');
            $geoClass->setKey($ipInfoDbApiKey);
            $location = $geoClass->getCity($element->geolocation_ip);
            $errorLoc = $geoClass->getError();
        }else{
            $location = $this->ip2locationTestApiKey($ip2LocationApiKey, $element->geolocation_ip);
        }
        ...
        if(empty($errorLoc) && !empty($location) && !empty($location->countryCode) && $location->countryCode != '-'){
            ...
            $element->geolocation_created = time();
            $element->geolocation_continent = $this->countryToContinent($location->countryCode);
	...
	function ip2locationTestApiKey($apiKey, $ipUser = null){
        if(empty($ipUser)){
            $userHelper = acymailing_get('helper.user');
            $ipUser = $userHelper->getIP();
        }

        $conn = curl_init('https://api.ip2location.io/?key='.trim($apiKey).'&ip='.$ipUser.'&format=json');
        curl_setopt($conn, CURLOPT_SSL_VERIFYPEER, true);
        curl_setopt($conn, CURLOPT_FRESH_CONNECT, true);
        curl_setopt($conn, CURLOPT_RETURNTRANSFER, 1);
        $data = curl_exec($conn);
        if (empty($data)) {
            $result = [curl_errno($conn) => curl_error($conn)];
        }
        curl_close($conn);
        ...
        $result = new stdClass();
        if(empty($data) || empty($data['latitude'])){
            $result->statusCode = 'noReturn';
            $result->ip = $ipUser;
            if(!empty($data['error']['error_message'])){
                $result->errorAPI = $data['error']['error_message'];
            }
        ...
        $result->countryName = $data['country_name'];
