...
define('ACYMAILING_CMS_SIMPLE', 'joomla');
...
define('ACYMAILING_CMSV', $jversion);
...
define('ACYMAILING_J37', version_compare($jversion, '3.7.0', '>='));
define('ACYMAILING_J39', version_compare($jversion, '3.9.0', '>='));
define('ACYMAILING_J40', version_compare($jversion, '4.0.0', '>='));
define('ACYMAILING_J44', version_compare($jversion, '4.4.0', '>='));
...
function acymailing_route($url, $xhtml = true, $ssl = null)
{
    if (ACYMAILING_J40) {
        global $Itemid;
        if (!acymailing_isAdmin() && !empty($Itemid) && strpos($url, 'Itemid') === false) {
...
function acymailing_getVar($type, $name, $default = null, $source = 'default', $mask = 0)
{
    if (ACYMAILING_J40) {
        if ($mask & ACY_ALLOWRAW) {
            $type = 'RAW';
        } elseif ($mask & ACY_ALLOWHTML) {
            $type = 'HTML';
        }
    ...
    if (is_string($result) && !($mask & ACY_ALLOWRAW)) {
        return acymailing_filterText($result);
    }
...
{
    if (is_string($attribs) && strpos($attribs, 'width') === false) {
        $stylePos = strpos($attribs, 'style="');
        if ($stylePos === false) {
            $attribs .= ' style="width: 220px;"';
        } else {
            $attribs = substr($attribs, 0, $stylePos).' style="width: 220px;"'.substr($attribs, $stylePos);
        }
    }
...
{
    $element = class_exists('JHtmlAcyselect') && !ACYMAILING_J44 ? 'acyselect' : 'select';
...
function acymailing_addScript($raw, $script, $type = 'text/javascript', $defer = true, $async = false)
{
...
function acymailing_addStyle($raw, $style, $type = 'text/css', $media = null, $attribs = [])
{
    $acyDocument = acymailing_getGlobal('doc');
...
function acymailing_trigger($method, $args = [])
{
    if (ACYMAILING_J40) {
...
{
    $tmpl = acymailing_getVar('cmd', 'tmpl');
...
function acymailing_enqueueMessage($message, $type = 'success')
{
    ...
    $result = str_replace(' onclick="', ' acy-data-onclick="', $result);
...
function acymailing_displayMessages()
{
    ...
    foreach ($sorted as $type => $message) {
        acymailing_display(str_replace('acy-data-onclick', 'onclick', $message), $type);
    }
}
...
function acymailing_isExtensionActive($extension)
{
...
{
    $folders = [ACYMAILING_ROOT.'plugins' => '', ACYMAILING_ROOT.'plugins'.DS.'user' => '', ACYMAILING_ROOT.'plugins'.DS.'system' => ''];
    $results = ['', '', ''];
...
function acymailing_frontendLink($link, $newsletter = true, $popup = false, $complete = false)
{
    ...
    if ($config->get('use_sef', 0) && strpos($link, '&ctrl=cron') === false && strpos($link, '&ctrl=url') === false) {
        if (ACYMAILING_J39) {
            $sh404SEF = defined('SH404SEF_IS_RUNNING') && SH404SEF_IS_RUNNING == 1;
            ...
            $json = acymailing_sef('&urls[0]='.base64_encode($link));
...
{
    $url = acymailing_rootURI().'index.php';
    $parameters = 'option=com_acymailing&ctrl=url&task=sef'.$links;

    if (function_exists('curl_exec')) {
        $conn = curl_init($url);
        curl_setopt($conn, CURLOPT_POSTFIELDS, $parameters);
        curl_setopt($conn, CURLOPT_SSL_VERIFYPEER, true);
        curl_setopt($conn, CURLOPT_FRESH_CONNECT, true);
        curl_setopt($conn, CURLOPT_RETURNTRANSFER, 1);

        $sefLink = curl_exec($conn);
        curl_close($conn);
    }

    if (empty($sefLink)) {
        $sefLink = acymailing_fileGetContent($url.'?'.$parameters);
    }

    return json_decode($sefLink, true);
}
...
function acymailing_sendAjaxResponse($message = '', $data = [], $success = true)
{
    $response = [
        'message' => $message,
        'data' => $data,
