...
class NewsletterViewNewsletter extends acymailingView
{
    ...
    public $mailReceiver;
    public $filterClass;
    public $nbTotalReceivers;
    public $nbTestReceivers;
    public $statMail;
    ...
    function listing()
    {
        ...
        $selectedTags = acymailing_getUserVar($paramBase."filter_tags", 'filter_tags', [], 'array');
        ...
        $searchMap = ['a.mailid', 'a.alias', 'a.subject', 'a.fromname', 'a.fromemail', 'a.replyname', 'a.replyemail', 'a.userid', 'b.'.$this->cmsUserVars->name, 'b.'.$this->cmsUserVars->username, 'b.'.$this->cmsUserVars->email];
        $filters = [];
        ...
        $selection = ['a.mailid', 'a.alias', 'a.subject', 'a.fromname', 'a.fromemail', 'a.replyname', 'a.replyemail', 'a.userid', 'b.'.$this->cmsUserVars->name.' AS name', 'b.'.$this->cmsUserVars->username.' AS username', 'b.'.$this->cmsUserVars->email.' AS email', 'a.created', 'a.frequency', 'a.senddate', 'a.published', 'a.type', 'a.visible', 'a.abtesting'];
        ...
        if (!empty($pageInfo->filter->order->value) && !in_array($pageInfo->filter->order->value, ['a.date', 'c.email'])) {
    ...
    function form()
    {
        ...
        if (acymailing_isAdmin()) {
            ...
            if (in_array($this->type, ['news', 'followup']) && acymailing_isAllowed($config->get('acl_tags_view', 'all'))) {
    ...
    function preview()
    {
        ...
        $mail->sendHTML = true;
        acymailing_trigger('acymailing_replaceusertags', [&$mail, &$receiver, false]);
        if (!empty($mail->altbody)) $mail->altbody = $mailerHelper->textVersion($mail->altbody, false);
        ...
        if (acymailing_isAdmin()) {
            ...
            if (acymailing_isAllowed($config->get('acl_'.$this->aclCat.'_spam_test', 'all'))) {
                if (!acymailing_level(1) && $config->get('new_install', 0) == 1) {
                    $acyToolbar->custom('', acymailing_translation('SPAM_TEST'), 'spamtest', false, '', '', '', acymailing_translation('ACY_ONLY_PAID_VERSION'));
                } else {
        ...
        if ((!empty($mailids) && strpos($mailids, ',') !== false)) {
            ...
            if (!empty($resDetail) && count($resDetail) != count($mailsArray)) {
                ...
                if (empty($resDetail)) {
                    ...
                    sort($savedIds);
                    sort($mailsArray);
                    if (!empty($abTestDetail['status']) && in_array($abTestDetail['status'], ['inProgress', 'testSendOver', 'abTestFinalSend']) && $savedIds != $mailsArray) {
                ...
                if (!empty($resDetail) && empty($queueCheck) && in_array($abTestDetail['status'], ['inProgress', 'abTestFinalSend'])) {
