	...
	function language(){
		...
		if($loadLatest || acymailing_getVar('cmd', 'task') == 'latest'){
			if(file_exists(acymailing_getLanguagePath(ACYMAILING_ROOT, $code))){
                ...
                $languagesContent = $updatemeHelper->call('public/download/translations?version=5&codes='.acymailing_getVar('cmd', 'code'));

                if (empty($languagesContent['result']['translations'])) {
                    acymailing_enqueueMessage(acymailing_translation('ACY_ERROR_LOAD_LATEST_TRANSLATION'), 'error');
                }else{
                    $file->content = array_pop($languagesContent['result']['translations']);
                }
			}else{
