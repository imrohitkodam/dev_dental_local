...
$uploadPath = ACYMAILING_MEDIA.'import'.DS.str_replace(['.', ' '], '_', substr($filename, 0, strpos($filename, $extension))).$extension;
...
<table <?php echo acymailing_isAdmin() ? 'class="acymailing_table"' : 'class="adminlist"'; ?> cellspacing="10" cellpadding="10" align="center" id="importdata">
    ...
    if (!acymailing_isAdmin()) {
        $allowedColumns = [];
        acymailing_trigger('onAcyPrepareImportColumns', [&$allowedColumns]);
    }
    ...
    if (acymailing_isAllowed($this->config->get('acl_extra_fields_import', 'all'))) {
        $createField = acymailing_selectOption(
            "2",
            acymailing_translation('ACY_CREATE_FIELD'),
            'value',
            'text',
    ...
    foreach ($fields as $oneField) {
        if ($oneField === 'filterflags') continue;

        $fieldAssignment[] = acymailing_selectOption(
            $oneField,
            $oneField,
            'value',
            'text',
            !empty($allowedColumns) && $oneField !== 'email' && !in_array($oneField, $allowedColumns)
        );
    }
    ...
    foreach ($columnNames as $key => &$oneColumn) {
        ...
        $attributes = 'size="1" onchange="checkNewCustom('.$key.')" style="width:180px;"';
        if (!acymailing_isAdmin() && !empty($allowedColumns)) {
            if (empty($default) || ($default === 'name' && !in_array('name', $allowedColumns))) $default = '1';
            if (!$noHeader) $attributes .= ' disabled="disabled"';
        }

        echo '<td align="center" valign="top">';
        echo acymailing_select(
            $fieldAssignment,
            'fieldAssignment'.$key,
            $attributes,
            'value',
