				...
				<table class="acymailing_smalltable">
                    ...
                    if (!empty($this->geolocfields)) {
						...
						<tr class="<?php echo "row$k"; ?>" id="userField_<?php echo $fieldName; ?>">
							...
							<td align="center" style="text-align:center">
                                <?php
                                $values = [acymailing_selectOption('asc', acymailing_translation('SEPARATOR_FIRST_GEOL_SAVED')), acymailing_selectOption('desc', acymailing_translation('ACYEXPORT_LAST_GEOL_SAVED'))];
                        ...
                        foreach ($this->geolocfields as $fieldName => $fieldType) {
                            if (in_array($fieldName, ['geolocation_id', 'geolocation_subid'])) continue;
                            ?>
							<tr class="<?php echo "row$k"; ?>" id="userField_<?php echo $fieldName; ?>">
								<td>
                                    <?php echo $fieldName ?>
					...
					<tr class="<?php echo "row$k";
                    $k = 1 - $k; ?>" id="userField_separator">
						<td>
                            <?php echo acymailing_translation('ACY_SEPARATOR'); ?>
						</td>
						<td align="center" nowrap="nowrap">
                            <?php
                            $values = [acymailing_selectOption('semicolon', acymailing_translation('SEPARATOR_SEMICOLON')), acymailing_selectOption('comma', acymailing_translation('SEPARATOR_COMMA'))];
                            $data = str_replace([';', ','], ['semicolon', 'comma'], $this->config->get('export_separator', ';'));
					...
					<?php if(!empty($_SESSION['acymailing']['acyexportquery'])){ ?>
						<tr class="<?php echo "row$k"; ?>" id="userField_oneline">
							<td>
								<?php echo acymailing_tooltip(acymailing_translation('ACY_EXPORT_ONE_LINE_DESC'), acymailing_translation('ACY_EXPORT_ONE_LINE'), '', acymailing_translation('ACY_EXPORT_ONE_LINE')); ?>
							</td>
							<td align="center" style="text-align:center">
								<?php echo acymailing_boolean("export_oneline", '', $this->config->get('export_oneline', 0) == 1 ? 1 : 0); ?>
							</td>
						</tr>
					<?php } ?>
				</table>
