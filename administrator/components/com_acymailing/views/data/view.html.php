		...
		if(acymailing_getVar('int', 'sessionquery')){
			...
			if(!empty($exportQuery)){
				...
				if(strpos($exportQuery, 'urlclick')){
					$otherFields = array('url.name', 'url.url', 'urlclick.mailid', 'urlclick.date', 'urlclick.ip', 'urlclick.click');
					$this->otherfields = $otherFields;
				}
				if(strpos($exportQuery, 'history')){
					$otherFields = array('hist.data', 'hist.date', 'hist.mailid');
					$this->otherfields = $otherFields;
				}
			}
		}
