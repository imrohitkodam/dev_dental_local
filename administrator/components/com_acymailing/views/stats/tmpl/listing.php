				...
				<tr class="<?php echo "row$k"; ?>">
						...
						<td align="center" style="text-align:center">
							...
							if(!empty($row->senthtml)){
								if($this->config->get('anonymous_tracking', 0) == 0){
                                    ...
                                    $link = '';
                                    if(acymailing_level(1) && $this->config->get('anonymous_tracking', 0) == 0) {
                                        $link = acymailing_completeLink((acymailing_isAdmin() ? '' : 'front').'stats&task=detaillisting&filter_status=open&filter_mail='.$row->mailid);
                                    }

                                    echo acymailing_tooltip($text, $title, '', $pourcent, $link);
								}else{
									echo $row->opentotal;
								}
							}
					...
					<?php if($this->menuparams->get('sent', '1') == 1){ ?>
						<td align="center" style="text-align:center">
							...
							$link = '';
                            if(acymailing_level(1) && $this->config->get('anonymous_tracking', 0) == 0) {
                                $link = acymailing_completeLink((acymailing_isAdmin() ? '' : 'front').'stats&task=detaillisting&filter_status=0&filter_mail='.$row->mailid);
                            }

							echo acymailing_tooltip($text, $title, '', $row->senthtml + $row->senttext, $link);
                            ?>
						</td>
					<?php } ?>
					<?php if(acymailing_level(3) && $this->menuparams->get('bounces', '1') == 1){ ?>
