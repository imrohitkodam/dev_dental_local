...
class NotificationViewNotification extends NewsletterViewNewsletter{
	...
	function listing(){
		...
		$queryFilters[] = '`type` = '.acymailing_escapeDB($this->type);
		...
		if(!empty($pageInfo->search)){
			...
			$queryFilters[] = "subject LIKE $searchVal OR body LIKE $searchVal";
		}
		...
		if(ACYMAILING_J16){
			...
			if(!empty($pageInfo->category)){
				$queryFilters[] = "alias LIKE '".acymailing_getEscaped($pageInfo->category, true)."-%'";
			}
			$catvalues = array();
		...
		$query = 'SELECT mailid, subject, alias, fromname, published, fromname, fromemail, replyname, replyemail FROM #__acymailing_mail WHERE ('.implode(') AND (', $queryFilters).')';
		...
		$queryCount = 'SELECT count(mailid) FROM #__acymailing_mail WHERE ('.implode(') AND (', $queryFilters).')';
