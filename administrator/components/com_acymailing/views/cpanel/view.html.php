...
class CpanelViewCpanel extends acymailingView{
    ...
    public $plugins;
    public $integrationplugins;
    public $disabledClass;
    public $disabledState;
    public $acycheckerInstalled;
    public $acycheckerLink;
    public $bounceaction;
    public $languages;
    public $elements;
    public $tabs;
    ...
    function display($tpl = null){
		...
		function confirmHTTPS(element){
			...
			if(clickedHTTPS){
				alert(\''.str_replace("'", "\'", acymailing_translation('ACY_SSLCHOICE_CONFIRMATION')).'\');';
		$js .= '
			}
		...
		if(acymailing_level(3)){
			...
			if($geolocAvailable){
				...
				$ip2location_api_key = '<input class="inputbox" type="text" id="ip2location_api_key" name="config[ip2location_api_key]" style="width:450px" value="'.$this->escape($config->get('ip2location_api_key', '')).'">';
        ...
        $disabledClass = '';
        $disabledState = '';
        if(acy_isAcyCheckerInstalled() && $config->get('email_verification', 0) == 0){
            $disabledClass = ' style="display: none;" ';
        }
        if(!acy_isAcyCheckerInstalled()){
            $disabledClass = ' class="acymailing_disabled" ';
            $disabledState = ' disabled="disabled" ';
        }
        $this->disabledClass = $disabledClass;
        $this->disabledState = $disabledState;
        $jsAcyChecker = "function enableAcyChekerTests(checkEmail){
						if(checkEmail == 1){document.getElementById('acychecker_config').style.display = 'block';}
						else{document.getElementById('acychecker_config').style.display = 'none';}
						};";
        $elements->acychecker = acymailing_boolean(
            "config[email_verification]",
            'onclick="enableAcyChekerTests(this.value)"'.$disabledState,
            $config->get('email_verification', 0)
        );
        acymailing_addScript(true, $jsAcyChecker);
        $this->acycheckerInstalled = acy_isAcyCheckerInstalled();
        $this->acycheckerLink = ACYMAILING_ACYCHECKER_WEBSITE.'?utm_source=acymailing_plugin&utm_campaign=get_acychecker&utm_medium=button_configuration_security';
