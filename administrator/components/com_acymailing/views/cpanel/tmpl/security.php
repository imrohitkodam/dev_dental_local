...
?><div id="page-security">
	...
	<div class="onelineblockoptions">
		...
		<table class="acymailing_table" cellspacing="1">
			...
			<tr class="acychecker_ad">
				<td colspan="4">
					<h6>
                        <?php echo acymailing_translation('ACY_ACYCHECKER_CONFIG_AD_TITLE'); ?>
						<img class="acychecker_logo" alt="logo AcyChecker" src="<?php echo ACYMAILING_IMAGES.'icons/logo_acychecker.png'; ?>" />
					</h6>
				</td>
			</tr>
			<tr class="acychecker_ad">
				<td class="acykey">
                    <?php echo acymailing_translation('ACY_ACYCHECKER_CHECK_SUBSCRIPTION'); ?>
				</td>
				<td>
                    <?php echo $this->elements->acychecker; ?>
				</td>
			</tr>
			<tr class="acychecker_ad">
				<td colspan="4">
					<div id="acychecker_config" <?php echo $this->disabledClass; ?>>
						<table class="acymailing_table">
							<tr class="acychecker_ad">
								<td><?php echo acymailing_translation('ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK'); ?></td>
							</tr>
                            <?php
                            $verificationOptions = [
                                'email_verification_non_existing' => ['ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_NON_EXISTING', ''],
                                'email_verification_disposable' => [
                                    'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_DISPOSABLE',
                                    'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_DISPOSABLE_DESC',
                                ],
                                'email_verification_free' => ['ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_FREE', 'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_FREE_DESC'],
                                'email_verification_role' => ['ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_ROLE', 'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_ROLE_DESC'],
                                'email_verification_acceptall' => [
                                    'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_ACCEPT_ALL',
                                    'ACY_ACYCHECKER_CHECK_SUBSCRIPTION_BLOCK_ACCEPT_ALL_DESC',
                                ],
                            ];
                            ...
                            foreach ($verificationOptions as $option => $label) {
								...
								<tr class="acychecker_ad">
									<td>
                                        <?php
                                        $text = acymailing_translation($label[0]);
                                        if (!empty($label[1])) {
                                            echo acymailing_tooltip(acymailing_translation($label[1]), $text, '', $text);
                                        } else {
                                            echo acymailing_translation($text);
                                        }
                                        ?>
									</td>
									<td>
                                        <?php echo acymailing_boolean('config['.$option.']', $this->disabledState, $this->config->get($option, 0)); ?>
									</td>
                                    <?php if ($first) { ?>
										<td rowspan="5" colspan="2" id="acyc_not_installed">
                                            <?php if (!$this->acycheckerInstalled) {
                                                echo acymailing_translation('ACY_ACYCHECKER_CONFIG_AD'); ?>
												<br /><br />
												<a target="_blank" href="<?php echo $this->acycheckerLink; ?>">
                                                    <?php echo acymailing_translation('ACY_ACYCHECKER_CONFIG_AD_BUTTON'); ?>
												</a>
                                            <?php } ?>
										</td>
                                        <?php
