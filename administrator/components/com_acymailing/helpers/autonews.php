...
class acyautonewsHelper
{
    ...
    private function _generateAutoNews($newNewsletter)
    {
        ...
        if (!empty($notifyUsers)) {
            ...
            $allUsers = explode(' ', trim(str_replace([';', ','], ' ', $notifyUsers)));
    ...
    {
        $results = acymailing_trigger('acymailing_generateautonews', [&$oneAutonews]);
        $return = true;
        ...
        if (is_numeric($oneAutonews->frequency)) {
            ...
            if (strpos($oneAutonews->frequency, 'on_') !== false) {
                ...
                if ($currentDayKey === false) {
                    ...
                    $newMail->senddate = empty($oneAutonews->senddate) ? time() + 86400 : $oneAutonews->senddate + 86400;
                } else {
                    if (isset($values[$currentDayKey + 1])) {
