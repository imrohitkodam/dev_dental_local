	...
	function replaceVideos($text){
		...
		if(preg_match_all('#<iframe[^>]*src="([^"]*vimeo[^"]*)"[^>]*>[^<]*</iframe>#Uis', $text, $matches)){
			foreach($matches[1] as $key => $match){
				if(substr($matches[1][$key], 0, 2) == '//') $matches[1][$key] = 'https:'.$matches[1][$key];
				$xml = acymailing_fileGetContent('https://vimeo.com/api/oembed.json?url='.urlencode($matches[1][$key]));
				...
				$text = str_replace($matches[0][$key], '<a target="_blank" href="'.($matches[1][$key]).'"><img class="donotresize" alt="" src="'.($xml->thumbnail_url_with_play_button).'" /></a>', $text);
			}
		}
