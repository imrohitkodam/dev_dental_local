...
class acybounceHelper
{
    ...
    public function init()
    {
        ...
        if ($this->report) {
            acymailing_display('The extension "' . $EXTENSION . '" could not be loaded, please change your PHP configuration to enable it or use the pop3 method without imap extension',
                               'error');
            if (!empty($warnings)) {
    ...
    private function _connectpear()
    {
        ...
        if (!$this->mailbox->connect($serverName, $port)) {
            ...
            if ($this->report) {
                acymailing_enqueueMessage("Error connecting to the server " . $this->server . " : " . $port, 'error');
            }
        ...
        if (empty($login) or isset($login->code)) {
            $warnings = ob_get_clean();
            if ($this->report) {
                acymailing_enqueueMessage('Identication error ' . $this->username . ':' . $this->password, 'error');
            }
            if (!empty($warnings) and $this->report) {
                acymailing_display($warnings, 'warning');
            }
    ...
    private function decodeMessage($attachments = false)
    {
        if ($this->usepear) {
            return $this->_decodeMessagepear($attachments);
        } else {
            return $this->_decodeMessageimap($attachments);
        }
    }

    private function _decodeMessagepear($attachments)
    {
                    ...
                    if (strpos($segment, "Content-Type: text/plain") !== false) {
                        ...
                        if (!empty($matches[1])) {
                            ...
                            foreach ($parts as $onePart) {
                                ...
                                if (strpos($onePart, 'charset=') !== false) {
                                    preg_match('#charset="([^"]+)"#Uis', $onePart, $charset);
                                    if (!empty($charset[1])) {
                                        $charset = trim(strtoupper($charset[1]));
                        ...
                        if (preg_match("#Content-Disposition: attachment;#i", $segment) === false) {
                            $filename = preg_replace('#[^a-zA-Z0-9]#Uis', '_', substr($filename, 0, $extensionPos));
                        } else {
                            $filename = substr($filename, 0, $extensionPos);
                        }

                        $uploadFolder = str_replace(['/', '\\'], DS, acymailing_getFilesFolder());
    ...
    private function _decodeMessageimap($attachments)
    {
    ...
    private function _uploadAttachment($attachment, $num)
    {
        ...
        try {
            if (acymailing_writeFile($pathToUpload . $filename . '.' . $extension, $data)) {
                if (strtolower($attachment->disposition) != "attachment" && !empty($attachment->id) && in_array(strtolower($attachment->subtype),
    ...
    public function handleMessages()
    {
        ...
        while (($msgNB > 0) && ($this->_message = $this->getMessage($msgNB))) {
            ...
            $this->_display('<b>' . acymailing_translation('JOOMEXT_SUBJECT') . ' : ' . strip_tags($this->_message->subject) . '</b>',
                            false,
            ...
            if (!empty($resultsVars[1])) {
                $this->_message->subid = $resultsVars[1];
            }
            ...
            if (empty($this->_message->mailid)) {
                ...
                if (!empty($resultsMailid[1])) {
                    $this->_message->mailid = $resultsMailid[1];
                }
            }
            ...
            if (!empty($this->stoptime) and time() > $this->stoptime) {
                break;
            }
        }
        ...
        if (!empty($this->bounceMessages)) {
            foreach ($this->bounceMessages as $mailid => $bouncedata) {
                ...
                if (!empty($bouncedata['subid'])) {
                    ...
                    acymailing_query('INSERT INTO ' . acymailing_table('userstats') . ' (mailid,subid,bounce,bouncerule) VALUES ' . implode(',',
                                                                                                                                            $valueInsert) . ' ON DUPLICATE KEY UPDATE `bounce` = `bounce` + 1, bouncerule=VALUES(bouncerule)');

                    $realUniqueBounces = acymailing_loadResult('SELECT COUNT(*) FROM ' . acymailing_table('userstats') . ' WHERE `bounce` = 1 AND `subid` IN (' . implode(',',
                                                                                                                                                                          $bouncedata['subid']) . ') AND `mailid` = ' . (int) $mailid);
    ...
    private function _handleRule(&$oneRule)
    {
        ...
        if (isset($oneRule->executed_on['subject'])) {
    ...
    private function _actionuser(&$oneRule)
    {
        ...
        if (empty($this->_message->subemail)) {
            $this->_message->subemail = $this->_message->subid;
        }
        ...
        if (isset($oneRule->action_user['stats']) && !empty($this->_message->mailid)) {
            ...
            if (empty($this->bounceMessages[$this->_message->mailid])) {
                ...
                $this->bounceMessages[$this->_message->mailid]['subid'] = [];
        ...
        if (!empty($oneRule->action_user['min']) && $oneRule->action_user['min'] > 1) {
            ...
            if ($nb < $oneRule->action_user['min']) {
                $message .= ' | ' . acymailing_translation_sprintf('BOUNCE_RECEIVED', $nb, $this->_message->subemail) . ' | ' . acymailing_translation_sprintf('BOUNCE_MIN_EXEC',
                                                                                                                                                               $oneRule->action_user['min']);
        ...
        if (isset($oneRule->action_user['sub']) && !empty($oneRule->action_user['subscribeto'])) {
            ...
            if (empty($status[$listId])) {
                $this->listsubClass->addSubscription($this->_message->subid, ['1' => [$listId]]);
            } elseif ($status[$listId]->status != 1) {
                $this->listsubClass->updateSubscription($this->_message->subid, ['1' => [$listId]]);
            }
        }
        ...
        if (isset($oneRule->action_user['unsub'])) {
            ...
            if (!empty($unsubLists)) {
                ...
                $this->listsubClass->updateSubscription($this->_message->subid, ['-1' => $unsubLists]);
            } else {
                $message .= ' | user ' . $this->_message->subemail . ' not subscribed';
            }
        }
        ...
        if (isset($oneRule->action_user['emptyq'])) {
            ...
            $message .= ' | user ' . $this->_message->subemail . ' ( ' . acymailing_translation('ACY_ID') . ' ' . intval($this->_message->subid) . ' ) queue: ' . acymailing_translation_sprintf('SUCC_DELETE_ELEMENTS',
                                                                                                                                                                                                 $affected);
        }
    ...
    private function _actionmessage(&$oneRule)
    {
        ...
        if (!empty($oneRule->action_message['forwardto'])) {
            ...
            if (substr_count($oneRule->action_message['forwardto'], '@') > 1) {
                $forwardAddresses = explode(';', str_replace([';', ','], ';', $oneRule->action_message['forwardto']));
            } else {
                $forwardAddresses = [$oneRule->action_message['forwardto']];
            }
    ...
    {
        if (empty($this->action) || empty($this->action->actions)) {
            return;
        }
    ...
    function passConditions()
    {
        ...
        if ($conditions['sender'] != 'all') {
            ...
            if ($conditions['sender'] == 'group') {
                $userid = acymailing_loadResult('SELECT ' . $this->cmsUserVars->id . ' AS id FROM ' . acymailing_table($this->cmsUserVars->table,
                                                                                                                       false) . ' WHERE ' . $this->cmsUserVars->email . ' = ' . acymailing_escapeDB($fromEmail));
                if (empty($userid)) {
            ...
            if ($conditions['sender'] == 'list' && !empty($conditions['listids']) && strtolower($conditions['listids']) != 'none') {
                ...
                $isAllowed = acymailing_loadResult(
                    'SELECT s.`subid` FROM ' . acymailing_table('subscriber') . ' AS s
                ...
                if (empty($isAllowed)) {
                    $this->_display(acymailing_translation('ACY_SENDER_NOT_ALLOWED') . ': ' . htmlspecialchars($fromEmail,
                                                                                                               ENT_QUOTES,
                                                                                                               'UTF-8') . ' is not an AcyMailing user or not subscribed to one of the allowed lists.');
        ...
        if ($conditions['subject'] != 'all') {
            ...
            if ($conditions['subject'] == 'begins' && strpos($this->_message->subject, $conditions['subjectvalue']) !== 0) {
                $passSubject = false;
            }
            if ($conditions['subject'] == 'ends' && strpos($this->_message->subject,
                                                           $conditions['subjectvalue']) !== strlen($this->_message->subject) - strlen($conditions['subjectvalue'])) {
                $passSubject = false;
            }
            if ($conditions['subject'] == 'contains' && strpos($this->_message->subject, $conditions['subjectvalue']) === false) {
                $passSubject = false;
            }
    ...
    function executeActions()
    {
        ...
        foreach ($this->action->actions as $i => $oneAction) {
            if (empty($oneAction['type']) || $oneAction['type'] == 'none') {
                continue;
            }

            $this->_display('<strong>Action ' . (intval($i) + 1) . '</strong>', true);

            if (in_array($oneAction['type'], ['forward', 'forwardlist'])) {
                ...
                if (empty($newsletters[$oneAction['template']])) {
                    ...
                    if (!empty($this->action->senderfrom)) {
                        ...
                        $newNewsletter->fromname = strip_tags($this->_decodeHeader($this->_message->header->from_name));
                    }

                    if (!empty($this->action->senderto)) {
                        $newNewsletter->replyemail = $this->_decodeHeader($this->_message->header->from_email);
                        $newNewsletter->replyname = strip_tags($this->_decodeHeader($this->_message->header->from_name));
                    }

                    if (!empty($this->attachments)) {
                        $newNewsletter->attach = serialize($this->attachments);
                    }

                    if ($oneAction['type'] == 'forwardlist') {
                        if (!empty($this->_message->fromaddress)) {
                            $newNewsletter->body = str_replace('{mailheader:from}', htmlspecialchars($this->_message->fromaddress, ENT_QUOTES, 'UTF-8'), $newNewsletter->body);
                        }
                        if (!empty($this->_message->toaddress)) {
                            $newNewsletter->body = str_replace('{mailheader:to}', htmlspecialchars($this->_message->toaddress, ENT_QUOTES, 'UTF-8'), $newNewsletter->body);
                        }

                        if (!empty($this->_message->ccaddress)) {
                            $newNewsletter->body = str_replace('{mailheader:cc}', htmlspecialchars($this->_message->ccaddress, ENT_QUOTES, 'UTF-8'), $newNewsletter->body);
                        }

                        if (!empty($this->_message->Date)) {
                            $newNewsletter->body = str_replace('{mailheader:date}', date("d.m.Y H:i", strtotime($this->_message->Date)), $newNewsletter->body);
                        }

                        $newNewsletter->body = str_replace(
                            [
                                '{mailheader:from}',
                                '{mailheader:to}',
                                '{mailheader:cc}',
            ...
            if ($oneAction['type'] == 'forward') {
                ...
                foreach ($receivers as $oneReceiver) {
                    if ($mailHelper->sendOne($newsletter->mailid, $oneReceiver)) {
                        $this->_display(acymailing_translation_sprintf('SEND_SUCCESS',
                                                                       $newsletter->subject . ' (' . $newsletter->mailid . ')',
                                                                       htmlspecialchars($oneReceiver, ENT_QUOTES, 'UTF-8')),
                                        true);
                    } else {
                        $this->_display(acymailing_translation_sprintf('SEND_ERROR',
                                                                       $newsletter->subject . ' (' . $newsletter->mailid . ')',
                                                                       htmlspecialchars($oneReceiver, ENT_QUOTES, 'UTF-8')));
                    }
                }
            ...
            if ($oneAction['type'] == 'forwardlist') {
                $receivers = [];
                $receivers[] = intval($oneAction['list']);
                $remove = [];

                $listMailClass->save($newsletters[$oneAction['template']], $receivers, $remove);

                $affected = $queueClass->queue($newsletters[$oneAction['template']], time());
            ...
            if ($oneAction['type'] == 'subscribe') {
                ...
                $subscribed = $listsubClass->addSubscription($subscriber->subid, ['1' => [$oneAction['list']]]);
            ...
            if ($oneAction['type'] == 'unsubscribe') {
                ...
                if (empty($subscriber->subid)) {
                    ...
                    $affected = $this->listsubClass->updateSubscription($subscriber->subid, ['-1' => [$oneAction['list']]]);
