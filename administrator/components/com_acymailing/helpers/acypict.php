...
class acypictHelper
{
    ...
    function resizePictures($input)
    {
        ...
        foreach ($results[1] as $onepicture) {
            ...
            $base = str_replace(['http://www.', 'https://www.', 'http://', 'https://'], '', ACYMAILING_LIVE);
            ...
            foreach ($replacements as $oneReplacement) {
                ...
                $imageUrl = str_replace([$oneReplacement, '/'], [ACYMAILING_ROOT, DS], urldecode($imageUrl));
                break;
            }
    ...
    function generateThumbnail($picturePath)
    {
        ...
        if (file_exists($newFile)) return ['file' => $newFile, 'width' => $newWidth, 'height' => $newHeight];
