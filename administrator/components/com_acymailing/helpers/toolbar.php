...
class acytoolbarHelper
{
    ...
    function custom($task, $text, $class, $listSelect = true, $onClick = '', $title = '', $link = '', $tooltipText = '')
    {

        $submit = empty($task) ? "return false" : "acymailing.submitbutton('".$task."')";
        $js = !empty($listSelect) ? "if(document.adminForm.boxchecked.value==0){alert('".str_replace(["'", '"'], ["\'", '\"'], acymailing_translation('ACY_SELECT_ELEMENT'))."');return false;}else{".$submit."}" : $submit;
        ...
        if (empty($link)) {
            ...
            if (!empty($tooltipText)) $button = acymailing_tooltip($tooltipText, '', '', $button);
        } else {
            $button = '<a style="color: white;" id="toolbar-'.$class.'" class="acytoolbar_'.$class.'" href="'.$link.'" target="_blank"><i class="acyicon-'.$class.'"></i><span>'.$text.'</span></a>';
        }
        ...
        $dropdownOptions = '<ul class="buttonOptions" style="margin: 0; text-align: left;">';
    ...
    {
        $url = 'https://docs.acymailing.com/v/old/';
        $this->custom('help', acymailing_translation('ACY_HELP'), 'help', false, '', '', $url);
    }
    ...
    {
        $ids = '';
        if (in_array($name, ['ABtesting', 'action'])) {
            $js = "
