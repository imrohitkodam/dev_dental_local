...
class acyuserHelper
{
    ...
    function exportdata($id)
    {
        ...
        $dateFields = ['created', 'confirmed_date', 'lastopen_date', 'lastclick_date', 'lastsent_date', 'userstats_opendate', 'userstats_senddate', 'urlclick_date', 'hist_date'];
        $excludedFields = ['userid', 'subid', 'html', 'key', 'source', 'filterflags'];
        ...
        foreach ($subscriber as $column => $value) {
            ...
            if (in_array($fields[$column]->type, ["singledropdown", "multipledropdown", "radio", "checkbox"])) {
        ...
        if (!empty($subscription)) {
            ...
            foreach ($subscription as $oneSubscription) {
                ...
                foreach ($oneSubscription as $column => $value) {
                    ...
                    if ($column == 'status') $value = str_replace(['-1', '1', '2'], ['Unsubscribed', 'Subscribed', 'Waiting for confirmation'], $value);
        ...
        if (!empty($geolocation)) {
            $dateFields = ['geolocation_created'];
            $excludedFields = ['geolocation_id', 'geolocation_subid'];
        ...
        $history = acymailing_loadObjectList(
        ...
        if (!empty($statistics)) {
            $dateFields = ['senddate', 'opendate'];
            $excludedFields = ['subid'];
