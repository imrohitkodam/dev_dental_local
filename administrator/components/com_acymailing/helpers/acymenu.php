...
class acymenuHelper{
	function display($selected = ''){
		...
		if(acymailing_isAllowed($config->get('acl_statistics_manage', 'all'))){
			...
			if(acymailing_level(1) && $config->get('anonymous_tracking', 0) == 0) $submenu['stats'][] = array(acymailing_translation('DETAILED_STATISTICS'), acymailing_completeLink('stats&task=detaillisting'), 'acyicon-detailed-stat');
		...
		$acysmsLink = '';
		if(acymailing_isAllowed($config->get('acl_configuration_manage', 'all')) && file_exists(ACYMAILING_ROOT.'components'.DS.'com_acysms') && JComponentHelper::isEnabled('com_acysms')){
		    $acysmsLink = '<a class="sendother" href="index.php?option=com_acymailing&ctrl=update&task=acysms">'.acymailing_translation('ACY_SMS').'&nbsp;&nbsp;<i class="acyicon-message"></i></a>';
        }
	...
	public function myacymailingarea(){
		...
		if(version_compare($currentVersion, $latestVersion, '>=')){
			...
			$menu .= '<div class="acyversion_needtoupdate myacymailingbuttons"><a class="acy_updateversion" href="https://www.acymailing.com/account/license/" target="_blank"><i class="acyicon-import"></i>'.acymailing_translation_sprintf('ACY_UPDATE_NOW', $latestVersion).'</a></div>';
		}
		...
		if(acymailing_level(1)){
			...
			if(empty($expirationDate) || $expirationDate == -1){
                ...
                $licenseTabConfig = acymailing_completeLink('cpanel');
				$menu .= '<div id="myacymailing_expiration">
                            <div class="acylicence_expired">
                                <span style="color:#c2d5f3; line-height: 16px;">'.acymailing_translation('ACY_ATTACH_LICENCE').' :</span>
                                <div>
                                    <a class="acy_attachlicence myacymailingbuttons" href="'.$licenseTabConfig.'" onclick="localStorage.setItem(\'acyconfig_tab\', \'config_license\');">
                                        <i class="acyicon-attach"></i>'.acymailing_translation('ACY_ATTACH_LICENCE_BUTTON').'
                                    </a>
                                </div>
                            </div>
                        </div>';
			}elseif($expirationDate < time()){
				$menu .= '<div id="myacymailing_expiration"><div class="acylicence_expired"><span class="acylicenceinfo">'.acymailing_translation('ACY_SUBSCRIPTION_EXPIRED').'</span><a class="acy_subscriptionexpired myacymailingbuttons" href="https://www.acymailing.com/account/license/" target="_blank"><i class="acyicon-renew"></i>'.acymailing_translation('ACY_SUBSCRIPTION_EXPIRED_LINK').'</a></div></div>';
			}else{
