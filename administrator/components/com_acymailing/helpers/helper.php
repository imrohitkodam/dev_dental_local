...
define('ACYMAILING_WEBSITE', 'https://www.acymailing.com/');
define('ACYMAILING_UPDATEME_API_URL', 'https://api.acymailing.com/');
define('ACYMAILING_COMPONENT_NAME_API', 'acymailing');
define('ACYMAILING_ACYCHECKER_WEBSITE', 'https://www.acychecker.com/');
...
function acymailing_makeCurlCall($url, $fields)
{
    $urlPost = '';
...
function acymailing_getEmailRegex($secureJS = false, $forceRegex = false)
{
    ...
    if ($secureJS) $regex = str_replace(['"', "'"], ['\"', "\'"], $regex);

    return $regex;
}

function acymailing_dump($arg, $ajax = false)
{
    ob_start();
    var_dump($arg);
    $result = ob_get_clean();
    ...
    if ($ajax) {
        file_put_contents(ACYMAILING_ROOT.'acymailing_debug.txt', $result, FILE_APPEND);
    } else {
        echo '<pre>'.$result.'</pre>';
    }
}
...
function acymailing_navigationTabs()
{
    ...
    $pages = [
        'subscriber' => [
            'ACY_SUBSCRIBER' => ['ctrl' => 'subscriber', 'task' => ''],
        ],
        'list' => [
            'LISTS' => ['ctrl' => 'list', 'task' => ''],
        ],
        'newsletter' => [
            'NEWSLETTERS' => ['ctrl' => 'newsletter', 'task' => ''],
        ],
        'stats' => [
            'STATISTICS' => ['ctrl' => 'stats', 'task' => 'listing'],
        ],
        'cpanel' => [
            'ACY_CONFIGURATION' => ['ctrl' => 'cpanel', 'task' => ''],
            'EXTRA_FIELDS' => ['ctrl' => 'fields', 'task' => ''],
            'BOUNCE_HANDLING' => ['ctrl' => 'bounces', 'task' => ''],
        ],
    ];

    if (acymailing_isAllowed($config->get('acl_subscriber_import', 'all'))) $pages['subscriber']['IMPORT'] = ['ctrl' => 'data', 'task' => 'import'];
    if (acymailing_isAllowed($config->get('acl_subscriber_export', 'all'))) $pages['subscriber']['ACY_EXPORT'] = ['ctrl' => 'data', 'task' => 'export'];
    if (acymailing_isAllowed($config->get('acl_lists_filter', 'all'))) $pages['subscriber']['ACY_MASS_ACTIONS'] = ['ctrl' => 'filter', 'task' => ''];

    if (acymailing_isAllowed($config->get('acl_distribution_manage', 'all'))) $pages['list']['ACY_DISTRIBUTION'] = ['ctrl' => 'action', 'task' => ''];

    if (acymailing_level(2) && acymailing_isAllowed($config->get('acl_autonewsletters_manage', 'all'))) {
        $pages['newsletter']['AUTONEWSLETTERS'] = ['ctrl' => 'autonews', 'task' => ''];
    }
    if (acymailing_level(3) && acymailing_isAllowed($config->get('acl_campaign_manage', 'all'))) {
        $pages['newsletter']['ACY_CAMPAIGNS'] = ['ctrl' => 'campaign', 'task' => ''];
    }
    if (acymailing_level(3) && acymailing_isAllowed($config->get('acl_simple_sending_manage', 'all'))) {
        $pages['newsletter']['SIMPLE_SENDING'] = ['ctrl' => 'simplemail', 'task' => 'edit'];
    }
    if (acymailing_isAllowed($config->get('acl_templates_manage', 'all'))) {
        $pages['newsletter']['ACY_TEMPLATES'] = ['ctrl' => 'template', 'task' => ''];
    }
    if (acymailing_isAllowed($config->get('acl_queue_manage', 'all'))) {
        $pages['newsletter']['QUEUE'] = ['ctrl' => 'queue', 'task' => ''];
    }

    if (acymailing_level(1) && $config->get('anonymous_tracking', 0) == 0) $pages['stats']['DETAILED_STATISTICS'] = ['ctrl' => 'stats', 'task' => 'detaillisting'];
    if (acymailing_level(1)) $pages['stats']['CLICK_STATISTICS'] = ['ctrl' => 'statsurl', 'task' => ''];
    if (acymailing_level(1)) $pages['stats']['CHARTS'] = ['ctrl' => 'diagram', 'task' => ''];
    ...
    foreach ($pages[$page] as $text => $oneCtrl) {
        ...
        $navigationTabs[] = '<li'.($active ? ' class="active"' : '').'><a href="'.acymailing_completeLink($oneCtrl['ctrl']).(empty($oneCtrl['task']) ? ''
                : '&task='.$oneCtrl['task']).'">'.acymailing_translation($text).'</a></li>';
    }

	$level = $config->get('level', 'enterprise');
	$version = $config->get('version', '5.13.0');
?>
    <div class="acytabsystem acymailing_top_menu">
        <ul class="acynavigationtabs nav nav-tabs">
			<?php echo implode('', $navigationTabs); ?>
			<li id="acymailing_top_menu_version">AcyMailing <?php echo $level; ?> <div style="color: red"><?php echo $version; ?></div></li>
		</ul>
	</div>
<?php
}
...
function acymailing_isRobot()
{
    ...
    if (!empty($_SERVER['REMOTE_ADDR'])
        && version_compare($_SERVER['REMOTE_ADDR'], '64.235.144.0', '>=')
        && version_compare($_SERVER['REMOTE_ADDR'], '64.235.159.255', '<=')) {
        return true;
    }
...
function acymailing_getFunctionsEmailCheck($controllButtons = [], $bounce = false)
{
...
function acymailing_getUpgradeLink($tolevel)
{
    ...
    return ' <a class="acyupgradelink" href="https://www.acymailing.com/account/license/" target="_blank">'.acymailing_translation(
            'ONLY_FROM_'.strtoupper($tolevel)
        ).'</a>';
}
...
function acymailing_absoluteURL($text)
{
    ...
    $text = str_replace(
        ['href="../undefined/', 'href="../../undefined/', 'href="../../../undefined//', 'href="undefined/', ACYMAILING_LIVE.'http://', ACYMAILING_LIVE.'https://'],
        ['href="'.$mainurl, 'href="'.$mainurl, 'href="'.$mainurl, 'href="'.ACYMAILING_LIVE, 'http://', 'https://'],
        $text
    );
    ...
    $text = preg_replace('#href="'.preg_quote(str_replace(['http://', 'https://'], '', $mainurl), '#').'#Ui', 'href="'.$mainurl, $text);
...
function acymailing_listingsearch($search)
{
    ...
    $searchBar .= '<input placeholder="'.acymailing_translation('ACY_SEARCH').'" type="text" name="search" id="search" value="'.htmlspecialchars(
            $search,
            ENT_COMPAT,
            'UTF-8'
        ).'" class="text_area" title="'.acymailing_translation('ACY_SEARCH').'"/>';
    $searchBar .= '<button style="float:none;" onclick="document.adminForm.task.value=\'\';document.adminForm.limitstart.value=0;this.form.submit();" class="btn tip hasTooltip" type="submit" title="'.acymailing_translation(
            'ACY_SEARCH'
        ).'"><i class="acyicon-search"></i></button>';
    $searchBar .= '<button style="float:none;margin-left:0px;" onclick="document.adminForm.task.value=\'\';document.adminForm.limitstart.value=0;document.getElementById(\'search\').value=\'\';this.form.submit();" class="btn tip hasTooltip" type="button" title="'.acymailing_translation(
            'JOOMEXT_RESET'
        ).'"><i class="acyicon-cancel"></i></button>';
    $searchBar .= '</div>';
    echo $searchBar;
}
...
function acymailing_initModule($params)
{
    ...
    if (!empty($moduleCSS)) {
        if ($includejs == 'header') {
            ...
            echo "\n".'<link rel="stylesheet" property="stylesheet" href="'.ACYMAILING_CSS.'module_'.$moduleCSS.'.css?v='.filemtime(
                    ACYMAILING_MEDIA.'css'.DS.'module_'.$moduleCSS.'.css'
                ).'" type="text/css" />'."\n";
        }
    }
}
...
function acymailing_footer()
{
    ...
    $text = '<!-- '.ACYMAILING_NAME.' Component powered by https://www.acymailing.com -->
		<!-- version '.$config->get('level').' : '.$config->get('version').' -->';
    if (acymailing_level(1) && !acymailing_level(4)) return $text;
    $level = $config->get('level');
    $text .= '<div class="acymailing_footer" align="center" style="text-align:center"><a href="'.ACYMAILING_WEBSITE.'?utm_source=acymailing-'.$level.'&utm_medium=front-end&utm_content=txt&utm_campaign=powered-by" target="_blank" title="'.ACYMAILING_NAME.' : '.str_replace(
            'TM ',
            ' ',
...
{
    $oneResult = acymailing_getVar('array', 'cid', [], '');
...
function acymailing_importFile($file, $uploadPath, $onlyPict, $maxwidth = '')
{
    ...
    if (file_exists($uploadPath.DS.$file["name"])) {
        ...
        if ($onlyPict) $additionalMsg .= '<br /><a style="color: blue; cursor: pointer;" onclick="confirmBox(\'rename\', \''.$file['name'].'\', \''.$nameFile.'.'.$ext.'\')">'.acymailing_translation(
                'ACY_RENAME_OR_REPLACE'
            ).'</a>';
    }
    ...
    foreach ($folders as $k => $folder) {
        ...
        if (strpos($folder, '{listalias}') !== false) {
            ...
            foreach ($allLists as $oneList) {
                $newFolders[] = str_replace('{listalias}', strtolower(str_replace([' ', '-'], '_', $oneList->alias)), $folders[$k]);
            }
        ...
        if (strpos($folder, '{groupid}') !== false || strpos($folder, '{groupname}') !== false) {
            ...
            if (ACYMAILING_J16) {
                ...
                $completeGroups = [$groupObject];
            }

            foreach ($completeGroups as $group) {
                $newFolders[] = str_replace(['{groupid}', '{groupname}'], [$group->id, strtolower(str_replace(' ', '_', $group->title))], $folders[$k]);
            }
...
function acymailing_getFolders($path, $filter = '.', $recurse = false, $full = false, $exclude = ['.svn', 'CVS', '.DS_Store', '__MACOSX'], $excludefilter = ['^\..*'])
{
    $path = acymailing_cleanPath($path);
...
function acymailing_createArchive($name, $files)
{
    ...
    foreach ($files as $file) {
        ...
        $cdrec .= pack('V', $crc).pack('V', $c_len).pack('V', $unc_len).pack('v', strlen($filename)).pack('v', 0).pack('v', 0).pack('v', 0).pack('v', 0).pack('V', 32).pack(
                'V',
                $old_offset
            ).$filename;
...
function acymailing_raiseError($code, $message)
{
    echo '<style>
        #acymailing_messages_error.alert{
        ...
        #acymailing_messages_error.alert-error{
            background-color: rgb(242, 222, 222);
            border-color: rgb(238, 211, 215);
            color: rgb(185, 74, 72);
        }
        </style>';
    acymailing_display('Error '.$code.': '.$message, 'error');
    http_response_code($code);
    exit;
}

function acy_isAcyCheckerInstalled()
{
    $installed = 'joomla' === 'joomla' && acymailing_isExtensionActive('com_acychecker');

    return $installed || ('joomla' === 'wordpress' && acymailing_isExtensionActive('acychecker/acychecker.php'));
}

function acy_utf8Encode($string)
{
...
function acy_utf8Decode($string)
{
    if (function_exists('mb_convert_encoding')) {
...
class acymailingClass
{
    ...
    function delete($elements)
    {
        ...
        foreach ($this->tables as $oneTable) {
            acymailing_trigger('onAcyBefore'.ucfirst($oneTable).'Delete', [&$elements]);
...
class acyEmoji
{
    ...
    private static function convertEmoji($text, $op)
    {
        ...
        if ($op == "ENCODE") {
            ...
            return @preg_replace_callback('/(\\\u[0-9a-f]{4})+/i', ['self', "decodeEmoji"], $text);
        }
    }
...
class acyPagination
{
    ...
    function getListFooter()
    {
        ...
        $onclick = $this->start - $this->value >= 0 ? '" onclick="document.adminForm.limitstart.value='.($this->start - $this->value).'; acymailing.submitform();"'
            : ' acypaginactive"';
...
class acyParameter
{
    ...
    function loadArray($array, $flattened = false, $separator = null)
    {
