...
require_once ACYMAILING_INC.'phpmailer'.DS.'exception.php';
require_once ACYMAILING_INC.'phpmailer'.DS.'smtp.php';
require_once ACYMAILING_INC.'phpmailer'.DS.'phpmailer.php';

use acymailingPHPMailer\Exception;
use acymailingPHPMailer\SMTP;
use acymailingPHPMailer\acymailingPHPMailer;
...
class acymailerHelper extends acymailingPHPMailer{
    ...
    function __construct(){
		...
		switch($this->config->get('mailer_method', 'phpmail')){
			...
			case 'elasticemail' :
				...
				if(is_numeric($port)){
					...
					include_once(ACYMAILING_INC.'phpmailer'.DS.'elasticemail.php');
        ...
        $maxLineLength = intval($this->config->get('word_wrapping', 0));
	...
	public function send(){
		...
		if(!$result){
			...
			$this->errorNumber = 1;
			if($this->report){
				$this->reportMessage = str_replace('Could not instantiate mail function', '<a target="_blank" href="https://docs.acymailing.com/faq/could-not-instantiate-mail-function" title="'.acymailing_translation('TELL_ME_MORE').'">Could not instantiate mail function</a>', $this->reportMessage);
	...
	public function sendOne($mailid, $receiverid){
		...
		$this->Body = preg_replace('#<img((?: [a-z-]+="[^"]*")*) alt( |/)#Uis', '<img$1 alt=""$2', $this->Body);
