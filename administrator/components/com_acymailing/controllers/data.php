...
class DataController extends acymailingController{
	...
	function doexport(){
		...
		$fieldsToExport = acymailing_getVar('array', 'exportdata', []);
		if(!in_array('1', array_values($fieldsToExport))) {
            return $this->exportError(acymailing_translation('ACY_EXPORT_SELECT_FIELD'));
        }
		...
		if(!empty($notAllowedFields)) {
            return $this->exportError('The field '.implode(', ', array_keys($notAllowedFields)).' is not in the allowed fields: '.implode(', ', array_keys($tableFields)));
        }
		...
		if(!empty($notAllowedFields)) {
            return $this->exportError('The field '.implode(', ', $notAllowedFields).' is not in the allowed fields: listid, listname');
        }

		$fieldsToExportOthers = acymailing_getVar('none', 'exportdataother');

		$fieldsToExportGeoloc = acymailing_getVar('array', 'exportdatageoloc', []);
		...
		if(acymailing_getVar('int', 'sessionquery')){
            $oneLine = acymailing_getVar('int', 'export_oneline', 1);
            if($oneLine == 0) $assocField = '';
		...
		do{
            ...
            $subids = [];
			foreach($allData as &$oneUser){
                $subids[$oneUser->subid] = true;

				foreach($dateFields as &$fieldName){
					if(isset($oneUser->$fieldName)) $oneUser->$fieldName = acymailing_getDate($oneUser->$fieldName, '%Y-%m-%d %H:%M:%S');
				}
			}
			...
			if(!empty($exportFieldsList)){
				...
				$queryList .= 'LEFT JOIN #__acymailing_list AS l ON ls.listid = l.listid
								WHERE s.subid IN ('.implode(',', array_keys($subids)).')';
				$resList = acymailing_loadObjectList($queryList);

				foreach($allData as $key => $oneSubscriber){
                    $listids = [];
                    $listnames = [];
                    foreach($resList as $oneList){
                        if($oneList->subid != $oneSubscriber->subid) continue;

                        if(!empty($oneList->listid)) $listids[] = $oneList->listid;
                        if(!empty($oneList->name)) $listnames[] = $oneList->name;
                    }
					foreach($exportFieldsList as $listField){
                        if ($listField === 'listid') {
                            $allData[$key]->listid = implode(' - ', $listids);
                        } else {
                            $allData[$key]->listname = implode(' - ', $listnames);
                        }
					}
				}
				unset($resList);
			...
			if(!empty($exportFieldsGeoloc) && !empty($allData)){
				...
				$resGeol = acymailing_loadObjectList(
                    'SELECT geolocation_subid,'.implode(', ', $exportFieldsGeoloc).' 
                    FROM (
                        SELECT * 
                        FROM #__acymailing_geolocation 
                        WHERE geolocation_subid IN ('.implode(',', array_keys($allData)).') 
                        ORDER BY geolocation_id '.$orderGeoloc.'
                    ) as geoloc 
                    GROUP BY geolocation_subid',
                    'geolocation_subid'
                );
				foreach($allData as $key => $oneSubscriber){
					foreach($exportFieldsGeoloc as $geolField){
						$value = empty($resGeol[$oneSubscriber->subid]) ? '' : $resGeol[$oneSubscriber->subid]->$geolField;
						$allData[$key]->$geolField = ($geolField == 'geolocation_created' ? acymailing_getDate($value, '%Y-%m-%d %H:%M:%S') : $value);
					}
				}
				unset($resGeol);
			}
			...
			foreach($allData as &$oneUser){
				if(!in_array('subid', $exportFields)) unset($oneUser->subid);
