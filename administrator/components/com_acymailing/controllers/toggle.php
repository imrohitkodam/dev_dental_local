...
class ToggleController extends acymailingController{
	...
	function configguessport($port, $value){
		...
		foreach($tests as $port => $server){
			...
			if($fp){
				...
				echo " errornum: ".$errno.' : '.acy_utf8Encode($errstr);
				echo '</span>';
			}
		}
		if(empty($total)){
	...
	function testApiKey(){
        ...
        $this->handleIpTestResult($test);
		exit;
	}

	function ip2locationTestApiKey(){
		$apiKey = acymailing_getVar('string', 'value', '');
		if(empty($apiKey)){
			echo '<span style="color:red">No API key</span><br />';
			exit;
		...
		$classGeoloc = acymailing_get('class.geolocation');
		$test = $classGeoloc->ip2locationTestApiKey($apiKey);

        $this->handleIpTestResult($test);
		exit;
	}

    function handleIpTestResult($test){
