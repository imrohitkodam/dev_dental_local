...
class CpanelController extends acymailingController
{
    ...
    function store()
    {
        ...
        if (!empty($formData['email_verification'])) {
            ...
            $registrationIntegrations = explode(',', $acyCheckerConfigClass->get('registration_integrations'));
            if ($disabledOptions) {
                $formData['email_verification'] = false;
                acymailing_enqueueMessage(acymailing_translation('ACY_ACYCHECKER_AUTO_DISABLED'), 'info');

                if (in_array('acymailing5', $registrationIntegrations)) {
                    unset($registrationIntegrations[array_search('acymailing5', $registrationIntegrations)]);
                    $acyCheckerConfigClass->save(['registration_integrations' => implode(',', $registrationIntegrations)]);
                }
            } else {
                if (!in_array('acymailing5', $registrationIntegrations)) {
                    $registrationIntegrations[] = 'acymailing5';
                }

                $registrationConditions = [];
                if (class_exists('\AcyMailing\Classes\ConfigurationClass')) {
                    $acyMailing7ConfigClass = new \AcyMailing\Classes\ConfigurationClass();
                }
                $newConfig = [];
                foreach ($verificationOptions as $acymOption => $acycOption) {
                    if (!empty($formData[$acymOption])) {
                ...
                if (!empty($acyMailing7ConfigClass)) {
                    $acyMailing7ConfigClass->save($newConfig);
                }

                $acyCheckerConfigClass->save(
                    [
        ...
        if (!$result) {
            ...
            if ($config->get('mailer_method') == 'smtp' && $config->get('smtp_secured') == 'ssl' && !function_exists('openssl_sign')) {
                acymailing_enqueueMessage(
                    'The PHP Extension openssl is not enabled on your server, this extension is required to use an SSL connection, please enable it',
                    'notice'
                );
    ...
    function checkDB()
    {
        ...
        $nbdeleted = acymailing_query(
            "DELETE listsub.* FROM #__acymailing_listsub as listsub LEFT JOIN #__acymailing_subscriber as sub ON sub.subid = listsub.subid WHERE sub.subid IS NULL"
        );
        if (!empty($nbdeleted)) {
        ...
        $nbdeleted = acymailing_query(
            "DELETE listsub.* FROM #__acymailing_listsub AS listsub LEFT JOIN #__acymailing_list AS b ON listsub.listid = b.listid WHERE b.listid IS NULL"
        );
        if (!empty($nbdeleted)) {
        ...
        if (!empty($fieldsDiff)) {
            echo '<span style="color:red;">At least one field is missing in the subscriber table or has not the same case between fields and subscriber table (they should all be lower case): <span style="font-weight: bold">'.implode(
                    ', ',
                    $fieldsDiff
                ).'</span>. You should only create fields using the custom fields interface.</span>';
        } else {
    ...
    function anonymize()
    {
        ...
        acymailing_query(
            'UPDATE #__acymailing_userstats SET open = 0, opendate = 0, ip = NULL, browser = NULL, browser_version = NULL, is_mobile = NULL, mobile_os = NULL, user_agent = NULL'
        );
    ...
    {
        $config = acymailing_getVar('array', 'config', []);
        $licenseKey = $config['license_key'];

        if (empty($licenseKey)) {
            $this->displayMessage('ACY_PLEASE_SET_A_LICENSE_KEY');
            $this->listing();
        ...
        if (empty($resultAttachLicenseOnUpdateMe['success']) || (!empty($resultAttachLicenseOnUpdateMe['code']) && !in_array($resultAttachLicenseOnUpdateMe['code'], [201, 403]))) {
        ...
        if (!empty($resultAttachLicenseOnUpdateMe['result']['message'])) {
            $this->displayMessage($resultAttachLicenseOnUpdateMe['result']['message']);
        }
    ...
    public function unlinkLicense()
    {
        ...
        if (!empty($resultUnlinkLicenseOnUpdateMe['success'])) {
            $this->config->save(['license_key' => '']);
        }
        ...
        if (!empty($resultUnlinkLicenseOnUpdateMe['result']['message'])) {
            $this->displayMessage($resultUnlinkLicenseOnUpdateMe['result']['message']);
        }
    ...
    public function activateCron($licenseKey = null)
    {
        ...
        if (!empty($result['result']) && $this->displayMessage($result['result']['message'])) {
    ...
    private function displayMessage($message)
    {
        $correspondences = [
            ...
            'ACY_PLEASE_SET_A_LICENSE_KEY' => ['message' => 'ACY_PLEASE_SET_A_LICENSE_KEY', 'type' => 'error'],
        ];
        ...
        if (empty($message)) {
            ...
            return false;
        } elseif (empty($correspondences[$message])) {
            acymailing_enqueueMessage(acymailing_translation($message), 'error');
    ...
    private function handleNewDkim(&$formData)
    {
        ...
        $publicKey = $config->get('dkim_public');
        ...
        if (empty($newDkimKeys['result']['private_key'])) {
            return;
        }

        $formData['dkim_private'] = $newDkimKeys['result']['private_key'];
        $formData['dkim_public'] = $newDkimKeys['result']['public_key'];
    }
}
