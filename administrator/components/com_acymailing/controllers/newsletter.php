...
class NewsletterController extends acymailingController{
	...
	function copy(){
		...
		foreach($cids as $oneMailid){
			$query = 'INSERT INTO `#__acymailing_mail` (`subject`, `body`, `altbody`, `published`'.$addSendDate.', `created`, `fromname`, `fromemail`, `replyname`, `replyemail`, `bccaddresses`, `type`, `visible`, `userid`, `alias`, `attach`, `html`, `tempid`, `key`, `frequency`, `params`,`filter`,`metakey`,`metadesc`,`summary`)';
			$query .= " SELECT CONCAT('copy_',`subject`), `body`, `altbody`, 0".$addSendDate.", ".$time.", `fromname`, `fromemail`, `replyname`, `replyemail`, `bccaddresses`, `type`, `visible`, ".$creatorId.", `alias`, `attach`, `html`, `tempid`, ".acymailing_escapeDB(acymailing_generateKey(8)).', `frequency`, `params`,`filter`,`metakey`,`metadesc`,`summary` FROM `#__acymailing_mail` WHERE `mailid` = '.(int)$oneMailid;
