...
class EditorController extends acymailingController{
	...
	private function _setJs(){
		...
		acymailing_addScript(false, $websiteurl.ACYMAILING_MEDIA_FOLDER.'/js/jquery/jquery.min.js?v='.@filemtime(ACYMAILING_ROOT.str_replace('/', DS, ACYMAILING_MEDIA_FOLDER).DS.'js'.DS.'jquery'.DS.'jquery.min.js'));
		...
		$js = "
				...
				function checkSelected(imageZone) {
					...
					if(imageZone){
						...
						if(parsedSelection[0].tagName == 'IMG'){
							...
							if(jQuery(selection).attr('src').substring(0,4) == 'http'){
								var imageUrl =  jQuery(selection).attr('src');
							}else{
								var imageUrl =  '".ACYMAILING_LIVE."' + jQuery(selection).attr('src');
							}
							displayImageFromUrl(imageUrl, 'success', name, width, height);
				...
				function validateImageModification() {
					...
					var dataString = toQueryString(form_params(form));
