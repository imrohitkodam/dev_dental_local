...
class sf_pdf {
	...
	function sf_pdf($options = array()) {
		...
		// Installation path
		define("K_PATH_MAIN", JPATH_COMPONENT_ADMINISTRATOR.'/assets' . "/tcpdf");
	
		// URL path
		define("K_PATH_URL", $mosConfig_live_site);
	
		// Fonts path
		define("K_PATH_FONTS", JPATH_COMPONENT_ADMINISTRATOR.'/assets'.'/tcpdf/fonts/');
		...
		require_once(JPATH_COMPONENT_ADMINISTRATOR.'/assets' . '/tcpdf/tcpdf.php');
		...
		$this->_engine->setHeaderData('', 0, JFactory::getApplication()->getCfg('sitename'), _PDF_GENERATED .' '. JHtml::_('date',time(), 'j F, Y, H:i') );
