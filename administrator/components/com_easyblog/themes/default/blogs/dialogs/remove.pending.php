...
<dialog>
	...
	<title><?php echo JText::_('COM_EASYBLOG_REMOVE_PENDING_POST_DIALOG_TITLE');?></title>
	<content>
		<form data-remove-form action="<?php echo JRoute::_('index.php');?>" method="post">
			<p class="mt-10 ml-10 mr-10"><?php echo JText::_('COM_EASYBLOG_REMOVE_PENDING_POST_DIALOG_CONTENT'); ?></p>

			<?php foreach ($ids as $id) { ?>
			<input type="hidden" name="ids[]" value="<?php echo $id;?>" />
			<?php } ?>

			<?php echo $this->fd->html('form.action', 'pending.remove'); ?>
		</form>
	</content>
	<buttons>
