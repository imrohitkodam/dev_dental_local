...
class EasyBlogFacebook extends EasyBlog
{
	...
	public function getImage(EasyBlogPost &$blog, $rawIntroText = '')
	{
		...
		// category default post cover image
		$hasCategoryDefaultPostCover = $blog->getCategoryDefaultPostCover();

		if ($hasCategoryDefaultPostCover) {
			$source = $hasCategoryDefaultPostCover;
			return $source;
		}
	...
	public function addOpenGraphTags($obj)
	{
		...
		if ($obj instanceof EasyBlogTableCategory) {
			return $this->addOpenGraphTagsForCategory($obj);
		}
	}
	 ...
	 * @access	public
	 */
	public function addOpenGraphTagsForCategory($category)
	{
		$title = $this->formatTitle($category->title);
		$url = $category->getExternalPermalink(null, true);

		$description = $this->formatDescription($category->getDescription());
		...
		$author = EB::user($category->created_by);
	...
	public function addOpenGraphTagsForPost(EasyBlogPost &$post)
	{
		...
		// Add the blog image.
		if (EB::image()->isImage($image)) {
			$this->addImageOpengraphTags($image);
			$this->addImageAltOpengraphTags($post);
			...
			$this->addImageOpengraphTags(EB::getPlaceholderImage(false, 'video'));
			$this->addVideoOpengraphTags($post->getImage('original', true, true));
		}
	...
	{
		$this->doc->setMetaData('og:site_name', FH::getSiteName(), 'property');
	}
	 ...
	 * @access	public
	 */
	public function addImageAltOpengraphTags($post)
	{
		$alt = $this->formatTitle($post->getCoverImageAlt());

		$this->doc->setMetaData('og:image:alt', $alt, 'property');
	}
	 ...
	 * @access	public
	 */
	public function addVideoOpengraphTags($videoUrl)
	{
		$this->doc->setMetaData('og:video', $videoUrl, 'property');
	}
