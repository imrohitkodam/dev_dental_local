...
class HtmlDiff {
	...
	private function Operations() {
		...
		$matches[] = new Detail( count( $this->oldWords ), count( $this->newWords ), 0 );
	...
	private function FindMatch( $startInOld, $endInOld, $startInNew, $endInNew ) {
		...
		return $bestMatchSize != 0 ? new Detail( $bestMatchInOld, $bestMatchInNew, $bestMatchSize ) : null;
	}
}

class Detail implements Countable {
