...
class EasyBlogSimplehtml extends EasyBlog
{
	...
	function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$dom = new eb_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
 ...
 * @package PlaceLocalInclude
 */
class eb_eb_simple_html_dom_node
{
...
//		$pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
		...
		foreach ($matches as $m) {
			...
			list($tag, $key, $val, $exp, $no_key) = array($m[1], '', null, '=', false);
...
class eb_simple_html_dom
{
	...
	protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new eb_eb_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		...
		// text
		$node = new eb_eb_simple_html_dom_node($this);
		++$this->cursor;
	...
	protected function read_tag()
	{
		...
		{
			$this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
			// This represents the change in the eb_simple_html_dom trunk from revision 180 to 181.
