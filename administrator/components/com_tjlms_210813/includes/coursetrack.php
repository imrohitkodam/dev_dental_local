 ...
 * @since  1.3.30
 */
class TjLmsCourseTrack extends CMSObject
{
	/**
	 * Id
	 *
	...
	public function load($userId, $courseId)
	{
		...
		$table = Table::getInstance('Coursetrack', 'TjlmsTable');
	...
	{
		if ($courseTrackData['totalLessons'] > 0 && $courseTrackData['completedLessons'] > 0)
		{
	...
	public function getCourseResumeModule($moduleData)
	{
		...
		$lessonTrackmodel = JModelLegacy::getInstance('lessonTrack', 'tjlmsModel', array('ignore_request' => true));
		...
		if (empty($lastAccessedLesson))
		{
			...
			$firstLesson = reset($firstModule->lessons);
		...
		$lastAccessedLessonKey = array_search($lastAccessedLesson[0]->id, array_column($moduleData[$lastAccessedLesson[0]->mod_id]->lessons, 'id'));
	...
	public function checkPassableLessonsPassed()
	{
		...
		if (!empty($passableLessons))
		{
			...
			// Fetch latest lessons track data
			$lessonTrackModel = Tjlms::model('Lessontrack', array('ignore_request' => true));
			$lessonTrackModel->setState('lesson_id', $passableLessons);
			$lessonTrackModel->setState('user_id', $this->user_id);
			$lessonTrackModel->setState('lessons_last_attempt', true);
			$lessonsTrack = $lessonTrackModel->getItems();
