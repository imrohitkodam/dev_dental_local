 ...
 * @since  1.0.0
 */
class TjlmsControllerLesson extends JControllerForm
{
	/**
	...
	public function save($key = null, $urlVar = null)
	{
		...
		try
		{
			...
			if ($data == false)
			{
				...
				echo new JResponseJson('', JText::_('COM_TJLMS_FORM_VALIDATATION_FAILED'), true);
			}
			...
			if ($model->save($data))
			{
				...
				echo new JResponseJson($result, JText::_('COM_TJLMS_FORM_SAVE_SUCCESS'));
			}
			else
			{
				$errors = $model->getErrors();
				$this->processErrors($errors);
				echo new JResponseJson('', JText::_('COM_TJLMS_FORM_SAVE_FAIL'), true);
			}
		}
		catch (Exception $e)
		{
	...
	public function updateassocfiles()
	{
		...
		{
			$result = 1;
			$msg = JText::_('COM_TJLMS_LESSON_FILES_UPLOADED');
		}
		else
		{
			$result = 0;
			$msg = JText::_('COM_TJLMS_LESSON_FILES_UPLOADED_ERROR');
		}
	...
	public function updateformat()
	{
		...
		try
		{
			...
			if ($mediaId)
			{
				...
				echo new JResponseJson($result, JText::_('COM_TJLMS_LESSON_FORMAT_UPLOADED'));
			}
			else
			{
				$errors = $model->getErrors();
		...
		{
			echo new JResponseJson($result, JText::_('COM_TJLMS_LESSON_FORMAT_UPLOADED'), true);
		}
	}
	...
	public function cancel($key = null)
	{
		...
		$modId = $data['mod_id'];
		$link = JRoute::_('index.php?option=com_tjlms&view=modules&course_id=' . $courseId . '&mod_id=' . $modId, false);
	...
	public function removeAssociatedFileFromLesson()
	{
		...
		try
		{
			...
			echo new JResponseJson($ret, JText::_("COM_TJLMS_LESSON_ASSOCIATED_FILE_REMOVED"));
		}
		catch (Exception $e)
		{
			echo new JResponseJson($e);
	...
	public function downloadMedia()
	{
		...
		JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_tjlms/models', 'TjlmsModel');
		...
		JFactory::getApplication()->redirect(JRoute::_('index.php?option=com_users'));
