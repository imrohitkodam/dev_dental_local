...
class TjlmsControllerUserimport extends JControllerForm
{
	...
	public function csvImport()
	{
		...
		$dateTime = str_replace(array(' ', '-', ':'), '_', JFactory::getDate());
	...
	public function saveCsvContent($file_to_upload, $notify_user)
	{
		...
		JLog::add(JText::sprintf('COM_TJLMS_MANAGEENROLLMENTS_LOG_CSV_FILE_NAME', $csvFileName), JLog::INFO, 'com_tjlms');

		JLog::add(JText::_("COM_TJLMS_MANAGEENROLLMENTS_LOG_CSV_START"), JLog::INFO, 'com_tjlms');
		...
		if (empty($userData))
		{
			array_push($messages, array('error' => JText::_('COM_TJLMS_TITLE_MANAGEENROLLMENTS_IMPORT_BLANK_FILE')));
		...
		foreach ($userData as $eachUser)
		{
			...
			if (!empty($eachUser['timezone']))
			{
				...
				else
				{
					...
					JLog::add(JText::sprintf($msg, $lineno), JLog::ERROR, 'com_tjlms');
					$badTimeZoneCnt++;
					continue;
				}
			}
			...
			else
			{
				...
				{
					array_push($messages, array('error' => JText::_('COM_TJLMS_CSV_IMPORT_COLUMN_MISSING')));
					$output['messages'] = $messages;
				...
				{
					$userId = JUserHelper::getUserId($eachUser['username']);
				}
				...
				foreach ($courseIds as $courseId)
				{
					if ($courseId)
					{
						...
						if (PluginHelper::isEnabled('tjlms', 'courseprerequisite'))
						{
							...
							$result = $dispatcher->trigger('checkPrerequisiteCourseStatus', array($courseId, $userId));
					...
					if (!empty($startDate) && !empty($dueDate))
					{
						...
						elseif (date("Y-m-d", strtotime($startDate)) < date("Y-m-d"))
						{
							...
							JLog::add(
							JText::sprintf($msg, date("Y-m-d", strtotime($startDate)), date("Y-m-d"), $userId, $courseId), JLog::ERROR, 'com_tjlms');
							$startLessToday++;
							continue;
						}
						elseif (date("Y-m-d", strtotime($startDate)) > date("Y-m-d", strtotime($dueDate)))
						{
							...
							JLog::add(
							JText::sprintf($msg, date("Y-m-d", strtotime($dueDate)), date("Y-m-d", strtotime($startDate)), $userId, $courseId), JLog::ERROR, 'com_tjlms');
							$startGtDue++;
							continue;
						}
						else
						{
							...
							{
								JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_ALREADY_ASSIGNED", $userId, $courseId), JLog::INFO, 'com_tjlms');
							...
							$options = array('element' => 'com_tjlms.course', 'element_id' => $courseId, 'plg_name' => 'jlike_tjlms', 'plg_type' => 'content');

							if ($recommendModel)
							{
								$recommendModel->assignRecommendUsers($data, $options, $notify_user);
								$msg = "COM_TJLMS_MANAGEENROLLMENTS_LOG_NEW_ASSIGN";
								JLog::add(JText::sprintf($msg, $userId, $courseId, $data['start_date'], $data['due_date']), JLog::INFO, 'com_tjlms');
								$assignmentMsg = 1;
							}
					...
					if (!$alreadyEnrolled)
					{
						...
						{
							JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_NEW_ENROLL", $userId, $courseId), JLog::INFO, 'com_tjlms');
							$enrollSuccess ++;
						}
					}
					elseif (!$assignmentMsg)
					{
						JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_ALREADY_ENROLL", $userId, $courseId), JLog::INFO, 'com_tjlms');
						$alreadyEnrolledCnt ++;
					}
				}
			}
			...
			if (!empty($removeCourses))
			{
				...
				foreach ($courseIds as $courseId)
				{
					...
					if (!empty($alreadyEnrolled))
					{
						...
						{
							JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_DE_ENROLL", $userId, $courseId), JLog::INFO, 'com_tjlms');
							$deenrollSuccess ++;
						}
					}
					else
					{
						JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_NOT_ALREADY_ENROLL", $userId, $courseId), JLog::INFO, 'com_tjlms');
		...
		JLog::add(JText::_("COM_TJLMS_MANAGEENROLLMENTS_LOG_CSV_END"), JLog::INFO, 'com_tjlms');

		// Log file Path
		$logFilepath = JRoute::_('index.php?option=com_tjlms&view=manageenrollments&task=downloadLog&prefix=enrollment');
		...
		{
			$logLink = '<a href="' . $logFilepath . '" >' . JText::_("COM_TJLMS_ENROLLMENT_CSV_SAMPLE") . '</a>';
			$logLink =	JText::sprintf('COM_TJLMS_LOG_FILE_PATH', $logLink);
		}
		// Handle Messages
		$message = JText::sprintf('COM_TJLMS_MANAGEENROLLMENTS_IMPORT_TOTAL_ROWS_CNT_MSG', $totalUser) . ' ' . $logLink;
		array_push($messages, array('success' => $message));

		if ($adminApproval == 1)
		{
			array_push($messages, array('notice' => JText::_('COM_TJLMS_USER_NEED_ADMIN_APPROVAL')));
		}

		if ($missingDetails > 0)
		{
			$message = ($missingDetails == 1) ? 'COM_TJLMS_MANAGEENROLLMENTS_MANDATORY_FIELDS_ONE' : 'COM_TJLMS_MANAGEENROLLMENTS_MANDATORY_FIELDS';
			array_push($messages, array('error' => JText::sprintf($message, $missingDetails)));
		}

		if ($userExists > 0)
		{
			$message = 'COM_TJLMS_TITLE_MANAGEENROLLMENTS_IMPORT_USER_EXIST';
			array_push($messages, array('notice' => JText::sprintf($message, $userExists)));
		}
		...
		if ($newUsers > 0)
		{
			...
			array_push($messages, array('success' => JText::sprintf($message, $newUsers)));
		}
		...
		if ($updateUsers > 0)
		{
			...
			array_push($messages, array('success' => JText::sprintf($message, $updateUsers)));
		}

		if ($notCreateUsers > 0)
		{
			$message = ($notCreateUsers == 1) ? 'COM_TJLMS_MANAGEENROLLMENTS_BAD_USERDATA' : 'COM_TJLMS_MANAGEENROLLMENTS_BAD_USERDATA';
			array_push($messages, array('error' => JText::sprintf($message, $notCreateUsers)));
		}
		...
		if ($enrollSuccess > 0)
		{
			...
			array_push($messages, array('success' => JText::sprintf($message, $enrollSuccess)));
		}
		...
		if ($deenrollSuccess > 0)
		{
			...
			array_push($messages, array('success' => JText::sprintf($message, $deenrollSuccess)));
		}
		...
		if ($assignSuccess > 0)
		{
			...
			array_push($messages, array('success' => JText::sprintf($message, $assignSuccess)));
		}
		...
		if ($alreadyEnrolledCnt > 0)
		{
			...
			array_push($messages, array('notice' => JText::sprintf($message, $alreadyEnrolledCnt)));
		}
		...
		if ($alreadyNotEnrolledCnt > 0)
		{
			...
			array_push($messages, array('notice' => JText::sprintf($message, $alreadyNotEnrolledCnt)));
		}
		...
		if ($assignedUpdated > 0)
		{
			...
			array_push($messages, array('notice' => JText::sprintf($message, $assignedUpdated)));
		}

		if ($badUserAccess > 0)
		{
			$message = 'COM_TJLMS_TITLE_MANAGEENROLLMENTS_IMPORT_BAD_USER_ACCESS';
			array_push($messages, array('error' => JText::sprintf($message, $badUserAccess)));
		}
		...
		if ($badCourses > 0)
		{
			...
			array_push($messages, array('error' => JText::sprintf($message, implode(',', $invalidCourses))));
		}
		...
		if ($badGroupsCnt > 0)
		{
			...
			array_push($messages, array('error' => JText::sprintf($message, implode(',', $badGroups))));
		}
		...
		if ($invalidDate > 0)
		{
			...
			array_push($messages, array('error' => JText::sprintf($message, $invalidDate)));
		}
		...
		if ($startLessToday > 0)
		{
			...
			array_push($messages, array('error' => JText::sprintf($message, $startLessToday)));
		}
		...
		if ($startGtDue > 0)
		{
			...
			array_push($messages, array('error' => JText::sprintf($message, $startGtDue)));
		}

		if ($badTimeZoneCnt > 0)
		{
			$message = ($badTimeZoneCnt == 1) ? 'COM_TJLMS_MANAGEENROLLMENTS_BAD_TIMEZONE_ONE' : 'COM_TJLMS_MANAGEENROLLMENTS_BAD_TIMEZONE';
			array_push($messages, array('error' => JText::sprintf($message, $badTimeZoneCnt)));
		}
	...
	public function createUpdateUser($userData)
	{
		...
		if ($user->bind($userData))
		{
			if ($user->save())
			{
				if ($newUser)
				{
					JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_NEW_USER", $user->id, json_encode($user->groups)), JLog::INFO, 'com_tjlms');
				}
				else
				{
					JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_UPDATED_USER", $user->id), JLog::INFO, 'com_tjlms');
				}
				...
				if (!$newUser)
				{
					...
					if (!empty($addGroups))
					{
						...
						foreach ($groupIds as $groupId)
						{
							...
							{
								JLog::add(
								JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_USERGROUP_ADD_FAIL", $userData['email'], $groupId, $e->getMessage()
								), JLog::ERROR, 'com_tjlms'
								);
							}
						}
					}
					...
					if (!empty($userData["removegroups"]))
					{
						...
						foreach ($groupIds as $groupId)
						{
							...
							{
								JLog::add(
								JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_USERGROUP_REMOVE_FAIL",
		...
		JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_ERROR_NEW_USER", $user->getError(), $userData['email']), JLog::ERROR, 'com_tjlms');
  ...
  */
	/*public function checkGroupExist($gid)
	{
		...
		{
			JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_INVALID_USER_GROUP", $gid), JLog::WARNING, 'com_tjlms');
		...
		if ($userId)
		{
			...
			if ($removeGroups)
			{
				...
				foreach ($removeGroups as $key => $removeGroup)
				{
					...
					if ($validGroup)
					{
						...
						if ($arrayIndex !== false)
						{
						JLog::add(JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_REMOVED_USER_GROUP", $userId, $removeGroup), JLog::WARNING, 'com_tjlms');
			...
			if ($addGroups)
			{
				...
				foreach ($addGroups as $key => $addGroup)
				{
					...
					if ($validGroup)
					{
						$msg = JText::sprintf("COM_TJLMS_MANAGEENROLLMENTS_LOG_USER_ADDED_TO_GROUP", $userId, $validGroup);
						JLog::add($msg, JLog::INFO, 'com_tjlms');
