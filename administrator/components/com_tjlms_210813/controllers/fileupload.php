...
class TjlmsControllerFileUpload extends JControllerForm
{
	...
	public function processupload()
	{
		...
		$isSafe = JFilterInput::isSafeFile($file_to_upload, $options);

		if (!$isSafe)
		{
			$return = 0;
			$msg =	JText::_("COM_TJLMS_ERROR_FILENOTSAFE_TO_UPLOAD", $filename = '');
		}
		else
		{
			...
			$filename = JFile::makeSafe($file_to_upload['name']);
			...
			if (!JFile::upload($file_attached, $uploads_dir, false, true))
			{
				$return = 0;
				$msg =	JText::sprintf("COM_TJLMS_ERROR_UPLOAD_ON_LOCAL", $filename);
				$uploadErrors = JFactory::getApplication()->getMessageQueue();
	...
	public function validateupload($file_to_upload, $format, $subformat, $lesson_id)
	{
		...
		if ($file_to_upload["error"] == UPLOAD_ERR_OK)
		{
			...
			if (($tjlmsparams->get('lesson_upload_size', 10) * 1024 * 1024) != 0)
			{
				...
				{
					$return = 0;
					$msg = JText::sprintf('COM_TJLMS_UPLOAD_SIZE_ERROR', $tjlmsparams->get('lesson_upload_size', 10, 'INT') . ' MB');
				}
			}
				...
				switch ($format)
				{
					case 'scorm':
							...
							if (in_array($fileext, $valid_extensions_arr))
							{
								...
								{
									$return = 0;
									$msg = JText::_("COM_TJLMS_VALID_SCORM_PACKAGE");
								}
							}
							else
							{
								$msg = JText::_("COM_TJLMS_VALID_ZIP_PACKAGE");
								$return = 0;
							}
						break;
					case 'tincanlrs':
							...
							if (in_array($fileext, $valid_extensions_arr))
							{
								...
								{
									$return = 0;
									$msg = JText::_("COM_TJLMS_VALID_TINCAN_PACKAGE");
								}
							}
							else
							{
					...
					case 'htmlzips':
							...
							if (in_array($fileext, $valid_extensions_arr))
							{
								...
								{
									$return = 0;
									$msg = JText::_("COM_TJLMS_VALID_HTMLZIP_PACKAGE");
								}
							}
							else
							{
					...
					case 'document':
							...
							if ($subformat == 'pdfviewer')
							{
								...
								$msg = JText::_("COM_TJLMS_UPLOAD_EXTENSION_ERROR");
							}

							if (!in_array($fileext, $valid_extensions_arr))
							{
								$return = 0;
								$msg = (!empty($msg)? $msg : JText::_("COM_TJLMS_VALID_DOCUMENT_UPLOAD"));
							}
						break;
					...
					case 'video':
						...
						if (!in_array($fileext, $valid_extensions_arr))
						{
								$msg = JText::_("COM_TJLMS_VALID_VIDEO_UPLOAD");
								$return = 0;
						}
		...
		{
			$return = 0;
			$msg  = JText::sprintf("COM_TJLMS_ERROR_UPLOADINGFILE", $file_to_upload['name']);
	...
	public function populate_tables()
	{
		...
		if (!in_array($format, $tjlmsnotallowedformats) && !empty($format_id))
		{
			...
			$model     = JModelLegacy::getInstance('lesson', 'TjlmsModel');
			...
			$ret['OUTPUT']['msg'] = JText::_("COM_TJLMS_FORM_SAVE_SUCCESS");
		}
		else
		{
			$ret['OUTPUT']['data'] = $format_id;
			$ret['OUTPUT']['msg'] = JText::_("COM_TJLMS_ERROR_FILENOTSAFE_TO_UPLOAD");
		}
	...
	public function uploadFormatsonResServer()
	{
		...
		$result = 1;
		$msg = JText::sprintf("COM_TJLMS_FORMAT_UPLOAD_SUCCESS_ON", $subformat);
		$warning = $upload = '';
		...
		/*if ($format == 'scorm' || $format == 'tjscorm')
		{
			...
			{
				$result = 0;
				$msg = JText::sprintf("COM_TJLMS_FORMAT_EXTRACT_ERROR");
			}
		}
		else*/
		if (!empty($subformat))
		{
			...
			JPluginHelper::importPlugin('tj' . $format);
			$upload_status = $dispatcher->trigger('upload_filesOn' . $subformat, array($lesson_id, $filename, $dest));
			...
			{
				$result = 0;
				$msg = JText::sprintf("COM_TJLMS_FORMAT_UPLOAD_ERORR", $subformat);
			}
			...
			/*if (!empty($upload_status[0]) && $upload_status[0])
			{
				...
				if ($error = $db->getErrorMsg())
				{
					$this->setError($error);
					$result = 0;
					$msg = JText::sprintf("COM_TJLMS_FORMATPARAM_STORE_ERORR", $res['column']);
				}
			}
			else
			{
				$result = 0;
				$resUpload = JText::sprintf("COM_TJLMS_FORMAT_UPLOAD_ERORR", $subformat);
			}*/
		}
	...
	public function parse_format()
	{
		...
		$results = $dispatcher->trigger('parse' . $subfomatopt . 'Format', array($lessonFormatData));
...
/*			$extract_dir = JPATH_SITE . '/media/com_tjlms/lessons/' . $lesson_id . '/scorm';
	...
	private function uploadcodeToMessage($code)
	{
		switch ($code)
		{
			case UPLOAD_ERR_INI_SIZE:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_INI_SIZE');
				break;
			case UPLOAD_ERR_FORM_SIZE:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_FORM_SIZE');
				break;
			case UPLOAD_ERR_PARTIAL:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_PARTIAL');
				break;
			case UPLOAD_ERR_NO_FILE:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_NO_FILE');
				break;
			case UPLOAD_ERR_NO_TMP_DIR:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_NO_TMP_DIR');
				break;
			case UPLOAD_ERR_CANT_WRITE:
				$message = JText::_('COM_TJLMS_ERROR_UPLOAD_ERR_CANT_WRITE');
				break;
			...
			default:
				$message = JText::_('COM_TJLMS_ERROR_UNKNOWN_ERROR');
				break;
		}
	...
	protected function check_create_htaccess($lesson_id)
	{
		...
		if (JFolder::exists($lesson_dir) && !JFile::exists($htaccessFile))
		{
			if (!is_writable($lesson_dir))
			{
				$errorMsg[] = JText::sprintf("COM_TJLMS_ERROR_FOLDER_NOT_WRITABLE", $lesson_dir);
			}
			...
			if (!$result)
			{
				$errorMsg[] = JText::sprintf("COM_TJLMS_ERROR_FILE_NOT_WRITABLE", $htaccessFile);
			}
		}
