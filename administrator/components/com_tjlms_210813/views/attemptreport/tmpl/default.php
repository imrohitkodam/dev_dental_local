...
<script type="text/javascript">
	...
	function updateAttemptData(event, attemptId, score, TotalMarks)
	{
		...
		{
			var msg = Joomla.JText._('COM_TJLMS_ENTER_NUMERNIC_MARKS');
		...
		if (event.which == 13)
		{
			...
			{
				var msg = Joomla.JText._('COM_TJLMS_ENTER_MARKS_GRT_TOTALMARKS');
				alert(msg);
			...
			if (isSure == true)
			{
				jQuery.ajax({
					...
					success: function(data)
					{
						...
						{
							var msg = Joomla.JText._('COM_TJLMS_UPDATED_MARKS_SUCCESSFULLY');
							alert(msg);
						}
					}
				});
	...
	function validateSubmit(fromCalendar,elem)
	{
		...
		{
			dispEnqueueMessage(Joomla.JText._('COM_TJLMS_ATTEMPTREPORT_INVALID_DATE_FORMAT'),'invalid_dates');
		}
		return true;
	}
	...
	function checkValidStartEnd(changedElem)
	{
		...
		if(changedElem && sdTimeStamp && edTimeStamp)
		{
			if(sdTimeStamp > edTimeStamp)
			{
				...
				dispEnqueueMessage(Joomla.JText._('COM_TJLMS_ATTEMPTREPORT_DATE_RANGE_VALIDATION'),'starts_ends');
				valid = false;
			}
		}
