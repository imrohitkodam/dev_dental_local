...
<form action="<?php echo JRoute::_('index.php?option=com_tjlms&view=modules&course_id='. $this->courseId); ?>" method="post" name="adminForm" id="lesson-format-form_<?php echo $this->formId;?>" class="form-validate form-horizontal lesson-format-form" >
	...
	<div class="container-fluid">
		...
		<div id="row-fluid">
			...
			{
				$subformat[] = JHTML::_('select.option', $sf['id'], $sf['name']);
			}
?>
			...
			<div class="lesson_format" id="<?php echo $this->format ?>">
				<div class="control-group">
					<div class="control-label">
						<label title="<?php echo JText::_("COM_TJLMS_".strtoupper($this->format)."_SUBFORMAT_OPTIONS");?>" ><?php echo JText::_("COM_TJLMS_".strtoupper($this->format)."_SUBFORMAT_OPTIONS");?></label>
					</div>
				...
				<?php
				 ...
				 echo JHTML::_('select.genericlist', $subformat, "lesson_format[subformat]",
				...
				foreach ($this->subformatOptions as $ind => $plugin)
				{
					...
					JPluginHelper::importPlugin($this->format, $plugin['id']);

					// Call the plugin and get the result
					$results = $dispatcher->trigger('getSubFormat_' . $plugin['id'] . 'ContentHTML', array($this->moduleId, $this->item->id, $this->item, $this->params, $this->formId));
				?>
				<?php
					$class="hide";
