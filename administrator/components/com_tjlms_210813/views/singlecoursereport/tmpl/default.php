...
<form action="<?php echo JRoute::_('index.php?option=com_tjlms&view=singlecoursereport'); ?>" method="post" name="adminForm" id="adminForm">
...
?>
	...
	<?php if (empty($this->items)) : ?>
		...
		?>
					...
					{
						echo JText::_('COM_TJLMS_SELECT_COURSE_MESSAGE');
					}
					else
					{
	...
	<table class="table table-striped" id="usersList">
		<thead>
			...
			<tr>
				...
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TJLMS_EMP_NAME', 'user_name', $listDirn, $listOrder); ?></th>
				<th class='left'><?php echo JHtml::_('grid.sort',  'COM_TJLMS_EMP_USERNAME', 'username', $listDirn, $listOrder); ?></th>
				<th class='left'><?php echo JText::_('COM_TJLMS_EMP_EMAIL'); ?></th>
				<th class='left'><?php echo JText::_('COM_TJLMS_EMP_GROUP'); ?></th>
				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_COURSE_CAT', 'cat.title', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_USER_ENROLLED_ON', 'eu.enrolled_on_time', $listDirn, $listOrder); ?>
				</th>
				<th class='left'>Due Date</th>
				<th class='left'><?php echo JText::_('COM_TJLMS_COURSE_COMPLETED_DATE'); ?></th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_COMPLETION', 'completion', $listDirn, $listOrder); ?>
				</th>

				<th class='left'>
					<?php echo JHtml::_('grid.sort',  'COM_TJLMS_REPORT_TIMESPENT', 'totaltimespent', $listDirn, $listOrder); ?>
				</th>
			<?php
			if (!empty($less_headers))
			{
				...
				{
					?>
					<th class='right'><?php echo JText::_( 'COM_TJLMS_LESSON_SCORE_STUDENT');?></th>
					<th class='left'><?php echo JText::_( 'COM_TJLMS_LESSON_TIME_SPENT_STUDENT');?></th>
					<th class='left'><?php echo JText::_( 'COM_TJLMS_LESSON_STATUS_STUDENT');?></th>
					<?php
				}
			}
			?>
		...
		?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				?></td>
				<td>
					<a href="<?php echo JUri::root() . 'administrator/index.php?option=com_tjlms&view=lessonreport&filter[coursefilter]=' . $item->course_id . '&filter[userfilter]=' .  $item->user_id; ?>"><?php echo $item->completion . '%'; ?></a>
				</td>
