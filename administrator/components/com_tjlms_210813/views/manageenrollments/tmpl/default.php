...
<?php } ?>
<form action="<?php echo JRoute::_('index.php?option=com_tjlms&view=manageenrollments'.$courseParam ); ?>" method="post" name="adminForm" id="adminForm">
			...
			<table class="table table-striped left_table" id="usersList">
				<thead>
					<tr>
						...
						<th class='left'>
							<?php echo JHtml::_('searchtools.sort',  'COM_TJLMS_ENROLMENT_USER_NAME', 'uc.name', $listDirn, $listOrder); ?>
						</th>

						<?php if (!$courseId) {	?>
						<th class='left'>
						<?php echo JHtml::_('searchtools.sort',  'COM_TJLMS_MANAGEENROLLMENTS_COURSE_ID', 'co.title', $listDirn, $listOrder); ?>
						</th>
						<?php } ?>

						<th class='left'>
							<?php echo JHtml::_('searchtools.sort',  'COM_TJLMS_ENROLMENT_USER_USERNAME', 'uc.username', $listDirn, $listOrder); ?>
						</th>

						<th class='left'>
							<?php echo JHtml::_('searchtools.sort',  'COM_TJLMS_ENROLMENT_START_DATE_TITLE', 'a.start_date', $listDirn, $listOrder); ?>
						</th>

						<th class='left'>
							<?php echo JHtml::_('searchtools.sort',  'COM_TJLMS_ENROLMENT_DUE_DATE_TITLE', 'a.due_date', $listDirn, $listOrder); ?>
						</th>
						<th class='left'>
							<?php  echo JHTML::tooltip(JText::_('COM_TJLMS_NOTIFY_ASSIGN_USER'), '','', JText::_('COM_TJLMS_NOTIFY_ASSIGN_USER'));?>
						</th>
						...
						<th class='left'>
							<?php	echo JHTML::tooltip(JText::_('COM_TJLMS_ENROLMENT_GROUP_TITLE'), '','', JText::_('COM_TJLMS_ENROLMENT_GROUP_TITLE')); ?>
						</th>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<?php if (!$courseId):	?>
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'manageenrollments.', $canChange, 'cb'); ?>
							</td>
						<?php endif; ?>
					<?php endif; ?>
					<td>
					...
					<td>
						...
						if($item->start_date)
						{
							$start_enroll_date = JHtml::date($item->start_date,'Y-m-d', true);
						}

						echo JHTML::calendar($start_enroll_date,'mycalendar', "start_{$i}", '%Y-%m-%d',array('size'=>'8','maxlength'=>'10','class'=>'input-small',));
							?>
					</td>
					<td>
						<?php
						...
						if ($item->due_date)
						{
							$due_date = JHtml::date($item->due_date,'Y-m-d', true);
						}
							echo JHTML::calendar($due_date,'mycalendar', "due_{$i}", '%Y-%m-%d',array('size'=>'8','maxlength'=>'10','class'=>'input-small',));
