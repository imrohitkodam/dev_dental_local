 ...
 * @since  1.0.0
 */
class TjlmsTablelesson extends JTable
{
	/**
	 * Constructor
	 *
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_tjlms.lesson.' . $array['id']))
		{
			$actions = JFactory::getACL()->getActions('com_tjlms', 'lesson');
			$default_actions = JFactory::getACL()->getAssetRules('com_tjlms.lesson.' . $array['id'])->getData();
			$array_jaccess = array();
	...
	public function check()
	{
		...
		if (!$this->alias)
		{
			$this->alias = trim(JFilterOutput::stringURLSafe($this->title));
		}
		...
		$table = JTable::getInstance('Lesson', 'TjlmsTable', array('dbo', $db));
	...
	public function delete($pk = null)
	{
		...
		if ($result && !empty($this->img))
		{
			...
			$result = JFile::delete(JPATH_ADMINISTRATOR . '/components/com_tjlms/image/lessons/' . $this->img);
		}
