...
class JFormFieldLmssubsplans extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html .= '<div class="com_tjlms_add_button span2">
					...
					title=' . JText::_("COM_TJLMS_ADD_BUTTON") . '>
	...
	public function fetchTimemeasure($fieldName, $value, &$node, $control_name, $k)
	{
		...
		$time_measure[] = JHTML::_('select.option', 'day', JText::_('COM_TJLMS_DAYS'));
		$time_measure[] = JHTML::_('select.option', 'week', JText::_('COM_TJLMS_WEEK'));
		$time_measure[] = JHTML::_('select.option', 'month', JText::_('COM_TJLMS_MONTH'));
		$time_measure[] = JHTML::_('select.option', 'year', JText::_('COM_TJLMS_YEAR'));
		$time_measure[] = JHTML::_('select.option', 'unlimited', JText::_('COM_TJLMS_UNLIMITED'));
	...
	public function fetchDuration($fieldName, $value, &$node, $control_name, $k, $timeMeasure)
	{
		...
		. '" aria-invalid="false" style="display:none">' . JText::_('COM_TJLMS_DURATION') . '</label>';
		...
		. JText::_('COM_TJLMS_DURATION') . '"  ' . $readOnly . ' value=' . $value . '>';
	}
	...
	public function fetchPrice($fieldName, $value, &$node, $control_name, $k)
	{
		...
		. JText::_('COM_TJLMS_PRICE') . '"  value=' . $value . '>';
		...
		$Price .= ' class="subs_plan_price " Onkeyup= "checkforalpha(this,46);" placeholder="' . JText::_('COM_TJLMS_PRICE') . '" value=' . $value . '>';
	...
	public function fetchPlanId($fieldName, $value, &$node, $control_name, $k)
	{
		...
		$PlanId .= ' class="subs_plan_id " placeholder="' . JText::_('COM_TJLMS_PLAN_ID') . '"  value=' . $value . '>';
...
<script>
	...
	function addClone(rId,rClass)
	{
				...
				"');\" title=\"<?php echo JText::_('COM_TJLMS_REMOVE_TOOLTIP'); ?>\" >";
		...
		function removeClone(rId, r_btndivId)
		{
			...
			if(flag == 2)
			{
				var remove_comfirm=confirm("<?php echo JText::_('LMS_SURE_REMOVE_SUBS_PLAN'); ?>")
