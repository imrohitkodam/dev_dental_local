 ...
 * @since  1.6
 */
class TjlmsModelTeacher_Report extends JModelLegacy
{
	/**
	...
	public function getTopScorer($course_id,$limit)
	{
		...
		{
			$topscorer->percentage = $topscorer->completed_lessons / $topscorer->no_of_lessons * 100;
			$topscorer->uname = JFactory::getUser($topscorer->user_id)->name;
	...
	public function getStudentwhoLiked($course_id)
	{
		...
		{
			$student = JFactory::getUser($enrolment_info->userid);
			$enrol_userid = $enrolment_info->userid;
			$popularStudent[$index]->username = JFactory::getUser($enrol_userid)->username;
