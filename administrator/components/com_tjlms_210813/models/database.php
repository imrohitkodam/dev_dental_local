 ...
 * @since  1.6
 */
class TjlmsModelDatabase extends JModelList
{
	...
	public function fixAlias($table_name, $table_prefix, $title_field='title')
	{
		...
		$table = JTable::getInstance($table_prefix, 'TjlmsTable', array('dbo', $db));
	...
	public function setDefaultCertificate()
	{
		...
		if (!$cer_rows)
		{
			...
			$row = JTable::getInstance('certificatetemplate', 'TjlmsTable', array());
			...
			foreach ($certificates as $cer_entry)
			{
				...
				else
				{
					...
					if ($cer_entry->course_track_id && $grant_date && $grant_date != '0000-00-00 00:00:00')
					{
						...
						$ctModel = JModelLegacy::getInstance('Coursetrack', 'TjlmsModel');
	...
	public function updateTrackEntries($limit=100, $start=0)
	{
		...
		foreach ($ct_entries as $track)
		{
			...
			JTable::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjlms/tables');
			$table = JTable::getInstance('Coursetrack', 'TjlmsTable', array('dbo', $db));
			$table->id = $track->id;
