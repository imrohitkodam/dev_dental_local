...
class TjlmsModelReports extends JModelList
{
	...
	public function getLessonFilter($created_by = 0)
	{
		...
		if (!empty($lessons))
		{
			...
			{
				$lessonFilter[] = JHTML::_('select.option', $eachLessons->id, $eachLessons->title);
			}
		}
	...
	public function getUserFilter($myteam = false)
	{
		...
		if ($myteam)
		{
			...
			JArrayHelper::toInteger($hasUsers);
	...
	public function getCatFilter($default = true)
	{
		...
		if ($default)
		{
			$obj = new stdClass;
			$obj->value = '';
			$obj->text = JText::_('COM_TJLMS_FILTER_SELECT_COURSE_CATEGORY');
			$obj->disable = '';
	...
	public function getNameFilter($myteam = false)
	{
		...
		$nameFilter[] = JHTML::_('select.option', '', JText::_('COM_TJLMS_FILTER_SELECT_USER'));

		foreach ($names as $eachName)
		{
			$nameFilter[] = JHTML::_('select.option', $eachName->id, $eachName->name);
		}
	...
	public function getPaymentProcessorFilter()
	{
		...
		$options = array();
		$options[] = JHTML::_('select.option', '', JText::_('COM_TJLMS_FILTER_SELECT_PAYMENT_PROCESSOR'));

		foreach ($gatewaysconfig as $gateway)
		{
			$options[] = JHTML::_('select.option', $gateway);
		}
