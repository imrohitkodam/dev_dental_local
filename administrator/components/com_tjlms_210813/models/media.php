 ...
 * @since  1.0.0
 */
class TjlmsModelMedia extends JModelAdmin
{
	// Set to 3 minutes
	...
	{
		$data = JFactory::getApplication()->getUserState('com_tmt.edit.test.data', array());
	...
	public function getMediaUrl($mediaObj, $getTimelyUrl = false)
	{
		...
		$timelyUrl = JRoute::_('index.php?option=com_tjlms&task=lesson.downloadMedia&mid=' . $mediaObj->id);
		...
		if ($getTimelyUrl)
		{
			...
			$timelyUrl = JUri::root() . substr(JRoute::_($timelyUrl), strlen(JUri::base(true)) + 1);
		}
	...
	public function saveMediaXref($mediaId, $clientId, $client, $isGallery = 0)
	{
		...
		if (!empty($tjmediaXrefTable->id))
		{
			...
			$oldTjmediaXrefTable = JTable::getInstance('Xref', 'TJMediaTable');
			...
			if (!$oldTjmediaXrefTable->id)
			{
				...
				$filetable = JTable::getInstance('Files', 'TJMediaTable');
				...
				if ($mediaLib->id)
				{
					if (!$mediaLib->delete())
					{
						$this->setError(JText::_($mediaLib->getError()));
