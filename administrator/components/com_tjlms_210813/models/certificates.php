 ...
 */
class TjlmsModelCertificates extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function getListQuery()
	{
		...
		{
			$query->select('IF(created_by.username IS NULL,"' . JText::_('COM_TJLMS_BLOCKED_USER') . '",created_by.username) AS created_by');
		}
		else
		{
			$query->select('IF(created_by.name IS NULL,"' . JText::_('COM_TJLMS_BLOCKED_USER') . '",created_by.name) AS created_by');
		}
	...
	public function getItems()
	{
		...
		{
			$oneItem->access = JText::_('COM_TJLMS_CERTIFICATES_ACCESS_OPTION_' . strtoupper($oneItem->access));
		}
	...
	{
		JArrayHelper::toInteger($certIds);
	...
	public function updateTags($oldTags, $newTags)
	{
		...
		foreach ($certificates as $certificate)
		{
			...
			$certTemplateTable = JTable::getInstance('certificatetemplate', 'TjlmsTable', array('dbo', $this->_db));
