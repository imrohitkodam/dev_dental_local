 ...
 * @since  1.0.0
 */
class TjlmsModelActivities extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getItems()
	{
		...
		foreach ($items as $index => $activity)
		{
			if ($show_user_or_username == 'name')
			{
				...
				$user_name	= JFactory::getUser($activity->actor_id)->name;
			}
			elseif ($show_user_or_username == 'username')
			{
				// Set user_name is user actual name
				$user_name	= JFactory::getUser($activity->actor_id)->username;
			}
			...
			switch ($activity->action)
			{
					case "ENROLL":
						...
						$text_to_show = JText::sprintf('COM_TJLMS_ACTIVITY_STREAM_ENROLL', $user_name, $course_title);
						break;
					case "ATTEMPT":
						...
						$text_to_show = JText::sprintf('COM_TJLMS_ACTIVITY_STREAM_ATTEMPT', $user_name, $attempt, $lesson_link, $course_title);
					...
					case "ATTEMPT_END":
						...
						$params       = json_decode($activity->params);
						$attempt      = $params->attempt;
						$text_to_show = JText::sprintf('COM_TJLMS_ACTIVITY_STREAM_ATTEMPT_END', $user_name, $attempt, $lesson_link, $course_title);
						break;
					case "COURSE_CREATED":
							$course_title = "<strong>" . $activity->element . "</strong>";
							$text_to_show = JText::sprintf('COM_TJLMS_COURSE_CREATED_STREAM', $user_name, $course_title);
						break;
					case "COURSE_RECOMMENDED":
							...
							if (isset($params->target_id))
							{
								...
								$course_title = "<strong>" . $activity->element . "</strong>";
								$text_to_show = JText::sprintf('COM_TJLMS_ON_RECOMMEND_COURSE_AS_LMS', $user_name, $course_title, JFactory::getUser($target_id)->name);
							}
						break;
					case "COURSE_COMPLETED":
							$course_title = "<strong>" . $activity->element . "</strong>";
							$text_to_show = JText::sprintf('COM_TJLMS_COURSE_COMPLETED_STREAM', $user_name, $course_title);
						break;

					case "LOGIN":
							$user_name = (isset($user_name) ? $user_name : '');
							$text_to_show = JText::sprintf('COM_TJLMS_ACTIVITY_LOGGEDIN', $user_name);
						break;
					case "LOGOUT":
							$user_name = (isset($user_name) ? $user_name : '');
							$text_to_show = JText::sprintf('COM_TJLMS_ACTIVITY_LOGGEDOUT', $user_name);
						break;
					default:
						$text_to_show = '';
						break;
