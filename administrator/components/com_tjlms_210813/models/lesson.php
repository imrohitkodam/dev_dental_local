...
class TjlmsModelLesson extends JModelAdmin
{
	...
	{
		// Get the form.
		JForm::addFormPath(JPATH_ADMINISTRATOR . '/components/com_tjlms/models/forms');
		JForm::addFieldPath(JPATH_ADMINISTRATOR . '/components/com_tjlms/models/fields');
	...
	{
		$app  = JFactory::getApplication();
		$format = $app->input->getString('format', 'scorm');
			...
			if ($item->id)
			{
				if (!empty($item->media_id))
				{
					...
					if (!empty($res))
					{
						...
						$dispatcher = JDispatcher::getInstance();
						$checkFormat = $dispatcher->trigger('additional' . $plg_name . 'FormatCheck', array($item->id, $res));
				...
				$assessModel = JModelLegacy::getInstance('Assessment', 'TjlmsModel');
	...
	public function save($data)
	{
		...
		if (!empty($data['image']) && (!$data['image']['error'] || $data['image']['error'] == UPLOAD_ERR_OK))
		{
			...
			{
				$this->setError(JText::_("COM_TMT_QUESTION_MEDIA_INVALID_FILE_TYPE_ERROR"));
		...
		if (parent::save($data))
		{
			...
			if (!empty($data['image']) && (!$data['image']['error'] || $data['image']['error'] == UPLOAD_ERR_OK))
			{
				...
				if (!$uploadedMedia)
				{
					$this->setError(JText::_($mediaModel->getError()));
	...
	public function deleteLesson($lesson)
	{
		JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_tjlms/models');
		$mediaModel = JModelLegacy::getInstance('Media', 'TjlmsModel');
		$lessonMappingModel = JModelLegacy::getInstance('CourseLessons', 'TjlmsModel');
		...
		try
		{
			...
			if ($format == 'scorm')
			{
				...
				if (JFolder::exists($extractedPath))
				{
