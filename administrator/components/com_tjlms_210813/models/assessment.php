 ...
 * @since  1.0.0
 */
class TjlmsModelAssessment extends JModelAdmin
{
	...
	public function save($assessmentData)
	{
		...
		$db    = JFactory::getDBO();
		JTable::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjlms/tables');
		$assessxreftable = JTable::getInstance('Assessmentxref', 'TjlmsTable', array('dbo', $db));
		$assessxreftable->load(array('lesson_id' => $assessmentData['lesson_id']));
		$properties = $assessxreftable->getProperties(1);
		$assessxreftable = JArrayHelper::toObject($properties, 'JObject');
		...
		if ($assessmentData['add_assessment'] == 1)
		{
			...
			$paramsTable = JTable::getInstance('Assessmentparameter', 'TjlmsTable', array('dbo', $db));
		...
		elseif ($assessmentData['add_assessment'] == 0)
		{
			...
			JTable::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjlms/tables');
			$lessonTable = JTable::getInstance('lesson', 'TjlmsTable', array('dbo', $db));
