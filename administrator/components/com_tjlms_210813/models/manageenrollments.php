...
class TjlmsModelManageenrollments extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
					$this->getState('list.select', 'b.id,b.state, b.course_id,b.user_id,b.enrolled_on_time,
					co.title, uc.name, uc.username,a.start_date,a.due_date,a.id as todo_id, co.catid')
		);
		...
		elseif ($published === '')
		{
			$query->where($this->_db->qn('b.state') . ' IN (0, 1)');
		}
	...
	public function delete($cid)
	{
		try
		{
			...
			foreach ($cid as $key => $eachEnrollment)
			{
				...
				if ($allowed_delete == 1)
				{
					$enrollDetails[$eachEnrollment]->course_id =	$Enrolledusers->course_id;
					$enrollDetails[$eachEnrollment]->user_id =	$Enrolledusers->user_id;
			...
			if ($cid != null)
			{
				...
				$dispatcher->trigger('onBeforeEnrolementsDelete', array($cid));
				...
				$dispatcher->trigger('onAfterEnrolementsDelete', array($cid, $enrollDetails));
			}
