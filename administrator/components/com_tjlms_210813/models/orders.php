			...
			try
			{
				...
				foreach ($lessonsParams as $lesson)
				{
					if (!empty($lesson->params))
					{
						...
						{
							$dispatcher->trigger('onAfterCourseStatusChanges', array($course->course_id, $lesson->id, $lesson->params, $status, $order_id));
						}
					}
				}
				...
				$dispatcher->trigger('onOrderAfterStatusChange', array($order_id, $status));
			...
			if ($enrollment_id)
			{
				...
				if (!empty($enrollmentData) && $enrollmentData->id)
				{
					...
					$res->state            = ($orderStatus == 'C') ? 1 : '-1';
						...
						PluginHelper::importPlugin('system');
						$dispatcher->trigger('onAfterEnrolUpdate', array(
																			$res->id,
																			$res->state
																		)
											);
