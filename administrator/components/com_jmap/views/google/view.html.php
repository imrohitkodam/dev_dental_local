...
class JMapViewGoogle extends JMapView {
	...
	protected function addDisplayToolbar() {
		...
		if($this->getModel()->getState('googlestats', 'analytics') == 'webmasters') {
			...
			JToolBarHelper::title( JText::_( 'COM_JMAP_GOOGLE_PAGESPEED_TEST' ), 'jmap' );
			$authenticationRequired = false;
		} else {
		...
		if ($user->authorise('core.edit', 'com_jmap') && $this->getModel()->getState('googlestats', 'analytics') == 'webmasters' && $this->isLoggedIn) {
			...
			JToolBarHelper::custom('google.exportXlsRecord', 'download', 'download', 'COM_JMAP_EXPORTXLS', false);
		}
		
		if ($user->authorise('core.edit', 'com_jmap') && $this->getModel()->getState('googlestats', 'analytics') == 'pagespeedfetch') {
			JToolBarHelper::custom('google.exportXlsPagespeed', 'download', 'download', 'COM_JMAP_EXPORTXLS', false);
		}
	...
	public function display($tpl = null) {
		...
		if($googleStatsState == 'webmasters') {
			...
			$this->document->addScriptDeclaration("jQuery(function(){jQuery('input[data-role=calendar]').datepicker({dateFormat : 'yy-mm-dd',firstDay : 1}).prev('span').on('click', function(){jQuery(this).datepicker('show');});});");
			...
			// Load resources, iframe script used for frontend module and custom backend template styling for the iframed contents
			$this->document->addScript(JUri::root(true) . '/administrator/components/com_jmap/js/tablesorter/jquery.tablesorter.js');
			$this->document->addStylesheet ( JUri::root ( true ) . '/administrator/components/com_jmap/css/pagespeed.css' );
			
			// Setup the iframe container
			$pageSpeedDomain = $this->getModel()->getState('pagespeedlink');
			$googleData['iframehtml'] = '<iframe id="jmap-analytics-frame" style="height:2000px" src="https://developers.google.com/speed/pagespeed/insights/?url=' . rawurlencode($pageSpeedDomain) . '&tab=desktop&hl=' . $this->getModel()->getState('pagespeed_language') . '"></iframe>';
