...
<form action="index.php" method="post" name="adminForm" id="adminForm">
	...
	<tbody>
	<?php
	$indexingWaiter = '<img class="hasTooltip metainfo-waiter" title="' . JText::_('COM_JMAP_METAINFO_COUNTERS_WAITER') . '" alt="' . JText::_('COM_JMAP_METAINFO_COUNTERS_WAITER') . '" src="' . JUri::base(true) . '/components/com_jmap/images/loading.gif" border="0" />';
	$k = 0;
	foreach ( $this->items as $row ) {
		...
		<tr>
			...
			<td align="center">
				<select class="robots_directive" data-bind="{robots}" id="jmap_metainfo_robots_<?php echo $k + 1;?>" data-robotsidentifier="<?php echo $k + 1;?>">
					<option value="">
					...
					<option value="max-snippet:-1,max-image-preview:large,max-video-preview:-1" <?php echo isset($row->metainfos->robots) && $row->metainfos->robots == 'optin' ? 'selected="selected"' : null;?>>
						<?php echo JText::_('COM_JMAP_USE_OPTIN');?>
					</option>
				</select>
			</td>
