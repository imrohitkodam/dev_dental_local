...
(function($) {
	var Metainfo = function() {
				...
				if (this.length) {
					return $(this).each(function charCountEachElement() {
						...
						var addElements = function() {
							...
							$this.container = $('<span/>');
							$this.counterCharacter = $('<span/>').attr('title', COM_JMAP_CHARACTERS).addClass('label labelmetainfo').addClass('label-primary').html( '<span class="jmapmetainfo_chars_count glyphicon glyphicon-align-left"></span><span class="jmap_metainfo_counter">' + 0 + '</span>'); 
							$this.counterPxDesktop = $('<span/>').attr('title', COM_JMAP_PIXEL_DESKTOP).addClass('label labelmetainfo').addClass('label-primary').html( '<span class="jmapmetainfo_pixel_count glyphicon glyphicon-blackboard"></span><span class="jmap_metainfo_counter_pixel">' + 0 + 'px</span>');
							$this.counterPxMobile = $('<span/>').attr('title', COM_JMAP_PIXEL_MOBILE).addClass('label labelmetainfo').addClass('label-primary').html( '<span class="jmapmetainfo_pixel_count glyphicon glyphicon-phone"></span><span class="jmap_metainfo_counter_pixel">' + 0 + 'px</span>');
						...
						$this.update = function() {
							...
							$this.counterCharacter.html('<span title="' + COM_JMAP_CHARACTERS + '" class="jmapmetainfo_chars_count glyphicon glyphicon-align-left"></span><span class="jmap_metainfo_counter">' + length + '</span>');
							$this.counterPxDesktop.html('<span title="' + COM_JMAP_PIXEL_DESKTOP + '" class="jmapmetainfo_pixel_count glyphicon glyphicon-blackboard"></span><span class="jmap_metainfo_counter_pixel">' + desktopLength + 'px</span>');
							$this.counterPxMobile.html('<span title="' + COM_JMAP_PIXEL_MOBILE + '" class="jmapmetainfo_pixel_count glyphicon glyphicon-phone"></span><span class="jmap_metainfo_counter_pixel">' + mobileLength + 'px</span>');
							// Warning class for the characters count
							if (options.limit < length) {
								$this.counterCharacter.removeClass('label-primary');
								$this.counterCharacter.addClass('label-danger');
							} else {
								$this.counterCharacter.removeClass('label-danger');
								$this.counterCharacter.addClass('label-primary');
							}
							// Warning class for title pixel count
							if($this.hasClass('metatitle')) {
								if (maxLengthTitleDesktop < desktopLength) {
									$this.counterPxDesktop.removeClass('label-primary');
									$this.counterPxDesktop.addClass('label-danger');
								} else {
									$this.counterPxDesktop.removeClass('label-danger');
									$this.counterPxDesktop.addClass('label-primary');
								}
								if (maxLengthTitleMobile < mobileLength) {
									$this.counterPxMobile.removeClass('label-primary');
									$this.counterPxMobile.addClass('label-danger');
								} else {
									$this.counterPxMobile.removeClass('label-danger');
									$this.counterPxMobile.addClass('label-primary');
								}
							}
		...
		var autoPopulateMetainfo = function() {
			...
			$.each(linksToAnalyze, function(index, link){
				...
				promisesCollection[index].then(function(responseData, status) {
					...
					if(titlePopulated || descPopulated || robotsPopulated) {
		...
		this.startMetaCount = function() {
			...
			$('span.labelmetainfo').tooltip({trigger:'hover', placement:'top', html : 1});
		};
