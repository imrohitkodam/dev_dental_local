...
class MijosqlModelMijosql extends MijosqlModel {
    ...
    public function _replacePrefix($query) {
        ...
        if (!is_writable($config_file)) {
            echo '<h2 style="color: red;">'.sprintf(JText::_('COM_MIJOSQL_CONFIG_NOT_WRITABLE'), $config_fname).'</h2>';
            return $msg;
        }
    ...
    public function exportToCsv($query) {
        ...
        if (!empty($rows)) {
            $comma = JText::_('COM_MIJOSQL_CSV_DELIMITER');
            $CR = "\r";
	...
	public function _getHtmlTable($query, $num, $db) {
       ...
       $body .= '<br />';
		...
		if (!empty($rows)) {
			...
			if ($_sel) {
				$body .= '<th>'.JText::_('COM_MIJOSQL_ACTION').'</th>';
			}
			...
			foreach($rows as $row) {
				...
				if ($_sel) {
					$body .= '<td align=center nowrap>';
						$body .= '<a href="index.php?option=com_mijosql&task=edit&ja_tbl_g='.base64_encode($table).'&ja_qry_g='.base64_encode($query).'&key='.$key.'&id='.$row[$key].'">';
							$body .= '<img border="0" src="components/com_mijosql/assets/images/icon-16-edit.png" alt="'.JText::_('COM_MIJOSQL_EDIT').'" title="'.JText::_('COM_MIJOSQL_EDIT').'" />';
						$body .= '</a>';
						$body .= '&nbsp;';
						$body .= '<a href="#" onclick="if (confirm(\'Are you sure you want to delete this record?\')) {this.href=\'index.php?option=com_mijosql&controller=mijosql&task=delete&ja_tbl_g='.base64_encode($table).'&ja_qry_g='.base64_encode($query).'&key='.$key.'&id='.$row[$key].'\'};">';
							$body .= '<img border="0" src="components/com_mijosql/assets/images/icon-16-delete.png" alt="'.JText::_('COM_MIJOSQL_DELETE').'" title="'.JText::_('COM_MIJOSQL_DELETE').'" />';
						$body .= '</a>';
					$body .= '</td>';
				}
