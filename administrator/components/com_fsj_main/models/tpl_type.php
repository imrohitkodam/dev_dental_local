...
class fsj_mainModeltpl_type extends JModelAdmin
{
	protected $text_prefix = 'fsj_main_tpl_type';
	...
	public function getTable($type = 'tpl_type', $prefix = 'JTablefsj_main', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('fsj_main.tpl_type', 'tpl_type', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_fsj_main.edit.tpl_type.data', array());

		if (empty($data)) {
			$data = $this->getItem();

			// Prime some default values.
			if ($this->getState('tpl_type.id') == 0) {
...
/* END PARENT SAVE CODE */
	...
	static function allowEdit($faq)
	{
		$user     = JFactory::getUser();
		$userId   = $user->get('id');
		$asset    = 'com_fsj_main.tpl_type.' . $faq->id;
