...
class fsj_mainModelplugin extends JModelAdmin
{
	protected $text_prefix = 'fsj_main_plugin';
	...
	public function getTable($type = 'plugin', $prefix = 'JTablefsj_main', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('fsj_main.plugin', 'plugin', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_fsj_main.edit.plugin.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('plugin.id') == 0) {
				...
				$data->set('type', JRequest::getInt('type', $app->getUserState('com_fsj_main.plugins.filter.type')));
	...
	public function save($data)
	{
		$db = JFactory::getDBO();
			...
			$params_handler = new JFormFieldfsjpluginparams();
			$params_handler->fsjpluginparams = json_decode("{}");
...
/* END PARENT SAVE CODE */
	...
	static function allowEdit($faq)
	{
		$user     = JFactory::getUser();
		$userId   = $user->get('id');
		$asset    = 'com_fsj_main.plugin.' . $faq->id;
