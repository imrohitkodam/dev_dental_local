...
class fsj_mainModeltpl extends JModelAdmin
{
	protected $text_prefix = 'fsj_main_tpl';
	...
	public function getTable($type = 'tpl', $prefix = 'JTablefsj_main', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('fsj_main.tpl', 'tpl', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_fsj_main.edit.tpl.data', array());

		if (empty($data)) {
			$data = $this->getItem();

			// Prime some default values.
			if ($this->getState('tpl.id') == 0) {
				$app = JFactory::getApplication();
				
				// need to load the state of any filters in
				$data->set('type', JRequest::getInt('type', $app->getUserState('com_fsj_main.tpls.filter.type')));
	...
	public function save($data)
	{
		$db = JFactory::getDBO();
			...
			$params_handler = new JFormFieldfsjtemplateedit();
			$params_handler->fsjtemplateedit = json_decode("{}");
...
/* END PARENT SAVE CODE */
	...
	public function canSave($data = array(), $key = 'id')
	{
				...
				if (isset($data->noedit) && $data->noedit)
					return false;
