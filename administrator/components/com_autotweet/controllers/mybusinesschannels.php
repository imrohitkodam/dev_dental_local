...
/**
 * AutotweetControllerMyBusinessChannels.
 *
 * @since       1.0
 */
class AutotweetControllerMyBusinessChannels extends ExtlyController
{
    /**
     * getCallbackUrl.
     *
    ...
    public function callback()
    {
        ...
        */
        ...
        try {
            ...
            $myBusinesscode = $this->input->getString('code');

            // Error throw
            if (!empty($myBusinesscode)) {
                ...
                $status = $myBusinessChannelHelper->authenticate($myBusinesscode);

                if (!$status) {
                    throw new Exception('Failed to authenticate MyBusinessChannel.');
                }

                $logger = AutotweetLogger::getInstance();
                $logger->log(\Joomla\CMS\Log\Log::INFO, 'MyBusinessChannel Authenticate OK.');
