...
class JFBConnectControllerAjax extends JFBConnectController
{
    ...
    function scsocialwidget()
    {
        ...
        $xmlPath = JPATH_ROOT .'/components/com_jfbconnect/libraries/provider/'.$provider.'/widget';

        if(!JFolder::exists($xmlPath))
        {
            $xmlPath = JPATH_ROOT . '/plugins/jfbconnect/provider_'. $provider.'/provider/'. $provider . '/widget';
        }

        JForm::addFieldPath($xmlPath);
        $xmlFile = $xmlPath.'/'.$widget.'.xml';
    ...
    public function showAccessField()
    {
        $model = JModelLegacy::getInstance('Channel', 'JFBConnectModel');
        $form = $model->getForm();
        ...
        $input = JFactory::getApplication()->input;
        $objects = $input->getString('objects',null);
        $plugins = $input->getString('plugins',null);
        ...
        $show = $view->checkIfObjectsAreContent($objects);
        $show = $show || $view->checkIfPluginsAreContent($plugins);
        $field = $view->getAccessFields($form, $show);
        echo $field;
        exit;
    }
}
