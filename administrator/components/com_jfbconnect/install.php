...
class com_JFBConnectInstallerScript
{
    ...
    public function __construct($installer)
    {
        ...
        foreach ($plugins as $p)
        {
            ...
            {
                $pluginFolder = explode('.', $parts[0]);
                $pluginFolder = $pluginFolder[0];
                if ($pluginFolder == 'installer' && version_compare(JVERSION, '3.2.0', '>='))
                    continue; // Don't install the installer plugin on Joomla 3.2+ as we use the extra_query param instead
    ...
    {
        $this->loadLanguage('com_jfbconnect', JPATH_ADMINISTRATOR);
        ...
        $this->migrateProfilePluginSettings612();

        $this->enableInstallerPlugin();
    ...
    private function installPackages()
    {
        ...
        <h3><?php echo JText::_('COM_JFBCONNECT_INSTALL_SUCCESS');?></h3>

        <?php
        echo '<table class="adminlist table table-striped" width="100%">';
        echo '<thead><tr><th class="title">'.JText::_('COM_JFBCONNECT_INSTALL_COL_EXTENSION').'</th><th width="40%">'.JText::_('COM_JFBCONNECT_INSTALL_COL_STATUS').'</th></tr></thead>';
        echo '<tbody>';
        echo '<tr class="row0"><td>'.JText::_('COM_JFBCONNECT_INSTALL_ROW_JFBC_LABEL').'</td><td><span style="color:green; font-weight:bold">'.JText::_('COM_JFBCONNECT_INSTALL_ROW_INSTALLED').'</span></td></tr>';
        foreach ($this->packages as $group => $packages)
        {
            echo '<tr><th><strong>' . $group . '</strong></th><th><center>'.JText::_('COM_JFBCONNECT_INSTALL_COL_STATUS').'</center></th></tr>';
    ...
    private function installPackageList($packages)
    {
        foreach ($packages as $package)
        {
            ...
            if (!$pkgInstall)
                $installed = '<span style="color:blue; font-weight:bold">'.JText::_('COM_JFBCONNECT_INSTALL_ROW_NOTINSTALLED').'</span>';
            else
            {
                $pkg = JInstallerHelper::unpack($this->installer->getParent()->getPath('source') . '/packages/' . $pkgFile);
                if ($installer->install($pkg['dir']))
                    $installed = '<span style="color:green; font-weight:bold">'.JText::_('COM_JFBCONNECT_INSTALL_ROW_INSTALLED').'</span>';
                else
                    $installed = '<span style="color:red; font-weight:bold">'.JText::_('COM_JFBCONNECT_INSTALL_ROW_INSTALLMANUALLY').'</span>';
    ...
    {
        ?>
        <p style="font-weight:bold; margin-top:20px"><?php echo JText::_('COM_JFBCONNECT_INSTALL_AUTOTUNE_DESC');?></p>
        <center><a href="index.php?option=com_jfbconnect&view=autotune"
                   ...
                   box-shadow:0 1px 3px 0 #565656;"><?php echo JText::_('COM_JFBCONNECT_INSTALL_AUTOTUNE_RUNNOW');?></a></center>
    <?php
    }
    ...
    private function enableInstallerPlugin()
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->update($db->qn('#__extensions'))
                ->set($db->qn('enabled') . '=1')
                ->where($db->qn('element') . '=' . $db->q('jfbconnect'))
    ...
    private function loadLanguage($extension, $basePath = JPATH_BASE)
    {
        // Load translations
