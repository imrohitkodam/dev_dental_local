...
class JFBConnectViewConfig extends JFBConnectAdminView
{
    function display($tpl = null)
    {
        ...
        foreach (JFBCFactory::getAllProviders() as $p)
        {
            ...
            if(!JFolder::exists($configPath))
                $configPath = JPATH_SITE . '/plugins/jfbconnect/provider_'. strtolower($p->systemName).'/provider/'. strtolower($p->systemName) . '/config/';
    ...
    function addToolbar()
    {
        JToolBarHelper::title('JFBConnect: Configuration', 'jfbconnect.png');
