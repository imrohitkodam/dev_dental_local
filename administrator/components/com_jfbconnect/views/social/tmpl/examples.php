...
$providersWithWidgets = JFBCFactory::getAllProviders();
?>
...
<div id="filter-bar" class="btn-toolbar">
    <div class="btn-group pull-left">
        <select name="filter_provider" id="filter_provider" class="input-large" onchange="this.form.submit()">
            ...
            <?php foreach($providersWithWidgets as $provider):?>
                <option value="<?php echo $provider->systemName; ?>" <?php if (strtolower($filter_provider) == $provider->systemName) echo 'selected="selected"'; ?>><?php echo $provider->name; ?></option>
            <?php endforeach; ?>
        </select>
    </div>
</div>
...
foreach($providersWithWidgets as $provider)
{
    if($filter_provider && $filter_provider != $provider->systemName) continue;

    $widgets = JFBCFactory::getAllWidgets($provider->systemName);
    foreach($widgets as $widget)
    {
        echo '<p> </p><h3>'.$provider->name . ' - '.$widget->getName().'</h3>';
        ...
        $xmlFile = JPATH_ROOT .'/components/com_jfbconnect/libraries/provider/'.$provider->systemName.'/widget/'.$widget->getSystemName().'.xml';
        if(!JFile::exists($xmlFile))
            $xmlFile = JPATH_ROOT .'/plugins/jfbconnect/provider_'. strtolower($provider->systemName).'/provider/'. strtolower($provider->systemName).'/widget/'.$widget->getSystemName().'.xml';
