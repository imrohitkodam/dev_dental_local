...
class JFBConnectViewChannel extends JFBConnectAdminView
{
    ...
    function displayAttributes()
    {
        ...
        $attribs .= $this->displayAutopostAttributes();
        $attribs .= "</div>\n";

        return $attribs;
    }

    function displayAutopostAttributes()
    {
        $attribs = '';
        $provider = JFBCFactory::provider($this->form->getValue('provider'));
        $channel = $provider->channel($this->form->getValue('type'));
        if($channel->canPostToNetwork())
        {
            $checkType = false;

            foreach ($this->form->getFieldset("autopost") as $field)
            {
                $fieldName = $field->getAttribute('name');
                if ($fieldName == 'autopost_plugins')
                {
                    $val = $field->__get('value');
                    $checkType = $checkType || $this->checkIfPluginsAreContent($val);
                    $attribs .= $this->formGetField($field);
                }
                else if ($fieldName == 'autopost_objects')
                {
                    $vals = $field->__get('value');

                    $checkType = $checkType || $this->checkIfObjectsAreContent($vals);
                    $attribs .= $this->formGetField($field);
                }
            }

            $attribs .= $this->getAccessFields($this->form, $checkType);

        }
        return $attribs;
    }

    function checkIfPluginsAreContent($val)
    {
        if (is_array($val))
            return in_array('content', $val) || in_array('k2', $val);
        return false;
    }

    function checkIfObjectsAreContent($vals)
    {
        $obj = JTable::getInstance('JFBConnectOpenGraphObject', 'Table');
        $check = false;
        if (is_array($vals))
        {
            ...
            {
                $obj->load($object);
                $check = $obj->plugin == 'content' || $obj->plugin == 'k2';
    ...
    function getAccessFields($form, $display)
    {
        if (!$display)
            return '';

        foreach ($form->getFieldset("access") as $field)
        {

            $fieldName = $field->getAttribute('name');
            if ($fieldName == 'autopost_access_type')
                return $this->formGetField($field);
        }
    }
