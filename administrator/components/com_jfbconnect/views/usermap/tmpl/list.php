...
<script>
    function addImage(containerId, url) {
        let avatarContainer = document.getElementById(containerId);

        UrlExists(url, avatarContainer, function(element, status){

            if( status === 200) // file found
            {
                element.innerHTML = '<img width="50" height="50" src="' + url +  '" />';
            }
        });
    }

    function UrlExists(url, element, cb){
        jQuery.ajax({
            url:      url,
            dataType: 'text',
            type:     'GET',
            complete:  function(xhr){
                if(typeof cb === 'function')
                    cb.apply(this, [element, xhr.status]);
            }
        });
    }
</script>
            ...
            for ($i = 0, $n = count($items); $i < $n; $i++)
            {
                ...
                <tr class="<?php echo "row$k"; ?>">
                    ...
                    <td align="center">
                        ...
                        {
                            echo '<div id="avatarImageContainer'.$row->provider_user_id.'">';
                            echo '<script>addImage("avatarImageContainer'.$row->provider_user_id.'", "'.$params->get('avatar_thumb').'")</script>';
                            echo '</div>';
                        }
                        ?>
                    </td>
