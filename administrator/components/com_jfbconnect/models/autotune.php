...
class JFBConnectModelAutoTune extends JModelLegacy
{
    ...
    public function makeSourceCoastRequest($task, $postData = null)
    {
        $ch = curl_init();
        $baseUrl = JURI::root();
        $baseUrl = base64_encode(urlencode($baseUrl));

        //JFBCFactory::log('Making SourceCoast Request: '.$task);

        $subscriptionId = $this->configModel->getSetting('sc_download_id');

        if (defined('JFBCDEV'))
            $url = "http://autotune.com/start.php";
        else
            $url = "https://autotune.sourcecoast.com/start.php";
        ...
        if ($response !== null && isset($response->authorization))
        {
            ...
            } else
            {
                if (isset($authorizationObject->error))
                    JFBCFactory::log($authorizationObject->error, 'error');
                else
                    JFBCFactory::log(JText::_('COM_JFBCONNECT_MSG_MISSING_ACTIVE_SUB'), 'error');
            }
        } else
            JFBCFactory::log($task . ' - ' . JText::_('COM_JFBCONNECT_MSG_CANNOT_FETCH_INFO'), 'error');
    ...
    */
    ...
    public function getAppConfig($forceUpdate = false)
    {
        ...
        if (!$appConfig || !$this->isUpToDate() || $forceUpdate)
        {
            ...
            {
                //$app = JFactory::getApplication();
                //JFBCFactory::log('Fetching Facebook Application Information');
    ...
    */
    ...
    /**
     * Fetch the latest network settings for each available social network
     */
    public function fetchNetworkSettings()
    {
        // Build the list of providers available in this release
        $providers = array();
        /** @var JFBConnectProvider $p */
        foreach (JFBCFactory::getAllProviders() as $p)
        {
            $details = [
                'api_setup' => (bool)!empty($p->appId) && !empty($p->secretKey)
            ];
            $details = json_encode($details);
            $providers[$p->systemName] = $details;
        }

        $data = $this->makeSourceCoastRequest('fetchNetworkSettings', $providers);

        $response = new stdClass();

        if ($data)
        {
            foreach ($data as $network => $settings)
            {
                $this->configModel->update($network . '_autotune_network_settings', $settings);
            }
            $response->success = true;
            $date = new JDate();
            $response->date = $date->format(JText::_('DATE_FORMAT_LC4'));
            $response->message = JText::_('COM_JFBCONNECT_MSG_FETCH_SUCCESS');
        } else
        {
            $response->success = false;
            $response->message = JText::_('COM_JFBCONNECT_MSG_FETCH_FAIL');
        }
        return $response;
    }
}