...
class JFBConnectModelUserMap extends JModelLegacy
{
    ...
    /***
     * Fetch an array of provider names that the user has authenticated with
     * @param $joomlaId - Joomla User ID
     */
    public function getUserProviders($joomlaId)
    {
        $query = $this->_db->getQuery(true);
        $query->select('provider')
            ->from($this->_db->qn('#__jfbconnect_user_map'))
