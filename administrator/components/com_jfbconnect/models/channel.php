...
class JFBConnectModelChannel extends JModelAdmin
{
    ...
    function save($data)
    {
        ...
        $data['attribs']['autopost_access_type'] = $data['autopost_access_type'];
    ...
    public function delete(&$pks)
    {
        ...
        foreach ($pks as $i => $pk)
        {
            if ($table->load($pk))
            {
                ...
                {
                    //Delete any autopost associated with this channel
                    $autopost = new JFBConnectModelAutopost($table->id);
                    $autopost->delete();
    ...
    protected function loadFormData()
    {
        ...
        if (empty($data))
        {
            ...
            $autopost = new JFBConnectModelAutopost($data->id);
            $autopostData = $autopost->getData();

            $apObjectsData = array();
            $apPluginsData = array();

            foreach($autopostData as $object)
            {
                if(is_numeric($object->opengraph_type))
                    $apObjectsData[] = $object->opengraph_type;
                else
                    $apPluginsData[]=$object->opengraph_type;
            }

            $data->set('autopost_objects', $apObjectsData);
            $data->set('autopost_plugins', $apPluginsData);
            $data->set('autopost_access_type', isset($data->attribs->autopost_access_type) ? $data->attribs->autopost_access_type : "1");
