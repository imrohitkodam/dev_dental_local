...
if ( ! class_exists('pkg_conditionalcontentInstallerScript'))
{
    class pkg_conditionalcontentInstallerScript
    {
        static $adapter;
        static $current_version;
        static $extensions         = [];
        static $file_string;
        static $min_joomla_version = [4 => '4.0'];
        static $min_php_version    = [4 => '8.1'];
        static $name;
        static $new_manifest;
        static $package_name;
        ...
        {
            return JFactory::getContainer()->get(JDatabaseInterface::class);
        }
        ...
        {
            $is_required = self::isRequired($string, $dependancy);

            if ( ! self::shouldUpdateDependancy($dependancy, $is_required))
            {
                self::removePackageFilesFromString($string, $dependancy);
            }
        }

        private static function isRequired($string, $package_name)
        {
            // <file type="package" id="package_name" required="true/false">pkg_package_name</file>
            $pattern = '#<file id="' . preg_quote($package_name, '#') . '" type="package"[^>]* required="(true|false)">#';
        ...
        {
            preg_match('#id="' . $id . '"[^>]*position="([^"]+)"[^>]*#', $xml, $match);
         ...
         * @param bool    $force
         */
        private static function publishExtension(
            $type,
            $name,
        ...
        private static function publishModule(
            $name,
        ...
        )
        {
            ...
            $module_position = $module_position ?: 'status';
        ...
        private static function shouldUpdateDependancy($package_name, $is_required = true)
        {
        ...
        {
            self::updatePackageId('regularlabs');
            self::updatePackageId('conditions');
        }

        private static function updatePackageId(string $alias)
        {
            $lib_package_id = self::getPackageId($alias);
            ...
            self::setPackageId($alias, $lib_package_id);
