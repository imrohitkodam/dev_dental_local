...
class plgEditorAcyEditor extends JPlugin
{
    function __construct(&$subject, $config)
    {
        ...
        if (!isset($this->params)) {
            ...
            $this->params = new acyParameter(empty($plugin->params) ? [] : $plugin->params);
        }
    }
    ...
    public function onInit()
    {
        ...
        acymailing_addScript(false, $websiteurl.'media/com_acymailing/js/jquery/jquery.min.js?v='.@filemtime(ACYMAILING_ROOT.'media'.DS.'com_acymailing'.DS.'js'.DS.'jquery'.DS.'jquery.min.js'));
    ...
    {
        $idIframe = "#".$id."_ifr";
        $initialisation = $this->GetInitialisationFunction($id, false);
    ...
    function onDisplay($name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null, $params = [])
    {
        if (empty($id)) {
            $id = $name;
        }
        ...
        $idIframe = $id."_ifr";
        $initialisation = $this->GetInitialisationFunction($id, true);
    ...
    function GetInitialisationFunction($id, $onDisplay)
    {
        ...
        $init = "Initialisation(\"$id\", \"$type\", \"$urlBase\", \"$urlAdminBase\", \"$cssurl\", \"$forceComplet\", \"$modeList\", \"$modeTemplate\", \"$modeArticle\", \"$joomla2_5\", \"$joomla3\", \"$isBack\", \"$tagAllowed\", \"$texteSuppression\", \"$tooltipSuppression\", \"$tooltipEdition\", \"$titleTemplateDelete\", \"$titleTemplateText\", \"$titleTemplatePicture\", \"$titleShowAreas\", \"$ckEditorFileVersion\");\n";
