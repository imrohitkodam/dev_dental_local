...
class PPAppEmail extends PPApp
{
	 ...
	 * @access	public
	 */
	public function onPayplansSubscriptionRenewalComplete($subscription, $invoice)
	{
		$this->triggerRenewalEmail($subscription, $invoice);
	}

	/**
	 * Track events after recurring payment the subscriptions
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	public function onPayplansSubscriptionRecurringRenewalComplete($subscription, $invoice)
	{
		if (!$this->helper->shouldSendEmailForEachRecurringPayment()) {
			return true;
		}
	...
	/**
	 * Event for renewal emails
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	private function triggerRenewalEmail($subscription, $invoice)
	{	
		if (!$this->helper->shouldSendRenewalEmail($subscription, $invoice)) {
			return;
		}

		return $this->helper->send($invoice);
	}
	...
	private function cartAbandoned()
	{
		...
		foreach ($invoices as $id => $row) {
			...
			$user = $subscription->getBuyer(true);
			...
			// Do not sent email if user already subscribe to this plan
			if (in_array($planId, $userPlanIds)) {
				return true;
			}
