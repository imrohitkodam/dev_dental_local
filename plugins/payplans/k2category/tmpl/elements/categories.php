...
require_once(JPATH_ROOT . '/plugins/payplans/k2category/app/lib.php');
...
<?php if ($lib->exists()) { ?>
	...
	<select name="<?php echo $name;?>[]" class="pp-autocomplete o-form-control" multiple="multiple" <?php echo $attributes;?>>
		...
		<option value="<?php echo $category->category_id;?>" <?php echo in_array($category->category_id, $value) ? 'selected="selected"' : '';?>><?php echo JText::_($category->name);?></option>
		<?php } ?>
	</select>
<?php } else { ?>
