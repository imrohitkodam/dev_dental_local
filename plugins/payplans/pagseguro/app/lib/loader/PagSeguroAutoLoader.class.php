...
class PagSeguroAutoloader {
	...
	private function __construct() {
		$folders = array('domain', 'exception', 'parser', 'service', 'utils', 'helper', 'config', 'resources', 'log');
		
		foreach($folders as $folder){
			$this->autoLoadFolder(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.$folder);
		}
	...
	public function autoLoadFolder($folder)
	{
		...
		if(is_array($files)){
			foreach($files as $file ){
				...
				$className = array_shift($temp);
				JLoader::register($className, $folder.DIRECTORY_SEPARATOR.$file);
			}
		}
	}
