...
?>
<?php if ($lib->exists()) { ?>
	<?php $sections = $lib->getSections(); ?>
	<select name="<?php echo $name;?>[]" class="pp-autocomplete o-form-control" multiple="multiple" <?php echo $attributes;?>>
		<?php foreach ($sections as $section) { ?>
		<option value="<?php echo $section->sec_id;?>" <?php echo in_array($section->sec_id, $value) ? 'selected="selected"' : '';?>><?php echo JText::_($section->name);?></option>
		<?php } ?>
	</select>
<?php } else { ?>
