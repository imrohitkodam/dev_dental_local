...
class PPAppEasysocialProfiletype extends PPApp
{
	...
	public function onPayplansSubscriptionAfterSave($prev, $new)
	{
		...
		if ($new->isActive()) {
			...
			return $this->helper->setEasysocialprofile($userId, $easysocialProfileId, true, $new->isActive());
		}

		// if subscription is hold
		if ($new->isOnHold()) {
			$easysocialProfileId = $this->getAppParam('esprofiletypeOnHold', 0);
			return $this->helper->setEasysocialprofile($userId, $easysocialProfileId, true, $new->isOnHold());
		}

		// if subscription is expired
		if ($new->isExpired()) {

			//check if user have active subscription for same ES Profiletype then do nothing
			$appId = $this->getId();
			$activeProfileId = $this->getAppParam('esprofiletypeOnactive', 0);
			$state = $this->helper->checkUserSubscription($user, $activeProfileId, $appId);
			...
			return $this->helper->setEasysocialprofile($userId, $easysocialProfileId, true, $new->isExpired());
		}
