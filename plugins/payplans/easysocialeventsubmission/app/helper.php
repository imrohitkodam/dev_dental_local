...
class PPHelperEasysocialeventsubmission extends PPHelperStandardApp
{
	...
	*/
	...
	/**
	* Retrieve ES events created by user.
	*
	* @since	4.2.0
	* @access	public
	*/
	public function getUserEasysocialEvents($userId, $restrictType, $restrictCategories)
	{
		$esLib = PP::easysocial();

		$results = $esLib->getAppUserEasysocialCluster(SOCIAL_TYPE_EVENT, $userId, $restrictType, $restrictCategories);
		return $results;
	}


	/**
	* Update event's state
	*
	* @since	4.2.0
	* @access	public
	*/
	public function toggleState($state, $eventIds)
	{
		$esLib = PP::easysocial();

		$result = $esLib->appToggleClusterState($state, $eventIds);
		return $result;
	}
