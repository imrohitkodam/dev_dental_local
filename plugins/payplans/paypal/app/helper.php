...
class PPHelperPaypal extends PPHelperPayment
{
	 ...
	 * @access	public
	 */
	public function getLanguageEncoding()
	{
		static $charset = null;

		if (is_null($charset)) {
			$charset = $this->params->get('charset', 'utf-8');
		}
	...
	public function getCallbackUrls(PPPayment $payment)
	{
		...
		if (is_null($callbacks)) {
			...
			$cancelURL = PPR::external('index.php?option=com_payplans&gateway=paypal&view=payment&task=complete&action=cancel&payment_key=' . $payment->getKey(), false) . PP::getExcludeTplQuery('checkout');
	...
	public function getRecurrenceTime($expTime)
	{
		...
		if (!empty($expTime['year'])) {
			...
			if (isset($expTime['month']) && $expTime['month']) {
				return [
					'period' => $expTime['year'] * 12 + $expTime['month'], 
					'unit' => 'M', 
					'frequency' => JText::_('COM_PAYPLANS_RECURRENCE_FREQUENCY_GREATER_THAN_ONE'),
					'message' => JText::_('COM_PAYPLANS_PAYMENT_APP_PAYPAL_RECURRING_MESSAGE')
				];
		...
		if (!empty($expTime['month'])) {
			...
			return [
				'period' => intval(($days + $expTime['day'])/7, 10), 
				'unit' => 'W', 
				'frequency' => JText::_('COM_PAYPLANS_RECURRENCE_FREQUENCY_GREATER_THAN_ONE'),
				'message' => JText::_('COM_PAYPLANS_PAYMENT_APP_PAYPAL_RECURRING_MESSAGE')
			];
	...
	{	
		...
		if ($data) {
			...
			$disallowedKeys = ['option', 'task', 'view', 'layout', 'gateway', 'id', 'Itemid', 'payment_key'];
