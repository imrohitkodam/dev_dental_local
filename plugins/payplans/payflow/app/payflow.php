...
class PPAppPayflow extends PPAppPayment
{
	...
	public function processPayment(PPPayment $payment, $invoiceCount)
	{
		...
		$message = JText::_('Recurring PaymentResponse');
		...
		foreach ($response as $key => $value) {
			...
			if (strpos($key, 'P_PNREF1') === false || in_array($value, $pnref_array)) {
				continue;
			}

			$index = substr($key, -1);

			$rowCode = (int) $response['P_RESULT1' . $index];
			$rowState = (int) $response['P_TRANSTATE1' . $index];
			$rowAmount = (float) $response['P_AMT1' . $index];
			$rowTransactionId = (string) $response['P_PNREF1' . $index];
			...
			if ($rowCode === 0 && $rowState == $transactionState && $rowAmount > 0) {
				...
				$transTime = PP::normalize($response, 'P_TRANSTIME1' . $index, '');
				$transResult = PP::normalize($response, 'P_RESULT1' . $index, '');
				$transTranslate = PP::normalize($response, 'P_TRANSLATE1' . $index, '');

				$params = new JRegistry();
				$params->set('P_PNREF1', $rowTransactionId);
				$params->set('P_TRANSTIME1', $transTime);
				$params->set('P_RESULT1', $transResult);
				$params->set('P_TRANSLATE1', $transTranslate);
