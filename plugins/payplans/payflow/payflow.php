...
class plgPayplansPayflow extends PPPlugins
{
	...
	public function onPayplansCron()
	{
		...
		foreach ($payments as $payment) {
			...
			$transactions = $this->getExistingTransactions($invoice->getId(), $response['RPREF'], 0, 0);

			if ($transactions) {
				foreach ($transactions as $transaction) {
					$transaction = PP::transaction($transaction);

					if ($transaction->get('gateway_txn_id') == $response['RPREF']) {
						return true;
					}
				}
			}
			...
			if ($response['RESULT'] == 0 ) {
				$transaction = PP::createTransaction($invoice, $payment, $response['RPREF'], 0, 0, $response);
	...
	private function getPayments($appIds = array())
	{
		...
		$now->subtractExpiration('000001000000');
