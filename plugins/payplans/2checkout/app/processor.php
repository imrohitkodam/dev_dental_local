...
class PP2CheckoutProcessor
{
	...
	{
		$transaction->message = 'COM_PAYPLANS_APP_2CHECKOUT_TRANSACTION_ORDER_CREATED_' . PPJString::strtoupper($data['invoice_status']);
	...
	public function fraud_status_changed($payment, $data, $transaction)
	{
		...
		$transaction->message = 'COM_PAYPLANS_APP_2CHECKOUT_TRANSACTION_FRAUD_STATUS_CHANGED_' . PPJString::strtoupper($status);
	...
	public function ship_status_changed($payment, $data, $transaction)
	{
		...
		$transaction->message = 'COM_PAYPLANS_APP_2CHECKOUT_TRANSACTION_SHIP_STATUS_CHANGED_' . PPJString::strtoupper($data['ship_status']);
	...
	public function invoice_status_changed($payment, $data, $transaction)
	{
		...
		$transaction->message = 'COM_PAYPLANS_APP_2CHECKOUT_TRANSACTION_STATUS_INVOICE_STATUS_CHANGED_' . PPJString::strtoupper($data['invoice_status']);
