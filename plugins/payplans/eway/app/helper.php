...
class PPHelperEway extends PPHelperPayment
{
	...
	{
		// get country code
		$country = strtolower(PP::normalize($data, 'country', ''));
		$countryCode = $this->getCountry($country);
		...
		$payload['Country'] =strtolower($countryCode);
		...
		try {
			...
			$defaultMsg = JText::_('COM_PAYPLANS_APP_EWAY_CREATE_CUSTOMER_ERROR');
			...
			PPLog::log(PPLogger::LEVEL_ERROR, $defaultMsg, $payment, $error, 'PayplansPaymentFormatter', '', true);

			// if the system detected the user enter invalid expiry date then show the clear message to the user on frontend
			if ($errorMsg === 'Credit Card expiry date must be valid.') {
				$defaultMsg = $defaultMsg . ': ' . $errorMsg;
			}

			$this->setError($defaultMsg);
			return false;
		}
	 ...
	 * @access	public
	 */
	public function processNonRecurringPayment(PPPayment $payment, PPInvoice $invoice, $data = [], $customerProfileId)
	{
	 ...
	 * @access	public
	 */
	public function processRecurringPayment(PPPayment $payment, PPInvoice $invoice, $recurrenceCount, $customerProfileId, $data = [])
	{
		...
		// If initial purchase discount applied on recurring plan then no need to decrese the recurrenec count
