...
class PPHelperEasysocialpagesubmission extends PPHelperStandardApp
{
	...
	* @access	public
	*/
	public function getUserEasysocialPages($userId, $restrictType, $restrictCategories)
	{
		$esLib = PP::easysocial();

		$results = $esLib->getAppUserEasysocialCluster(SOCIAL_TYPE_PAGE, $userId, $restrictType, $restrictCategories);
		return $results;
	}
	...
	* @access	public
	*/
	public function toggleState($state, $pageIds)
	{
		$esLib = PP::easysocial();

		$result = $esLib->appToggleClusterState($state, $pageIds);
		return $result;
	}
