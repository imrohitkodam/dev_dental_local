...
class plgPayplansEuvat extends PPPlugins
{
	...
	{
		if (!($view instanceof PayPlansViewCheckout) && !($view instanceof PayPlansViewInvoice && PP::isFromAdmin())) {
			return true;
		}
	...
	public function onPayplansTaxRequest($invoiceKey, $country, $purpose = 0, $businessNo = '', $businessName = '')
	{
		...
		list($invoice, $error, $proceed, $success) = $helper->doTaxRequest($invoiceKey, $country, $purpose, $businessNo, $businessName);
		...
		return $ajax->resolve($html, $total, $error, $success);
