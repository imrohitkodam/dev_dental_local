...
class PPHelperPaypalPro extends PPHelperPayment
{
	...
	public function createRecurringProfile($invoice, $payment, $payloadData)
	{
		...
		// Add deley in profile start date
		$time = '000000000025';
		$date = $date->addExpiration($time);

		// Default Amount
		$amount = $invoice->getTotal();
		$payloadData['PROFILEREFERENCE'] = urlencode($payment->getKey());
		$payloadData['PROFILESTARTDATE'] = str_replace("GMT", "T", $date->format("Y-m-dTH:i:s", null, true))."Z";
	...
	public function getCountries()
	{
		...
		if (is_null($countries)) {
			...
			// Option for select country added
			$none = new stdClass();
			$none->value = 0;
			$none->title = JText::_('COM_PP_SELECT_COUNTRY');
	...
	{ 
		...
		if(!empty($expTime['day'])){
			...
			return array(
						'period' => $days, 'unit' => 'Day', 'frequency' => JText::_('COM_PAYPLANS_RECURRENCE_FREQUENCY_GREATER_THAN_ONE'), 'message' => JText::_('COM_PAYPLANS_PAYMENT_APP_PAYPAL_PRO_RECURRING_MESSAGE'));
		}
		...
		if(!empty($expTime['month'])){
			...
			return array('period' => $month, 'unit' => 'Month', 'frequency' => JText::_('COM_PAYPLANS_RECURRENCE_FREQUENCY_GREATER_THAN_ONE'), 'message' => JText::_('COM_PAYPLANS_PAYMENT_APP_PAYPAL_PRO_RECURRING_MESSAGE'));
		}
		
		// years
		if(!empty($expTime['year'])){		
			return array('period' => $expTime['year'], 'unit' => 'Year', 'frequency' => JText::_('COM_PAYPLANS_RECURRENCE_FREQUENCY_GREATER_THAN_ONE'), 'message' => JText::_('COM_PAYPLANS_PAYMENT_APP_PAYPAL_PRO_RECURRING_MESSAGE'));
		}
