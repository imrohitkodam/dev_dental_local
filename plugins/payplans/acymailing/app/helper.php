...
class PPHelperAcymailing extends PPHelperStandardApp
{
	...
	public function addOrRemoveFromAcymailingList($userid, $addToList, $removeFromList, $subscriptionId)
	{
		...
		$subscribe = is_array($addToList) ? $addToList : [$addToList]; 
		
		// user who want to remove this list id
		$removeListIds = is_array($removeFromList) ? $removeFromList : [$removeFromList];
		...
		if (!empty($removeListIds)) {
			...
			foreach ($removeListIds as $listId) {
				...
				if (is_array($listId)){
					$this->addOrRemoveFromAcymailingList($userid, [], $listId, $subscriptionId);
	...
	public function checkUserSubscription($user, $acymailingListIds)
	{
		...
		$acymailingApp = PPHelperApp::getAvailableApps('acymailing');
