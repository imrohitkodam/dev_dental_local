...
class PPAppK2item extends PPApp
{
	...
	public function onPayplansAccessCheck(PPUser $user)
	{
		...
		if ($view == 'itemlist' && ($layout == 'category' || $task == 'category')) {
			...
			// Get the current url which is accessing
			$currentUri = PPR::getCurrentURI();

			$apprestriction = (is_array($apprestriction))? $apprestriction : [$apprestriction];
			...
			if (array_intersect($categories, $apprestriction)) {
				...
				if ($applyAll == false) {
					...
					// Set current url in session , so after login user will redirect to correct k2 item
					$this->setRedirectUrl($currentUri);
	...
	public function onK2PrepareContent($item, $params, $limitstart)
	{
		...
		if (!$accessibleItems || !$allowed) {
			...
			$url = PPR::_('index.php?option=com_payplans&view=plan');
	...
	/**
	 * Set Redirect url on session so , after login user will redirectk2 item which is accessing
	 *
	 * @since   4.0.0
	 * @access  public
	 */
	...
	{
		// Set the session for redirect url
		$session = JFactory::getSession();
		$session->set('k2_item_redirect_url', $redirectUrl);
...
class PayplansAppK2itemFormatter extends PayplansAppFormatter
{
	...
	public function getVarFormatter()
	{
		$rules = [
			...
			'app_params' => [
				'formatter'=> 'PayplansAppK2itemFormatter',
