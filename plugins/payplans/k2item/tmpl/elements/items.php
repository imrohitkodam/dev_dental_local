...
require_once(JPATH_ROOT . '/plugins/payplans/k2item/app/lib.php');
...
?>
<?php if ($lib->exists()) { ?>
	<?php $items = $lib->getItems(); ?>
	<select name="<?php echo $name;?>[]" class="pp-autocomplete o-form-control" multiple="multiple" <?php echo $attributes;?>>
		<?php foreach ($items as $item) { ?>
		<option value="<?php echo $item->item_id;?>" <?php echo in_array($item->item_id, $value) ? 'selected="selected"' : '';?>><?php echo JText::_($item->title);?></option>
		<?php } ?>
	</select>
<?php } else { ?>
