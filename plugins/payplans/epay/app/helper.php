...
class PPHelperEpay extends PPHelperPayment
{
	...
	* @access public
	*/
	public function processRecurringPayment(PPPayment $payment, $invoiceCount = 0, $failureCount = 0)
	{
		$invoice = $payment->getInvoice();
		...
		if (!$result->authorizeResult) {
			PPLog::log(PPLogger::LEVEL_ERROR, JText::_('COM_PAYPLANS_APP_EPAY_ERROR_RECURRING_PAYMENT'), $payment, (array) $result, 'PayplansPaymentFormatter');
