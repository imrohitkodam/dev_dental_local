...
class PPAppSkrill extends PPAppPayment
{
	...
	public function onPayplansPaymentNotify(PPPayment $payment, $data, $controller)
	{
		...
		if ($helper->isTransactionFailed($status)) {
			...
			PPLog::log(PPLogger::LEVEL_ERROR, 'COM_PAYPLANS_APP_MONEYBOOKERS_LOGGER_ERROR_IN_PAYMENT', $payment, $response, 'PayplansPaymentFormatter', '', true);
		...
		if ($helper->isPaymentNotification($status)) {
			...
			if ($merchantEmail !== PPJString::strtolower($data['pay_to_email'])) {
