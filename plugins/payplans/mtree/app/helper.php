...
class PPHelperMtree extends PPHelperStandardApp
{
	...
	public function getChildCategory($categories)
	{
		...
		$query = ' SELECT `cat_id` as cat_id FROM `#__mt_cats` WHERE `cat_parent` IN ( '.implode(',', $categories).' ) AND `cat_published`=1';
	...
	{
		$db = PP::db();
		$query =  ' SELECT `link_id` as link_id FROM `#__mt_links` WHERE `user_id`= '.$userId;
	...
	{
		$db = PP::db();
		$query =  ' SELECT count(*) FROM `#__mt_cl` WHERE `cat_id` IN ('.implode(',', $categories).") AND `link_id` IN ('".implode("', '", $items)."')";
