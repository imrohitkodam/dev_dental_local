...
class plgPayPlansMtree extends PPPlugins
{
	...
	public function onPayplansAccessCheck(PPUser $user)
	{
		...
		if ($view != 'addlisting' && !(in_array($task, ['addlisting','savelisting']))) {
			return;
		}
		...
		foreach ($apps as $app) {
			...
			$count = (int) $app->getAppParam('publish_listings_on_active', 0);

			if ($restrictionType == 'any') {

				// restrict user if not subscribed to any plan
				if (!$planIds) {
				...
				// check if user listing submission limit reached
			...
			$categories = $app->getAppParam('restrict_specific', []);
			...
			if (array_intersect($categories, $currentCategoryId)) {
				...
				// if user not subscribed to plan then restrict to view listing
				...
				$totalPostsInCategories = (int) $helper->getTotalUserItemsInCategory($posts, $categories);

				// check if user listing submission limit reached
				if ($totalPostsInCategories >= $count) {
