...
class PPAppebanx extends PPAppPayment
{
	...
	public function onPayplansPaymentForm(PPPayment $payment, $data = null)
	{
		...
		if ($invoice->isRecurring()) {
			$formUrl = $helper->getFormUrl($payment->getKey());
			
			$this->set('formUrl', $formUrl);
			$this->set('payment', $payment);
			
			return $this->display('form_recurring');
		}

		$user = $payment->getBuyer(true);
		$params = $this->getAppParams();

		$countryCode = $helper->getCountry($user);
	...
	public function onPayplansPaymentAfter(PPPayment $payment, &$action, &$data, $controller)
	{
		...
		if (isset($data['initiateRecurring'])) {

			$helper = $this->getHelper();
			$tokenUrl = $helper->getTokenUrl();

			$user = $payment->getBuyer(true);
			$countryCode = $helper->getCountry($user);
			$cardExpiry = $data['ebanx_exp_month']."/".$data['ebanx_exp_year'];

			$payload = array(
							'request_body' => array(
								'integration_key' => $helper->getIntegrationKey(),
								'payment_type_code' => $data['payment_type_code'],
								'country' => $countryCode,
								'card' => array (
											'card_number' => $data['card_number'],
											'card_name' => $data['card_name'],
											'card_due_date' => $cardExpiry,
											'card_cvv' => $data['card_cvv']
								),
							)
						);
			
			$response = $helper->connect($tokenUrl, $payload, true);
			...
			$gatewayParams->set('ebanx_token', $response['token']);
			$gatewayParams->set('payment_type_code', $response['payment_type_code']);
