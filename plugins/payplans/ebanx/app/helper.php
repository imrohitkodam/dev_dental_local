...
class PPHelperEbanx extends PPHelperPayment
{
	 ...
	 * @access	public
	 */
	public function connect($url, $payload, $token = false)
	{
		// Ensure that integration_key is always sent to Ebanx
		if (!isset($payload['integration_key']) && !$token) {
	...
	/**
	 * Retrieves the Integration Key for Ebanx
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	...
	{
		static $integrationKey = null;

		if (is_null($integrationKey)) {
			$integrationKey = $this->params->get('integration_key', '');
		}
	...
	/**
	 * Retrieves the Token API url for Ebanx
	 *
	 * @since	4.0.0
	 * @access	public
	 */
	...
	{
		$cuountryId = $user->getCountry();
		$countryCode = 'BR';
		if ($cuountryId) {
			
			$table = PP::table('Country');
			$table->load(array('country_id' => $cuountryId));

			$countryCode = $table->isocode2;
		}
