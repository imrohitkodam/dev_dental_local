...
{	
	...
	{		
		...
		if ($data->type === 'return.created') {
			$paymentData = $data->data->original->tags;
	...
	public function onPayplansPaymentNotify(PPPayment $payment, $data, $controller)
	{
		...
		if ($paymentData->type === 'RETURN') {
			$gatewayTransactionId = isset($paymentData->original->order) ? $paymentData->original->order : '';
		}
		...
		if ($paymentData->completed) {
			...
			if ($paymentData->type === 'RETURN') {
				...
				$message = 'COM_PP_PAYMENT_APP_FASTSPRING_TRANSACTION_REFUNDED';
			} else {
				$amount = $invoice->getTotal();
