...
{
	private $acypluginsHelper;
    private $multicatsOn;
    private $newMulticats;
    public $newslanguage;
    public $currentcatid;
    public $readmore;
    public $tags;
    ...
    public function __construct(&$subject, $config)
    {
        ...
        $this->multicatsOn = false;
        if (defined('ACYMAILING_CMSV') && version_compare(ACYMAILING_CMSV, '3.4.0', '>=')) {
            if (JComponentHelper::isInstalled('com_multicats') && JComponentHelper::isEnabled('com_multicats')) {
                $this->multicatsOn = true;
            }
        } else {
            try {
                if (file_exists(JPATH_ADMINISTRATOR . DS . 'com_multicats') && JComponentHelper::isEnabled('com_multicats')) {
                    $this->multicatsOn = true;
                }
            } catch (Exception $exc) {
                $this->multicatsOn = false;
    ...
    function onAcymailingtagcontent_show()
    {
        return $this->acymailingtagcontent_show();
    }
    ...
    function acymailingtagcontent_show()
    {
        ...
        if ($pictureHelper->available()) {
            $picts[] = acymailing_selectOption("resized", acymailing_translation('RESIZED'));
        }
        ...
        $searchFields = ['a.id', 'a.title', 'a.alias', 'a.created_by', 'b.name', 'b.username'];
			...
			<table width="100%" class="acymailing_table">
				...
				<tr id="format" class="acyplugformat">
					...
					<td valign="top"><?php echo acymailing_radio($picts, 'pict', 'size="1" onclick="updateTag();"', 'value', 'text', $pageInfo->pict); ?>
						<span id="pictsize" <?php if ($pageInfo->pict != 'resized') {
                            echo 'style="display:none;"';
                        } ?>><br /><?php echo acymailing_translation('CAPTCHA_WIDTH') ?>
		...
		<div class="onelineblockoptions">
			<table class="acymailing_table_options">
				<tr>
					...
					<td nowrap="nowrap">
                        <?php echo acymailing_select(
                            $categoriesValues,
                            'filter_cat',
                            'class="inputbox" size="1" onchange="document.adminForm.submit( );"',
                            'value',
                            'text',
                            (int) $pageInfo->filter_cat
                        ); ?>
					</td>
				</tr>
			</table>
			...
			<table class="acymailing_table" cellpadding="1" width="100%">
				<thead>
					<tr>
						...
						<th class="title">
                            <?php echo acymailing_gridSort(
                                acymailing_translation(ACYMAILING_J16 ? 'COM_CONTENT_PUBLISHED_DATE' : 'START PUBLISHING'),
                                'a.publish_up',
		...
		<div class="onelineblockoptions">
			<table width="100%" class="acymailing_table">
				<tr>
					...
					<td colspan="2">
                        <?php echo acymailing_radio(
                            $contenttype,
                            'contenttypeauto',
                            'size="1" onclick="updateAutoTag();"',
                            'value',
                            'text',
                            $this->params->get('default_type', 'intro')
                        ); ?>
					</td>
					<td id="languagesauto">
                        <?php $jflanguages = acymailing_get('type.jflanguages');
				...
				<tr id="formatauto" class="acyplugformat">
					...
					<td valign="top"><?php echo acymailing_translation('DISPLAY_PICTURES'); ?></td>
					<td valign="top"><?php echo acymailing_radio(
                            $picts,
                            'pictauto',
                            'size="1" onclick="updateAutoTag();"',
                            'value',
                            'text',
                            $this->params->get('default_pict', '1')
                        ); ?>
						<span id="pictsizeauto" <?php if ($this->params->get(
                                'default_pict',
                                '1'
                            ) != 'resized') {
                            echo 'style="display:none;"';
							...
							x <?php echo acymailing_translation('CAPTCHA_HEIGHT') ?>
							<input name="pictheightauto"
								   type="text"
								   onchange="updateAutoTag();"
								   value="<?php echo $this->params->get('maxheight', '150'); ?>"
								   style="width:30px;" />
						</span>
					</td>
				</tr>
				<tr>
					...
					<td>
                        <?php echo acymailing_radio(
                            ...
                            'value',
                            'text',
                            $this->params->get('default_titlelink', 'link')
                        ); ?>
					</td>
					<td>
                        <?php echo acymailing_translation('AUTHOR_NAME'); ?>
					...
					<td>
                        <?php echo acymailing_radio(
                            ...
                            'value',
                            'text',
                            (string) $this->params->get('default_author', '0')
                        ); ?>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
                        ...
                        if (ACYMAILING_J16) {
                            $values['publish_up'] = 'COM_CONTENT_PUBLISHED_DATE';
                        }
            ...
            <?php
            if (ACYMAILING_J37 && !empty($customFields)) {
                echo '<div class="onelineblockoptions">
					<span class="acyblocktitle">' . acymailing_translation('EXTRA_FIELDS') . '</span>
					<table class="acymailing_table" cellpadding="1">';
                foreach ($groups as $oneGroup) {
				...
				<tbody>
                    ...
                    foreach ($categories as $row) {
                        ...
                        if (!ACYMAILING_J16 && $currentSection != $row->section) {
                            ?>
							<tr id="content_sec<?php echo $row->secid ?>_cat0"
								class="<?php echo "row$k"; ?>"
								onclick="applyAutoContent(<?php echo $row->secid ?>,0,'<?php echo "row$k" ?>');"
                        ...
                        if (!ACYMAILING_J16) {
                            ?>
							<tr id="content_sec<?php echo $row->secid ?>_cat<?php echo $row->catid ?>"
								class="<?php echo "row$k"; ?>"
								onclick="applyAutoContent(<?php echo $row->secid ?>,<?php echo $row->catid ?>,'<?php echo "row$k" ?>');"
								style="cursor:pointer;">
								<td class="acytdcheckbox"></td>
								<td>
								</td>
							...
							<tr id="content_cat<?php echo $row->id ?>"
								class="<?php echo "row$k"; ?>"
								onclick="applyAutoContent(<?php echo $row->id ?>,'<?php echo "row$k" ?>');"
								style="cursor:pointer;">
								<td class="acytdcheckbox"></td>
								<td>
                                    <?php
    ...
    private function _replaceArticles(&$email)
    {
        ...
        $this->currentcatid = - 1;
        $this->readmore = empty($email->template->readmore) ? acymailing_translation('JOOMEXT_READ_MORE')
            : '<img class="readmorepict" src="' . ACYMAILING_LIVE . $email->template->readmore . '" alt="' . acymailing_translation('JOOMEXT_READ_MORE', true) . '" />';
    ...
    private function _replaceContent(&$tag)
    {
        ...
        if (!ACYMAILING_J16) {
            $query = 'SELECT a.*, b.name as authorname, c.alias AS catalias, c.title AS cattitle, c.image AS catpict, s.alias as secalias, s.title AS sectitle FROM ' . acymailing_table(
                    'content',
                    false
                ) . ' AS a ';
        ...
        if (empty($article)) {
            if (acymailing_isAdmin()) {
                acymailing_enqueueMessage('The article "' . $tag->id . '" could not be loaded', 'notice');
            }
        ...
        if ($this->params->get('integration') == 'jreviews' && !empty($article->images)) {
            ...
            if (!empty($firstpict[0])) {
                $picturePath = file_exists(ACYMAILING_ROOT . 'images' . DS . 'stories' . DS . str_replace('/',
                                                                                                          DS,
                                                                                                          $firstpict[0])) ? ACYMAILING_LIVE . 'images/stories/' . $firstpict[0]
                    : ACYMAILING_LIVE . 'images/' . $firstpict[0];
        ...
        if (!empty($tag->lang)) {
            $link .= (strpos($link, '?') ? '&' : '?') . 'lang=' . substr($tag->lang, 0, strpos($tag->lang, ACYMAILING_J16 ? '-' : ','));
        }
        ...
        if (empty($tag->lang) && !empty($article->language) && $article->language != '*') {
            if (!isset($this->langcodes[$article->language])) {
                $this->langcodes[$article->language] = acymailing_loadResult(
                    'SELECT sef FROM #__languages WHERE lang_code = ' . acymailing_escapeDB($article->language) . ' ORDER BY `published` DESC LIMIT 1'
                );
        ...
        if ($tag->type != "title") {
            ...
            if (ACYMAILING_J16 && !empty($article->images) && !empty($tag->pict) && empty($tag->nomainimage)) {
                ...
                if (!empty($images->$pictVar)) {
                    ...
                    if (!empty($tag->link) && empty($tag->nopictlink)) {
                        $picthtml .= '<a target="_blank" href="' . $link . '" style="text-decoration:none" >';
                    }
                    $alt = '';
                    $altVar = $pictVar . '_alt';
            ...
            if (!empty($tag->custom)) {
                ...
                $articleCFValues = acymailing_loadObjectList(
                    'SELECT fv.value, f.id, f.fieldparams, f.params, f.type, f.label, f.name, f.default_value 
																FROM #__fields AS f 
                ...
                foreach ($fields as $oneField) {
                    if (!empty($oneField['field']->fieldparams)) {
                        $oneField['field']->fieldparams = json_decode($oneField['field']->fieldparams, true);
                    }
                    $oneField['field']->params = json_decode($oneField['field']->params, true);

                    if ($oneField['values'][0] === null) {
                        if (($oneField['field']->type == 'user' && empty($oneField['field']->default_value)) || ($oneField['field']->type != 'user' && strlen(
                                                                                                                                                           $oneField['field']->default_value
                                                                                                                                                       ) == 0)) {
                            continue;
                        }
                        $oneField['values'] = [$oneField['field']->default_value];
                    }
                    ...
                    foreach ($oneField['values'] as &$oneFieldVal) {
                        switch ($oneField['field']->type) {
                            ...
                            case 'imagelist':
                                ...
                                if (strlen($oneField['field']->fieldparams['directory']) > 1) {
                                    $oneFieldVal = '/' . $oneFieldVal;
                                } else {
                                    $oneField['field']->fieldparams['directory'] = '';
                                }
                            ...
                            case 'calendar':
                                ...
                                if (!empty($tag->{$oneField['field']->name . 'format'})) {
                                    $format = acymailing_translation($tag->{$oneField['field']->name . 'format'});
                                }
                                $oneFieldVal = JHTML::_('date', $oneFieldVal, $format);
                                break;
                        }
                    }
                    ...
                    if ($oneField['field']->params['showlabel'] == '1') {
                        ...
                        if ($oneField['field']->type == 'imagelist') {
                            $label .= '<br/>';
                        }
            ...
            if (file_exists(JPATH_SITE . DS . 'plugins' . DS . 'attachments') && empty($tag->noattach)) {
                ...
                if (!empty($attachments)) {
                    ...
                    foreach ($attachments as $oneAttachment) {
                        $afterArticle .= '<br /><a target="_blank" href="' . $oneAttachment->url . '">' . (empty($oneAttachment->display_name) ? $oneAttachment->filename
                                : $oneAttachment->display_name) . '</a>';
                    }
                }
            }
            ...
            if (!empty($tag->share)) {
                ...
                foreach ($shareOpt as $socialNetwork) {
                    ...
                    if ($socialNetwork == 'facebook') {
                        ...
                        $picSrc = (file_exists(ACYMAILING_MEDIA . 'plugins' . DS . 'facebook.png') ? 'media/com_acymailing/plugins/facebook.png'
                            : 'media/com_acymailing/images/facebookshare.png');
                        $altText = 'Facebook';
                    } elseif ($socialNetwork == 'twitter') {
                        $linkShare = 'https://twitter.com/share?url=' . urlencode($nonsefLink) . '&text=' . urlencode($article->title);
                        $picSrc = (file_exists(ACYMAILING_MEDIA . 'plugins' . DS . 'twitter.png') ? 'media/com_acymailing/plugins/twitter.png'
                            : 'media/com_acymailing/images/twittershare.png');
                        $altText = 'Twitter';
                        ...
                        $picSrc = (file_exists(ACYMAILING_MEDIA . 'plugins' . DS . 'linkedin.png') ? 'media/com_acymailing/plugins/linkedin.png'
                            : 'media/com_acymailing/images/linkedin.png');
                        $altText = 'LinkedIn';
                        ...
                        $knownNetwork = false;
                        acymailing_display('Network not found: ' . $socialNetwork . '. Available networks are facebook, twitter, linkedin and mailto.', 'warning');
                    }
                    if ($knownNetwork) {
                        array_push(
                            $links,
                            '<a target="_blank" href="' . $linkShare . '" title="' . acymailing_translation_sprintf(
                                'SOCIAL_SHARE',
                                $altText
                            ) . '"><img alt="' . $altText . '" src="' . $picSrc . '" /></a>'
                        );
                    }
                }
        ...
        if (!empty($tag->jtags) && version_compare(JVERSION, '3.1.0', '>=')) {
            $tags = acymailing_loadObjectList(
                'SELECT t.id, t.alias, t.title FROM #__tags AS t JOIN #__contentitem_tag_map AS m ON t.id = m.tag_id WHERE t.published = 1 AND m.type_alias = "com_content.article" AND m.content_item_id = ' . intval(
                    $tag->id
                )
            );
            if (!empty($tags)) {
        ...
        if (!(empty($tag->cattitle) && empty($tag->catpict)) && ((!strpos($article->catid, ',') && $this->currentcatid != $article->catid) || (strpos(
                                                                                                                                                   $article->catid,
                                                                                                                                                   ','
                                                                                                                                               ) && !in_array($this->currentcatid,
                                                                                                                                                              explode(',',
                                                                                                                                                                      $article->catid))))) {
            ...
            if (!empty($tag->catpict) && !empty($article->catpict)) {
                ...
                if (!empty($tag->cattitlelink)) {
                    $resultTitle = '<a target="_blank" href="index.php?option=com_content&view=category&id=' . $this->currentcatid . '">' . $resultTitle . '</a>';
                }
            } else {
                if (!empty($tag->cattitlelink)) {
                    $resultTitle = '<a target="_blank" href="index.php?option=com_content&view=category&id=' . $this->currentcatid . '">' . $resultTitle . '</a>';
    ...
    function onAcymailing_generateautonews(&$email)
    {
        return $this->acymailing_generateautonews($email);
    }
    ...
    function acymailing_generateautonews(&$email)
    {
        ...
        foreach ($tags as $oneTag => $parameter) {
            ...
            if ($this->multicatsOn && $this->newMulticats && (!empty($selectedArea) || !empty($parameter->excludedcats))) {
                $query .= ' JOIN `#__multicats_content_catid` as mcc ON a.id = mcc.item_id ';
            }
            ...
            if (!empty($parameter->filter) && !empty($email->params['lastgenerateddate'])) {
                $condition = '(`publish_up` > \'' . date('Y-m-d H:i:s', $email->params['lastgenerateddate'] - date('Z')) . '\' AND `publish_up` < \'' . date(
                        'Y-m-d H:i:s',
                        $time - date('Z')
                    ) . '\')';
                if ($parameter->filter === 'modify') {
                    $modify = '(`modified` > \'' . date('Y-m-d H:i:s', $email->params['lastgenerateddate'] - date('Z')) . '\' AND `modified` < \'' . date(
                            'Y-m-d H:i:s',
                            $time - date('Z')
                        ) . '\')';
                    if (!empty($parameter->maxpublished)) {
                        $modify = '(' . $modify . ' AND `publish_up` > \'' . date(
                                'Y-m-d H:i:s',
                                time() - date('Z') - ((int) $parameter->maxpublished * 60 * 60 * 24)
                            ) . '\')';
                    }
                    $condition .= ' OR ' . $modify;
                }
            ...
            $where[] = '`publish_down` > \'' . date('Y-m-d H:i:s', $time - date('Z')) . '\' OR `publish_down` ' . (ACYMAILING_J40 ? 'IS NULL' : '= 0');
            ...
            if (!empty($parameter->min) && count($allArticles) < $parameter->min) {
                $return->status = false;
                $return->message = 'Not enough articles for the tag ' . $oneTag . ' : ' . count($allArticles) . ' / ' . $parameter->min . ' between ' . acymailing_getDate(
                        $email->params['lastgenerateddate']
                    ) . ' and ' . acymailing_getDate($time);
            }
            ...
            if (!empty($allArticles)) {
                if (file_exists(ACYMAILING_MEDIA . 'plugins' . DS . 'autocontent.php')) {
                    ...
                    foreach ($allArticles as $oneArticleId) {
                        ...
                        if (!empty($parameter->custom)) {
                            $args[] = 'custom:' . $parameter->custom;
                        }
                        ...
                        if (!empty($parameter->nomainimage)) {
                            $args[] = 'nomainimage';
                        }
