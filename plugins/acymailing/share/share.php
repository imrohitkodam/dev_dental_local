...
class plgAcymailingShare extends JPlugin
{
    ...
    function onAcymailingtagshare_show(){
		return $this->acymailingtagshare_show();
	}
	...
	function acymailingtagshare_show()
    {
        ...
        $uploadedFile = acymailing_getVar('array', 'socialfile', [], 'files');
        ...
        foreach ($results as $var => $allresults) {
            foreach ($allresults[0] as $numres => $tagname) {
                ...
                if ($tag->network == 'facebook') {
                    $link = 'https://www.facebook.com/sharer.php?u='.urlencode($archiveLink).'&t='.urlencode($email->subject);
                    ...
                    $link = 'https://twitter.com/share?url='.urlencode($archiveLink).'&text='.urlencode($email->subject);
                    ...
                    $link = 'https://www.linkedin.com/shareArticle?mini=true&url='.urlencode($archiveLink).'&title='.urlencode($email->subject);
    ...
    {
        $variables = ['subject', 'body', 'altbody'];
