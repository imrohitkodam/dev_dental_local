...
class plgAcymailingTaguser extends JPlugin
{
    ...
    function onAcymailingtaguser_show(){
		return $this->acymailingtaguser_show();
	}
	...
	function acymailing_replaceusertags(&$email, &$user, $send = true)
    {
        ...
        if (empty($this->customFields) && ACYMAILING_J37) {
        ...
        if (strpos($filter['map'], 'customfield_') !== false) {
            ...
            if (in_array($filter['map'], array('registerDate', 'lastvisitDate'))) {
                ...
                if (is_numeric($filter['value'])) {
                    $filter['value'] = date('Y-m-d H:i:s', $filter['value']);
                }
                $type = 'datetime';
            }
