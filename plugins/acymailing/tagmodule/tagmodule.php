...
class plgAcymailingTagmodule extends JPlugin{
	...
	function onAcymailingtagmodule_show(){
		return $this->acymailingtagmodule_show();
	}
	...
	function acymailing_replacetags(&$email, $send = true){
		...
		@ini_set('user_agent', "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/109.0");
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				$loc = ACYMAILING_LIVE.'index.php?option=com_acymailing&tmpl=component'.$mailid.'&ctrl=moduleloader&acymodid='.$allresults[1][$i].'&seckey='.urlencode($config->get('security_key')).'&time='.time().$lang.$item;
				if(function_exists('curl_init') AND ($this->params->get('getmethod') == 'curl' OR !ini_get('allow_url_fopen'))){
					$ch = curl_init();

					$loc = ACYMAILING_LIVE.'index.php';
					$urlPost = 'option=com_acymailing&tmpl=component'.$mailid.'&ctrl=moduleloader&acymodid='.$allresults[1][$i].'&seckey='.urlencode($config->get('security_key')).'&time='.time().$lang.$item;
	...
	function onTestPlugin(){
		...
		if($this->params->get('getmethod') == 'curl'){
			...
			$loc = ACYMAILING_LIVE.'index.php';
			$urlPost = 'option=com_acymailing&tmpl=component&ctrl=moduleloader&seckey='.$config->get('security_key').'&time='.time().$item;
			...
			if($result === false){
				...
				acymailing_display('Result returned by the curl call: '.$result, 'info');
			}
			curl_close($ch);
		}else{
