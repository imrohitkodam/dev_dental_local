...
class plgAcymailingTagsubscription extends JPlugin
{
    ...
    function onAcymailingtagsubscription_show(){
		return $this->acymailingtagsubscription_show();
	}
	...
	function onAcymailing_replaceusertags(&$email, &$user, $send = true){
		return $this->acymailing_replaceusertags($email, $user, $send);
	}
	...
	function acymailing_replaceusertags(&$email, &$user, $send = true)
	{
        ...
        if (!empty($user->subid) && !empty($this->unsubscribeLink[$email->mailid]) && empty($this->addedListUnsubscribe[$email->mailid][$user->subid]) && $this->params->get('listunsubscribe', 0) && method_exists($email, 'addCustomHeader')) {
            ...
            $this->addedListUnsubscribe[$email->mailid][$user->subid] = true;
    ...
    private function _getattachedlistid($email, $subid)
    {
        ...
        if (in_array($type, array('news', 'autonews', 'action'))) {
            if (!empty($subid)) {
    ...
    private function _replacesubscriptiontags(&$email)
    {
        ...
        $tags = array();
		$this->addedListUnsubscribe[$email->mailid] = [];
    ...
    function replaceSubscriptionTag(&$allresults, $i, &$email)
    {
        ...
        $this->unsubscribeLink[$email->mailid] = true;
