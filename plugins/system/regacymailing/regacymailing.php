...
class plgSystemRegacymailing extends JPlugin{
	...
	function initAcy(){
        ...
        $acyHelper = rtrim(JPATH_ADMINISTRATOR, $ds).$ds.'components'.$ds.'com_acymailing'.$ds.'helpers'.$ds.'helper.php';
		if(!file_exists($acyHelper) || !include_once($acyHelper)) return false;
	...
	function onAfterRender(){
		...
		if(version_compare($jversion, '4.0.0', '>=')) {
			$this->components['com_users'] = array('view' => array('registration', 'profile', 'user'), 'edittasks' => array('profile', 'user'), 'lengthafter' => ACYMAILING_J40 ? 400 : 200, 'email' => array('jform[email2]', 'jform[email1]'), 'password' => 'jform[password2]', 'displayBackend' => true, 'displayLoggedin' => true, 'checkLayout' => array('profile' => 'edit'));
        }elseif(version_compare($jversion, '1.6.0', '>=')){
			$this->components['com_users'] = array('view' => array('registration', 'profile', 'user'), 'edittasks' => array('profile', 'user'), 'lengthafter' => ACYMAILING_J40 ? 400 : 200, 'email' => array('jform[email2]', 'jform[email]'), 'password' => 'jform[password2]', 'displayBackend' => true, 'displayLoggedin' => true, 'checkLayout' => array('profile' => 'edit'));
		}else{
