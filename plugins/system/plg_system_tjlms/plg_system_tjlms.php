 ...
 * @since  1.0.0
 */
class PlgSystemplg_System_Tjlms extends JPlugin
{
	...
	public function onAfterRoute()
	{
		...
		elseif ($app->isClient('site'))
		{
			if ($option == 'com_tmt')
			{
				...
				$quizLangs = \JLanguageHelper::parseIniFile(JPATH_SITE . '/language/en-GB/' . $langFile);
	...
	public function onAfterCourseEnrol($actorId, $state, $courseId, $enrolledBy, $notify_user = 1, $courseStatus = '', $timestart = '', $timeend = '')
	{
		if ($this->comtjlmsHelperObj)
		{
			...
			$this->addCourseTrackEntry($courseId, $actorId, 0, $courseStatus, $timestart, $timeend);
			...
			foreach ($lessons as $lesson)
			{
				...
				$dispatcher->trigger('courseEnrolPostProcessing', array(
												$plug_name,
												$courseId,
												$actorId,
												$state,
	...
	public function onAfterCourseCompletion($actorId, $courseId, $lessonId = 0)
	{
		if ($this->comtjlmsHelperObj)
		{
			...
			if (empty($ifCourseAlreadyCompleted))
			{
				...
				$this->addCourseTrackEntry($courseId, $actorId, $lessonId);
			}
	...
	public function OnAfterLessonCreation($courseId, $lessonId, $lesson)
	{
		...
		$this->addCourseTrackEntry($courseId);
	}
	...
	public function OnAfterLessonFormatUploaded($lessonId, $mediaId)
	{
		...
		// Update course track for number of lessons and status
				$this->addCourseTrackEntry($obj->course_id);
		...
		// Trigger all "invitex" plugins method that renders the button/image
		$dispatcher->trigger('onAfterLessonCreationTrigger', array(
										$plug_type,
										$obj->course_id,
										$lessonId,
										$obj
	...
	public function onAfterLessonCompletion($lessonId, $attempt, $actorId)
	{
		...
		$this->addCourseTrackEntry($obj->course_id, $actorId, $lessonId);
	...
	{
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin('tjtextmedia', 'joomlacontent');
		$dispatcher->trigger('isLessonArticle', array($cid, &$lesson_articles));
		...
		if (!empty($lesson_articles))
		{
			...
			{
				$Content = JTable::getInstance('Content', 'JTable', array('dbo', $db));
	...
	public function onBeforeLessonDelete($courseId, $lessonIds, $mediaIds)
	{
		foreach ($lessonIds as $lessonId)
		{
			...
			$dispatcher->trigger('onBeforeLessonDeletion', array($plug_type, $courseId, $lessonId, $obj));
		}
	...
	{
		$enrollTable = JTable::getInstance('Enrolledusers', 'TjlmsTable', array('dbo', $this->db));
