...
else
{
	...
	//		$appIds = array_filter($appIds);
							...
							if ($payplan_details)
							{
								...
								foreach ($payplan_details AS $plan_details)
								{
									if ($extra_added_plan)
									{
										...
										// if ($plan_details->invoice_status == 402)
										// {
											$planData[] = array(
												'userid' => $userid,
		 ...
		 * @since   1.6
		 */
		public function updateSyncStatus($sync_id, $current_date, $oldCourses)
		{
			try
			{
				if ($sync_id)
		...
		//				$db->quoteName('old_shika_course_plan') . ' = ' . $db->quote($oldCourses)
					);
