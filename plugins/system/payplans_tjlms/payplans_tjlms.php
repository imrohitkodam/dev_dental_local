...
else
{
	...
	class PlgSystemPayplans_Tjlms extends XiPlugin
		...
		public function onAjaxsyncData()
		{
			...
			$response = array('appData' => $mainArray, 'remove_plans' => $remove_plan ,'count' => count($mainArray));
		...
		public function renderPayplansDetails($planId, $userid)
		{
			if ($planId)
			{
				...
				$selectList  .= 'i.invoice_id, s.subscription_id, s.plan_id, i.status AS invoice_status';
		...
		public function onAjaxCreateOrder()
		{
			...
			foreach ($dataArray as $data)
			{
				...
				foreach ($coursePlans as $plan)
				{
					...
					$query->select("a.*");
					$query->from('`#__tjlms_payplanApp` as a');
					$query->leftJoin('`#__tjlms_orders` AS o ON a.`tjlms_order_id` = o.`id`');
					$query->where('a.pp_order_id = ' . (int) $xrefplArray->pp_order_id);
					$query->where('a.course_id = ' . (int) $course_Plans[0]->course_id);
					$query->where('a.user_id = ' . (int) $userid);
					$query->where('a.tjlms_order_id != 0');

					$db->setQuery($query);

					$CourseOrderId = $db->loadObjectList();

					$isNew   = 0;
					$xrefID  = $CourseOrderId[0]->id;
					$orderId = 0;

					$newCourseStatus = "C";

					// Check shika order is created or not
					if (empty($CourseOrderId[0]->tjlms_order_id))
					{
						$isNew                          = 1;
		...
		public function deleteShikaOrder($coursePlanIds, $userid)
		{
			...
			foreach ($coursePlanIds as $deletePlanIds)
			{
				...
				$pp_subscription_id = $query->select("tjlms_order_id")
								->from('`#__tjlms_payplanApp` as a')
								->where('a.course_plan_id = ' . (int) $deletePlanIds)
