...
class TemporarySuperUsers extends DisableObsoleteAdmins
{
	 ...
	 * through Joomla's com_users.
	 *
	 * **MESSAGE BODY INTENTIONALLY LEFT BLANK. DO NOT REMOVE.**
	 *
    ...
    /**
     * If the user is deleting a Joomla user, we must delete the record inside the Temp Admin table as well
     *
     * @param array $user
     * @param bool $success
     * @param string $msg
    ...
    {
        // Deleting the user was blocked, no need to do anything
        if (!$success)
        {
            return;
        }

        // Delete any user that is linked inside the Temporary Admin table. Even if it's not there, that's not a big deal
        $db    = $this->db;
        ...
        {
            // Do not die if we got a database error
        }
    }
