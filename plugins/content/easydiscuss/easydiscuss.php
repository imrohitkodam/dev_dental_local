...
class plgContentEasyDiscuss extends JPlugin
{
	...
	public function addResponses(&$article, &$post)
	{
		...
		// if the limitstart still showing empty, we need to set this default to 0
		if (!$limitstart) {
			$limitstart = 0;
		}
	...
	/**
	 * Returns the frontend SEF URL to a specific article that is compatible with the Joomla 3.x and Joomla 4
	 *
	 * @since	5.0.6
	 * @access	public
	 */
	public function getSiteArticleURL($url)
	{
		if (!$url) {
			return $url;
		}
		...
		if (method_exists('JRoute', 'link')) {
			$url = JRoute::link('site', $url, false, null);

		} else {
			$mode = ['mode' => JROUTER_MODE_SEF];

			$router = new JRouterSite($mode);
	...
	private function getArticleURL(&$article)
	{
		...
		if ($this->extension === 'com_content') {
			...
			if ($sefEnabled) {
				$url = $this->getSiteArticleURL($url);
			} else {
			...
			// Tidying up the url
			$permalink = str_replace('component/content/article/', '', $url);
			$permalink = $domain . '/' . ltrim($permalink , '/');
			return $permalink;
		}
		...
		if ($this->extension === 'com_easyblog') {
			...
			$permalink = EBR::getRoutedURL('index.php?option=com_easyblog&view=entry&id=' . $article->id, false, true);
			return $permalink;
		}
		...
		if ($this->extension === 'com_k2') {
			...
			$permalink = K2HelperRoute::getItemRoute($article->id . ':' . $article->alias , $article->catid . ':' . $category->alias);
			$permalink = $domain . '/' . ltrim($permalink , '/');
			return $permalink;
		}
	}
	...
	public function createDiscussion(&$article , $isNew = true)
	{
		...
		$text = isset($article->introtext) ? $article->introtext : '';
		...
		if ($this->extension == 'com_easyblog') {
			...
			// if still don't have any content here then we need to retrieve it from the intro/content column
			if (!$text) {
				$text = $article->intro . ' ' . $article->content;
			}
		}
		...
		if ($config->get('layout_editor') == 'bbcode') {
			...
			$text = $this->strip_only($text, '<script>', true);
			$text = $this->strip_only($text, '<object>', true);
			$text = $this->trimEmptySpace($text);
