 ...
 * @since  1.0.0
 */
class PlgTjtextmediaJoomlacontent extends JPlugin
{
	/**
	 ...
	 * @since 1.0.0
	 */
	public function getSubFormat_tjtextmediaContentInfo($config=array('joomlacontent'))
	{
	 ...
	 * @since 1.0.0
	 */
	public function getSubFormat_joomlacontentContentHTML($mod_id , $lesson_id, $lesson, $comp_params)
	{
		$result = array();
		...
		$layout = JPluginHelper::getLayoutPath($this->_type, $this->_name, 'creator');
		include $layout;
	 ...
	 * @since 1.0.0
	 */
	public function joomlacontentrenderPluginHTML($config)
	{
		...
		ob_start();
		$layout = $layout = JPluginHelper::getLayoutPath($this->_type, $this->_name, 'default');
		include $layout;
		$html = ob_get_contents();
		ob_end_clean();
	...
	public function additionaljoomlacontentFormatCheck($lessonId, $mediaObj)
	{
		return $mediaObj;
	}
	 ...
	 * @since 1.0.0
	 * */
	public function joomlaContent_updatedata()
	{
	 ...
	 * @since 1.0.0
	 */
	public function getjoomlacontentLaunchButtonHtml($lesson)
	{
		...
		{
			$span = '<i rel="popover" class="icon-lock" ></i><span class="lesson_attempt_action">' . JText::_("PLG_TJTEXTMEDIA_JC_LAUNCH") . '</span>';
	...
	public function articleIsPublished($contentId)
	{
		...
		{
			return $msg = JText::sprintf("PLG_TJTEXTMEDIA_JC_ACCESS_DENIED");
		}
		elseif ($result->publish_down < $now && $result->publish_down != 0)
		{
			return $msg = JText::sprintf("PLG_TJTEXTMEDIA_JC_ARTICLE_EXPIRED");
		}
