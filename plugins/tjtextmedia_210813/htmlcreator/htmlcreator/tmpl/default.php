...
.tjlms-wrapper
{
 ...
 <div id="iframecontent" style="display: none;">
        <link type="text/css" rel="stylesheet" href="<?php echo JUri::root(true). '/plugins/tjtextmedia/' . $this->_name . '/' . $this->_name .'/assets/default/content.css' ?>" />
		<link href="<?php echo JUri::root(true). '/plugins/tjtextmedia/' . $this->_name . '/' . $this->_name .'/assets/scripts/contentbuilder.css'; ?>" rel="stylesheet" type="text/css" />
		<?php echo $html; ?>
 </div>
...
$dispatcher->trigger('onContentPrepare', array ('com_tjlms.lesson', &$item, &$item, $offset));

$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_tjlms.lesson', &$item, &$item, $offset));
echo trim(implode("\n", $results));
...
<div class="<?php echo COM_TJLMS_WRAPPER_DIV; ?>">
	<div id="contentarea" class="container">
		 <iframe src="about:blank" name="tjhtmliframe" width="100%" id="tjhtmliframeid" frameborder="0"> </iframe>
	</div>
</div>
<input type="hidden" id="htmlTimeSpent" value=''>
<?php

$results = $dispatcher->trigger('onContentAfterDisplay', array('com_tjlms.lesson', &$item, &$item, $offset));
echo trim(implode("\n", $results));
