...
class SppagebuilderAddonPlan_addon extends SppagebuilderAddons 
{
	/**
	 * Tests if Payplans exists
	 *
	 * @since	4.2.0
	 * @access	public
	 */
	...
	public function render() 
	{
		...
		$plan = $this->getPlanDetails($settings->plan);
		...
		$price = $plan->getPrice();
		$price_symbol = $plan->getCurrency();
		$duration = self::getPlanDuration($plan);
		$pricing_content = (isset($settings->pricing_content) && $settings->pricing_content) ? $settings->pricing_content : $plan->getDescription();
		$button_text = (isset($settings->button_text) && $settings->button_text) ? $settings->button_text : '';
		$button_url = $plan->getSelectPermalink();
		...
		$button_attribs .= ' href="' . $plan->getSelectPermalink() . '"';
		...
		$pricesymbol = ($price_symbol) ? '<span class="pp-pricing-price-symbol">' . $price_symbol . '</span>' : '';
		...
		$output .= ($title) ? '<'.$heading_selector.' class="pp-addon-title sppb-pricing-title">' . $title . '</'.$heading_selector.'>' : '';
		if ($price_position == 'after' ) {
			$output .= '<div class="pp-pricing-price-container">';
		...
		if ($pricing_content) {
			$output .= '<div class="pp-pricing-features">';
			$output .= '<ul>';
	...
	public function css() 
	{
		...
		$pricing_content_margin_bottom_xs = (isset($settings->pricing_content_margin_bottom_xs) && $settings->pricing_content_margin_bottom_xs) ? 'margin-bottom: '. $settings->pricing_content_margin_bottom_xs .'px;' : '';
		...
		if ($price_symbol_style) {
			$css .= $addon_id . ' .pp-pricing-price-symbol {';
		...
		if ($pricing_content_margin_bottom) {
			$css .= $addon_id . ' .pp-pricing-features {';
			$css .= $pricing_content_margin_bottom;
			$css .= '}';
		}

		if ($pricing_content_style) {
			$css .= $addon_id . ' .pp-pricing-features ul li {';
			$css .= $pricing_content_style;
			$css .= '}';
		}

		if ($price_container_style) {
			$css .= $addon_id . ' .pp-pricing-price-container {';
		...
		if ($pricing_hover_style) {
			...
			$css .= $addon_id . ':hover .pp-pricing-features ul li {';
			$css .= $pricing_hover_color;
			$css .= '}';
			$css .= $addon_id . ':hover .pp-pricing-price-container {';
	...
	public static function getTemplate()
	{
		...
		// Get plan list
		$plans = PP::model('plan')->loadRecords([
			'published' => 1,
			'visible' => 1
		]);
		...
		foreach ($plans as $plan) {
			...
			// set plan details
			$plan->price = $planData->getPrice();
			$plan->price_symbol = $planData->getCurrency();
			$plan->duration = self::getPlanDuration($planData);
			$plan->button_url = $planData->getSelectPermalink();
			$result[$plan->plan_id] = $plan;
		}

		$output ='
		<#
			let plans = ' . json_encode($result) . ';

			if (data.plan == "") {
				data.title = "'. JText::_("Please Select Plan") . '";
			}

			if (plans[data.plan]) {
				data.title = plans[data.plan].title;
				data.pricing_content = plans[data.plan].description;
				data.price = plans[data.plan].price;
				data.price_symbol = plans[data.plan].price_symbol;
				data.duration = plans[data.plan].duration;
				data.button_url = plans[data.plan].button_url;
			}
			...
			if(data.price_symbol){
				price_symbol = \'<span class="pp-pricing-price-symbol">\'+data.price_symbol+\'</span>\';
			}
		...
		<style type="text/css">
			<# if( _.isObject(data.price_margin_bottom)) { #>
				#sppb-addon-{{ data.id }} .pp-pricing-price-container {
			...
			<# if( data.price_symbol_color ) { #>
				#sppb-addon-{{ data.id }} .pp-pricing-price-symbol {
			...
			<# if(_.isObject(data.pricing_content_gap)){ #>
				#sppb-addon-{{ data.id }} .pp-pricing-features ul li {
			...
			<# if(_.isObject(data.pricing_content_margin_bottom)){ #>
				#sppb-addon-{{ data.id }} .pp-pricing-features {
			...
			#>
				...
				#sppb-addon-{{ data.id }}:hover .pp-pricing-features ul li {
					color:{{data.pricing_hover_color}};
				}
				#sppb-addon-{{ data.id }}:hover .pp-pricing-price-container {
		...
		<div class="sppb-addon sppb-addon-pricing-table {{ data.alignment }} {{ data.class }}">
			<div class="sppb-pricing-box {{ data.featured }}">
				<div class="sppb-pricing-header">
					...
					<# } #>
					<# if( price_position == "after" ) { #>
						<div class="pp-pricing-price-container">
							<# if( data.price ) { #>
