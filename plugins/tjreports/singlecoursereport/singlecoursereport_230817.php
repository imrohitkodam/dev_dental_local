...
class TjreportsModelSinglecoursereport extends TjreportsModelReports
{
	...
	public function getItems()
	{
		...
		foreach ($items as &$item)
		{
			...
			{
				$lessonId 		= (int) $key;
				$score = $lesson_status = $attempts_done = $timestart = $timeend = ' - ';
				$time_spent = 0;
				...
				$item[$key]['lesson::lesson_status'] = $lesson_status;
				...
				{
					$attemptStatus = (!empty($lesson_track_id)) ? Text::_('COM_TJLMS_ATTEMPTREPORT_STATE_EXPIRED') : Text::_('COM_TJLMS_ATTEMPTREPORT_STATE_ACTIVE');
				}
				...
				if ($attempts_done)
				{
					...
					$item[$key]['lesson::attempts_done'] = '<a href="' . JRoute::_($link) . '">' . $attempts_done . '</a>';
				}
	...
	public function displayFilters()
	{
		...
		array_unshift($groups, JHtml::_('select.option', '', Text::_('COM_TJLMS_ENROLLED_USER_ACCESS')));
		...
		{
			$filterHtml = JHtml::_('select.genericlist', $reportOptions, 'filters[report_filter]',
