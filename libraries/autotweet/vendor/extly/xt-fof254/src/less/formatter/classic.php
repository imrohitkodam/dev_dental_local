 ...
 * @since    2.0
 */
class XTF0FLessFormatterClassic
{
    ...
    public function block($block)
    {
        ...
        $isSingle = !$this->disableSingle &&
            null === $block->type && 1 == count($block->lines);

        if (!empty($block->selectors)) {
            $this->indentLevel++;

            $selectorSeparator = $this->breakSelectors ? $this->selectorSeparator.$this->break.$pre : $this->selectorSeparator;
     ...
     *
     * @return bool True if the block has no lines or children
     */
