...
/**
 * This class is taken near verbatim (changes marked with **XTF0F** comment markers) from:
 *
 * lessphp v0.3.9
...
class XTF0FLess
{
    ...
    public function compileFile($fname, $outFname = null)
    {
        ...
        $type = $this->compile(file_get_contents($fname), $fname);

        $this->importDir = $oldImport;

        if (null !== $outFname) {
            /** XTF0F - BEGIN CHANGE * */
            return XTF0FPlatform::getInstance()->getIntegrationObject('filesystem')->fileWrite($outFname, $type);
            /** XTF0F - END CHANGE * */
        }
    ...
    public function cachedCompile($in, $force = false)
    {
        ...
        if (null !== $root) {
            return ['root' => $root, 'compiled' => $this->compileFile($root), 'files' => $this->allParsedFiles(), 'updated' => time()];
        } else {
    ...
    {
        /** XTF0F - BEGIN CHANGE * */
        return XTF0FPlatform::getInstance()->getIntegrationObject('filesystem')->fileExists($name);
        /** XTF0F - END CHANGE * */
    }
     ...
     * @param string          $out          Out
     * @param XTF0FLessParser $sourceParser Source parser
    ...
    protected function compileBlock($block)
    {
        switch ($block->type) {
            ...
            case 'directive':
                ...
                break;
            default:
                $this->throwError(sprintf('unknown block type: %s%s', $block->type, PHP_EOL));
        }
    }
    ...
    {
        foreach ($this->sortProps($block->props) as $type) {
            $this->compileProp($type, $block, $out);
        }
    }
            ...
            foreach ($query as $q) {
                switch ($q[0]) {
                    case 'mediaType':
                        ...
                        break;
                    case 'mediaExp':
                        $parts[] = isset($q[2]) ? sprintf('(%s: ', $q[1]).
                            $this->compileValue($this->reduce($q[2])).')' : sprintf('(%s)', $q[1]);
        ...
        if (null === $childQueries) {
            ...
            foreach ($queries as $query) {
                foreach ($childQueries as $childQuery) {
                    $out[] = array_merge($query, $childQuery);
                }
            }
        }
    ...
    protected function multiplySelectors($selectors)
    {
        ...
        if (null === $parentSelectors) {
            ...
            foreach ($selectors as &$selector) {
                $this->expandParentSelectors($selector, '');
            }
        ...
        foreach ($parentSelectors as $parentSelector) {
            foreach ($selectors as $selector) {
                $count = $this->expandParentSelectors($selector, $parentSelector);

                // Don't prepend the parent tag if & was used
                $out[] = $count > 0 ? trim($selector) : trim($parentSelector.' '.$selector);
            }
        }
    ...
    protected function patternMatch($block, $callingArgs)
    {
        ...
        if (empty($block->args)) {
            return $block->isVararg || 0 == $numCalling;
        }
    ...
    protected function findBlocks($searchIn, $path, $args, $seen = [])
    {
        ...
        if (isset($searchIn->children[$name])) {
            ...
            if (1 == count($path)) {
                ...
                foreach ($blocks as $block) {
                    $subMatches = $this->findBlocks($block, array_slice($path, 1), $args, $seen);
    ...
    protected function compileProp($prop, $block, $out)
    {
        ...
        switch ($prop[0]) {
            ...
            case 'directive':
                [, $name, $value] = $prop;
                $out->lines[] = sprintf('@%s ', $name).$this->compileValue($this->reduce($value)).';';
                break;
            case 'comment':
                $out->lines[] = $prop[1];
                break;
            ...
            case 'import_mixin':
                ...
                break;
            default:
                $this->throwError(sprintf('unknown op: %s%s', $prop[0], PHP_EOL));
        }
    }
    ...
    protected function compileValue($value)
    {
        switch ($value[0]) {
            ...
            case 'color':
                ...
                if (!empty($this->formatter->compressColors) && ($h[1] === $h[2] && $h[3] === $h[4] && $h[5] === $h[6])) {
                    $h = '#'.$h[1].$h[3].$h[5];
                }
    ...
    {
        $type = $this->assertNumber($arg);

        return ['number', floor($type), $arg[2]];
    }
    ...
    protected function lib_round($arg)
    {
        ...
        return ['number', round($type), $arg[2]];
    }
    ...
    protected function lib_darken($args)
    {
        ...
        $type = $this->toHSL($color);
        $type[3] = $this->clamp($type[3] - $delta, 100);

        return $this->toRGB($type);
    }
    ...
    protected function lib_saturate($args)
    {
        ...
        $type = $this->toHSL($color);
        $type[2] = $this->clamp($type[2] + $delta, 100);
    ...
    {
        $type = $this->toHSL($this->assertColor($color));
    ...
    {
        if ('list' != $args[0] || count($args[2]) < 3) {
            $this->throwError('mix expects (color1, color2, weight)');
        }

        [$first, $second, $weight] = $args[2];
    ...
    protected function lib_contrast($args)
    {
        ...
        [$inputColor, $darkColor, $lightColor] = $args[2];

        $inputColor = $this->assertColor($inputColor);
        $darkColor = $this->assertColor($darkColor);
        $lightColor = $this->assertColor($lightColor);
        $type = $this->toHSL($inputColor);
    ...
    protected function funcToColor($func)
    {
        ...
        if ('hsl' == $fname || 'hsla' == $fname) {
            ...
            foreach ($rawComponents as $rawComponent) {
                $rawComponent = $this->reduce($rawComponent);

                if ($i < 4) {
                    if ('number' == $rawComponent[0] && '%' == $rawComponent[2]) {
                        $components[] = 255 * ($rawComponent[1] / 100);
                    } else {
                        $components[] = (float) ($rawComponent[1]);
                    }
                } elseif (4 == $i) {
                    if ('number' == $rawComponent[0] && '%' == $rawComponent[2]) {
                        $components[] = 1.0 * ($rawComponent[1] / 100);
                    } else {
                        $components[] = (float) ($rawComponent[1]);
                    }
                } else {
    ...
    protected function injectVariables($args)
    {
        $this->pushEnv();
        /** XTF0F -- BEGIN CHANGE * */
        $xtf0FLessParser = new XTF0FLessParser($this, __METHOD__);
        /** XTF0F -- END CHANGE * */
        foreach ($args as $name => $strValue) {
            ...
            $xtf0FLessParser->count = 0;
            $xtf0FLessParser->buffer = (string) $strValue;

            if (!$xtf0FLessParser->propertyValue($value)) {
                throw new Exception(sprintf('failed to parse passed in variable %s: %s', $name, $strValue));
            }
     ...
     * @param type $name X
     *
     * @return XTF0FLessParser
     */
    protected function makeParser($name)
    {
        /** XTF0F -- BEGIN CHANGE * */
        $xtf0FLessParser = new XTF0FLessParser($this, $name);
        /** XTF0F -- END CHANGE * */
        $xtf0FLessParser->writeComments = $this->preserveComments;
     ...
     * New formatter
     *
     * @return XTF0FLessFormatterLessjs
     */
    protected function newFormatter()
    {
        /** XTF0F -- BEGIN CHANGE * */
        $className = 'XTF0FLessFormatterLessjs';
        /** XTF0F -- END CHANGE * */
        if (!empty($this->formatterName)) {
            ...
            /** XTF0F -- BEGIN CHANGE * */
            $className = 'XTF0FLessFormatter'.ucfirst($this->formatterName);
            /** XTF0F -- END CHANGE * */
        }
