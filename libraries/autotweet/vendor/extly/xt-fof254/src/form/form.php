...
/**
 * XTF0FForm is an extension to JForm which support not only edit views but also
 * browse (record list) and read (single record display) views based on XML
 * forms.
 *
 * @since    2.0
 */
class XTF0FForm extends JForm
{
    /**
     ...
     *
     * @return object XTF0FForm instance
     *
     * @since   2.0
     *
    ...
    public static function getInstance($name, $data = null, $options = [], $replace = true, $xpath = false)
    {
        ...
        if (!isset($forms[$name])) {
            ...
            if ($data === '' || $data === '0') {
                throw new InvalidArgumentException(sprintf('XTF0FForm::getInstance(name, *%s*)', gettype($data)));
            }
            ...
            if ('<' === substr(trim($data), 0, 1)) {
                if (false == $forms[$name]->load($data, $replace, $xpath)) {
                    throw new RuntimeException('XTF0FForm::getInstance could not load form');
                }
            } elseif (false == $forms[$name]->loadFile($data, $replace, $xpath)) {
                throw new RuntimeException('XTF0FForm::getInstance could not load file '.$data.'.xml');
            }
        }
    ...
    public function loadCSSFiles()
    {
        ...
        if (!empty($cssfiles)) {
            ...
            foreach ($cssfiles as $cssfile) {
                XTF0FTemplateUtils::addCSS(trim($cssfile));
            }
        }
        ...
        if (!empty($lessfiles)) {
            ...
            foreach ($lessfiles as $def) {
                ...
                XTF0FTemplateUtils::addLESS(trim($lessfile), $alt);
            }
        }
    }
    ...
    public function loadJSFiles()
    {
        ...
        foreach ($jsfiles as $jsfile) {
            XTF0FTemplateUtils::addJS(trim($jsfile));
        }
    }
    ...
    /**
     * Attaches a XTF0FModel to this form
     *
     * @param XTF0FModel &$xtf0FModel The model to attach to the form
     *
     * @return void
     */
    public function setModel(XTF0FModel &$xtf0FModel)
    {
        $this->model = $xtf0FModel;
    }

    /**
     * Returns the XTF0FModel attached to this form
     *
     * @return XTF0FModel
     */
    ...
    /**
     * Attaches a XTF0FView to this form
     *
     * @param XTF0FView &$xtf0FView The view to attach to the form
     *
     * @return void
     */
    public function setView(XTF0FView &$xtf0FView)
    {
        $this->view = $xtf0FView;
    }

    /**
     * Returns the XTF0FView attached to this form
     *
     * @return XTF0FView
     */
    ...
    /**
     * Method to get an array of XTF0FFormHeader objects in the headerset.
     *
     * @return array the array of XTF0FFormHeader objects in the headerset
     *
     * @since   2.0
     */
    ...
    /**
     * Method to get a header field represented as a XTF0FFormHeader object.
     *
     * @param string $name  the name of the header field
     * @param string $group the optional dot-separated form group path on which to find the field
     * @param mixed  $value the optional value to use as the default for the field
     *
     * @return mixed the XTF0FFormHeader object for the field or boolean false on error
     *
     * @since   2.0
     */
    public function getHeader($name, $group = null, $value = null)
    {
        // Make sure there is a valid XTF0FForm XML document.
    ...
    public function removeHeader($name, $group = null)
    {
        ...
        if (!($this->xml instanceof SimpleXMLElement)) {
            throw new UnexpectedValueException(sprintf('%s::getFieldAttribute `xml` is not an instance of SimpleXMLElement', static::class));
        }
    ...
    /**
     * Proxy for {@link XTF0FFormHelper::addFieldPath()}.
     *
    ...
    {
        return XTF0FFormHelper::addFieldPath($new);
    }

    /**
     * Proxy for {@link XTF0FFormHelper::addHeaderPath()}.
     *
     * @param mixed $new a path or array of paths to add
     *
     * @return array the list of paths that have been added
    ...
    {
        return XTF0FFormHelper::addHeaderPath($new);
    }

    /**
     * Proxy for XTF0FFormHelper::addFormPath().
     *
     * @param mixed $new a path or array of paths to add
     *
     * @return array the list of paths that have been added
     *
     * @see     XTF0FFormHelper::addFormPath()
     * @since   2.0
     */
    public static function addFormPath($new = null)
    {
        return XTF0FFormHelper::addFormPath($new);
    }

    /**
     * Proxy for XTF0FFormHelper::addRulePath().
     *
     * @param mixed $new a path or array of paths to add
     *
     * @return array the list of paths that have been added
     *
     * @see XTF0FFormHelper::addRulePath()
     * @since   2.0
     */
    public static function addRulePath($new = null)
    {
        return XTF0FFormHelper::addRulePath($new);
    }
    ...
    /**
     * Method to load, setup and return a XTF0FFormHeader object based on field data.
     *
    ...
    protected function loadHeader($element, $group = null, $value = null)
    {
        ...
        // Setup the XTF0FFormHeader object.
        $field->setForm($this);
    ...
    /**
     * Proxy for {@link XTF0FFormHelper::loadFieldType()}.
     *
     * @param string $type the field type
     * @param bool   $new  flag to toggle whether we should get a new instance of the object
     *
     * @return mixed XTF0FFormField object on success, false otherwise
     *
     * @since   2.0
     */
    protected function loadFieldType($type, $new = true)
    {
        return XTF0FFormHelper::loadFieldType($type, $new);
    }

    /**
     * Proxy for {@link XTF0FFormHelper::loadHeaderType()}.
     *
     * @param string $type the field type
     * @param bool   $new  flag to toggle whether we should get a new instance of the object
     *
     * @return mixed XTF0FFormHeader object on success, false otherwise
     *
     * @since   2.0
     */
    protected function loadHeaderType($type, $new = true)
    {
        return XTF0FFormHelper::loadHeaderType($type, $new);
    }

    /**
     * Proxy for {@link XTF0FFormHelper::loadRuleType()}.
     *
     ...
     *
     * @see     XTF0FFormHelper::loadRuleType()
     * @since   2.0
     */
    protected function loadRuleType($type, $new = true)
    {
        return XTF0FFormHelper::loadRuleType($type, $new);
    }
}
