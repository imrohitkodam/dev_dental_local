...
/**
 * XTF0FForm's helper class.
 * Provides a storage for filesystem's paths where XTF0FForm's entities reside and
 * methods for creating those entities. Also stores objects with entities'
 * prototypes for further reusing.
 *
 * @since    2.0
 */
class XTF0FFormHelper extends JFormHelper
{
    /**
    ...
    /**
     * Attempt to import the XTF0FFormHeader class file if it isn't already imported.
     * You can use this method outside of JForm for loading a field for inheritance or composition.
     *
     * @param string $type type of a field whose class should be loaded
     *
    ...
    public static function loadClass($entity, $type)
    {
        ...
        $altClass = ucfirst($altPrefix, '_').'Form'.ucfirst($entity, '_').ucfirst($type, '_');
