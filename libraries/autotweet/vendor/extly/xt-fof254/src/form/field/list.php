 ...
 * @since    2.0
 */
class XTF0FFormFieldList extends JFormFieldList implements XTF0FFormField
{
    ...
    public static function getOptionName($data, $selected = null, $optKey = 'value', $optText = 'text')
    {
        ...
        foreach ($data as $elementKey => &$element) {
            if (is_array($element)) {
                $key = null === $optKey ? $elementKey : $element[$optKey];
     ...
     *
     * @since	Ordering is available since XTF0F 2.1.b2.
     */
    protected function getOptions()
    {
        ...
        if ($this->element['order'] && 'false' !== $this->element['order']) {
        ...
        if ($this->element['order_case_sensitive'] && 'true' == $this->element['order_case_sensitive']) {
        ...
        if ($order) {
            ...
            XTF0FUtilsArray::sortObjects($sortOptions, $order, 'asc' == $order_dir ? 1 : -1, $order_case_sensitive, false);
        }
        ...
        foreach ($sortOptions as $sortOption) {
            ...
            $tmp = JHtml::_('select.option', (string) $option['value'], $name, 'value', 'text', ('true' === (string) $option['disabled']));
