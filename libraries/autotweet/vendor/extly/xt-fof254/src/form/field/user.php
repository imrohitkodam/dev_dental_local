 ...
 * @since    2.0
 */
class XTF0FFormFieldUser extends JFormFieldUser implements XTF0FFormField
{
    ...
    public function getStatic()
    {
        ...
        if ('false' == $this->element['show_username']) {
            $show_username = false;
        }

        if ('true' == $this->element['show_email']) {
            $show_email = true;
        }

        if ('true' == $this->element['show_name']) {
            $show_name = true;
        }

        if ('true' == $this->element['show_id']) {
            $show_id = true;
        }
    ...
    public function getRepeatable()
    {
        ...
        if ('false' == $this->element['show_email']) {
            $show_email = false;
        }

        if ('false' == $this->element['show_name']) {
            $show_name = false;
        }

        if ('false' == $this->element['show_id']) {
            $show_id = false;
        }

        if ('false' == $this->element['show_avatar']) {
            $show_avatar = false;
        }
        ...
        if ($show_avatar) {
            ...
            if ('plugin' === $avatar_method) {
                // Use the user plugins to get an avatar
                XTF0FPlatform::getInstance()->importPlugin('user');
                $jResponse = XTF0FPlatform::getInstance()->runPlugins('onUserAvatar', [$user, $avatar_size]);
                ...
                $scheme = XTF0FPlatform::getInstance()->isCli() ? 'http' : JURI::getInstance()->getScheme();
