 ...
 * @since    2.0
 */
class XTF0FFormFieldCalendar extends JFormFieldCalendar implements XTF0FFormField
{
    ...
    protected function getCalendar($display)
    {
        ...
        $formatJS = $format;
        $formatPHP = str_replace(['%', 'H:M:S', 'B'], ['', 'H:i:s', 'F'], $formatJS);

        // Check for empty date values
        if (empty($this->value) || $this->value == XTF0FPlatform::getInstance()->getDbo()->getNullDate() || '0000-00-00' == $this->value) {
            $this->value = $default;
        }
