 ...
 * @since    2.1
 */
class XTF0FFormFieldRules extends JFormFieldRules implements XTF0FFormField
{
     ...
     * not found. Our fixes are surrounded by `XTF0F Library fix` remarks.
     *
    ...
    protected function getInput25()
    {
        ...
        foreach ($this->element->children() as $el) {
            if ('action' == $el->getName()) {
                $actions[] = (object) ['name' => (string) $el['name'], 'title' => (string) $el['title'],
        ...
        foreach ($groups as $group) {
            ...
            $canCalculateSettings = ($group->parent_id || $component !== '' && $component !== '0');
            ...
            foreach ($actions as $action) {
                ...
                $html[] = '<option value=""'.(null === $assetRule ? ' selected="selected"' : '').'>'
                    .JText::_(empty($group->parent_id) && ($component === '' || $component === '0') ? 'JLIB_RULES_NOT_SET' : 'JLIB_RULES_INHERITED').'</option>';
                $html[] = '<option value="1"'.(true === $assetRule ? ' selected="selected"' : '').'>'.JText::_('JLIB_RULES_ALLOWED')
                    .'</option>';
                $html[] = '<option value="0"'.(false === $assetRule ? ' selected="selected"' : '').'>'.JText::_('JLIB_RULES_DENIED')
                    .'</option>';

                $html[] = '</select>&#160; ';

                // If this asset's rule is allowed, but the inherited rule is deny, we have a conflict.
                if ((true === $assetRule) && (false === $inheritedRule)) {
                ...
                if ($canCalculateSettings) {
                    ...
                    if (true !== JAccess::checkGroup($group->value, 'core.admin', $assetId)) {
                        if (null === $inheritedRule) {
                            $html[] = '<span class="icon-16-unset">'.JText::_('JLIB_RULES_NOT_ALLOWED').'</span>';
                        } elseif (true === $inheritedRule) {
                            $html[] = '<span class="icon-16-allowed">'.JText::_('JLIB_RULES_ALLOWED').'</span>';
                        } elseif (false === $inheritedRule) {
                            if (false === $assetRule) {
        ...
        if ('component' === $section || null == $section) {
        ...
        $js = "window.addEvent('domready', function(){ new Fx.Accordion($$('div#permissions-sliders.pane-sliders .panel h3.pane-toggler'),"
            ...
            .\Joomla\CMS\Uri\Uri::getInstance()->getInt('jpanesliders_permissions-sliders'.$component, 0, 'cookie').', show: '
            .\Joomla\CMS\Uri\Uri::getInstance()->getInt('jpanesliders_permissions-sliders'.$component, 0, 'cookie').', alwaysHide:true, opacity: false}); });';
    ...
    protected function getInput3x()
    {
        ...
        // Building tab nav
        $html[] = '<ul class="xt-nav xt-nav-tabs">';
        ...
        foreach ($groups as $group) {
            ...
            foreach ($actions as $action) {
                ...
                if ($canCalculateSettings) {
                    ...
                    if (true !== JAccess::checkGroup($group->value, 'core.admin', $assetId)) {
                        if (null === $inheritedRule) {
                            $html[] = '<span class="xt-label xt-label-error">'.JText::_('JLIB_RULES_NOT_ALLOWED').'</span>';
                        } elseif (true === $inheritedRule) {
                            $html[] = '<span class="xt-label xt-label-success">'.JText::_('JLIB_RULES_ALLOWED').'</span>';
                        } elseif (false === $inheritedRule) {
                            if (false === $assetRule) {
                                $html[] = '<span class="xt-label xt-label-error">'.JText::_('JLIB_RULES_NOT_ALLOWED').'</span>';
                            } else {
                        ...
                        $html[] = '<span class="xt-label xt-label-success"><i class="icon-lock icon-white"></i> '.JText::_('JLIB_RULES_ALLOWED_ADMIN')
                            .'</span>';
                    } elseif ('core.admin' === $action->name) {
                        // Special handling for  groups that have global admin because they can't  be denied.
                        // The admin rights can be changed.
                        ...
                        // Other actions cannot be changed.
                        $html[] = '<span class="xt-label xt-label-error"><i class="icon-lock icon-white"></i> '
        ...
        $html[] = '<div class="xt-alert xt-alert-info">';
