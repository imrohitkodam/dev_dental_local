...
class XTF0FTableRelations
{
    ...
    /**
     * Create a relations object based on the provided XTF0FTable instance
     *
     * @param XTF0FTable $table The table instance used to initialise the relations
     */
    public function __construct(XTF0FTable $table)
    {
        // Store the table
        $this->table = $table;
        ...
        $this->tableType = XTF0FInflector::singularize($this->tableType);
        ...
        foreach ($table->getKnownFields() as $field) {
            ...
            $itemName = XTF0FInflector::singularize($parts[1]);
        ...
        $configRelations = $table->getConfigProvider()->get($key, []);
     ...
     * @param bool   $default    add as the default child relation?
     *
     * @return void
     */
     ...
     * @param string $remoteKey  is the remote table's FK column, default: componentname_itemname_id
     * @param bool   $default    Is this the default parent relationship?
     *
     * @return void
     */
     ...
     * @param string $localKey   is the column containing our side of the FK relation, default: our primary key
     * @param string $remoteKey  is the remote table's FK column, default: componentname_itemname_id
     * @param bool   $default    is this the default children relationship?
     *
     * @return void
     */
     ...
     * @param bool   $default       is this the default multiple relation?
     */
    ...
    public function getSiblings($itemName = null)
    {
        ...
        $tableClass = ucfirst($tableNameParts[0]).'Table'.ucfirst(XTF0FInflector::singularize($tableNameParts[1]));
    ...
    /**
     * Returns a XTF0FTable object based on a given relation
     *
    ...
    protected function getTableFromRelation($relation)
    {
        ...
        $tableClassParts = XTF0FInflector::explode($tableClass);
        ...
        $table = XTF0FTable::getInstance($tableClassParts[2], ucfirst($tableClassParts[0]).ucfirst($tableClassParts[1]));
    ...
    /**
     * Returns a XTF0FDatabaseIterator based on a given relation
     *
     * @param array $relation Indexed array holding relation definition.
     *                        tableClass => name of the related table class
     *                        localKey   => name of the local key
    ...
    protected function addBespokeSimpleRelation($relationType, $itemName, $tableClass, $localKey, $remoteKey, $default)
    {
        ...
        $this->relations[$relationType][$itemName] = [
            'tableClass'	=> $tableClass,
            'localKey'		=> $localKey,
            'remoteKey'		=> $remoteKey,
        ];
     ...
     *
     * @param bool   $pivot         Is this a many to many relation involving a pivot table?
     * @param string $itemName      is how it will be known locally to the getRelatedItems method (plural)
     * @param string $tableClass    if skipped it is defined automatically as ComponentnameTableItemname
     * @param string $localKey      is the column containing our side of the FK relation, default: componentname_itemname_id
     * @param string $remoteKey     is the remote table's FK column, default: componentname_itemname_id
    ...
    protected function normaliseParameters($pivot, &$itemName, &$tableClass, &$localKey, &$remoteKey, &$ourPivotKey, &$theirPivotKey, &$pivotTable)
    {
        ...
        if (empty($tableClass)) {
            ...
            if (1 == count($tableClassParts)) {
                array_unshift($tableClassParts, $this->componentName);
            }

            if ('joomla' == $tableClassParts[0]) {
                $tableClassParts[0] = 'J';
            }

            $tableClass = ucfirst($tableClassParts[0]).'Table'.ucfirst(XTF0FInflector::singularize($tableClassParts[1]));
        }
        ...
        if (empty($pivotTable)) {
            $pivotTable = '#__'.strtolower($this->componentName).'_'.
                            strtolower(XTF0FInflector::pluralize($this->tableType)).'_';
     ...
     * @param bool   $pluralise Should I pluralise the name? If not, I will singularise it
     *
    ...
    protected function normaliseItemName($itemName, $pluralise = false)
    {
        ...
        if (count($itemNameParts) > 1) {
            ...
            $name = $pluralise ? XTF0FInflector::pluralize($name) : XTF0FInflector::singularize($name);
            $itemNameParts[] = $name;

            $itemName = XTF0FInflector::implode($itemNameParts);
        }
        // Otherwise we singularise/pluralise the remaining part
        else {
            $name = array_pop($itemNameParts);
            $itemName = $pluralise ? XTF0FInflector::pluralize($name) : XTF0FInflector::singularize($name);
        }
