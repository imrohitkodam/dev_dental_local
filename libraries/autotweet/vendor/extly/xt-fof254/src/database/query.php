 ...
 *
 * @property XTF0FDatabaseQueryElement $type
 * @property XTF0FDatabaseQueryElement $select
 * @property XTF0FDatabaseQueryElement $group
 * @property XTF0FDatabaseQueryElement $having
 */
abstract class XTF0FDatabaseQuery
{
    /**
     * @var XTF0FDatabaseDriver the database driver
     *
     * @since  11.1
     */
    protected $db = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the query element for a generic query (type = null)
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the select element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the delete element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the update element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the insert element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the from element
     *
     * @since  11.1
     */
    protected $from = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the join element
     *
     * @since  11.1
     */
    protected $join = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the set element
     *
     * @since  11.1
     */
    protected $set = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the where element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the group by element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the having element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the column list for an INSERT statement
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the values list for an INSERT statement
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the order element
     *
     * @since  11.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the call element
     *
     * @since  12.1
     */
    protected $call = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the exec element
     *
     * @since  12.1
     */
    protected $exec = null;
    ...
    /**
     * @var XTF0FDatabaseQueryElement the union element
     *
     * @since  12.1
     */
    ...
    /**
     * @var XTF0FDatabaseQueryElement the unionAll element
     *
     * @since  13.1
     */
     ...
     * Class constructor.
     *
     * @param XTF0FDatabaseDriver $xtf0FDatabaseDriver the database driver
     *
     * @since   11.1
     */
    public function __construct(XTF0FDatabaseDriver $xtf0FDatabaseDriver = null)
    {
        $this->db = $xtf0FDatabaseDriver;
    }
    ...
    public function __toString()
    {
        ...
        if ($this instanceof XTF0FDatabaseQueryLimitable) {
    ...
    public function call($columns)
    {
        ...
        if (null === $this->call) {
            $this->call = new XTF0FDatabaseQueryElement('CALL', $columns);
        } else {
    ...
    public function columns($columns)
    {
        if (null === $this->columns) {
            $this->columns = new XTF0FDatabaseQueryElement('()', $columns);
        } else {
    ...
    {
        if (!($this->db instanceof XTF0FDatabaseDriver)) {
    ...
    {
        return '<pre class="XTF0FDatabasequery">'.str_replace('#__', $this->db->getPrefix(), $this).'</pre>';
    }
    ...
    {
        $this->type = 'delete';
        $this->delete = new XTF0FDatabaseQueryElement('DELETE', null);
     ...
     *
     * Note that 'e' is an alias for this method as it is in XTF0FDatabaseDriver.
     *
    ...
    public function exec($columns)
    {
        ...
        if (null === $this->exec) {
            $this->exec = new XTF0FDatabaseQueryElement('EXEC', $columns);
        } else {
     ...
     *                              This can be a XTF0FDatabaseQuery object (or a child of it) when used
     *                              as a subquery in FROM clause along with a value for $subQueryAlias.
     * @param string $subQueryAlias alias used when $tables is a XTF0FDatabaseQuery
     *
     * @return XTF0FDatabaseQuery returns this object to allow chaining
     *
    ...
    public function from($tables, $subQueryAlias = null)
    {
        if (null === $this->from) {
            ...
            $this->from = new XTF0FDatabaseQueryElement('FROM', $tables);
        } else {
    ...
    {
        if (null === $this->group) {
            $this->group = new XTF0FDatabaseQueryElement('GROUP BY', $columns);
        } else {
    ...
    public function having($conditions, $glue = 'AND')
    {
        if (null === $this->having) {
            ...
            $this->having = new XTF0FDatabaseQueryElement('HAVING', $conditions, sprintf(' %s ', $glue));
        } else {
    ...
    {
        $this->type = 'insert';
        $this->insert = new XTF0FDatabaseQueryElement('INSERT INTO', $table);
    ...
    public function join($type, $conditions)
    {
        ...
        $this->join[] = new XTF0FDatabaseQueryElement(strtoupper($type).' JOIN', $conditions);
    ...
    {
        if (null === $this->order) {
            $this->order = new XTF0FDatabaseQueryElement('ORDER BY', $columns);
        } else {
     ...
     *
     * Note that 'q' is an alias for this method as it is in XTF0FDatabaseDriver.
     *
     * Usage:
     ...
     *
     * Note that 'qn' is an alias for this method as it is in XTF0FDatabaseDriver.
     *
     * Usage:
     * $query->quoteName('#__a');
     * $query->qn('#__a');
    ...
    public function select($columns)
    {
        ...
        if (null === $this->select) {
            $this->select = new XTF0FDatabaseQueryElement('SELECT', $columns);
        } else {
    ...
    public function set($conditions, $glue = ',')
    {
        if (null === $this->set) {
            $glue = strtoupper($glue);
            $this->set = new XTF0FDatabaseQueryElement('SET', $conditions, "\n\t{$glue} ");
        } else {
    ...
    {
        $this->type = 'update';
        $this->update = new XTF0FDatabaseQueryElement('UPDATE', $table);
    ...
    {
        if (null === $this->values) {
            $this->values = new XTF0FDatabaseQueryElement('()', $values, '),(');
        } else {
    ...
    public function where($conditions, $glue = 'AND')
    {
        if (null === $this->where) {
            $glue = strtoupper($glue);
            $this->where = new XTF0FDatabaseQueryElement('WHERE', $conditions, sprintf(' %s ', $glue));
        } else {
     ...
     *
     * @param mixed  $query    the XTF0FDatabaseQuery object or string to union
     * @param bool   $distinct true to only return distinct rows from the union
     * @param string $glue     the glue by which to join the conditions
     *
     * @return mixed the XTF0FDatabaseQuery object on success or boolean false on failure
     *
    ...
    public function union($query, $distinct = false, $glue = '')
    {
        ...
        // Get the XTF0FDatabaseQueryElement if it does not exist
        if (null === $this->union) {
            $this->union = new XTF0FDatabaseQueryElement($name, $query, $glue);
        }
    ...
    public function format($format)
    {
        ...
        $func = function ($match) use ($query, $args, &$i) {
            ...
            $replacement = !$index || !isset($args[$index]) ? '' : $args[$index];
     ...
     *
     * @param mixed  $query    the XTF0FDatabaseQuery object or string to union
     * @param bool   $distinct not used - ignored
    ...
    public function unionAll($query, $distinct = false, $glue = '')
    {
        ...
        if (null === $this->unionAll) {
            $this->unionAll = new XTF0FDatabaseQueryElement($name, $query, $glue);
        }
