 ...
 * @since  11.3
 */
class XTF0FDatabaseQueryPostgresql extends XTF0FDatabaseQuery implements XTF0FDatabaseQueryLimitable
{
    /**
     ...
     *
     * @return XTF0FDatabaseQueryPostgresql returns this object to allow chaining
     *
     * @since   11.3
     */
     ...
     *
     * @return XTF0FDatabaseQueryPostgresql FOR UPDATE query element
     *
     * @since   11.3
     */
    public function forUpdate($table_name, $glue = ',')
    {
        ...
        if (null === $this->forUpdate) {
            $glue = strtoupper($glue);
            $this->forUpdate = new XTF0FDatabaseQueryElement('FOR UPDATE', 'OF '.$table_name, $glue . ' ');
        } else {
     ...
     * @param string $glue       The glue by which to join the conditions. Defaults to ',' .
     *
     * @return XTF0FDatabaseQueryPostgresql FOR SHARE query element
     *
     * @since   11.3
     */
    public function forShare($table_name, $glue = ',')
    {
        ...
        if (null === $this->forShare) {
            $glue = strtoupper($glue);
            $this->forShare = new XTF0FDatabaseQueryElement('FOR SHARE', 'OF '.$table_name, $glue . ' ');
        } else {
     ...
     *
     * @return XTF0FDatabaseQueryPostgresql NO WAIT query element
     *
     * @since   11.3
     */
    public function noWait()
    {
        ...
        if (null === $this->noWait) {
            $this->noWait = new XTF0FDatabaseQueryElement('NOWAIT', null);
        }
    ...
    {
        if (null === $this->limit) {
            $this->limit = new XTF0FDatabaseQueryElement('LIMIT', (int) $limit);
        }
    ...
    {
        if (null === $this->offset) {
            $this->offset = new XTF0FDatabaseQueryElement('OFFSET', (int) $offset);
        }
    ...
    public function returning($pkCol)
    {
        if (null === $this->returning) {
            $this->returning = new XTF0FDatabaseQueryElement('RETURNING', $pkCol);
        }
