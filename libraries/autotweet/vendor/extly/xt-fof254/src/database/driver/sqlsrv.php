 ...
 * @since  12.1
 */
class XTF0FDatabaseDriverSqlsrv extends XTF0FDatabaseDriver
{
    /**
     ...
     *
     * @return XTF0FDatabaseDriverSqlsrv returns this object to support chaining
     *
     * @since   12.1
     */
    public function dropTable($tableName, $ifExists = true)
    {
        ...
        if ($ifExists) {
            $this->setQuery(
                'IF EXISTS(SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '.$xtf0FDatabaseQuery->quote($tableName).') DROP TABLE '.$tableName
            );
        } else {
    ...
    public function execute()
    {
        ...
        if (0 == strncmp(ltrim(strtoupper($query)), 'SELECT', strlen('SELECT'))) {
    ...
    protected function getTableConstraints($tableName)
    {
        ...
        $this->setQuery(
            'SELECT CONSTRAINT_NAME FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = '.$xtf0FDatabaseQuery->quote($tableName)
        );
    ...
    {
        sqlsrv_free_stmt($cursor ?: $this->cursor);
    }
    ...
    protected function checkFieldExists($table, $field)
    {
        ...
        $query = sprintf("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '%s' AND COLUMN_NAME = '%s'", $table, $field).
