 ...
 * @since  12.1
 */
class XTF0FDatabaseDriverPostgresql extends XTF0FDatabaseDriver
{
    /**
    ...
    /**
     * XTF0FDatabaseDriverPostgresqlQuery object returned by getQuery
     *
     * @var XTF0FDatabaseDriverPostgresqlQuery
     *
     * @since  12.1
     */
    ...
    public function connect()
    {
        ...
        $dsn .= sprintf('dbname=%s user=%s password=%s', $this->options['database'], $this->options['user'], $this->options['password']);
    ...
    public function getNumRows($cur = null)
    {
        ...
        return pg_num_rows((int) $cur !== 0 ? $cur : $this->cursor);
    }

    /**
     * Get the current or query, or new XTF0FDatabaseQuery object.
     *
     * @param bool $new   false to return the last query set, True to return a new XTF0FDatabaseQuery object
     * @param bool $asObj false to return last query as string, true to get XTF0FDatabaseQueryPostgresql object
     *
    ...
    public function getQuery($new = false, $asObj = false)
    {
        if ($new) {
            ...
            if (!class_exists('XTF0FDatabaseQueryPostgresql')) {
                throw new RuntimeException('XTF0FDatabaseQueryPostgresql Class not found.');
            }

            $this->queryObject = new XTF0FDatabaseQueryPostgresql($this);
    ...
    {
        $this->connect();
        $xtf0FDatabaseQuery = $this->getQuery(false, true);
        $table = $xtf0FDatabaseQuery->__get('insert')->getElements();
     ...
     *
     * @return XTF0FDatabaseDriverPostgresql returns this object to support chaining
     *
     * @since   12.1
     *
     * @throws RuntimeException
    ...
    public function sqlValue($columns, $field_name, $field_value)
    {
        switch ($columns[$field_name]) {
            ...
            case 'serial':
            case 'numeric,':
                $val = 0 == strlen($field_value) ? 'NULL' : $field_value;
                break;
    ...
    public function insertObject($table, &$object, $key = null)
    {
        ...
        if ($key) {
            $xtf0FDatabaseQuery->returning($key);
    ...
    public function getStringPositionSql($substring, $string)
    {
        ...
        $query = sprintf('SELECT POSITION( %s IN %s )', $substring, $string);
        $this->setQuery($query);
     ...
     * @param bool   $utf     true if the database supports the UTF-8 character set, not used in PostgreSQL "CREATE DATABASE" query
     *
    ...
    public function replacePrefix($query, $prefix = '#__')
    {
        ...
        if (strpos($query, "'")) {
            ...
            if (strpos($query, 'currval')) {
                ...
                for ($nIndex = 1; $nIndex < $counter; $nIndex += 2) {
            ...
            $counter = count($explodedQuery);

            for ($nIndex = 0; $nIndex < $counter; $nIndex += 2) {
    ...
    {
        return pg_fetch_object($cursor ?? $this->cursor, null, $class);
    }
    ...
    {
        pg_free_result($cursor ?: $this->cursor);
    }
