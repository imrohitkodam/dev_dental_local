 ...
 */
abstract class XTF0FDatabaseDriver extends XTF0FDatabase implements XTF0FDatabaseInterface
{
    /**
    ...
    /**
     * @var int the number of SQL statements executed by the database driver
     *
     * @since  11.1
     */
    protected $count = 0;
    ...
    /**
     * @var int the affected row limit for the current SQL statement
     *
     * @since  11.1
     */
    protected $limit = 0;
    ...
    /**
     * @var int the affected row offset to apply for the current SQL statement
     *
     * @since  11.1
     */
    protected $offset = 0;
    ...
    /**
     * @var bool true if the database engine supports UTF-8 Multibyte (utf8mb4) character encoding
     *
     * @since  CMS 3.5.0
     */
    ...
    /**
     * @var int The database error number
     *
     * @since       11.1
     * @deprecated  12.1
     */
    ...
    /**
     * @var array XTF0FDatabaseDriver instances container
     *
     * @since  11.1
     */
    ...
    /**
     * @var int the depth of the current transaction
     *
     * @since  12.3
     */
    ...
    public static function getConnectors()
    {
        ...
        foreach ($iterator as $file) {
            ...
            if ('mysql.php' === $fileName && \PHP_MAJOR_VERSION >= 7) {
                continue;
            }

            // Derive the class name from the type.
            $class = str_ireplace('.php', '', 'XTF0FDatabaseDriver'.ucfirst(trim($fileName)));
    ...
    public static function getInstance($options = [])
    {
        ...
        if ('mysql' === $options['driver'] && \PHP_MAJOR_VERSION >= 7) {
            // Check if we have support for the other MySQL drivers
            $mysqliSupported = XTF0FDatabaseDriverMysqli::isSupported();
            $pdoMysqlSupported = XTF0FDatabaseDriverPdomysql::isSupported();
     ...
     * @param bool   $rethrow   True to rethrow database exceptions. Default: false (exceptions are suppressed)
     *
    ...
    /**
     * Adds a function callable just before disconnecting the database. Parameter of the callable is $this XTF0FDatabaseDriver
     *
     ...
     *
     * @return XTF0FDatabaseDriver returns this object to support chaining
     *
     * @since   11.4
     *
     * @throws RuntimeException
    ...
    public function getAlterTableCharacterSet($tableName)
    {
        ...
        $queries = [];
        $queries[] = sprintf('ALTER TABLE %s CONVERT TO CHARACTER SET %s COLLATE %s', $quotedTableName, $charset, $collation);
        ...
        if (is_array($columns)) {
            foreach ($columns as $column) {
                ...
                $default = null === $col->Default ? '' : "DEFAULT '".$this->q($col->Default)."'";
                $columnMods[] = sprintf('MODIFY COLUMN `%s` %s CHARACTER SET %s COLLATE %s %s %s', $col->Field, $col->Type, $charset, $newCollation, $null, $default);
            }
        }

        if ($columnMods !== []) {
            $queries[] = sprintf('ALTER TABLE %s ', $quotedTableName).
                implode(',', $columnMods).
                sprintf(' CHARACTER SET %s COLLATE %s', $charset, $collation);
        }
     ...
     *
     * @return XTF0FDatabaseExporter an exporter object
     *
     * @since   12.1
     *
     * @throws RuntimeException
    ...
    {
        // Derive the class name from the driver.
        $class = 'XTF0FDatabaseExporter'.ucfirst($this->name);
     ...
     *
     * @return XTF0FDatabaseImporter an importer object
     *
     * @since   12.1
     *
     * @throws RuntimeException
    ...
    {
        // Derive the class name from the driver.
        $class = 'XTF0FDatabaseImporter'.ucfirst($this->name);
    ...
    public function getName()
    {
        if (empty($this->name)) {
            ...
            $className = str_replace('XTF0FDatabaseDriver', '', $className);
    ...
    public function getServerType()
    {
        if (empty($this->serverType)) {
            ...
            if (false !== stristr($name, 'mysql')) {
                $this->serverType = 'mysql';
            } elseif (false !== stristr($name, 'postgre')) {
                $this->serverType = 'postgresql';
            } elseif (false !== stristr($name, 'oracle')) {
                $this->serverType = 'oracle';
            } elseif (false !== stristr($name, 'sqlite')) {
                $this->serverType = 'sqlite';
            } elseif (false !== stristr($name, 'sqlsrv')) {
                $this->serverType = 'mssql';
            } elseif (false !== stristr($name, 'mssql')) {
                $this->serverType = 'mssql';
            } else {
     ...
     *
     * @param bool $new false to return the current query object, True to return a new XTF0FDatabaseQuery object
     *
    ...
    public function getQuery($new = false)
    {
        if ($new) {
            // Derive the class name from the driver.
            $class = 'XTF0FDatabaseQuery'.ucfirst($this->name);
     ...
     *
     * @return XTF0FDatabaseIterator a new database iterator
     *
     * @since   12.1
     *
     * @throws RuntimeException
    ...
    {
        // Derive the class name from the driver.
        $iteratorClass = 'XTF0FDatabaseIterator'.ucfirst($this->name);
    ...
    {
        if (class_exists('JLog')) {
            JLog::add('XTF0FDatabaseDriver::getUTFSupport() is deprecated. Use XTF0FDatabaseDriver::hasUTFSupport() instead.', JLog::WARNING, 'deprecated');
        }
     ...
     * MySQL databases.
     *
     * @return bool true if the database engine supports UTF-8 Multibyte
     *
     * @since   CMS 3.5.0
     */
    ...
    public function insertObject($table, &$object, $key = null)
    {
        ...
        foreach (get_object_vars($object) as $k => $v) {
            ...
            if (is_array($v) || is_object($v) || null === $v) {
                continue;
            }
    ...
    public function loadNextObject($class = 'stdClass')
    {
        ...
        // Execute the query and get the result set cursor.
        if (null === $cursor && !($cursor = $this->execute())) {
     ...
     * @throws RuntimeException
     *
     * @deprecated  4.0 (CMS)  Use XTF0FDatabaseDriver::getIterator() instead
     */
    ...
    public function setQuery($query, $offset = 0, $limit = 0)
    {
        ...
        if ($query instanceof XTF0FDatabaseQueryLimitable) {
    ...
    public function updateObject($table, &$object, $key, $nulls = false)
    {
        ...
        foreach (get_object_vars($object) as $k => $v) {
            ...
            if (is_array($v) || is_object($v) || '_' === $k[0]) {
                continue;
            }
