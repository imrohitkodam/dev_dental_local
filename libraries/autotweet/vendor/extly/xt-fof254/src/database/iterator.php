 ...
 * Database iterator
 */
abstract class XTF0FDatabaseIterator implements Iterator
{
    /**
     * The database cursor.
     */
    ...
    /**
     * The current database record as a XTF0FTable object.
     *
     * @var XTF0FTable
     */
    ...
    /**
     * A XTF0FTable object created using the class type $class, used by getTable
     *
     * @var XTF0FTable
     */
     ...
     */
    public function __construct($cursor, $column, $class, $config = [])
    {
        // Figure out the type and prefix of the class by the class name
        $parts = XTF0FInflector::explode($class);

        if (3 != count($parts)) {
            throw new InvalidArgumentException('Invalid table name, expected a pattern like ComponentTableFoobar got '.$class);
        }

        $this->_tableObject = XTF0FTable::getInstance($parts[2], ucfirst($parts[0]).ucfirst($parts[1]))->getClone();
     ...
     * @throws InvalidArgumentException
     */
    public static function &getIterator($dbName, $cursor, $column, $class, $config = [])
    {
        $className = 'XTF0FDatabaseIterator'.ucfirst($dbName);
    ...
    /**
     * Returns the data in $this->_current as a XTF0FTable instance
     *
     * @return XTF0FTable
     *
