...
class XTF0FToolbar
{
    ...
    public function __construct($config = [])
    {
        ...
        $perms = (object) [
            'manage'	 => $xtf0FPlatform->authorise('core.manage', $this->input->getCmd('option', 'com_foobar')),
            'create'	 => $xtf0FPlatform->authorise('core.create', $this->input->getCmd('option', 'com_foobar')),
            'edit'		 => $xtf0FPlatform->authorise('core.edit', $this->input->getCmd('option', 'com_foobar')),
            'editstate'	 => $xtf0FPlatform->authorise('core.edit.state', $this->input->getCmd('option', 'com_foobar')),
            'delete'	 => $xtf0FPlatform->authorise('core.delete', $this->input->getCmd('option', 'com_foobar')),
        ];
        ...
        if (!XTF0FPlatform::getInstance()->isBackend()) {
            ...
            if ($this->renderFrontendButtons) {
                ...
                XTF0FPlatform::getInstance()->loadTranslations('');
     ...
     *
     * @return XTF0FToolbar The toolbar instance for the component
     */
     ...
     * @param \Joomla\CMS\Input\Input $input An optional input object used to determine the defaults
     *
     * @return void
     */
    public function renderToolbar($view = null, $task = null, $input = null)
    {
        ...
        $render_toolbar = 'component' != $this->input->getCmd('tmpl', '');
        ...
        $toolbar = $xtf0FConfigProvider->get(
    ...
    {
        if (XTF0FPlatform::getInstance()->isBackend() || $this->renderFrontendSubmenu) {
            $this->renderSubmenu();
        }

        if (!XTF0FPlatform::getInstance()->isBackend() && !$this->renderFrontendButtons) {
            return;
        }
    ...
    protected function getMyViews()
    {
        ...
        if (!empty($allFolders)) {
            foreach ($allFolders as $allFolder) {
                ...
                // View already added
                if (in_array(XTF0FInflector::pluralize($view), $t_views)) {
                    continue;
                }
                ...
                if (!$meta) {
                    $plural = XTF0FInflector::pluralize($view);
        ...
        XTF0FUtilsArray::sortObjects($to_order, 'ordering');
        $views = XTF0FUtilsArray::getColumn($to_order, 'view');

        // If not using the metadata file, let's put the cpanel view on top
        if (!$using_meta) {
            $cpanel = array_search('cpanels', $views, true);
        ...
        switch ($type) {
            ...
            case 'custom':
                ...
                $listSelect = isset($attributes['list_select']) ?
                    XTF0FStringUtils::toBool($attributes['list_select']) : true;
            ...
            case 'preview':
                ...
                $update_editors = isset($attributes['update_editors']) ?
                    XTF0FStringUtils::toBool($attributes['update_editors']) : false;
            ...
            case 'help':
                ...
                $ref = $attributes['help'];
                $com = isset($attributes['com']) ? XTF0FStringUtils::toBool($attributes['com']) : false;
            ...
            case 'new':
                if ($this->perms->create) {
                    ...
                    $check = isset($attributes['check']) ?
                        XTF0FStringUtils::toBool($attributes['check']) : false;
            ...
            case 'trash':
                if ($this->perms->editstate) {
                    ...
                    $check = isset($attributes['check']) ?
                        XTF0FStringUtils::toBool($attributes['check']) : true;
