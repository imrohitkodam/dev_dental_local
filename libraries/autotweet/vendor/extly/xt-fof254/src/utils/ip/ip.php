...
abstract class XTF0FUtilsIp
{
    ...
    public static function getIp()
    {
        if (null === static::$ip) {
            ...
            if (!empty($ip) && ('0.0.0.0' != $ip) && function_exists('inet_pton') && function_exists('inet_ntop')) {
                $myIP = @inet_pton($ip);
     ...
     *
     * @return bool True if it's IPv6
     */
    public static function isIPv6($ip)
    {
        return (bool) strstr($ip, ':');
    }
     ...
     *
     * @return bool|null True if it's in the list
     */
    public static function IPinList($ip, $ipTable = '')
    {
        ...
        if (!is_array($ipTable)) {
            if (false !== strpos($ipTable, ',')) {
                $ipTable = explode(',', $ipTable);
                $ipTable = array_map(fn($x) => trim($x), $ipTable);
            } else {
    ...
    protected static function detectAndCleanIP()
    {
        ...
        if ((false !== strstr($ip, ',')) || (false !== strstr($ip, ' '))) {
            $ip = str_replace(' ', ',', $ip);
            $ip = str_replace(',,', ',', $ip);
            $ips = explode(',', $ip);
            $ip = '';
    ...
    {
        $unpacked = 4 == strlen($inet) ? unpack('A4', $inet) : unpack('A16', $inet);
