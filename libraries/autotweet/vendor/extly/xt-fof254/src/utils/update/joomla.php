 ...
 */
class XTF0FUtilsUpdateJoomla extends XTF0FUtilsUpdateExtension
{
    /**
    ...
    {
        $xtf0FUtilsUpdateCollection = new XTF0FUtilsUpdateCollection();

        return $xtf0FUtilsUpdateCollection->getExtensionUpdateSource($url, 'file', 'joomla', $jVersion);
    }
    ...
    public function sanitiseVersion($version)
    {
        ...
        if (false !== $alphaQualifierPosition) {
            ...
            if ($betaRevision === '' || $betaRevision === '0') {
                $betaRevision = 1;
            }
        ...
        } elseif (false !== $betaQualifierPosition2) {
        ...
        } elseif (false !== $rcQualifierPosition3) {
    ...
    public function getUpdates($sources = [], $jVersion = null)
    {
        ...
        foreach ($allUpdates as $allUpdate) {
            ...
            if ('current' == $allUpdate['upgrade']) {
                $sections[0] = 'installed';
            } elseif (version_compare($allUpdate['version'], $currentMinVersion, 'ge') && version_compare($allUpdate['version'], $currentMaxVersion, 'le')) {
                $sections[0] = 'current';
            } else {
                $sections[0] = '';
            }

            $sections[1] = $allUpdate['lts'] ? 'lts' : 'sts';

            if ($allUpdate['testing']) {
                $sections = ['test'];
            }
            ...
            foreach ($sections as $section) {
                ...
                if (version_compare($allUpdate['version'], $existingVersionForSection, 'ge')) {
                    $ret[$section]['version'] = $allUpdate['version'];
                    $ret[$section]['package'] = $allUpdate['downloads'][0]['url'];
                    $ret[$section]['infourl'] = $allUpdate['infourl']['url'];
                }
            }
        }
