...
class XTF0FUtilsFilescheck
{
    ...
    public function __construct($option, $version, $date)
    {
        ...
        $xtf0FDatabaseQuery = $xtf0FDatabaseDriver->getQuery(true)->select('*')->from($xtf0FDatabaseDriver->qn('#__extensions'))
                    ->where($xtf0FDatabaseDriver->qn('element').' = '.$xtf0FDatabaseDriver->q($this->option))
                    ->where($xtf0FDatabaseDriver->qn('type').' = '.$xtf0FDatabaseDriver->q('component'));
        $extension = $xtf0FDatabaseDriver->setQuery($xtf0FDatabaseQuery)->loadObject();
    ...
    public function slowCheck($idx = 0)
    {
        ...
        $xtf0FUtilsTimer = new XTF0FUtilsTimer(3.0, 75.0);

        while ($xtf0FUtilsTimer->getTimeLeft() && (($idx < $totalFiles) || ($idx < $totalFolders))) {
            if ($idx < $totalFolders) {
