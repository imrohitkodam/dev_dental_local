...
class EDbProxyHelper
{
    ...
    public function flushInserts()
    {
        ...
        foreach ($this->bulk_tables as $bulk_table) {
            // No inserts
            if (!array_key_exists($bulk_table, $this->bulk_inserts)) {
                continue;
            }

            $tableq = $db->qn($bulk_table);

            $insertSet = $this->bulk_inserts[$bulk_table];
    ...
    public function flushUpdates()
    {
        ...
        foreach ($this->bulk_tables as $bulk_table) {
            // No updates
            if (!array_key_exists($bulk_table, $this->bulk_updates)) {
                continue;
            }

            $tableq = $db->qn($bulk_table);
            $pk_field = $this->bulk_tables_pk_field[$bulk_table];

            $fieldSet = $this->bulk_updates[$bulk_table];
            $pkSet = $this->bulk_updates_pks[$bulk_table];
    ...
    {
        $this->bulk_tables = [];
        $this->bulk_tables_pk_field = [];
        $this->bulk_inserts = [];
        $this->bulk_updates = [];
        $this->bulk_updates_pks = [];
        $this->bulk_updates_pks_flushed = [];
    }
