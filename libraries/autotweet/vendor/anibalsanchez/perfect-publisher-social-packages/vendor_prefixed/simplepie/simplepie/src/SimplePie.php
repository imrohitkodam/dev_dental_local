...
use XTS_BUILD\SimplePie\Cache\Psr16;
...
use XTS_BUILD\SimplePie\HTTP\Response;
...
class SimplePie
{
     ...
     * SimplePie Name
     */
    public const NAME = 'XTS_SimplePie';
    ...
    {
        // trigger_error(sprintf('SimplePie\SimplePie::set_file() is deprecated since SimplePie 1.9.0, please use "XTS_BUILD\SimplePie\SimplePie::set_http_client()" or "XTS_BUILD\SimplePie\SimplePie::set_raw_data()" instead.'), \E_USER_DEPRECATED);
    ...
    {
        // @trigger_error(sprintf('SimplePie\SimplePie::set_cache_location() is deprecated since SimplePie 1.8.0, please use "XTS_BUILD\SimplePie\SimplePie::set_cache()" instead.'), \E_USER_DEPRECATED);
    ...
    {
        trigger_error(sprintf('"%s()" is deprecated since SimplePie 1.3, please use "XTS_BUILD\SimplePie\SimplePie::set_cache()" instead.', __METHOD__), \E_USER_DEPRECATED);
    ...
    {
        trigger_error(sprintf('"%s()" is deprecated since SimplePie 1.3, please use "XTS_BUILD\SimplePie\SimplePie::get_registry()" instead.', __METHOD__), \E_USER_DEPRECATED);
    ...
    {
        // trigger_error(sprintf('"%s()" is deprecated since SimplePie 1.8.0, please use "XTS_BUILD\SimplePie\SimplePie::set_cache_namefilter()" instead.', __METHOD__), \E_USER_DEPRECATED);
            ...
            if (!empty($this->data)) {
                ...
                elseif (!isset($this->data['cache_expiration_time']) || $this->data['cache_expiration_time'] < time()) {
                    ...
                    if (isset($this->data['headers']['last-modified']) || isset($this->data['headers']['etag'])) {
                        $headers = [
                            'Accept' => XTS_SimplePie::DEFAULT_HTTP_ACCEPT_HEADER,
                        ];
    ...
    public function get_items(int $start = 0, int $end = 0)
    {
        if (!isset($this->data['items'])) {
            if (!empty($this->multifeed_objects)) {
                $this->data['items'] = XTS_SimplePie::merge_items($this->multifeed_objects, $start, $end, $this->item_limit);
                if (empty($this->data['items'])) {
                    return [];
                }
...
class_alias('SimplePie\SimplePie', 'XTS_SimplePie');
