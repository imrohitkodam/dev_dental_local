...
use XTS_BUILD\SimplePie\XML\Declaration\Parser;
...
class Misc
{
    ...
    {
        $iri = \XTS_BUILD\SimplePie\IRI::absolutize(new \XTS_BUILD\SimplePie\IRI($base), $relative);
        if ($iri === false) {
            return false;
        }
        return $iri->get_uri();
    ...
    public static function get_element(string $realname, string $string)
    {
        ...
        if (preg_match_all("/<($name)" . \XTS_BUILD\SimplePie\SimplePie::PCRE_HTML_ATTRIBUTE . "(>(.*)<\/$name>|(\/)?>)/siU", $string, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE)) {
    ...
    {
        $iri = new \XTS_BUILD\SimplePie\IRI($url);
        return [
    ...
    {
        $iri = new \XTS_BUILD\SimplePie\IRI('');
        $iri->scheme = $scheme;
    ...
    {
        $parser = \XTS_BUILD\SimplePie\Parse\Date::get();
    ...
    public static function entities_decode(string $data)
    {
        ...
        $decoder = new \XTS_BUILD\SimplePie_Decode_HTML_Entities($data);
    ...
    public static function atom_03_construct_type(array $attribs)
    {
        if (isset($attribs['']['mode']) && strtolower(trim($attribs['']['mode'])) === 'base64') {
            $mode = \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_BASE64;
        } else {
            $mode = \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_NONE;
        }
        if (isset($attribs['']['type'])) {
            switch (strtolower(trim($attribs['']['type']))) {
                case 'text':
                case 'text/plain':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_TEXT | $mode;

                case 'html':
                case 'text/html':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_HTML | $mode;

                case 'xhtml':
                case 'application/xhtml+xml':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_XHTML | $mode;

                default:
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_NONE | $mode;
            }
        }
    ...
    public static function atom_10_construct_type(array $attribs)
    {
        if (isset($attribs['']['type'])) {
            switch (strtolower(trim($attribs['']['type']))) {
                case 'text':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_TEXT;

                case 'html':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_HTML;

                case 'xhtml':
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_XHTML;

                default:
                    return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_NONE;
            }
        }
        return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_TEXT;
    }
    ...
    public static function atom_10_content_construct_type(array $attribs)
    {
        if (isset($attribs['']['type'])) {
            ...
            if (in_array(substr($type, -4), ['+xml', '/xml']) || substr($type, 0, 5) === 'text/') {
                ...
                return \XTS_BUILD\SimplePie\SimplePie::CONSTRUCT_BASE64;
            }
        }
     ...
     */
    public static function xml_encoding(string $data, \XTS_BUILD\SimplePie\Registry $registry)
    {
...
JS;
    ...
    {
        return \XTS_BUILD\SimplePie\SimplePie::NAME . '/' . \XTS_BUILD\SimplePie\SimplePie::VERSION . ' (Feed Parser; ' . \XTS_BUILD\SimplePie\SimplePie::URL . '; Allow like Gecko) Build/' . static::get_build();
    }
    ...
    {
        $info = 'SimplePie ' . \XTS_BUILD\SimplePie\SimplePie::VERSION . ' Build ' . static::get_build() . "\n";
...
class_alias('XTS_BUILD\SimplePie\Misc', 'XTS_SimplePie_Misc', false);
