...
class MySQL extends DB
{
    ...
    public function __construct(string $location, string $name, $type)
    {
        ...
        $this->options = array_replace_recursive($this->options, \XTS_BUILD\SimplePie\Cache::parse_URL($location));
    ...
    public function save($data)
    {
        ...
        if ($data instanceof \XTS_BUILD\SimplePie\SimplePie) {
            $data = clone $data;
    ...
    public function load()
    {
        ...
        if ($query->execute() && ($row = $query->fetch())) {
            ...
            if ($items !== 0) {
                if (isset($data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_10]['feed'][0])) {
                    $feed = &$data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_10]['feed'][0];
                } elseif (isset($data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_03]['feed'][0])) {
                    $feed = &$data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_03]['feed'][0];
                } elseif (isset($data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RDF]['RDF'][0])) {
                    $feed = &$data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RDF]['RDF'][0];
                } elseif (isset($data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RSS_20]['rss'][0])) {
                    $feed = &$data['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RSS_20]['rss'][0];
                } else {
                    $feed = null;
                }
                ...
                if ($feed !== null) {
                    ...
                    if ($query->execute()) {
                        while ($row = $query->fetchColumn()) {
                            $feed['child'][\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_10]['entry'][] = unserialize((string) $row);
                        }
                    } else {
                        return false;
                    }
...
class_alias('XTS_BUILD\SimplePie\Cache\MySQL', 'XTS_SimplePie_Cache_MySQL');
