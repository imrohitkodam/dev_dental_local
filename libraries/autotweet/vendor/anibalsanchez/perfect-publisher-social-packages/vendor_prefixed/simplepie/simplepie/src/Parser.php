...
use XTS_BUILD\SimplePie\XML\Declaration\Parser as DeclarationParser;
use XMLParser;
...
class Parser implements RegistryAware
{
    ...
    public function tag_open($parser, string $tag, array $attributes)
    {
        ...
        if (isset($attribs[\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XML]['base'])) {
            $base = $this->registry->call(Misc::class, 'absolutize_url', [$attribs[\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XML]['base'], end($this->xml_base)]);
            if ($base !== false) {
                $this->xml_base[] = $base;
        ...
        if (isset($attribs[\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XML]['lang'])) {
            $this->xml_lang[] = $attribs[\XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XML]['lang'];
        } else {
        ...
        if ($this->current_xhtml_construct >= 0) {
            ...
            if (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XHTML) {
            ...
            if ((end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_03 && in_array(end($this->element), ['title', 'tagline', 'copyright', 'info', 'summary', 'content']) && isset($attribs['']['mode']) && $attribs['']['mode'] === 'xml')
            || (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ATOM_10 && in_array(end($this->element), ['rights', 'subtitle', 'summary', 'info', 'title', 'content']) && isset($attribs['']['type']) && $attribs['']['type'] === 'xhtml')
            || (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RSS_20 && in_array(end($this->element), ['title']))
            || (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RSS_090 && in_array(end($this->element), ['title']))
            || (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_RSS_10 && in_array(end($this->element), ['title']))) {
    ...
    public function tag_close($parser, string $tag)
    {
        if ($this->current_xhtml_construct >= 0) {
            $this->current_xhtml_construct--;
            if (end($this->namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_XHTML && !in_array(end($this->element), ['area', 'base', 'basefont', 'br', 'col', 'frame', 'hr', 'img', 'input', 'isindex', 'link', 'meta', 'param'])) {
                $this->data['data'] .= '</' . end($this->element) . '>';
            }
        }
    ...
    public function split_ns(string $string)
    {
        ...
        if (!isset($cache[$string])) {
            if ($pos = strpos($string, $this->separator)) {
                ...
                if (strtolower($namespace) === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ITUNES) {
                    $namespace = \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_ITUNES;
                }

                // Normalize the Media RSS namespaces
                if ($namespace === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS_WRONG ||
                    $namespace === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS_WRONG2 ||
                    $namespace === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS_WRONG3 ||
                    $namespace === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS_WRONG4 ||
                    $namespace === \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS_WRONG5) {
                    $namespace = \XTS_BUILD\SimplePie\SimplePie::NAMESPACE_MEDIARSS;
                }
...
class_alias('XTS_BUILD\SimplePie\Parser', 'XTS_SimplePie_Parser');
