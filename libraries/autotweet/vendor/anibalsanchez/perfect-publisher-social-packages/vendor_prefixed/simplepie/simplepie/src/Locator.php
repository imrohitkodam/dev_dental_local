...
class Locator implements RegistryAware
{
     ...
     */
    public function find(int $type = \XTS_BUILD\SimplePie\SimplePie::LOCATOR_ALL, ?array &$working = null)
    {
        ...
        if ($type & ~\XTS_BUILD\SimplePie\SimplePie::LOCATOR_NONE) {
            $this->get_base();
        }

        if ($type & \XTS_BUILD\SimplePie\SimplePie::LOCATOR_AUTODISCOVERY && $working = $this->autodiscovery()) {
            return $working[0];
        }

        if ($type & (\XTS_BUILD\SimplePie\SimplePie::LOCATOR_LOCAL_EXTENSION | \XTS_BUILD\SimplePie\SimplePie::LOCATOR_LOCAL_BODY | \XTS_BUILD\SimplePie\SimplePie::LOCATOR_REMOTE_EXTENSION | \XTS_BUILD\SimplePie\SimplePie::LOCATOR_REMOTE_BODY) && $this->get_links()) {
            if ($type & \XTS_BUILD\SimplePie\SimplePie::LOCATOR_LOCAL_EXTENSION && $working = $this->extension($this->local)) {
                return $working[0];
            }

            if ($type & \XTS_BUILD\SimplePie\SimplePie::LOCATOR_LOCAL_BODY && $working = $this->body($this->local)) {
                return $working[0];
            }

            if ($type & \XTS_BUILD\SimplePie\SimplePie::LOCATOR_REMOTE_EXTENSION && $working = $this->extension($this->elsewhere)) {
                return $working[0];
            }

            if ($type & \XTS_BUILD\SimplePie\SimplePie::LOCATOR_REMOTE_BODY && $working = $this->body($this->elsewhere)) {
                return $working[0];
            }
        }
        return null;
    ...
    public function get_base()
    {
        ...
        if ($this->dom === null) {
            throw new \XTS_BUILD\SimplePie\Exception('DOMDocument not found, unable to use locator');
        }
    ...
    public function get_rel_link(string $rel)
    {
        ...
        if ($this->dom === null) {
            throw new \XTS_BUILD\SimplePie\Exception('DOMDocument not found, unable to use '.
                                          'locator');
        }
        if (!class_exists('DOMXpath')) {
            throw new \XTS_BUILD\SimplePie\Exception('DOMXpath not found, unable to use '.
                                          'get_rel_link');
        }
...
class_alias('XTS_BUILD\SimplePie\Locator', 'XTS_SimplePie_Locator', false);
