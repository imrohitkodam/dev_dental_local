...
class File implements Response
{
     ...
     */
    public $method = \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_NONE;
    ...
    public function __construct(string $url, int $timeout = 10, int $redirects = 5, ?array $headers = null, ?string $useragent = null, bool $force_fsockopen = false, array $curl_options = [])
    {
        if (function_exists('idn_to_ascii')) {
            $parsed = \XTS_BUILD\SimplePie\Misc::parse_url($url);
            if ($parsed['authority'] !== '' && !ctype_print($parsed['authority'])) {
                ...
                $url = \XTS_BUILD\SimplePie\Misc::compress_parse_url($parsed['scheme'], $authority, $parsed['path'], $parsed['query'], null);
            }
        }
        $this->url = $url;
        ...
        if (preg_match('/^http(s)?:\/\//i', $url)) {
            ...
            if (!$force_fsockopen && function_exists('curl_exec')) {
                $this->method = \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_REMOTE | \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_CURL;
                $fp = curl_init();
                $headers2 = [];
                ...
                if (version_compare(\XTS_BUILD\SimplePie\Misc::get_curl_version(), '7.10.5', '>=')) {
                ...
                curl_setopt($fp, CURLOPT_REFERER, \XTS_BUILD\SimplePie\Misc::url_remove_credentials($url));
                ...
                if (curl_errno($fp)) {
                    ...
                    $responseHeaders = \XTS_BUILD\SimplePie\HTTP\Parser::prepareHeaders((string) $responseHeaders, $info['redirect_count'] + 1);
                    $parser = new \XTS_BUILD\SimplePie\HTTP\Parser($responseHeaders, true);
                    if ($parser->parse()) {
                        ...
                        if ((in_array($this->status_code, [300, 301, 302, 303, 307]) || $this->status_code > 307 && $this->status_code < 400) && ($locationHeader = $this->get_header_line('location')) !== '' && $this->redirects < $redirects) {
                            $this->redirects++;
                            $location = \XTS_BUILD\SimplePie\Misc::absolutize_url($locationHeader, $url);
                            if ($location === false) {
                ...
                $this->method = \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_REMOTE | \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_FSOCKOPEN;
            ...
            $this->method = \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_LOCAL | \XTS_BUILD\SimplePie\SimplePie::FILE_SOURCE_FILE_GET_CONTENTS;
...
class_alias('XTS_BUILD\SimplePie\File', 'XTS_SimplePie_File');
