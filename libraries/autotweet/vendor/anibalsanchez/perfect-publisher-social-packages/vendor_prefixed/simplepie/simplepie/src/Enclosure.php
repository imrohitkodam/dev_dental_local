...
class Enclosure
{
    ...
    public function __construct(
        ...
        if (function_exists('idn_to_ascii')) {
            $parsed = \XTS_BUILD\SimplePie\Misc::parse_url($link ?? '');
            if ($parsed['authority'] !== '' && !ctype_print($parsed['authority'])) {
                ...
                $this->link = \XTS_BUILD\SimplePie\Misc::compress_parse_url($parsed['scheme'], $authority, $parsed['path'], $parsed['query'], $parsed['fragment']);
            }
        }
    ...
    public function get_duration(bool $convert = false)
    {
        if ($this->duration !== null) {
            if ($convert) {
                $time = \XTS_BUILD\SimplePie\Misc::time_hms($this->duration);
                return $time;
            }
    ...
    {
        if ($this->link !== null) {
            $url = \XTS_BUILD\SimplePie\Misc::parse_url($this->link);
            if ($url['path'] !== '') {
...
class_alias('XTS_BUILD\SimplePie\Enclosure', 'XTS_SimplePie_Enclosure');
