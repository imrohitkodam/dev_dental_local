...
namespace XTS_BUILD\Tumblr\API;
...
class RequestHandler
{
    ...
    public function __construct()
    {
        ...
        $this->signatureMethod = new \XTS_BUILD\Eher\OAuth\HmacSha1();
        $this->client = new \XTS_BUILD\GuzzleHttp\Client(array(
    ...
    {
        $this->consumer = new \XTS_BUILD\Eher\OAuth\Consumer($key, $secret);
    }
    ...
    {
        $this->token = new \XTS_BUILD\Eher\OAuth\Token($token, $secret);
    }
    ...
    public function request($method, $path, $options)
    {
        ...
        $oauth = \XTS_BUILD\Eher\OAuth\Request::from_consumer_and_token(
            $this->consumer,
            $this->token,
            $method,
            $url,
        ...
        $uri = new \XTS_BUILD\GuzzleHttp\Psr7\Uri($url);
        $guzzleOptions = [
            'headers' => [
