; (function (jQuery, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        this.theForm = jQuery(this.options.formSelector);

        // Filters
        this.filterButton = jQuery(this.options.formSelector + ' ' + this.options.filterBtnSelector);
        this.filterContainer = jQuery(this.options.formSelector + ' ' + this.options.filterContainerSelector);
        this.filtersHidden = this.options.filtersHidden;

        // List fields
        this.listButton = jQuery(this.options.formSelector + ' ' + this.options.listBtnSelector);
        this.listContainer = jQuery(this.options.formSelector + ' ' + this.options.listContainerSelector);
        this.listHidden = this.options.listHidden;

        // Main container
        this.mainContainer = jQuery(this.options.mainContainerSelector);

        // Search
        this.searchButton = jQuery(this.options.formSelector + ' ' + this.options.searchBtnSelector);
        this.searchField = jQuery(this.options.formSelector + ' ' + this.options.searchFieldSelector);
        this.searchString = null;
        this.clearButton = jQuery(this.options.clearBtnSelector);

        // Ordering
        this.orderCols = jQuery(this.options.formSelector + ' ' + this.options.orderColumnSelector);
        this.orderField = jQuery(this.options.formSelector + ' ' + this.options.orderFieldSelector);

        // Limit
        this.limitField = jQuery(this.options.formSelector + ' ' + this.options.limitFieldSelector);
    ...
    Plugin.prototype = {
        init: function () {
            ...
            var filter_fields = self.getFilterFields();

            filter_fields.each(function (i, element) {
                if (self.checkFilter(element))
                    self.filtersHidden = false;                
            });
            ...
            this.orderCols.click(function () {
                ...
                // Order to set
                var newOrderCol = jQuery(this).attr('data-order');
        ...
        clear: function () {
            ...
            if (self.clearListOptions) {
                ...
                jQuery('#list_limit').val(self.options.defaultLimit);
                if (self.chosenSupport) {
        ...
        activeFilter: function (element) {
            ...
            jQuery(chosenId).addClass('active');
        },
        deactiveFilter: function (element) {
            ...
            var chosenId = '#' + jQuery(element).attr('id') + '_chosen';
            jQuery(chosenId).removeClass('active');
        },
        ...
        hideContainer: function (container) {
            ...
            jQuery(container).removeClass('shown');
        },
        showContainer: function (container) {
            jQuery(container).show('fast');
            jQuery(container).addClass('shown');
        },
        toggleContainer: function (container) {
            if (jQuery(container).hasClass('shown')) {
        ...
        createOrderField: function () {
            ...
            if (!this.orderField.length) {
                this.orderField = jQuery('<input>').attr({
                    type: 'hidden',
        ...
        updateFieldValue: function (field, newValue) {
            ...
            if (type === 'hidden' || type === 'text') {
                ...
                var desiredOption = field.find('option').filter(function () { return jQuery(this).val() == newValue; });
        ...
        findOption: function (select, value) {
            return select.find('option').filter(function () { return jQuery(this).val() == value; });
        }
    };
...
jQuery(document).ready( function () {
    // fix name of the itmes per page dropdown
    jQuery('#limit').attr('name', "list[limit]");
});