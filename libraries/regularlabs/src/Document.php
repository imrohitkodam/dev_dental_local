...
class Document
{
    ...
    public static function getToolbar(): JToolbar
    {
        $doc = self::get();
        return method_exists($doc, 'getToolbar') ? $doc->getToolbar() : JToolbar::getInstance();
    }
    ...
    public static function isAdmin(bool $exclude_login = \false): bool
    {
        ...
        $is_admin = self::isClient('administrator') && (!$exclude_login || !\RegularLabs\Library\User::isGuest()) && \RegularLabs\Library\Input::get('task', '') != 'preview' && !(\RegularLabs\Library\Input::get('option', '') == 'com_finder' && \RegularLabs\Library\Input::get('format', '') == 'json');
