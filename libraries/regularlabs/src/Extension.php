...
class Extension
{
    ...
    public static function cleanCache($group = null)
    {
        $options = ['defaultgroup' => $group ?: JFactory::getApplication()->getInput()->get('option'), 'cachebase' => JFactory::getApplication()->get('cache_path', JPATH_CACHE)];
        try {
            /** @var CallbackController $cache */
            $cache = JFactory::getContainer()->get(CacheControllerFactoryInterface::class)->createCacheController('callback', $options);
            $cache->clean();
    ...
    {
        if (\RegularLabs\Library\User::isGuest()) {
            return \false;
        }
        if (!$require_core_auth) {
            return \true;
        }
        if (!\RegularLabs\Library\User::authorise('core.edit', 'com_content') && !\RegularLabs\Library\User::authorise('core.edit.own', 'com_content') && !\RegularLabs\Library\User::authorise('core.create', 'com_content')) {
            return \false;
        }
        return \true;
    }
    ...
    public static function isInstalled(string $extension, string $type = 'component', string $folder = 'system'): bool
    {
        ...
        switch ($type) {
            ...
            case 'plugin':
                return file_exists(JPATH_PLUGINS . '/' . $folder . '/' . $extension . '/' . $extension . '.xml');
            case 'module':
                $extension = str_replace('mod_', '', $extension);
                return file_exists(JPATH_ADMINISTRATOR . '/modules/mod_' . $extension . '/' . $extension . '.xml') || file_exists(JPATH_ADMINISTRATOR . '/modules/mod_' . $extension . '/mod_' . $extension . '.xml') || file_exists(JPATH_SITE . '/modules/mod_' . $extension . '/' . $extension . '.xml') || file_exists(JPATH_SITE . '/modules/mod_' . $extension . '/mod_' . $extension . '.xml');
            case 'library':
