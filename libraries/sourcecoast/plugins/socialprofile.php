...
class SocialProfilePlugin extends JPlugin
{
            ...
            foreach ($fieldMap as $fieldId => $socialField)
            {
                ...
                if ($value != null && $value != "")
                {
                    ...
                    { // This is a field with multiple, comma separated values
                        ...
                        $value = JFBConnectUtilities::r_implode(', ', $value);
                    }
    ...
    protected function importSocialAvatar()
    {
        ...
        $data = JFBConnectUtilities::getRemoteContent($avatarURL);
        if ($data)
        {
            $baseImgPath = $this->getAvatarPath();
            $tmpImgName = 'scprofile_' . $this->joomlaId . '_pic_tmp.jpg';
            $path = $baseImgPath . '/' . $tmpImgName;
            ...
            if ($type != 2)
            {
                ...
                imagecopyresampled($image_p, $origImage, 0, 0, 0, 0, $width, $height, $width, $height);
    ...
    protected function importCoverPhoto()
    {
        ...
        $data = JFBConnectUtilities::getRemoteContent($cover->get('url'));
    ...
    protected function getAutoUsername($providerProfile)
    {
        ...
        return JFBConnectUtilities::getAutoUsername($providerProfile->get('first_name'), $providerProfile->get('last_name'), $providerProfile->get('email'), $providerPrefix, $this->socialId, $usernamePrefixFormat);
    }
    ...
    <table>
        ...
        foreach ($profileFields as $profileField)
        {
            ...
            foreach ($providers as $provider)
            {
                $this->loadSettings($provider->systemName);
                $fieldMap = $this->getFieldMap($provider->systemName);
                if (property_exists($fieldMap, $profileField->id))
                {
                ...
                $html .= '<select name="profiles_' . $this->profileName . '_' . $provider->systemName . '_field_map' . $profileField->id . '">';
                foreach ($profile->getProviderFields() as $name => $providerField)
                {
                    if ((string) $name == $selectedValue)
                        $selected = 'selected';
                    else
                        $selected = '';

                    if (property_exists($providerField, 'title'))
