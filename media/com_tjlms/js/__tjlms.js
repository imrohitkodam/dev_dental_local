				...
				({
					...
					success: function(data)
					{
						if (data.data)
						{
							...
							var opt = jQuery('<optgroup id="'+ ele.id +'" label="'+ ele.title +'" >');
...
function updateData(plugObject) {
	jQuery.ajax({
...
function closePopup(lessonLaunch, redirectUrl) {
	if (lessonLaunch == 'tab') {
		...
		var c = url.searchParams.get("layout");
		if (c == 'thankyou')
		{
			window.location = redirectUrl;
		}
		...
		{
			window.location = window.location.href + '&layout=thankyou';
		}
	}
}
...
function openAssignRecommendPopups(url) {
