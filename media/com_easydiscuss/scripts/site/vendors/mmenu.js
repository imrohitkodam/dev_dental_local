...
mmenu_oncanvas.prototype.setPage = function (page) {
	...
	if (!page) {
		...
		// Note : The Issue starting from Elegant template then Easydiscuss demo site
		// just in case next day encounter another issue again then need to recheck these 2 template

		// If site uses nav on the top level there could be an issue
		// If it doesn't find out any selector then we need to manually find the selector to join
		if (!configs.page.selector) {
