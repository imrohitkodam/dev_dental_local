ed.define('jquery.atwho', ['edjquery', 'jquery.caret'], function($) {
...
DEFAULT_CALLBACKS = {
	...
	matcher: function(flag, subtext, should_startWithSpace, acceptSpaceBar) {
		...
		regexp = new RegExp(flag + "([A-Za-z" + _a + "-" + _y + "0-9_" + space + "\'\.\+\-" + latin + "]*)$|" + flag + "([^\\x00-\\xff]*)$", 'gi');
	...
	highlighter: function(li, query) {
		...
		regexp = new RegExp(">\\s*(\\w*?)(" + query.replace("+", "\\+") + "\\s*)(\\s*\\w*)\\s*<", 'ig');
...
TextareaController = (function(superClass) {
	...
	TextareaController.prototype.insert = function(content, $li) {
		...
		$inputor.val(text);
		carpos = startStr.length + content.length;

		if (content == '/label ~ ') {
			carpos = carpos - 1;
		}

		$inputor.caret('pos', carpos, {
			iframe: this.app.iframe
		});
		if (!$inputor.is(':focus')) {
			$inputor.focus();
		}
		// $inputor.trigger('paste');
