...
var BskyValidation = Core.ExtlyModel.extend({
  url: function () {
    return Core.SefHelper.route('index.php?option=com_autotweet&view=channels&task=getBskyValidation');
  }
});
/*!
 * @package     Extly.Solutions
...
var MyBusinessValidation = Core.ExtlyModel.extend({
  url: function () {
    return Core.SefHelper.route('index.php?option=com_autotweet&view=channels&task=getMyBusinessValidation');
  }
});
/*!
 * @package     Extly.Solutions
...
var TwitterV2Validation = Core.ExtlyModel.extend({
  url: function () {
    return Core.SefHelper.route('index.php?option=com_autotweet&view=channels&task=getTwitterV2Validation');
  }
});
/*!
 * @package     Extly.Solutions
...
var WebpushrValidation = Core.ExtlyModel.extend({
  url: function () {
    return Core.SefHelper.route('index.php?option=com_autotweet&view=channels&task=getWebpushrValidation');
  }
});
/*!
 * @package     Extly.Solutions
...
var BskyValidations = Backbone.Collection.extend({
  model: BskyValidation
});
/*!
 * @package     Extly.Solutions
 * @subpackage  com_perfectpub - Publish your content easily and engage your audience.
...
var MyBusinessValidations = Backbone.Collection.extend({
  model: MyBusinessValidation
});
/*!
 * @package     Extly.Solutions
 * @subpackage  com_perfectpub - Publish your content easily and engage your audience.
...
var TwitterV2Validations = Backbone.Collection.extend({
  model: TwitterV2Validation
});
/*!
 * @package     Extly.Solutions
 * @subpackage  com_perfectpub - Publish your content easily and engage your audience.
...
var WebpushrValidations = Backbone.Collection.extend({
  model: WebpushrValidation
});
...
var FbChannelView = Backbone.View
  .extend({
    ...
    onChangeChannel: function () {
      ...
      // Disable Show Url on Facebook Channels
...
var FbValidationView = Backbone.View.extend({
  events: {
    "click #fbauth-tab": "onFbAsyncInit",
    "click #fbvalidationbutton": "onValidationReq",
    "click #fbchannel-tab": "onCheckFbApp",
  ...
  onChangeOwnApi: function (e) {
    ...
    if (authorizeCanvas) {
      ...
      this.$("#canvas_page")
        ...
        .removeClass("validate-facebookapp");
  ...
  onCheckFbApp: function () {
    var view = this;
    var fbappTab = view.$("#fbapp-tab");
    var fbauthTab = view.$("#fbauth-tab");
    var fbchannelTab = view.$("#fbchannel-tab");
    ...
    if (!_.isEmpty(id) && !_.isEmpty(s) && s.length > 30) {
      fbauthTab.removeClass("disabled");
      fbchannelTab.removeClass("disabled");
    }

    if (fbauthTab.hasClass("disabled")) {
      setTimeout(function () {
        fbappTab.show();
      }, 100);
  ...
  getPermsScope: function (view) {
    var requireGroupsPerms = view.$("#fb_api_perms_groups").val() === "1";
    var requireInstagramPerms = view.$("#fb_api_perms_instagram").val() === "1";

    if (requireGroupsPerms) {
      return view.$("#fb_api7_perms_groups_detail").val();
    }

    if (requireInstagramPerms) {
      return view.$("#fb_perms_instagram_detail").val();
    }

    return view.$("#fb_api7_perms").val();
  },
  ...
  onFbAsyncInit: function () {
    ...
    var fbScope = view.getPermsScope(view);
  ...
  // Same method in file fbextend.js
  fbStatusChangeCallback: function (response) {
    const view = this;
...
var MyBusinessValidationView = Backbone.View.extend({
  events: {
    'click #mybusinessvalidationbutton': 'onValidationReq'
  },
  ...
  onValidationReq: function () {
    var view = this,
      ...
      channelId = view.$('#channel_id').val().trim(),
      developerKey = view.$('#client_id').val().trim(),
      clientSecret = view.$('#client_secret').val().trim(),

      token = view.$('#XTtoken').attr('name');

    view.$('#channel_id').val(channelId);
    view.$('#client_id').val(developerKey);
    view.$('#client_secret').val(clientSecret);
...
var TwitterV2ValidationView = Backbone.View.extend({
  events: {
    'click #twitterv2validationbutton': 'onValidationReq'
  },
