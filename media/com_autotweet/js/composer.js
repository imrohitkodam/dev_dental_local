...
angular
  ...
  .controller(
    ...
    function ($scope, $rootScope, $sce, Requests, Agenda,
      ...
      _this.success = function (response) {
        ...
        if ((response.status) && (_this.redirectOnSuccess)) {
          if (Joomla.submitform) {
            return Joomla.submitform('cancel');
          }

          return Joomla.submitbutton('cancel');
        }
      ...
      _this.resetToolbarButtonListener = function (id) {
        var toolbarSaveNewDiv = document.getElementById(id);

        if (!toolbarSaveNewDiv) {
          return;
        }

        var buttonInsideDiv = toolbarSaveNewDiv.querySelector('button');

        if (!buttonInsideDiv) {
          return;
        }

        var clonedButton = buttonInsideDiv.cloneNode(true);
        buttonInsideDiv.parentNode.replaceChild(clonedButton, buttonInsideDiv);
      }
      ...
      _this.resetToolbarButtonListener('toolbar-apply');
      ...
      _this.resetToolbarButtonListener('toolbar-save');
      ...
      _this.resetToolbarButtonListener('toolbar-save-new');
      ...
      holder = document.getElementById('XTF0FHeaderHolder');
