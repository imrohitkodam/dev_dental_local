FD50.plugin("bootstrap", function($) {
...
+function ($) { "use strict";
	...
	// COLLAPSE DATA-API
	// =================
	$(document).on('click.bs.collapse.data-api', '[data-eb-toggle=collapse]', function (e) {
		var $this   = $(this), href
		...
		if (!data || !data.transitioning) {
			if ($parent) $parent.find('[data-eb-toggle=collapse][data-parent="' + parent + '"]').not($this).addClass('collapsed')
...
+function ($) { "use strict";
	...
	function clearMenus() {
		// Fixed duplicate background remove method with ES bootstrap. #1467
		// Also to fix the backdrop not clearing on mobile. #2957
		if (!window["Foundry/Bootstrap"] || $(backdrop).length > 0) {
			$(backdrop).remove()
		}
...
+function ($) { "use strict";
	...
	, trigger: 'click'
	, content: ''
	, template: '<div id="eb" class="eb-popover"><div class="arrow"></div><h3 class="eb-popover-title"></h3><div class="eb-popover-content"></div></div>'
	})
	...
	Popover.prototype.setContent = function () {
		...
		$tip.find('.eb-popover-title')[this.options.html ? 'html' : 'text'](title)
		$tip.find('.eb-popover-content')[this.options.html ? 'html' : 'text'](content)

		$tip.removeClass('fade top bottom left right in')

		// IE8 doesn't accept hiding via the `:empty` pseudo selector, we have to do
		// this manually by checking the contents.
		if (!$tip.find('.eb-popover-title').html()) $tip.find('.eb-popover-title').hide()
	}
...
/* ========================================================================
 * Custom events for easyblog
 * ========================================================================
 * This is used throughout the front end and back end of EasyBlog
 * ======================================================================== */
...
$(document).on('mouseover.tooltip.data-eb-api', '[data-eb-provide=tooltip]', function(event) {

	var element = $(this);
	var isMobile = element.parents('#eb').is('.w320, .w480');
	var html = element.data('html') !== undefined ? element.data('html') : false;
	...
	element
		.tooltip({
			...
			html: html,
			animation: false,
			template: '<div id="eb" class="eb tooltip tooltip-eb"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
			container: 'body'
		})
		.tooltip("show");
});
