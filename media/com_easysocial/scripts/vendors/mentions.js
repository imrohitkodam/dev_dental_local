...
$.Controller("Mentions", {
	...
	*/
	...
	reflect: function() {
		...
		// Resetting the input if required. #5165
		if (self.resetInput === true) {
			caretAfter.start = wholeText.length;
			caretAfter.end = wholeText.length;
		}
...
$.Controller("Mentions.Autocomplete", {
	...
	use: function(item) {
		...
		// add space character after selected a mentions.
		self.addSpace();
	},

	addSpace: function() {

		var markers = self.mentions.getMarkers();

		var lastMarker = markers[markers.length-1];

		// spawn a next marker
		var marker = lastMarker.spawn();
