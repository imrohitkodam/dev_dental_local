...
class HoursFieldWidgetsItem extends EasySocial
{
	 ...
	 * @access  public
	 */
	public function coverFooter(SocialGroup $group, SocialTableField $field)
	{
		$output = $this->processData($group, $field);
	 ...
	 * @access  public
	 */
	public function processData($group, $field, $layout = 'display')
	{
		$theme = ES::themes();
		...
		// Get the field value
		$data = $field->getData($group->id, $group->getType());
		...
		if ($data->hours_type == 'selected' && $data->days) {
			...
			if (in_array($currentDay, $openDays)) {
				...
				if (!$isLegacy && $hasMultiple) {
					...
					$openingString .= ' ' . (isset($data->days->$currentDay->start_period) ? (string) $data->days->$currentDay->start_period : '');
					$closingString .= ' ' . (isset($data->days->$currentDay->end_period) ? (string) $data->days->$currentDay->end_period : '');
					...
					if ($timeFormat == 'h:iA') {
						if (!isset($data->days->$currentDay->start_period)) {
							$data->days->$currentDay->start_period = $opening->format('a');
						}
						if (!isset($data->days->$currentDay->end_period)) {
							$data->days->$currentDay->end_period = $closing->format('a');
						}
					} else {
						$data->days->$currentDay->start_period = '';
						$data->days->$currentDay->end_period = '';
					}
		...
		$output = $theme->output('fields/group/hours/widgets/' . $layout);
	...
	public function getTodayDetails($data, $timeFormat)
	{
		...
		for ($t = 0; $t < $count; $t++) {
			...
			$dayOpeningString .= ' ' . (isset($data->days->$today->start_period[$t]) ? $data->days->$today->start_period[$t] : '');
			$dayClosingString .= ' ' . (isset($data->days->$today->end_period[$t]) ? $data->days->$today->end_period[$t] : '');

			$dayOpening = JFactory::getDate($dayOpeningString);
			$dayClosing = JFactory::getDate($dayClosingString);

			if ($timeFormat == 'h:iA') {
				if (!isset($data->days->$today->start_period[$t])) {
					$data->days->$today->start_period[$t] = $dayOpening->format('a');
				}
				if (!isset($data->days->$today->end_period[$t])) {
					$data->days->$today->end_period[$t] = $dayClosing->format('a');
				}
			} else {
				$data->days->$today->start_period[$t] = '';
				$data->days->$today->end_period[$t] = '';
			}
	...
	public function getFullDetails($data, $timeFormat, $open)
	{
		...
		for ($i=1; $i <= 7; $i++) {
			...
			// Fixed issue with sunday is being treated as '7' instead of '0'. #3447
			if ($i == 7) {
				$day = 0;
			}
			...
			// 2 - tuesday
			// and so on..
			$dayString = JFactory::getDate()->dayToString($day);
			...
			if (!$isLegacy && $hasMultiple) {
				...
				for ($t = 0; $t < $count; $t++) {
					...
					$dayOpeningString .= ' ' . (isset($data->days->$i->start_period) ? $data->days->$i->start_period[$t] : '');
					$dayClosingString .= ' ' . (isset($data->days->$i->end_period) ? $data->days->$i->end_period[$t] : '');

					$dayOpening = JFactory::getDate($dayOpeningString);
					$dayClosing = JFactory::getDate($dayClosingString);

					if ($timeFormat == 'h:iA') {
						if (!isset($data->days->$i->start_period[$t])) {
							$data->days->$i->start_period[$t] = $dayOpening->format('a');
						}

						if (!isset($data->days->$i->end_period[$t])) {
							$data->days->$i->end_period[$t] = $dayClosing->format('a');
						}
					} else {
						$data->days->$i->start_period[$t] = '';
						$data->days->$i->end_period[$t] = '';
					}
					...
					// we need to check if the closing is the next day
					if ($dayClosing->toSql() < $dayOpening->toSql()) {
						$tomorrowString = JFactory::getDate()->dayToString($day + 1, true);
						$string .= ' (' . $tomorrowString . ')';
					}
			...
			$dayOpeningString .= ' ' . (isset($data->days->$i->start_period) ? $data->days->$i->start_period : '');
			$dayClosingString .= ' ' . (isset($data->days->$i->end_period) ? $data->days->$i->end_period : '');
			...
			if ($timeFormat == 'h:iA') {
				if (!isset($data->days->$i->start_period)) {
					$data->days->$i->start_period = $dayOpening->format('a');
				}
				if (!isset($data->days->$i->end_period)) {
					$data->days->$i->end_period = $dayClosing->format('a');
				}
			} else {
				$data->days->$i->start_period = '';
				$data->days->$i->end_period = '';
			}
			...
			if ($dayClosing->toSql() < $dayOpening->toSql()) {
				...
				$tomorrowString = JFactory::getDate()->dayToString($tomorrowIdx, true);
	...
	public function getNextOpen($data, $timeFormat)
	{
		...
		if (isset($data->days->$today)) {
			...
			// reset the values
			if (!$isLegacy && !$hasMultiple) {
				$data = $this->resetDataArray($data, $today);
			}
			...
			$closingString .= ' ' . (isset($data->days->$today->end_period) ? $data->days->$today->end_period : '');	

			$closing = JFactory::getDate($closingString);

			if ($timeFormat == 'h:iA') {
				if (!isset($data->days->$today->end_period)) {
					$data->days->$today->end_period = $closing->format('a');
				}
			} else {
				$data->days->$today->end_period = '';
			}
		...
		$nextOpeningString .= ' ' . (isset($data->days->$nextOpen->start_period) ? $data->days->$nextOpen->start_period : '');

		$nextOpening = JFactory::getDate($nextOpeningString . ' +' . $count . ' day');

		if ($timeFormat == 'h:iA') {
			if (!isset($data->days->$nextOpen->start_period)) {
				$data->days->$nextOpen->start_period = $nextOpening->format('a');
			}
		} else {
			$data->days->$nextOpen->start_period = '';
		}
	...
	public function today($data, $timeFormat)
	{
		...
		$todayOpeningString .= ' ' . (isset($data->days->$today->start_period) ? $data->days->$today->start_period : '');
		$todayClosingString .= ' ' . (isset($data->days->$today->end_period) ? $data->days->$today->end_period : '');
		...
		if ($timeFormat == 'h:iA') {
			if (!isset($data->days->$today->start_period)) {
				$data->days->$today->start_period = $todayOpening->format('a');
			}
			if (!isset($data->days->$today->end_period)) {
				$data->days->$today->end_period = $todayClosing->format('a');
			}
		} else {
			$data->days->$today->start_period = '';
			$data->days->$today->end_period = '';
		}
	...
	public function yesterday($data, $timeFormat)
	{
		...
		$yesterdayOpeningString .= ' ' . (isset($data->days->$yesterday->start_period) ? $data->days->$yesterday->start_period : '');
		$yesterdayClosingString .= ' ' . (isset($data->days->$yesterday->end_period) ? $data->days->$yesterday->end_period : '');
		...
		if ($timeFormat == 'h:iA') {
			if (!isset($data->days->$yesterday->start_period)) {
				$data->days->$yesterday->start_period = $yesterdayOpening->format('a');
			}
			if (!isset($data->days->$yesterday->end_period)) {
				$data->days->$yesterday->end_period = $yesterdayClosing->format('a');
			}
		} else {
			$data->days->$yesterday->start_period = '';
			$data->days->$yesterday->end_period = '';
		}
