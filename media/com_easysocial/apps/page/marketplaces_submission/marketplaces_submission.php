...
class SocialPageAppMarketplaces_submission extends SocialAppItem
{
	/**
	...
	public function onPrepareStoryPanel($story)
	{
		...
		// Ensure that marketplace in page is allowed
		$page = ES::page($story->cluster);

		// Check if user are allowed to create event.
		if (!$page->canCreateListing() || !$page->canAccessMarketplaces()) {
			return;
		}
	...
	public function onBeforeStorySave(&$template, &$stream, &$content)
	{
		...
		// The author will follow the stream Post As value
		$listing->post_as = $template->post_as ? $template->post_as : SOCIAL_TYPE_USER;
