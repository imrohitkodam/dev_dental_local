...
class SocialPageAppFeedsHookNotificationComments
{
	...
	public function execute(&$item)
	{
		...
		if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
			$title = JText::sprintf('APP_PAGE_FEEDS_NOTIFICATIONS_USER_REPLIED_COMMENT_ON_FEED', ES::user($item->actor_id)->getName(), $page->getTitle());


			if ($stream->actor_id == $item->target_id) {
				$title = JText::sprintf('APP_PAGE_FEEDS_NOTIFICATIONS_USER_REPLIED_COMMENT_ON_YOUR_FEED', ES::user($item->actor_id)->getName());
			}
