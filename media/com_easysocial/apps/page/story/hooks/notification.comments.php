...
class SocialPageAppStoryHookNotificationComments
{
	...
	public function execute(SocialTableNotification &$item)
	{
		...
		// The stream could have been deleted. We need to ensure that it isn't processed
		if (!$stream->id) {
			return;
		}
		...
		if ($item->context_type == 'story.page.create') {
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_POST_PAGE', ES::user($item->actor_id)->getName(), $page->getTitle());


				if ($stream->actor_id == $item->target_id) {
					$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_YOUR_POST_PAGE', ES::user($item->actor_id)->getName(), $page->getTitle());
				}
			...
			$string = ES::string()->computeNoun('APP_PAGE_STORY_USER_POSTED_COMMENT_ON_USERS_POST', count($users));
		...
		if ($item->context_type == 'links.page.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_LINK', ES::user($item->actor_id)->getName(), $page->getTitle(), $link->title);


				if ($stream->actor_id == $item->target_id) {
					$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_YOUR_LINK', ES::user($item->actor_id)->getName(), $page->getTitle(), $link->title);
				}
			...
			// We need to generate the notification message differently for the author of the item and the recipients of the item.
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$string = ES::string()->computeNoun('APP_PAGE_STORY_USER_COMMENTED_ON_LINK_' . strtoupper($stream->post_as), count($users));
				$item->title = JText::sprintf($string, $names, $page->getName(), $link->title);
				return $item;
			}

			// This is for 3rd party viewers
			$string = ES::string()->computeNoun('APP_PAGE_STORY_USER_COMMENTED_ON_USERS_LINK', count($users));
			$item->title = JText::sprintf($string, $names, ES::user($stream->actor_id)->getName(), $page->getName(), $link->title);
	...
	private function notificationPhotos($names, $users, &$item)
	{
		...
		if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
			$title = JText::sprintf('APP_PAGE_STORY_USER_COMMENTED_ON_SHARED_PHOTO', ES::user($item->actor_id)->getName(), $page->getTitle());


			if ($stream->actor_id == $item->target_id) {
				$title = JText::sprintf('APP_PAGE_STORY_USER_COMMENTED_ON_YOUR_SHARED_PHOTO', ES::user($item->actor_id)->getName(), $page->getTitle(), ES::user($stream->actor_id)->getName());
			}
