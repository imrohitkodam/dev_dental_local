	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'news.page.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_ANNOUNCEMENT', ES::user($item->actor_id)->getName(), $page->getTitle(), ES::user($news->created_by)->getName());


				if ($news->created_by == $item->target_id) {
					$title = JText::sprintf('APP_PAGE_STORY_USER_REPLIED_COMMENT_ON_YOUR_ANNOUNCEMENT', ES::user($item->actor_id)->getName(), $page->getTitle());
				}
