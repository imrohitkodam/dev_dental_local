	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'news.group.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_GROUP_NEWS_USER_REPLIED_COMMENT_ON_YOUR_ANNOUNCEMENT', ES::user($item->actor_id)->getName(), $group->getTitle(), ES::user($news->created_by)->getName());


				if ($news->created_by == $item->target_id) {
					$title = JText::sprintf('APP_GROUP_NEWS_USER_REPLIED_COMMENT_ON_YOUR_ANNOUNCEMENT', ES::user($item->actor_id)->getName(), $group->getTitle());
				}
			...
			$item->title = JText::sprintf('APP_GROUP_NEWS_USER_COMMENTED_ON_USERS_ANNOUNCEMENT', $names, ES::user($news->created_by)->getName(), $group->getName());
