	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'files.group.uploaded') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_GROUP_FILES_USER_REPLIED_COMMENT_ON_FILE', ES::user($item->actor_id)->getName(), $group->getTitle(), ES::user($file->user_id)->getName());


				if ($file->user_id == $item->target_id) {
					$title = JText::sprintf('APP_GROUP_FILES_USER_REPLIED_COMMENT_ON_YOUR_FILE', ES::user($item->actor_id)->getName(), $group->getTitle());
				}
			...
			$item->title = JText::sprintf('APP_GROUP_FILES_USER_COMMENTED_ON_USERS_FILE', $names, ES::user($file->user_id)->getName(), $group->getName());
