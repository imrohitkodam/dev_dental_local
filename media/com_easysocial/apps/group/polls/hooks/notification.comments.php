	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'polls.group.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_GROUP_POLLS_USERS_REPLIED_COMMENT_ON_POLL', ES::user($item->actor_id)->getName(), $poll->title, $group->getTitle(), ES::user($poll->created_by)->getName());


				if ($poll->created_by == $item->target_id) {
					$title = JText::sprintf('APP_GROUP_POLLS_USERS_REPLIED_COMMENT_ON_YOUR_POLL', ES::user($item->actor_id)->getName(), $poll->title, $group->getTitle());
				}
			...
			if ($poll->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				...
				$item->title = JText::sprintf($langString, $names, $poll->title, $group->getTitle());
