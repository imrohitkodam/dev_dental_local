	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'news.event.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_NEWS_USER_REPLIED_COMMENT_ON_ANNOUNCEMENT', ES::user($item->actor_id)->getName(), ES::user($event->creator_uid)->getName(), $event->getTitle());

				if ($event->creator_uid == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_NEWS_USER_REPLIED_COMMENT_ON_YOUR_ANNOUNCEMENT', ES::user($item->actor_id)->getName());
				}
