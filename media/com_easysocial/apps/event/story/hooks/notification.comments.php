...
class SocialEventAppStoryHookNotificationComments
{
	...
	public function execute(SocialTableNotification &$item)
	{
		...
		if ($item->context_type == 'story.event.create') {
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_POST', ES::user($item->actor_id)->getName(), $event->getTitle(), ES::user($stream->actor_id)->getName());

				if ($stream->actor_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_YOUR_POST', ES::user($item->actor_id)->getName(), $event->getTitle());
				}
		...
		if ($item->context_type == 'links.event.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_LINK', ES::user($item->actor_id)->getName(), $event->getTitle(), $link->title, ES::user($stream->actor_id)->getName());

				if ($stream->actor_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_YOUR_LINK', ES::user($item->actor_id)->getName(), $event->getTitle(), $link->title);
				}
	...
	private function notificationPhotos($names, $users, &$item)
	{
		...
		if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
			$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_SHARED_PHOTO', ES::user($item->actor_id)->getName(), $event->getTitle(), ES::user($stream->actor_id)->getName());

			if ($stream->actor_id == $item->target_id) {
				$title = JText::sprintf('APP_EVENT_STORY_USER_REPLIED_COMMENT_ON_YOUR_SHARED_PHOTO', ES::user($item->actor_id)->getName(), $event->getTitle());
			}
