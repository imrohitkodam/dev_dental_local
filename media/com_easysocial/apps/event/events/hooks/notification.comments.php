...
class SocialEventAppEventsHookNotificationComments
{
	public function execute($item)
	{
		...
		if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
			$title = JText::sprintf('APP_USER_EVENTS_USER_REPLIED_COMMENT_ON_EVENT', ES::user($item->actor_id)->getName(), $owner->getName());

			if ($owner->id == $item->target_id) {
				$title = JText::sprintf('APP_USER_EVENTS_USER_REPLIED_COMMENT_ON_YOUR_EVENT', ES::user($item->actor_id)->getName());
			}
		...
		if ($item->target_type === SOCIAL_TYPE_USER && $item->target_id == $owner->id) {
			$item->title = JText::sprintf(ES::string()->computeNoun('APP_USER_EVENTS_USER_COMMENTED_ON_YOUR_EVENT_' . strtoupper($verb), count($users)), $names);

			return $item;
		}

		$item->title = JText::sprintf(ES::string()->computeNoun('APP_USER_EVENTS_USER_COMMENTED_ON_USERS_EVENT_' . strtoupper($verb), count($users)), $names, $owner->getName());
