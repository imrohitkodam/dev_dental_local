	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'albums.event.create') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_PHOTO_ALBUM', ES::user($item->actor_id)->getName(), $album->get('title'), ES::user($album->user_id)->getName());

				if ($album->user_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_YOUR_PHOTO_ALBUM', ES::user($item->actor_id)->getName(), $album->get('title'));
				}
		...
		if ($item->context_type == 'stream.event.upload') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_PHOTO_SHARED_ON_THE_STREAM', ES::user($item->actor_id)->getName(), ES::user($photo->user_id)->getName());

				if ($photo->user_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_YOUR_PHOTO_SHARED_ON_THE_STREAM', ES::user($item->actor_id)->getName());
				}
			...
			if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$string = ES::string()->computeNoun('APP_EVENT_PHOTOS_NOTIFICATIONS_COMMENTED_YOUR_PHOTO_SHARED_ON_THE_STREAM', count($users));
		...
		if ($item->context_type == 'photos.event.updateCover') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_EVENT_COVER', ES::user($item->actor_id)->getName(), $event->getTitle());

				if ($photo->user_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_YOUR_EVENT_COVER', ES::user($item->actor_id)->getName(), $event->getTitle());
				}
			...
			// We need to determine if the user is the owner
			if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_COMMENTED_ON_YOUR_EVENT_COVER', $names, $event->getTitle());

				return;
			}

			// For other users, we just post a generic message
			$item->title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_COMMENTED_ON_COVER', $names, $event->getTitle());
		...
		if ($item->context_type == 'photos.event.uploadAvatar') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_YOUR_EVENT_AVATAR', ES::user($item->actor_id)->getName(), $event->getTitle());

				if ($photo->user_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_AVATAR', ES::user($item->actor_id)->getName(), $event->getTitle());
				}
			...
			// We need to determine if the user is the owner
			if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_COMMENTED_ON_YOUR_EVENT_AVATAR', $names, $event->getTitle());

				return;
			}

			// For other users, we just post a generic message
			$item->title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_COMMENTED_ON_AVATAR', $names, $event->getTitle());
		...
		if ($item->context_type == 'photos.event.upload' || $item->context_type == 'photos.event.add') {
			...
			if ($item->cmd == 'comments.replied' && $item->target_type == SOCIAL_TYPE_USER) {
				$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_PHOTO', ES::user($item->actor_id)->getName(), ES::user($photo->user_id)->getName());

				if ($photo->user_id == $item->target_id) {
					$title = JText::sprintf('APP_EVENT_PHOTOS_NOTIFICATIONS_REPLIED_COMMENT_ON_YOUR_PHOTO', ES::user($item->actor_id)->getName());
				}
