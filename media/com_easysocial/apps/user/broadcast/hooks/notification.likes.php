...
class SocialUserAppStoryHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'comments.user.like') {
            ...
            if ($comment->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_YOUR_COMMENT_ITEM', count($users));
            ...
            if ($item->actor_id == $comment->created_by && count($users) == 1) {
                ...
                $item->title    = JText::sprintf('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_USERS_COMMENT_ITEM' . FD::user($item->actor_id)->getGenderLang(), FD::user($item->actor_id)->getName());

                return;
            }

            // For other users, we just post a generic message
            $langString     = FD::string()->computeNoun('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_USERS_COMMENT_ITEM', count($users));
            $item->title    = JText::sprintf($langString, $names, FD::user($comment->created_by)->getName());
            return;
        }
        ...
        if ($item->context_type == 'story.user.create') {
            ...
            if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_YOUR_STATUS_UPDATE', count($users));
                $item->title    = JText::sprintf($langString, $names);
            ...
            if ($item->actor_id == $stream->actor_id && count($users) == 1) {
                ...
                // We do not need to pluralize here since we know there's always only 1 recipient
                $item->title    = JText::sprintf('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_STATUS_UPDATE' . FD::user($item->actor_id)->getGenderLang(), FD::user($stream->actor_id)->getName());

                return;
            }

            // For other users, we just post a generic message
            $langString     = FD::string()->computeNoun('APP_USER_STORY_NOTIFICATIONS_USER_LIKES_USERS_STATUS_UPDATE', count($users));
