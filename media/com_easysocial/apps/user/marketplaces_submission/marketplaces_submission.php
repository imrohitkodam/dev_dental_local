...
class SocialUserAppMarketplaces_submission extends SocialAppItem
{
	/**
	...
	public function onPrepareStoryPanel($story)
	{
		...
		// Ensure that the user has access to create listing
		if (!$this->my->getAccess()->get('marketplaces.sell') || !$this->getApp()->hasAccess($this->my->profile_id)) {
			return;
		}
	...
	public function onBeforeStorySave(&$template, &$stream, &$content)
	{
		...
		$photos = $in->get('marketplace_photos', [], 'default');
		...
		$template->setParams(json_encode($params));
	}
}
