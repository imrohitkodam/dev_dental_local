...
class JFBConnectProfileVk extends JFBConnectProfile
{
    ...
    */
    public function fetchProfile($userId, $fields)
    {
        ...
        if (!empty($fields))
        {
            ...
            //we set the email since it was provided in the token
    ...
    */
    public function fetchStatus($providerUserId)
    {
        ...
        $url .= '&count=1&extended=0&filter=owner';
        $url .= '&v=' . $this->apiVersion;
        try
        {
            $data = $this->provider->client->query($url);
        ...
        {
            //TODO: add config?
            $status .= '<br /><a target="_blank" href="https://vk.com/wall'.$providerUserId.'_'.$status_id.'">'.JText::_('COM_JFBCONNECT_FACEBOOK_STATUS_VIEW_ORIGINAL_POST').'</a>';
        }
