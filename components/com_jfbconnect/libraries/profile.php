...
class JFBConnectProfile
{
    ...
    {
        $this->providerFields = ['0' => 'None'];
        $this->providerFields = array_merge($this->providerFields, (array)$this->provider->networkSettings->get('profile_fields'));
    }

    public function getPermissionsForFields($fields)
    {
        $perms = array();
        $availableFields = $this->provider->networkSettings->get('profile_fields');
        if (!empty($availableFields) && !empty($fields))
        {

            foreach ($fields as $field)
            {
                foreach ($availableFields as $key => $af)
                {
                    if ($key == $field)
                    {
                        if (property_exists($af, 'scope'))
                            $perms[] = $af->scope;
                        break;
                    }
                }
