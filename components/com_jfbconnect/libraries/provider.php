...
class JFBConnectProvider extends JObject
{
    ...
    function __construct()
    {
        ...
        $networkSettings = $this->configModel->get($this->systemName . "_autotune_network_settings");
        $registry = new JRegistry();
        $registry->loadObject($networkSettings);
        $this->networkSettings = $registry;
    ...
    */
    ...
    {
        $currentScope = $this->getUserScope($uid);
    ...
    {
        $neededScope = array();
        if (empty($scope)) // Don't do any checks if no scope is necessary
            return $neededScope;

        // get current scope for the user
        $token = $this->client->getToken();
        $currentScope = $this->getUserScope($this->getProviderUserId(), $token);
        foreach ($scope as $s)
        {
    ...
    public function getLoginButtonWithImage($params, $loginClass, $loginID)
    {
        ...
        $buttonAltTitle = JText::sprintf('COM_JFBCONNECT_LOGIN_USING_SOCIAL', $this->name);
    ...
    public function getLoginButtonInfo()
    {
        return new LoginButtonInfo($this->name, $this->loginPrefix);
    }
    ...
    */
    ...
    /**
     * Return any information about the specified scope gathered from Autotune
     * @param $scope
     */
    public function getScopeDetails($scope)
    {
        if ($this->networkSettings->exists('scopes.' . $scope))
            return $this->networkSettings->get('scopes.' . $scope);

        $response = new stdClass();
        $response->description = "No details available";
        return $response;
    }
}