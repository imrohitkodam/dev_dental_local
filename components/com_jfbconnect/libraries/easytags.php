...
class JFBConnectEasytags
{
    ...
    public function replaceGraphTags($contents)
    {
        ...
        //Check to see if we need to exclude tags for this component
        $currentComponent = JRequest::getCmd('option');
        $blockComponents = array();
        if(!empty($skipComponents))
        {
            //$blockComponents = unserialize($skipComponents);//we do the unserialize in the config model
            $blockComponents = array_map('trim', $skipComponents); //Trim whitespace from each array element
        }

        if(empty($skipComponents) || !in_array($currentComponent, $blockComponents))
        {
