...
class JFBConnectProviderLinkedin extends JFBConnectProvider
{
    ...
    function setupAuthentication()
    {
        ...
        $options->set('authurl', 'https://www.linkedin.com/oauth/v2/authorization');
        $options->set('tokenurl', 'https://www.linkedin.com/oauth/v2/accessToken');
        $options->set('authmethod', 'bearer');
        $options->set('getparam', 'access_token');
    ...
    function fetchNewScope($scope)
    {
        ...
        if (!$alreadyFetching) {
            ...
            $this->client->setOption('scope', implode(' ', $this->profile->getRequiredScope()) . ' ' . implode(' ', $scope));
