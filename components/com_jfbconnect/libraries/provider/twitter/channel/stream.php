...
class JFBConnectProviderTwitterChannelStream extends JFBConnectChannel
{
    public function setup()
    {
        ...
        $this->postCharacterMax = $this->provider->networkSettings->get('channel_char_count', 140);
        $this->urlLength = $this->provider->networkSettings->get('channel_url_length', 23);
        $this->postSuccessMessage = 'COM_JFBCONNECT_CHANNELS_TWITTER_STREAM_POST_SUCCESS';
    }
    ...
    public function getStream($stream)
    {
        ...
        if ($feed === false)
        {
            ...
            $data = array();
            $data['include_rts'] = $this->options->get('include_rts') ? 'true' : 'false';

            $path = $this->provider->options->get('api.url') . 'statuses/user_timeline.json';
            $feedResponse = $this->provider->client->oauthRequest($path, 'GET', $params, $data);
    ...
    public function performPost(JRegistry $data)
    {
        ...
        $urlLength = $this->urlLength; // +1 for https
        $status = substr($message, 0, $this->postCharacterMax - $urlLength - 1) . " " . $link;
