...
class JFBConnectUtilities
{
    ...
    static function getExtraShareButtons($url, $dataCount, $size, $showFacebookLikeButton, $showFacebookShareButton, $showTwitterButton, $renderKeyString, $showLinkedInButton = false, $fbWidth = 50, $showPinterestButton = false, $pinnedImage = "", $pinnedText = "")
    {
        ...
        {
            $extraButtonText .= '{JLinkedShare href=' . $url . $renderKeyString . '}';
        }
        if ($showTwitterButton)
        {
            ...
            $extraButtonText .= '{SCTwitterShare href=' . $url . " layout=" . $dataCount . $sizeParam . $renderKeyString . '}';
        }
        ...
        if ($showFacebookLikeButton)
        {
            ...
            $extraButtonText .= '{JFBCLike href=' . $url . " layout=" . $dataCount . ' size=' . $size .' share=' . $shareString . $renderKeyString . '}';
        }
    ...
    {
        $href = JFBConnectUtilities::forceScheme(JURI::current());
    ...
    static function stripQuery($juri)
    {
    ...
    static function getLinkFromMenuItem($itemId, $isLogout)
    {
        ...
        if ($item)
        {
            ...
            else if ($item->type == 'alias') //Alias menu item
            {
                ...
                if ($isLogout && JFBConnectUtilities::isMenuRegistered($aliasedId))
                    $link = 'index.php';
                else
            ...
            {
                if ($isLogout && JFBConnectUtilities::isMenuRegistered($itemId))
                    $link = 'index.php';
                else
    ...
    static function r_implode($glue, $pieces)
    {
        foreach ($pieces as $r_pieces)
        {
            if (is_array($r_pieces))
            {
                ...
                $retVal[] = JFBConnectUtilities::r_implode($glue, $r_pieces);
            }
            else
            {
    ...
    static function getAutoUsername($firstName, $lastName, $email, $genericPrefixStr, $socialUserId, $usernamePrefixFormat)
    {
        ...
        else if ($usernamePrefixFormat == AUTONAME_FIRSTLAST) //first.last
        {
            ...
            $firstName = JFBConnectUtilities::strtolower($firstName);
            $lastName = JFBConnectUtilities::strtolower($lastName);
            if ($lastName != '')
        ...
        else if ($usernamePrefixFormat == AUTONAME_FIRLAS) //firlas
        {
            ...
            $firstPrefix = JFBConnectUtilities::substr($firstName, 0, max(3, 6 - $lastLength));
            $lastPrefix = JFBConnectUtilities::substr($lastName, 0, max(3, 6 - $firstLength));
        ...
        if ($prefix != '')
        {
            $suffix = JFBConnectUtilities::getUsernameUniqueNumber($prefix);
    ...
    {
        $postUsername = JFBConnectUtilities::getPostData($postData, 'username');
        if ($postUsername != '')
        {
    ...
    {
        $postName = JFBConnectUtilities::getPostData($postData, 'name');
        if ($postName != '')
        {
            $liName = $postName;
        }
    ...
    {
        $postEmail1 = JFBConnectUtilities::getPostData($postData, 'email1');
        $postEmail2 = JFBConnectUtilities::getPostData($postData, 'email2');
