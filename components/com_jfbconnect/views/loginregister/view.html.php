...
class JFBConnectViewLoginregister extends JViewLegacy
{
    function display($tpl = null)
    {
        ...
        $args = array($provider->systemName);
        ...
        JFBConnectUtilities::getDisplayEmail($postData, $profile->get('email'), $email1, $email2);
        ...
        else if (JFBCFactory::config()->get('registration_show_username') > USERNAME_LEAVE_BLANK)
        {
            ...
            $providerUsername = JFBConnectUtilities::getAutoUsername($profile->get('first_name'), $profile->get('last_name'), $profile->get('email'), $provider->usernamePrefix, $providerUserId, $usernamePrefixFormat);
        }

        $providerMemberName = JFBConnectUtilities::getDisplayNameByFullName($postData, $profile->get('full_name'));
        ...
        JFBConnectUtilities::setJFBCNewMappingEnabled();
