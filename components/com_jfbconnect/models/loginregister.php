...
class JFBConnectModelLoginRegister extends JModelLegacy
{
    function createNewUser($provider)
    {
        ...
        {
            JFBCFactory::log(JText::_('COM_JFBCONNECT_SESSION_TIMEOUT'), 'error');
            return false;
        }
        ...
        if ($formData === false)
        {
            ...
            for ($i = 0, $n = count($errors); $i < $n && $i < 3; $i++)
            {
                ...
                {
                    JFBCFactory::log($errors[$i]->getMessage(), 'warning');
                }
                else
                {
                    JFBCFactory::log($errors[$i], 'warning');
                }
            }
        ...
        if ($jUser === false)
        {
            ...
            JFBCFactory::log(JText::sprintf('COM_USERS_REGISTRATION_SAVE_FAILED', $this->getError()), 'warning');
            return false;
        }
    ...
    public function getLoginRedirect($provider = null)
    {
        ...
        )
        {
            ...
            $redirect = JFBConnectUtilities::getLinkFromMenuItem($itemId, false);
        }
    ...
    {
        $tcEnabled = JPluginHelper::isEnabled('user', 'terms');
        if($tcEnabled)
        {
            $requestData = JRequest::getVar('jform', array(), 'post', 'array');
            if (empty($requestData['terms']['terms']))
            {
                $this->setError(JText::_('PLG_USER_TERMS_FIELD_ERROR'));
                return false;
            }
        }

        $privacyEnabled = JPluginHelper::isEnabled('system', 'privacyconsent');
        if($privacyEnabled)
        {
            $requestData = JRequest::getVar('jform', array(), 'post', 'array');
            if (empty($requestData['privacyconsent']['privacy']))
            {
                $this->setError(JText::_('PLG_SYSTEM_PRIVACYCONSENT_FIELD_ERROR'));
                return false;
            }
        }
        ...
        //set user groups
        $user->set('groups', JFBCFactory::config()->get('registration_usegroups'));
    ...
    function autoCreateUser($providerUserId, $provider)
    {
        ...
        {
            $password_length = JFBCFactory::config()->getSetting('random_password_length');
            $this->_newUserPassword = JUserHelper::genRandomPassword($password_length);
        ...
        $username = JFBConnectUtilities::getAutoUsername($profile->get('first_name'), $profile->get('last_name'), $profile->get('email'), $provider->usernamePrefix, $providerUserId, $usernamePrefixFormat);
    ...
    function &getBlankUser($user, $activationMode)
    {
        ...
        //set user groups
        $instance->set('groups', JFBCFactory::config()->get('registration_usegroups'));
        ...
        if (!$instance->save())
        {
            JFBCFactory::log($instance->getError(), 'error');
            $instance = null;
        }
    ...
    private function sendNewUserEmails(&$user, $providerName)
    {
        ...
        if (JFBConnectUtilities::endswith($newEmail, "@unknown.com"))
            return;
        ...
        if ($return !== true)
        {
            ...
            $query->clear()
                //->select($db->quoteName(array('name', 'email', 'sendEmail', 'id'))) // possible joomla bug as it select the name instead of the id
