...
class FSS_Attach_Handler
{
	...
	function __construct($options = null, $initialize = true, $error_messages = null) {
		$this->options = array(
			'script_url' => $this->get_full_url().'/', // used in get_download_url and for setting up delete url
			'upload_dir' => JPATH_ROOT.'/tmp/fss/incoming/',
			'upload_url' => $this->get_full_url().'/files/', // used in get_download_url
			'user_dirs' => false,
			'mkdir_mode' => 0755,
			'param_name' => 'files',
	...
	protected function get_download_url($file_name, $version = null, $direct = false) {
		return JRoute::_('index.php?option=com_fss&view=attach&task=process&upload_token=' . JRequest::getVar('upload_token') . '&' . $this->get_singular_param_name(). '=' . rawurlencode($file_name) . '&download=1', false);
	}

	protected function set_additional_file_properties($file) {
		$file->deleteUrl = JRoute::_('index.php?option=com_fss&view=attach&task=process&upload_token=' . JRequest::getVar('upload_token') . '&' . $this->get_singular_param_name(). '=' . rawurlencode($file->name), false);
	...
	protected function validate($uploaded_file, $file, $error, $index) {
		...
		protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
			...
			if ($this->validate($uploaded_file, $file, $error, $index)) {
				...
				$file_size = $file->size = $this->get_file_size($file_path, $append_file);
				$this->set_additional_file_properties($file);
			}
			return $file;
		}
		...
		public function get_file_type($file_path, $default = '') {
