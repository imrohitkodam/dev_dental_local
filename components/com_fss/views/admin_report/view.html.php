 ...
 * @deprecated 1be37729f951d054c7a48e44765ff592
**/
		...
		foreach ($this->filter as $filter)
		{
			if (isset($filter->type) && $filter->type == "daterange")
			{
				...
				$js = "
					...
					jQuery(document).ready( function () {
					    cal_{$filter->name} = new dhtmlXCalendarObject(['{$filter->name}_from', '{$filter->name}_to']);
						...
						cal_{$filter->name}.setDateFormat('%Y-%m-%d');
					});
			...
			} elseif (isset($filter->type) && $filter->type == "date")
			{
				...
				$js = "
					...
					jQuery(document).ready( function () {
					    cal_{$filter->name} = new dhtmlXCalendarObject(['{$filter->name}']);
						cal_{$filter->name}.setDate('{$from}');
						cal_{$filter->name}.hideTime();
						// init values
						var t = new Date();
