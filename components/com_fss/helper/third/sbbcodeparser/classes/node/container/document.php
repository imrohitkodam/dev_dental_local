...
class Node_Container_Document extends Node_Container
{
	...
	public static function stripSpeechMarks($in)
	{
		$in = trim($in);
		$in = trim($in, "\"'");
		...
		{
			if (substr($in, 0, strlen($char)) == $char) $in = substr($in, strlen($char));	
			if (substr($in, -strlen($char)) == $char) $in = substr($in, 0, -strlen($char));	
		}
	...
	public static function default_bbcodes()
	{
		return array(
			...
			new BBCode('quoted', '<div class="quoted_text_cont"><span class="quoted_text_show small muted">' . \JText::_("FSS_SHOW_QUOTED_TEXT") . '</span><div class="quoted_text">%content%</div></div>'),
			...
			new BBCode('div', function($content, $attribs, $node)
			{
				$output = "<div ";
												
				if (isset($attribs['class'])) $output .= " class='" . self::stripSpeechMarks($attribs['class']) . "' ";
				if (isset($attribs['style'])) $output .= " style='" . self::stripSpeechMarks($attribs['style']) . "' ";
				$output .= " >" . $content . "</div>";
						
				return $output;
			}),
		
			new BBCode('class', function($content, $attribs, $node)
			{
				$output = "<div ";			
				$output .= " class='" . self::stripSpeechMarks($attribs['default']) . "' ";
				$output .= " >" . $content . "</div>";
						
				return $output;
			}),
		
			new BBCode('sclass', function($content, $attribs, $node)
			{
				$output = "<span ";			
				$output .= " class='" . self::stripSpeechMarks($attribs['default']) . "' ";
				$output .= " >" . $content . "</span>";
			...
			new BBCode('icon', function($content, $attribs, $node)
			{
				$output = "<i class='icon-" . $content . "'></i>";									
				return $output;
			}),
	...
	private function parse_attribs($attribs)
	{
		...
		{
			// links were getting parsed incorrectly when they had = in them, so if we start with an =, take that till the first space and
			// use it as the default
			if ($attribs[0] == "=")
			{
				$endpos = strlen($attribs);
				if (strpos($attribs, " ") !== false)
					$endpos = strpos($attribs, " ");
				
				$ret['default'] = htmlentities(substr($attribs, 1, $endpos), ENT_QUOTES | ENT_IGNORE, "UTF-8");
				$attribs = substr($attribs, $endpos+1);
			}
	...
	public function detect_links()
	{
		$this->loop_text_nodes(function($child) {
			...
			preg_match_all("/(?:(?:https?|ftp):\/\/|(?:www|ftp)\.)(?:[a-zA-Z0-9\-\.]{1,255}\.[a-zA-Z]{1,20})(?::[0-9]{1,5})?(?:\/[^\s'\"]*)?(?:(?<![,\)\.])|[\S])/u",
				$child->get_text(),
				$matches,
