 ...
 * @copyright Copyright 2012, Leaf Corcoran
...
if (!class_exists("fss_lessc"))
{
	class fss_lessc {
		...
		protected function injectVariables($args) {
			$this->pushEnv();
			$parser = new fss_lessc_parser($this, __METHOD__);
		...
		protected function makeParser($name) {
			$parser = new fss_lessc_parser($this, $name);
		...
		protected function newFormatter() {
			$className = "fss_lessc_formatter_lessjs";
			if (!empty($this->formatterName)) {
				if (!is_string($this->formatterName))
				return $this->formatterName;
				$className = "fss_lessc_formatter_$this->formatterName";
			}
...
// syntax tree
if (!class_exists("fss_lessc_parser"))
{
	class fss_lessc_parser {
		...
		public function __construct($lessc, $sourceName = null) {
			...
			if (!self::$operatorString) {
				self::$operatorString =
					'('.implode('|', array_map(array('fss_lessc', 'preg_quote'),
							array_keys(self::$precedence))).')';

				$commentSingle = fss_lessc::preg_quote(self::$commentSingle);
				$commentMultiLeft = fss_lessc::preg_quote(self::$commentMultiLeft);
				$commentMultiRight = fss_lessc::preg_quote(self::$commentMultiRight);
		...
		protected function isDirective($dirname, $directives) {
			...
			$pattern = implode("|",
				array_map(array("fss_lessc", "preg_quote"), $directives));
		...
		protected function expressionList(&$exps) {
			...
			$exps = fss_lessc::compressList($values, ' ');
			return true;
		}
		...
		public function propertyValue(&$value, $keyName = null) {
			...
			$value = fss_lessc::compressList($values, ', ');
			return true;
		}
		...
		protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
			...
			$stop = array("'", '"', "@{", $end);
			$stop = array_map(array("fss_lessc", "preg_quote"), $stop);
		...
		protected function literal($what, $eatWhitespace = null) {
			...
			if (!isset(self::$literalCache[$what])) {
				self::$literalCache[$what] = fss_lessc::preg_quote($what);
			}
		...
		protected function to($what, &$out, $until = false, $allowNewline = false) {
			...
			if (!$this->match('('.$validChars.'*?)'.fss_lessc::preg_quote($what), $m, !$until)) return false;
...
if (!class_exists("fss_lessc_formatter_classic"))
{
	class fss_lessc_formatter_classic {
		public $indentChar = "  ";
...
if (!class_exists("fss_lessc_formatter_compressed"))
{
	class fss_lessc_formatter_compressed extends fss_lessc_formatter_classic {
...
if (!class_exists("fss_lessc_formatter_lessjs"))
{
	class fss_lessc_formatter_lessjs extends fss_lessc_formatter_classic {
		public $disableSingle = true;
