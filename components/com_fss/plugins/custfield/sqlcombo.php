	...
	function parseText($text, $context)
	{
		...
		if (is_array($context))
		{
			foreach ($_POST as $key => $value)
			{
				...
				if ($key == "prodid") $parser->setVar('prod_id', $value);
				if ($key == "deptid") $parser->setVar('ticket_dept_id', $value);
			}


			if (isset($context['ticketid']))
			{
			...
			if (isset($context['userid']))
			{
				...
				$parser->setVar('user_id', (int)$context['userid']);
			}
