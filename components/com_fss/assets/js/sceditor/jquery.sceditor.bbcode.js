...
; (function (jQuery, window, document) {
	 ...
	 * same as doing: jQuery(editor.tmpl("html", {params...}));</p>
	 *
	 * @param {string} name
	 * @param {Object} params
	 ...
	 */
    jQuery.sceditor = function (el, options) {
        var IE_VER = jQuery.sceditor.ie;
		 ...
		 * @private
		 */
        var original = el; //el.get ? el.get(0) : el;
		 ...
		 */
        base.commands = jQuery.extend(true, {}, (options.commands || jQuery.sceditor.commands));
		 ...
		 * @memberOf jQuery.sceditor.prototype
		 */
        base.opts = options = jQuery.extend({}, jQuery.sceditor.defaultOptions, options);
        ...
        init = function () {
            ...
            $editorContainer = jQuery('<div class="sceditor-container" />')
            ...
            // full editing
            if (!jQuery.sceditor.isWysiwygSupported)
                base.toggleSourceMode();

            updateActiveButtons();

            var loaded = function () {
                jQuery(window).unbind('load', loaded);
        ...
        initPlugins = function () {
            ...
            pluginManager = new jQuery.sceditor.PluginManager(base);
        ...
        initLocale = function () {
            ...
            locale = jQuery.sceditor.locale[options.locale];

            if (!locale) {
                lang = options.locale.split('-');
                locale = jQuery.sceditor.locale[lang[0]];
            }
        ...
        initEditor = function () {
            ...
            $sourceEditor = jQuery('<textarea></textarea>').hide();
            $wysiwygEditor = jQuery('<iframe frameborder="0"></iframe>');
            ...
            $wysiwygDoc = jQuery(doc);
            $wysiwygBody = jQuery(doc.body);
            ...
            rangeHelper = new jQuery.sceditor.rangeHelper(wysiwygEditor.contentWindow);
        ...
        initEvents = function () {
            ...
            jQuery(original.form)
				.bind('reset', handleFormReset)
				.submit(base.updateOriginal);

            jQuery(window).bind('resize orientationChanged', handleWindowResize);
        ...
        initToolBar = function () {
            ...
            $toolbar = jQuery('<div class="sceditor-toolbar" unselectable="on" />');
            jQuery.each(groups, function (idx, group) {
                $group = jQuery('<div class="sceditor-group" />');

                jQuery.each(group.split(','), function (idx, button) {
                    // The button must be a valid command and not excluded
                    if (!commands[button] || jQuery.inArray(button, exclude) > -1)
                        return;
            ...
            jQuery(options.toolbarContainer || $editorContainer).append($toolbar);
        };
        ...
        initCommands = function () {
            jQuery.each(base.commands, function (name, cmd) {
                ...
                if (cmd.forceNewLineAfter && jQuery.isArray(cmd.forceNewLineAfter))
                    requireNewLineFix = jQuery.merge(requireNewLineFix, cmd.forceNewLineAfter);
        ...
        initResize = function () {
            ...
            mouseUpFunc = function (e) {
                ...
                jQuery(document).unbind('touchmove mousemove', mouseMoveFunc);
                jQuery(document).unbind('touchend mouseup', mouseUpFunc);
            ...
            $grip.bind('touchstart mousedown', function (e) {
                ...
                $cover.show();
                jQuery(document).bind('touchmove mousemove', mouseMoveFunc);
                jQuery(document).bind('touchend mouseup', mouseUpFunc);
        ...
        autofocus = function () {
            ...
            {
                jQuery.sceditor.dom.removeWhiteSpace(body);
        ...
        base.dimensions = function (width, height, save) {
            ...
            if (width !== false) {
                ...
                if (jQuery.sceditor.ios && $wysiwygBody)
        ...
        base.maximize = function (maximize) {
            ...
            // IE 6 fix
            if (IE_VER < 7)
                jQuery('html, body').toggleClass('sceditor-maximize', maximize);
        ...
        base.destroy = function () {
            ...
            jQuery(document).unbind('click', handleDocumentClick);
            jQuery(window).unbind('resize orientationChanged', handleWindowResize);
        ...
        base.createDropDown = function (menuItem, dropDownName, content, ieUnselectable) {
            ...
            $dropdown = jQuery('<div class="sceditor-dropdown sceditor-' + dropDownName + '" />')
				.css(dropDownCss)
				.append(content)
        ...
        handlePasteData = function (elm, pastearea) {
            ...
            jQuery.sceditor.dom.fixNesting(pastearea);
            ...
            if (pluginManager.hasHandler('toSource'))
                pasteddata = pluginManager.callOnlyFirst('toSource', pasteddata, jQuery(pastearea));
        ...
        base.wysiwygEditorInsertHtml = function (html, endHtml, overrideCodeBlocking) {
            ...
            if (!overrideCodeBlocking && (jQuery(currentBlockNode).is('code') || jQuery(currentBlockNode).parents('code').length !== 0))
                return;
            ...
            scrollTop = $wysiwygBody.scrollTop() || $wysiwygDoc.scrollTop();
            scrollTo = (jQuery.sceditor.dom.getOffset($marker[0]).top + ($marker.outerHeight(true) * 1.5)) - editorHeight;
            $marker.hide();
        ...
        base.wysiwygEditorInsertText = function (text, endText) {
            base.wysiwygEditorInsertHtml(jQuery.sceditor.escapeEntities(text), jQuery.sceditor.escapeEntities(endText));
        };
            ...
            if (!base.inSourceMode()) {
                ...
                if (end) {
                    var html = base.getRangeHelper().selectedHtml(),
						frag = jQuery('<div>').appendTo(jQuery('body')).hide().html(html);
        ...
        base.getWysiwygEditorValue = function (filter) {
            ...
            jQuery.sceditor.dom.fixNesting($wysiwygBody[0]);
        ...
        replaceEmoticons = function (node) {
            ...
            if (!options.emoticonsEnabled || jQuery(node).parents('code').length)
                return;

            var doc = node.ownerDocument,
				emoticonCodes = [],
				emoticonRegex = [],
				emoticons = jQuery.extend({}, options.emoticons.more, options.emoticons.dropdown, options.emoticons.hidden);

            jQuery.each(emoticons, function (key) {
                if (options.emoticonsCompat)
                    emoticonRegex[key] = new RegExp('(>|^|\\s|\xA0|\u2002|\u2003|\u2009|&nbsp;)' + jQuery.sceditor.regexEscape(key) + '(\\s|$|<|\xA0|\u2002|\u2003|\u2009|&nbsp;)');
                    ...
                    if (node.nodeType !== 3) {
                        ...
                        while (emoticonIdx--) {
                            ...
                            if (startIdx > -1) {
                                ...
                                parsedHtml = jQuery.sceditor.dom.parseHTML(_tmpl('emoticon', {
                                    key: key,
        ...
        base.toggleSourceMode = function () {
            ...
            if (!jQuery.sceditor.isWysiwygSupported && base.inSourceMode())
                return;
        ...
        handleCommand = function (caller, command) {
            ...
            if (base.inSourceMode()) {
                if (command.txtExec) {
                    if (jQuery.isArray(command.txtExec))
            ...
            if (jQuery.isFunction(command.exec))
        ...
        base.execCommand = function (command, param) {
            var executed = false,
				$parentNode = jQuery(rangeHelper.parentNode());
        ...
        checkNodeChanged = function () {
            ...
            if (currentNode !== node) {
                ...
                $editorContainer.trigger(jQuery.Event('nodechanged', { oldNode: oldNode, newNode: currentNode }));
            }
        };
        ...
        appendNewLine = function () {
            ...
            jQuery.sceditor.dom.rTraverse($wysiwygBody[0], function (node) {
                ...
                if ((node.nodeType === 3 && !/^\s*$/.test(node.nodeValue)) || name === 'br' ||
					(IE_BR_FIX && !node.firstChild && !jQuery.sceditor.dom.isInline(node, false))) {
        ...
        handleEvent = function (e) {
            ...
            delete clone.type;
            customEvent = jQuery.Event((e.target === sourceEditor ? 'scesrc' : 'scewys') + e.type, clone);
        ...
        base.blur = function (handler, excludeWysiwyg, excludeSource) {
            if (jQuery.isFunction(handler))
                ...
                if (!$blurElm)
                    $blurElm = jQuery('<input style="position:absolute;width:0;height:0;opacity:0;border:0;padding:0;filter:alpha(opacity=0)" type="text" />').appendTo($editorContainer);
        ...
        emoticonsKeyPress = function (e) {
            ...
            if (jQuery(currentBlockNode).is('code') || jQuery(currentBlockNode).parents('code').length)
                return;

            if (!base.emoticonsCache) {
                base.emoticonsCache = [];

                jQuery.each(jQuery.extend({}, options.emoticons.more, options.emoticons.dropdown, options.emoticons.hidden), function (key, url) {
        ...
        emoticonsCheckWhitespace = function () {
            ...
            currentEmoticons = jQuery.map(currentEmoticons, function (emoticon) {
                ...
                previousText = previousText + jQuery(emoticon).data('sceditor-emoticon');
        ...
        base.emoticons = function (enable) {
            ...
            if (enable) {
                ...
                $wysiwygBody.find('img[data-sceditor-emoticon]').replaceWith(function () {
                    return jQuery(this).data('sceditor-emoticon');
                });
        ...
        base.css = function (css) {
            if (!inlineCss)
                inlineCss = jQuery('<style id="#inline" />').appendTo($wysiwygDoc.find('head'))[0];
        ...
        handleBackSpace = function (e) {
            ...
            if (!parent || jQuery(parent).is('body'))
                return;
        ...
        currentStyledBlockNode = function () {
            ...
            while (!jQuery.sceditor.dom.hasStyling(block)) {
                if (!(block = block.parentNode) || jQuery(block).is('body'))
                    return;
            }
        ...
        base.clearBlockFormatting = function (block) {
            ...
            if (!jQuery(block).is('p,div'))
                jQuery.sceditor.dom.convertElement(block, 'p');
	 ...
	 * @type {Boolean}
	 */
    jQuery.sceditor.ios = /iPhone|iPod|iPad| wosbrowser\//i.test(navigator.userAgent);
	 ...
	 * @return {Boolean}
	 */
    jQuery.sceditor.isWysiwygSupported = (function () {
        var match, isUnsupported,
			contentEditable = jQuery('<div contenteditable="true">')[0].contentEditable,
	 ...
	 * @memberOf jQuery.sceditor
	 */
    jQuery.sceditor.regexEscape = function (str) {
	 ...
	 * @since 1.4.1
	 */
    jQuery.sceditor.escapeEntities = function (str, quote) {
        if (!str)
            return str;
	 ...
	 * @since 1.4.5
	 */
    jQuery.sceditor.escapeUriScheme = function (url) {
        var path,
    ...
    jQuery.sceditor.commands = {
        ...
        font: {
            _dropDown: function (editor, caller, callback) {
                var fonts = editor.opts.fonts.split(','),
					...
					/** @private */
					clickFunc = function () {
					    callback(jQuery(this).data('font'));
            ...
            exec: function (caller) {
                ...
                jQuery.sceditor.command.get('font')._dropDown(
					editor,
					caller,
					function (fontName) {
        ...
        size: {
            _dropDown: function (editor, caller, callback) {
                var content = jQuery('<div />'),
					...
					clickFunc = function (e) {
					    callback(jQuery(this).data('size'));
					    editor.closeDropDown(true);
					    e.preventDefault();
					};
            ...
            exec: function (caller) {
                ...
                jQuery.sceditor.command.get('size')._dropDown(
					editor,
					caller,
					function (fontSize) {
        ...
        color: {
            _dropDown: function (editor, caller, callback) {
                var i, x, color, colors,
					...
					html = [],
					cmd = jQuery.sceditor.command.get('color');
                ...
                content.append(cmd._htmlCache)
					.find('a')
					.click(function (e) {
					    callback(jQuery(this).attr('data-color'));
					    editor.closeDropDown(true);
					    e.preventDefault();
					});
            ...
            exec: function (caller) {
                ...
                jQuery.sceditor.command.get('color')._dropDown(
					editor,
					caller,
					function (color) {
        ...
        indent: {
                ...
                if ($firstBlock.is('ul,ol,menu')) {
                    ...
                    if (window.Range && range instanceof Range) {
                        ...
                        if (startContainerParent !== startContainerParent.parentNode.firstElementChild ||
							...
							(jQuery(endContainerParent).is('li') && endContainerParent !== endContainerParent.parentNode.lastElementChild)) {
                            return 0;
                        }
                    }
            ...
            exec: function () {
                var editor = this,
					$elm = jQuery(editor.getRangeHelper().getFirstBlockParent());
        ...
        outdent: {
            state: function (parents, firstBlock) {
                return jQuery(firstBlock).is('ul,ol,menu') || jQuery(firstBlock).parents('ul,ol,menu').length > 0 ? 0 : -1;
            },
            exec: function () {
                var editor = this,
					$elm = jQuery(editor.getRangeHelper().getFirstBlockParent());
        ...
        unlink: {
            state: function () {
                var $current = jQuery(this.currentNode());
        ...
        emoticon: {
            exec: function (caller) {
                ...
                var createContent = function (includeMore) {
                    var emoticonsCompat = editor.opts.emoticonsCompat,
						...
						$content = jQuery('<div />'),
						$line = jQuery('<div />').appendTo($content),
						emoticons = jQuery.extend({}, editor.opts.emoticons.dropdown, includeMore ? editor.opts.emoticons.more : {}),
						perLine = 0;
                    ...
                    jQuery.each(emoticons, function (code, emoticon) {
                        $line.append(
							jQuery('<img />').attr({
							    ...
							    editor.insert(startSpace + jQuery(this).attr('alt') + endSpace, null, false).closeDropDown(true);
							    return false;
							})
						);
                    ...
                    if (!includeMore) {
                        $content.append(
							jQuery(editor._('<a class="sceditor-more">{0}</a>', editor._('More'))).click(function () {
            ...
            txtExec: function (caller) {
                jQuery.sceditor.command.get('emoticon').exec.call(this, caller);
            },
        ...
        youtube: {
            ...
            exec: function (caller) {
                ...
                jQuery.sceditor.command.get('youtube')._dropDown(
					editor,
					caller,
					function (id) {
        ...
        date: {
            ...
            exec: function () {
                this.insertText(jQuery.sceditor.command.get('date')._date(this));
            },
            txtExec: function () {
                this.insertText(jQuery.sceditor.command.get('date')._date(this));
            },
        ...
        time: {
            ...
            exec: function () {
                this.insertText(jQuery.sceditor.command.get('time')._time());
            },
            txtExec: function () {
                this.insertText(jQuery.sceditor.command.get('time')._time());
            },
	 ...
	 */
    jQuery.sceditor.rangeHelper = function (w, d) {
        ...
        _prepareHTML = function (node, endNode, returnHtml) {
            ...
            $lastChild = jQuery(lastChild);

            if (jQuery.sceditor.dom.canHaveChildren(lastChild)) {
        ...
        base.getFirstBlockParent = function (n) {
            var func = function (node) {
                if (!jQuery.sceditor.dom.isInline(node, true))
                    return node;
        ...
        base.raplaceKeyword = function (keywords, includeAfter, keywordsSorted, longestKeyword, requireWhiteSpace, currrentChar) {
            ...
            while (i--) {
                ...
                keywordRegex = new RegExp('(?:[\\s\xA0\u2002\u2003\u2009])' + jQuery.sceditor.regexEscape(keyword) + '(?=[\\s\xA0\u2002\u2003\u2009])');
	...
	/** @lends jQuery.sceditor.dom */
	{
	    ...
	    hasStyling: function (elm) {
	        ...
	        return elm && (!$elm.is('p,div') || elm.className || $elm.attr('style') || !jQuery.isEmptyObject($elm.data()));
	    },
	    ...
	    convertElement: function (elm, newElement) {
	        ...
	        while (i--) {
	            ...
	            if (!jQuery.sceditor.ie || attr.specified) {
	                // IE < 8 doesn't return the CSS for the style attribute
	                if (jQuery.sceditor.ie < 8 && /style/i.test(attr.name))
	    ...
	    isInline: function (elm, includeCodeAsBlock) {
	        ...
	        return jQuery.sceditor.dom.blockLevelList.indexOf('|' + elm + '|') < 0;
	    },
	    ...
	    findCommonAncestor: function (node1, node2) {
	        ...
	        return jQuery(node1).parents().has(jQuery(node2)).first();
	    },
	    ...
	    getSibling: function (node, previous) {
	        ...
	        return jQuery.sceditor.dom.getSibling(node.parentNode, previous);
	    },
	    ...
	    removeWhiteSpace: function (root, preserveNewLines) {
	        var nodeValue, nodeType, next, previous, cssWS, nextNode, trimStart, sibling,
				getSibling = jQuery.sceditor.dom.getSibling,
				isInline = jQuery.sceditor.dom.isInline,
	        ...
	        while (node) {
	            ...
	            if (nodeType === 1 && node.firstChild) {
	                cssWS = jQuery(node).css('whiteSpace');

	                // pre || pre-wrap with any vendor prefix
	                if (!/pre(?:\-wrap)?$/i.test(cssWS))
	                    jQuery.sceditor.dom.removeWhiteSpace(node, /line$/i.test(cssWS));
	            }
	            ...
	            if (nodeType === 3 && nodeValue) {
	                ...
	                while (jQuery(sibling).hasClass('sceditor-ignore'))
	    ...
	    getStyle: function (elm, property) {
	        ...
	        if (!jQuery.sceditor.dom._propertyCache)
	            jQuery.sceditor.dom._propertyCache = {};

	        propertyCache = jQuery.sceditor.dom._propertyCache;
	        if (!propertyCache[property])
	            propertyCache[property] = jQuery.camelCase(property);
	    ...
	    hasStyle: function (elm, property, values) {
	        var styleValue = jQuery.sceditor.dom.getStyle(elm, property);

	        if (!styleValue)
	            return (!values || styleValue === values || (jQuery.isArray(values) && jQuery.inArray(styleValue, values) > -1));
	 ...
	 */
    jQuery.sceditor.PluginManager = function (owner) {
        /**
		 * Alias of this
		 * @private
		 * @type {Object}
        ...
        base.exists = function (plugin) {
            if (plugin in jQuery.sceditor.plugins) {
                plugin = jQuery.sceditor.plugins[plugin];
        ...
        base.isRegistered = function (plugin) {
            ...
            while (i--)
                if (plugins[i] instanceof jQuery.sceditor.plugins[plugin])
                    return true;
        ...
        base.register = function (plugin) {
            ...
            plugin = new jQuery.sceditor.plugins[plugin]();
            plugins.push(plugin);
	...
	/** @lends jQuery.sceditor.command */
	{
		 ...
		 */
	    get: function (name) {
	        return jQuery.sceditor.commands[name] || null;
	    },
		 ...
		 *
		 * @example
		 * jQuery.sceditor.command.set("hello",
		 * {
		 *     exec: function() {
		 *         alert("Hello World!");
		 *     }
	    ...
	    set: function (name, cmd) {
	        ...
	        cmd = jQuery.extend(jQuery.sceditor.commands[name] || {}, cmd);

	        cmd.remove = function () { jQuery.sceditor.command.remove(name); };

	        jQuery.sceditor.commands[name] = cmd;
	        return this;
	    },
	    ...
	    remove: function (name) {
	        if (jQuery.sceditor.commands[name])
	            delete jQuery.sceditor.commands[name];
	 ...
	 */
    jQuery.sceditor.defaultOptions = {
		 ...
		 * @type {String}
		 */
        locale: jQuery('html').attr('lang') || 'en',
		 ...
		 *
		 * See jQuery.sceditor.BBCodeParser.defaults for list of valid options
		 * @type {Object}
		 */
        parserOptions: {},
    ...
    jQuery.fn.sceditor = function (options) {
        ...
        if (!options.runWithoutWysiwygSupport && !jQuery.sceditor.isWysiwygSupported)
            return;

        this.each(function () {

            $this = this.jquery ? this : jQuery(this);
            ...
            if (options === 'state')
                ...
                (new jQuery.sceditor(this, options));
        });
        ...
        return ret.length === 1 ? ret[0] : jQuery(ret);
    };
...
(function ($, window, document) {
    ...
    var escapeEntities = jQuery.sceditor.escapeEntities;
    var escapeUriScheme = jQuery.sceditor.escapeUriScheme;
	 ...
	 * @since v1.4.0
	 */
    jQuery.sceditor.BBCodeParser = function (options) {
        // make sure this is not being called as a function
        if (!(this instanceof jQuery.sceditor.BBCodeParser))
            return new jQuery.sceditor.BBCodeParser(options);
        ...
        TokenizeToken.prototype = {
            ...
            splitAt: function (splitAt) {
                ...
                if (typeof splitAt !== 'number')
                    splitAt = jQuery.inArray(splitAt, base.children);
        ...
        init = function () {
            base.opts = jQuery.extend({}, jQuery.sceditor.BBCodeParser.defaults, options);
            base.bbcodes = jQuery.sceditor.plugins.bbcode.bbcodes;
        };
        ...
        tokenizeTag = function (type, val) {
            ...
            if (type === 'open' && (matches = val.match(/\[([^\]\s=]+)(?:([^\]]+))?\]/))) {
                ...
                if (matches[2] && (matches[2] = jQuery.trim(matches[2])))
            ...
            if (!name || (type === 'open' || type === 'close') && !jQuery.sceditor.plugins.bbcode.bbcodes[name]) {
                type = 'content';
                name = '#';
            }
        ...
        tokenizeAttrs = function (attrs) {
            var matches,
				...
				*/
				...
				unquote = jQuery.sceditor.plugins.bbcode.stripQuotes,
				ret = {};
        ...
        isChildAllowed = function (parent, child) {
            ...
            if (allowedChildren && jQuery.inArray(child.name || '#', allowedChildren) < 0)
                return false;
            ...
            var token, bbcode, curTok, clone, i, previous, next,
				...
				closesCurrentTag = function (name) {
				    return currentOpenTag() &&
						...
						bbcode.closedBy &&
						jQuery.inArray(name, bbcode.closedBy) > -1;
				};
        ...
        fixNesting = function (children, parents, insideInline, rootArr) {
            ...
            for (i = 0; i < children.length; i++) {
                ...
                if (!isInline(token) && insideInline) {
                    ...
                    if ((parentIndex = jQuery.inArray(parent, parentParentChildren)) > -1) {
                        // remove the block level token from the right side of the split
                        // inline element
                        right.children.splice(jQuery.inArray(token, right.children), 1);
        ...
        removeEmpty = function (tokens) {
            ...
            while (i--) {
                ...
                if (isTokenWhiteSpace(token.children) && bbcode && !bbcode.isSelfClosing && !bbcode.allowsEmpty)
                    tokens.splice.apply(tokens, jQuery.merge([i, 1], token.children));
            }
        };
            ...
            while (tokens.length > 0) {
                ...
                if (token.type === tokenType.open) {
                    ...
                    if (bbcode && bbcode.html) {
                        ...
                        if (!jQuery.isFunction(bbcode.html)) {
                            token.attrs['0'] = content;
                            html = jQuery.sceditor.plugins.bbcode.formatBBCodeString(bbcode.html, token.attrs);
                        }
                        else
        ...
        convertToBBCode = function (toks) {
            ...
            while (toks.length > 0) {
                ...
                quoteType = (bbcode ? bbcode.quoteType : null) || base.opts.quoteType || jQuery.sceditor.BBCodeParser.QuoteType.auto;
		 ...
		 *
		 * @param {String} str
		 * @param {jQuery.sceditor.BBCodeParser.QuoteType} quoteType
		 * @param {String} name
		 * @return {String}
		 * @private
		 */
        quote = function (str, quoteType, name) {
            var QuoteTypes = jQuery.sceditor.BBCodeParser.QuoteType,
				needsQuotes = /\s|=/.test(str);

            if (jQuery.isFunction(quoteType))
	 ...
	 * @since v1.4.0
	 */
    jQuery.sceditor.BBCodeParser.QuoteType = {
	 ...
	 * @type {Object}
	 */
    jQuery.sceditor.BBCodeParser.defaults = {
        /**
		 ...
		 * @type {jQuery.sceditor.BBCodeParser.QuoteType}
		 * @since 1.4.1
		 */
        quoteType: jQuery.sceditor.BBCodeParser.QuoteType.auto
    };

    /**
	 * Deprecated, use jQuery.sceditor.plugins.bbcode
	 *
	 * @class sceditorBBCodePlugin
	 * @name jQuery.sceditor.sceditorBBCodePlugin
	 * @deprecated
	 */
    jQuery.sceditorBBCodePlugin =
	/**
	 ...
	 * @since 1.4.1
	 */
	jQuery.sceditor.plugins.bbcode = function () {
	    var base = this;
	    ...
	    formatString = jQuery.sceditor.plugins.bbcode.formatString;
	    base.bbcodes = jQuery.sceditor.plugins.bbcode.bbcodes;
	    base.stripQuotes = jQuery.sceditor.plugins.bbcode.stripQuotes;
	    ...
	    mergeSourceModeCommands = function (editor) {
	        var getCommand = jQuery.sceditor.command.get;
	        ...
	        var merge = {
	            ...
	            orderedlist: {
	                txtExec: function (caller, selected) {
	                    ...
	                    jQuery.sceditor.plugins.bbcode.bbcode.get('');
	            ...
	            link: {
	                txtExec: function (caller, selected) {
	                    var display = /^[a-z]+:\/\//i.test(jQuery.trim(selected)) ? null : selected,
							url = prompt(this._('Enter URL:'), (display ? 'http://' : jQuery.trim(selected))),
	        ...
	        editor.commands = jQuery.extend(true, {}, merge, editor.commands);
	    };
	    ...
	    buildBbcodeCache = function () {
	        jQuery.each(base.bbcodes, function (bbcode) {
	            if (base.bbcodes[bbcode].tags)
	                jQuery.each(base.bbcodes[bbcode].tags, function (tag, values) {
	            ...
	            if (base.bbcodes[bbcode].styles)
	                jQuery.each(base.bbcodes[bbcode].styles, function (style, values) {
	                    var isBlock = base.bbcodes[bbcode].isInline === false;
	    ...
	    handleStyles = function ($element, content, blockLevel) {
	        var styleValue, format,
				getStyle = jQuery.sceditor.dom.getStyle;
	        ...
	        jQuery.each(stylesToBbcodes[blockLevel], function (property, bbcodes) {
	            ...
	            jQuery.each(bbcodes, function (bbcode, values) {
	                if (!values || jQuery.inArray(styleValue.toString(), values) > -1) {
	                    format = base.bbcodes[bbcode].format;

	                    if (jQuery.isFunction(format))
	    ...
	    handleTags = function ($element, content, blockLevel) {
	        ...
	        if (tagsToBbcodes[tag] && tagsToBbcodes[tag][blockLevel]) {
	            ...
	            jQuery.each(tagsToBbcodes[tag][blockLevel], function (bbcode, bbcodeAttribs) {
	                ...
	                if (bbcodeAttribs) {
	                    ...
	                    jQuery.each(bbcodeAttribs, function (attrib, values) {
	                        // if the $element has the bbcodes attribute and the bbcode attribute
	                        // has values check one of the values matches
	                        if (!$element.attr(attrib) || (values && jQuery.inArray($element.attr(attrib), values) < 0))
	                            return;
	        ...
	        var isInline = jQuery.sceditor.dom.isInline;
	        if (blockLevel && (!isInline(element, true) || tag === 'br')) {
	            ...
	            while (previousSibling && (jQuery(previousSibling).hasClass('sceditor-ignore') ||
					(previousSibling.nodeType === 1 && isInline(previousSibling, true) && !previousSibling.firstChild))) {
	                previousSibling = previousSibling.previousSibling;
	            }

	            while (jQuery(parentLastChild).hasClass('sceditor-ignore'))
	    ...
	    base.signalToSource = function (html, $body) {
	        var $tmpContainer, bbcode,
				parser = new jQuery.sceditor.BBCodeParser(base.opts.parserOptions);
	        ...
	        //if (!$body) {
	            if (typeof html === 'string') {
	                ...
	                $tmpContainer = jQuery('<div />').css('visibility', 'hidden').appendTo(document.body).html(html);
	                $body = $tmpContainer;
	            }
	            else
	                $body = jQuery(html);
	        ...
	        jQuery.sceditor.dom.removeWhiteSpace($body[0]);
	        ...
	        bbcode = bbcode.replace(/\&lbracket\;/g, '&#91;');
	        bbcode = bbcode.replace(/\&rbracket\;/g, '&#93;');

	        if (base.opts.bbcodeTrim)
	            bbcode = jQuery.trim(bbcode);
	    ...
	    base.elementToBbcode = function ($element) {
	        return (function toBBCode(node, vChildren) {
	            ...
	            jQuery.sceditor.dom.traverse(node, function (node) {
	                ...
	                if (typeof vChildren === 'object') {
	                    isValidChild = jQuery.inArray(tag, vChildren) > -1;
	    ...
	    base.signalToWysiwyg = function (text, asFragment) {
	        var parser = new jQuery.sceditor.BBCodeParser(base.opts.parserOptions),
				html = parser.toHTML(base.opts.bbcodeTrim ? jQuery.trim(text) : text);

	        html = html.replace(/\&\#91\;/g, '[');
	        html = html.replace(/\&\#93\;/g, ']');
	        html = html.replace(/\&amp\;\#91\;/g, '[');
	        html = html.replace(/\&amp\;\#93\;/g, ']');
	    ...
	    removeFirstLastDiv = function (html) {
	        var node, next, removeDiv,
				$output = jQuery('<div />').hide().appendTo(document.body),
				output = $output[0];

	        removeDiv = function (node, isFirst) {
	            // Don't remove divs that have styling
	            if (jQuery.sceditor.dom.hasStyling(node))
	                return;

	            if (IE_BR_FIX || (node.childNodes.length !== 1 || !jQuery(node.firstChild).is('br'))) {
	            ...
	            if (isFirst) {
	                ...
	                if (node !== lastChild && jQuery(lastChild).is('div') && node.nextSibling === lastChild)
	        ...
	        if ((node = output.firstChild) && jQuery(node).is('div'))
	            removeDiv(node, true);

	        if ((node = output.lastChild) && jQuery(node).is('div'))
	            removeDiv(node);
	 ...
	 * @since v1.4.0
	 */
    jQuery.sceditor.plugins.bbcode.stripQuotes = function (str) {
	 ...
	 * @since v1.4.0
	 */
    jQuery.sceditor.plugins.bbcode.formatString = function () {
        var undef,
			args = arguments;
	 ...
	 * @since 1.4.5
	 */
    jQuery.sceditor.plugins.bbcode.formatBBCodeString = function (str, obj) {
	 ...
	 * @return {String}
	 */
    var normaliseColour = jQuery.sceditor.plugins.bbcode.normaliseColour = function (color) {
        var m, toHex;
    ...
    jQuery.sceditor.plugins.bbcode.bbcodes = {
        // START_COMMAND: Bold
        b: {
            tags: {
                b: null,
        ...
        font: {
            ...
            quoteType: jQuery.sceditor.BBCodeParser.QuoteType.never,
        ...
        quote: {
            ...
            format: function (element, content) {
                ...
                if ($cite.length === 1 || $elm.data('author')) {
                    ...
                    content = this.elementToBbcode(jQuery(element));
                    author = '=' + author;
        ...
        icon: {
            ...
            format: function (element, content) {
                element = element.attr('data-icon-id');

                return element ? '[icon]' + element + '[/icon]' : content;
            },
            html: '<i class="icon-{0}" data-icon-id="{0}"></i>'
        },
        // END_COMMAND
        ...
        class: {
            ...
            format: function (element, content) {
                var cls = element.attr('data-class-id');

                // make sure this link is not an e-mail, if it is return e-mail BBCode

                return '[class=' + cls + ']' + content + '[/class]';
            },
            html: function (token, attrs, content) {
                return '<div class="' + attrs.defaultattr + '" data-class-id="' + attrs.defaultattr + '">' + content + '</div>';
            }
        },
        ...
        sclass: {
            ...
            format: function (element, content) {
                ...
                return '[sclass=' + cls + ']' + content + '[/sclass]';
            },
            html: function (token, attrs, content) {
                return '<span class="' + attrs.defaultattr + '" data-class-id="' + attrs.defaultattr + '">' + content + '</span>';
            }
        },
	...
	/** @lends jQuery.plugins.bbcode.bbcode */
	{
		 ...
		 */
	    get: function (name) {
	        return jQuery.sceditor.plugins.bbcode.bbcodes[name] || null;
	    },
	    ...
	    set: function (name, bbcode) {
	        var bbcodes = jQuery.sceditor.plugins.bbcode.bbcodes;

	        if (!name || !bbcode)
	            return false;

	        // merge any existing command properties
	        bbcode = jQuery.extend(bbcodes[name] || {}, bbcode);
	 ...
	 * @deprecated
	 */
    jQuery.fn.sceditorBBCodePlugin = function (options) {
        options = options || {};
