...
?>
<div class="pp-plans__item" data-plans-item data-group-id="<?php echo $group->getId(); ?>">
	<div class="pp-plan-card<?php echo $group->getPlanHighlighter() ? ' is-highlight' : '' ?><?php echo $group->getBadgeVisible() ? ' has-badges' : ''; ?>">
		<div class="pp-plan-card__hd">
			...
			<div class="pp-plan-card__title">
				<h4><?php echo PPJString::ucfirst(JText::_($group->getTitle())); ?></h4>
			</div>

			<div class="pp-plan-card__desc">
				<?php echo PPJString::ucfirst(JText::_($group->getTeasertext()));?>
			</div>
		</div>

		<?php if ($group->getDescription(true)) { ?>
		<div class="pp-plan-card__bd">
			<div class="pp-plan-card__features">
				<?php echo JText::_($group->getDescription(true)); ?>
			</div>
		</div>
		<?php } ?>
		...
		<div class="pp-plan-card__ft">
			<div class="pp-plan-card__forms">
				<?php echo $this->fd->html('button.link', PPR::_('index.php?option=com_payplans&view=plan&task=subscribe&group_id=' . $group->getId()), 'COM_PAYPLANS_GROUP_BUTTON', 'primary'); ?>
			</div>
		</div>
	</div>
</div>