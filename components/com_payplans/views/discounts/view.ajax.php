...
class PayPlansViewDiscounts extends PayPlansSiteView
{
	...
	public function check()
	{
		...
		if (!$invoice->getId() || !$exists || !$table->prodiscount_id) {
			// apply discount in amount from backend
			if (PP::isSiteAdmin() && is_numeric($code)) {
				return $this->applyAdminDiscount($invoice, $code);
			}
		...
		// Create a new log
		$data = [];
		$data['Coupon_Code'] = $discount->getCouponCode();
		$data['Invoice_ID'] = $invoice->getId();
		$data['Discount_ID'] = $discount->getId();
		$data['User_ID'] = $invoice->getBuyer()->getId();

		if ($discount->isForExtension()) {
			$data['Extended_Time'] = $discount->getParams('extend_time_discount');
		}

		if (!$discount->isForExtension()) {
			$data['Discount_Amount'] = $modifier->amount;
	...
	public function applyAdminDiscount($invoice, $code)
	{
		$modifier = new stdClass();
		$modifier->message = 'Admin discount';
		$modifier->type = 'fixed_admin';
		$modifier->reference = $code;
		$modifier->frequency = PP_MODIFIER_FREQUENCY_ONE_TIME;

		$amount = floatval($code);

		if (floatval($amount) == PP_ZERO) {
			return $this->reject(JText::_('COM_PP_DISCOUNT_INVALID_AMOUNT'));
		}
