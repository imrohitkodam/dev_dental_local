...
class PayPlansViewDashboard extends PayPlansSiteView
{
	...
	public function display($tpl = null)
	{
		...
		$options = [
			'userId' => $user->getId(), 
			'hidePendingOrder' => $hidePendingOrders
		];
		...
		$this->page->title('COM_PP_PAGE_TITLE_DASHBOARD');
		...
		if (!$subscriptions && $task == 'noaccess') {
			return parent::display('site/denied/default/default');
		}
	...
	public function subscription()
	{
		...
		$customDetails = $model->getPlanCustomDetails($plan, PP_CUSTOM_DETAILS_TYPE_SUBSCRIPTION);

		$subscriptionParams = $subscription->getParams();

		$title = JText::sprintf('COM_PP_PAGE_TITLE_DASHBOARD_SUBSCRIPTION', $plan->getTitle(), $subscription->getKey());
	...
	public function download()
	{
		...
		PP::setMeta();
		$this->page->title('COM_PP_PAGE_TITLE_USER_DOWNLOAD');
	...
	public function preferences()
	{
		...
		$canEditBusinessDetails = $easysocial->canEditBusinessDetails();

		// When it is allowed to edit, then we know that we need to get the updated business details
		...
		PP::setMeta();
		$this->page->title('COM_PP_PAGE_TITLE_USER_EDIT_PREFERENCES');
