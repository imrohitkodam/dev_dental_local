...
if (isset($userscormdata->status)) {
  if (!isset($userscormdata->{'cmi.exit'}) || (($userscormdata->{'cmi.exit'} == 'time-out') || ($userscormdata->{'cmi.exit'} == 'normal'))) {
      $userscormdata->entry = 'ab-initio';
  } else {
    if (isset($userscormdata->{'cmi.exit'}) && (($userscormdata->{'cmi.exit'} == 'suspend') || ($userscormdata->{'cmi.exit'} == 'logout'))) {
...
function DoRequest(httpReq,url,param) {
    ...
    /*var canUseBeaconAPI = function() {
        return (window.mod_scorm_is_window_closing && navigator && navigator.sendBeacon && FormData);
    };
...
function SCORMapi1_3() {
    ...
    var CMILangString500 = '^(\{lang=([a-zA-Z]{2,3}|i|x)(\-[a-zA-Z0-9\-]{2,8})?\})?([^\{].{0,500}$)?';
...
//        'fill-in':{'pre':'^(((\{case_matters=(true|false)\})(\{order_matters=(true|false)\})?)|((\{order_matters=(true|false)\})(\{case_matters=(true|false)\})?))(.*?)$',
    ...
    var datamodel =  {
        ...
        'cmi.completion_status':{'defaultvalue':'<?php echo !empty($userscormdata->{'cmi.completion_status'})?$userscormdata->{'cmi.completion_status'}:'unknown' ?>', 'format':CMICStatus, 'mod':'rw'},
        'cmi.completion_threshold':{'defaultvalue':<?php echo !empty($userscormdata->threshold)?'\''.$userscormdata->threshold.'\'':'\'\'' ?>, 'mod':'r'},
        'cmi.credit':{'defaultvalue':'<?php echo !empty($userscormdata->credit)?$userscormdata->credit:'' ?>', 'mod':'r'},
        'cmi.entry':{'defaultvalue':'<?php echo $userscormdata->entry ?>', 'mod':'r'},
        'cmi.exit':{'defaultvalue':'<?php echo !empty($userscormdata->{'cmi.exit'})?$userscormdata->{'cmi.exit'}:'' ?>', 'format':CMIExit, 'mod':'w'},
        ...
        'cmi.interactions.n.description':{'pattern':CMIIndex, 'format':CMILangString500, 'mod':'rw'},
        'cmi.launch_data':{'defaultvalue':<?php echo !empty($userscormdata->datafromlms)?'\''.$userscormdata->datafromlms.'\'':'\'\'' ?>, 'mod':'r'},
        'cmi.learner_id':{'defaultvalue':'<?php echo $userscormdata->student_id ?>', 'mod':'r'},
        'cmi.learner_name':{'defaultvalue':'<?php addslashes($userscormdata->student_name); ?>', 'mod':'r'},
        'cmi.learner_preference._children':{'defaultvalue':student_preference_children, 'mod':'r'},
        'cmi.learner_preference.audio_level':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.learner_preference.audio_level'})?'\''.$userscormdata->{'cmi.learner_preference.audio_level'}.'\'':'\'1\'' ?>, 'format':CMIDecimal, 'range':audio_range, 'mod':'rw'},
        'cmi.learner_preference.language':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.learner_preference.language'})?'\''.$userscormdata->{'cmi.learner_preference.language'}.'\'':'\'\'' ?>, 'format':CMILang, 'mod':'rw'},
        'cmi.learner_preference.delivery_speed':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.learner_preference.delivery_speed'})?'\''.$userscormdata->{'cmi.learner_preference.delivery_speed'}.'\'':'\'1\'' ?>, 'format':CMIDecimal, 'range':speed_range, 'mod':'rw'},
        'cmi.learner_preference.audio_captioning':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.learner_preference.audio_captioning'})?'\''.$userscormdata->{'cmi.learner_preference.audio_captioning'}.'\'':'\'0\'' ?>, 'format':CMISInteger, 'range':text_range, 'mod':'rw'},
        'cmi.location':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.location'})?'\''.$userscormdata->{'cmi.location'}.'\'':'\'\'' ?>, 'format':CMIString1000, 'mod':'rw'},
        'cmi.max_time_allowed':{'defaultvalue':<?php echo !empty($userscormdata->attemptAbsoluteDurationLimit)?'\''.$userscormdata->attemptAbsoluteDurationLimit.'\'':'\'\'' ?>, 'mod':'r'},
        'cmi.mode':{'defaultvalue':'<?php echo $userscormdata->mode ?>', 'mod':'r'},
        ...
        'cmi.objectives.n.description':{'pattern':CMIIndex, 'format':CMILangString500, 'mod':'rw'},
        'cmi.progress_measure':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.progress_measure'})?'\''.$userscormdata->{'cmi.progress_measure'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'range':progress_range, 'mod':'rw'},
        'cmi.scaled_passing_score':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.scaled_passing_score'})?'\''.$userscormdata->{'cmi.scaled_passing_score'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'range':scaled_range, 'mod':'r'},
        'cmi.score._children':{'defaultvalue':score_children, 'mod':'r'},
        'cmi.score.scaled':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.score.scaled'})?'\''.$userscormdata->{'cmi.score.scaled'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'range':scaled_range, 'mod':'rw'},
        'cmi.score.raw':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.score.raw'})?'\''.$userscormdata->{'cmi.score.raw'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'mod':'rw'},
        'cmi.score.min':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.score.min'})?'\''.$userscormdata->{'cmi.score.min'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'mod':'rw'},
        'cmi.score.max':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.score.max'})?'\''.$userscormdata->{'cmi.score.max'}.'\'':'\'\'' ?>, 'format':CMIDecimal, 'mod':'rw'},
        'cmi.session_time':{'format':CMITimespan, 'mod':'w', 'defaultvalue':'PT0H0M0S'},
        'cmi.success_status':{'defaultvalue':'<?php echo !empty($userscormdata->{'cmi.success_status'})?$userscormdata->{'cmi.success_status'}:'unknown' ?>', 'format':CMISStatus, 'mod':'rw'},
        'cmi.suspend_data':{'defaultvalue':<?php echo !empty($userscormdata->{'cmi.suspend_data'})?'\''.$userscormdata->{'cmi.suspend_data'}.'\'':'\'\'' ?>, 'format':CMIString64000, 'mod':'rw'},
        'cmi.time_limit_action':{'defaultvalue':<?php echo !empty($userscormdata->timelimitaction)?'\''.$userscormdata->timelimitaction.'\'':'\'\'' ?>, 'mod':'r'},
        'cmi.total_time':{'defaultvalue':'<?php echo !empty($userscormdata->{'cmi.total_time'})?$userscormdata->{'cmi.total_time'}:'PT0H0M0S' ?>', 'mod':'r'},
...
<?php
    ...
    $scormhelper->scorm_reconstitute_array_element($version, $userscormdata, 'cmi.comments_from_learner', array());
    $scormhelper->scorm_reconstitute_array_element($version, $userscormdata, 'cmi.comments_from_lms', array());
?>
    ...
    <?php } ?>
        ...
        if ((Initialized) && (!Terminated)) {
			...
			// Replace new line char
                        if (elementmodel == 'cmi.interactions.n.description')
                        {
                            value = value.replace(/\n/g, " ");
                        }
...
<?php } ?>
    ...
    function AddTime (first, second) {
        ...
        // Scorm 2004 fix when time is not calculated
        if (timestring == 'P')
        {
      return '';
    }
...
/*    jQuery.ajax({
    ...
    {
      var results = String(response).split('\n');
      ...
      <?php if($this->jDebug == 1){ ?>
          retresult = ('true' == response) ? 'true' : 'false';
          errorCode = (response =='true')? '0' : '101';

          console.log("Storing data response" + errorCode)
        <?php } ?>
    }
    });*/
    ...
    if (navigator && navigator.sendBeacon)
    {
      ...
      var result = DoRequest(myRequest, root_url+ 'index.php?option=com_tjlms&task=scorm.storescotrackdata', datastring);
