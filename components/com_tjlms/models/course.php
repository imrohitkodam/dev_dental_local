...
class TjlmsModelcourse extends AdminModel
{
	...
	public function addCertEntry($courseId, $userId = '', $issuedDate = '')
	{
		...
		if (!empty($courseData->certificate_id))
		{
			...
			// Set certificate issue date - always use current date for "Generated" field
			// Note: "First Completed" date comes from first_completion_date in course_track table
			$currentDate = Factory::getDate()->toSQL();
			$tjCert->setIssuedDate($currentDate);

			// Get Certificate Replacements - pass current date for granted_date
			$replacements 	= $this->getReplacementTags($courseId, $userId, $currentDate);
	...
	public function getReplacementTags($courseId, $userId = '', $issuedDate = '')
	{
		...
		if ($courseId)
		{
			...
			$certificate->granted_date = (!empty($issuedDate) &&
					$issuedDate != '0000-00-00 00:00:00')  ? HTMLHelper::date($issuedDate, $dateFormat) : HTMLHelper::date(Factory::getDate()->toSQL(), $dateFormat);
