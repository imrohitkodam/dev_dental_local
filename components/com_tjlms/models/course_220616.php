...
class TjlmsModelcourse extends AdminModel
{
			...
			try
			{
				...
				if (!empty($modules))
				{
					...
					foreach ($lessons as $ind => $lesson)
					{
						...
						$checkFormat = $dispatcher->trigger('additional' . $plg_name . 'FormatCheck', array($lesson->id, $lesson));
						...
						if ($oluser_id)
						{
							...
							$lessonLastaccessedon[] = $lesson->userStatus['lastAccessedOnUTCdate'];
							/*$lesson->userStatus['score'] = Text::_('COM_TJLMS_N_A');
	...
	public function CheckIfUserHasProgress($course_id, $user_id)
	{
		...
		// Add Table Path
		Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjlms/tables');
		$courseTrack = Table::getInstance('Coursetrack', 'TjlmsTable', array('dbo', $db));
	...
	public function addCertEntry($courseId, $userId = '')
	{
		...
		if (!empty($courseData->certificate_id))
		{
			...
			{
				$validateCertificate = $tjCert::validateCertificate($checkIfIssued[0]->unique_certificate_id);

				// If certificate is expired then don't generate certificate
			...
			$replacements 	= $this->getReplacementTags($courseId, $userId);
	...
	public function getcertificateHTML($courseId, $userId = '')
	{
		...
		$certtmpl = Table::getInstance('certificatetemplate', 'TjlmsTable', array('dbo', $db));
	 ...
	 * @since  1.0.0
	 */
	public function getReplacementTags($courseId, $userId = '')
	{
		$lmsparams  = ComponentHelper::getParams('com_tjlms');
		...
		if ($courseId)
		{
			...
			$result->course->title           = $courseDetails->title;
			$result->enroll->studentname     = $user->name;
			$result->enroll->studentusername = $user->username;
			...
			$result->certificate->cert_id = $certificateData[0]->unique_certificate_id;
			...
			if (!empty($courseFieldsData->fields))
			{
				...
				foreach ($courseFieldsValueArray as $key => $value)
				{
					...
					$result->coursefield->$value = $courseFieldsValue;
				}
			}

			$result->certificate->granted_date = HTMLHelper::date(Factory::getDate()->toSQL(), $dateFormat);
			...
			{
				$result->certificate->expiry_date = HTMLHelper::date($certificateExpiryDate, $dateFormat);
			}

			$result->course->total_time = $this->getTotalTimeSpentOnCourse($courseId, $user->id);

			// Trigger to replace tag
			PluginHelper::importPlugin('system');
			$dispatcher = JDispatcher::getInstance();
			$dispatcher->trigger('onBeforeTjlmsTagReplace', array($courseId, $userId, &$result));
	...
	public function getData($pk = null)
	{
		...
		if ($enable_tags == 1)
		{
			$course->course_tags = new JHelperTags;
		...
		if ($userId)
		{
			...
			$jlikeRecommModel = BaseDatabaseModel::getInstance('Recommendations', 'JlikeModel', array('ignore_request' => true));
			...
			$course->assignmentDueDate = $jlikeRecommData[0]->due_date;
		}
	...
	public function enrollmentStatus($courseData)
	{
		...
		else
		{
			...
			if ($courseData->type == 1)
			{
				...
				if (($course->userOrder->id  && !in_array($course->userOrder->status, array("P", "C")) && !$course->userEnrollment->id)
					|| ($course->userEnrollment->id && $course->userEnrollment->expired == 1 && $course->userOrder->status != "P")
					|| !$course->userOrder->id || (!in_array($course->userOrder->status, array("I", "P", "C"))))
				{
					$course->allowBuy    = 1;
			...
			elseif (!$course->userEnrollment->id && $course->canEnroll && !$autoEnroll &&
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->item[$pk]))
		{
			try
			{
				...
				if (empty($course->id))
				{
					return JError::raiseError(404, Text::_('COM_TJLMS_ERROR_CONTENT_NOT_FOUND'));
				}
	 ...
	 * @since  1.0
	 */
	public function getSubscriptionPlans($courseId)
	{
		try
		{
			$db    = Factory::getDBO();
