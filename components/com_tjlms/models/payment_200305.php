 ...
 * @since  1.0.0
 */
class TjlmsModelpayment extends JModelLegacy
{
	/**
	 * Constructor
	 *
	...
	public function getPaymentVars($pg_plugin, $orderid)
	{
		...
		$vars->submiturl     = JRoute::_("index.php?option=com_tjlms&task=payment.confirmpayment&processor={$pg_plugin}");
		$orderUrl = 'index.php?option=com_tjlms&view=orders';
		$vars->return        = JUri::root() . substr(
														JRoute::_($orderUrl . $guest_email . "&orderid=" . $pass_data->order_id . "&Itemid=" . $orderItemid), strlen(JUri::base(true)) + 1
													);
		$cancleUrl = "index.php?option=com_tjlms&view=buy&layout=cancel&processor={$pg_plugin}&Itemid=" . $chkoutItemid;
		$vars->cancel_return = JURI::root() . substr(
														JRoute::_($cancleUrl), strlen(JURI::base(true)) + 1
													);
		...
		$vars->notify_url = JURI::root() . "index.php?option=com_tjlms&task=payment.notify&order_id="
. $pass_data->order_id . $guest_email . "&processor=" . $pg_plugin;
		$vars->url = JURI::root() . $processUrl;
	...
	public function processpayment($post, $pg_plugin, $order_id)
	{
		...
		if ($data)
		{
			...
			{
				$orderUrl = 'index.php?option=com_tjlms&view=orders&orderid=' . ($order_id) . '&processor={$pg_plugin}&Itemid=' . $orderItemid;
				$return_resp['return'] = JURI::root() . substr(JRoute::_($orderUrl, false), strlen(JURI::base(true)) + 1);
			}
			...
			foreach ($lessons as $lesson)
			{
				...
				$dispatcher = JDispatcher::getInstance();
				JPluginHelper::importPlugin($plug_type);

				// Trigger all "invitex" plugins method that renders the button/image
				$dispatcher->trigger('onAfterCourseBuy', array(
												$orderDetails,
												$lesson->params,
												$pg_plugin
											)
		...
		{
			$return_resp['msg'] = JText::_('COM_TJLMS_ORDER_ERROR');
		}
	...
	public function updateOrder($id, $userid, $data, $return_resp)
	{
		...
		if ($data['status'] == 'C' and $return_resp['status'] == 1)
		{
			...
			if ($oldStatus != 'C')
			{
				$comtjlmsHelper->sendInvoiceEmail($id);
			}
		}
		elseif (!empty($data['status']))
		{
	...
	public function updateStatus($data, $id)
	{
		...
		$res                 = new stdClass;
		$res->id             = $id;
		$res->mdate          = $comtjlmsHelper->getDateInUtc(JHtml::date('now', 'Y-m-d H:i:s', true));
		...
		if ($data['status'] == 'C' && $approvalForPaidCourses == 0)
		{
			$tjlmsCoursesHelper = new tjlmsCoursesHelper;
