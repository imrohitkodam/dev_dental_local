...
Log::add('5 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
		...
		if ($lesson_id > 0 && $oluser_id > 0)
		{
			...
			{
				Log::add('4 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
				$object->id = $track->id;
			...
			{
				Log::add('3 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
				$object->id = '';
...
Log::add('1 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
			// TRIGGERS....FOR LESSON AND COURSE COMPLETION.
			if ($lesson_status == 'completed' || $lesson_status == 'passed' || $lesson_status == 'failed')
			{
				Log::add('2 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
				$TjlmsLessonHelper = new TjlmsLessonHelper;
				...
				if ($prev_status != 'completed' && $prev_status != 'passed' && $prev_status != 'failed')
				{
					...
					PluginHelper::importPlugin('system');
				Log::add('21 prev_status: ' . $prev_status . ', User ID ' . $user_id, Log::INFO, 'tjlms_helper');
		...
		try
		{
			if (!empty($actorId))
			{
				...
				if (!$result)
				{
					...
					// Set first_completion_date if course is completed on first entry
					...
					{
						$courseTrack->first_completion_date = '0000-00-00 00:00:00';
					}
				...
				else
				{
					...
					*/
					...
					if ($courseProgress['status'] == 'C')
					{
						// Set first_completion_date only once - when course is completed for the first time
						$nullDate = Factory::getDbo()->getNullDate();
						if (empty($result->first_completion_date) || $result->first_completion_date == $nullDate || $result->first_completion_date == '0000-00-00 00:00:00')
						{
							$courseTrack->first_completion_date = Factory::getDate()->toSQL();
						}
