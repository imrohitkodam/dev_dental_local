...
class TjlmsCoursesHelper
{
	...
	public function getcourseInfo($course_id)
	{
		try
		{
			$db   = JFactory::getDBO();
			$query = $db->getQuery(true);
			$query->select(array('c.*', $db->qn('cat.published'), $db->qn('cat.access', 'catAccess')));
			...
			if ($result && JFactory::getDate()->toSql() < $result->start_date)
			{
				$result->state = 0;
			}
	 ...
	 * @since   1.0
	 */
	public function getCourseSubplans($courseId)
	{
		try
		{
			$db   = JFactory::getDBO();
	...
	public function getCourseProgress($courseId, $oluserId)
	{
		...
		if ($courseId > 0)
		{
			try
			{
				...
				$query->where($db->qn('l.start_date') . " <= '" . JHtml::date('now', 'Y-m-d H:i:s', true) . "' ");
				$db->setQuery($query);
	...
	public function getLessonBycondition($course_id, $condition, $order, $user_id, $columns = array("l.*"))
	{
		...
		{
			$lesson->image = $this->tjlmsLessonHelper->getLessonImage((array) $lesson, 'M_');
		}
	...
	public function enroll_user($admin_approval = 0, $enrolled_by = 0, $post = array(), $csv = 0, $notify_user = 1)
	{
		...
		$data = new stdclass;
		$data->course_id = $c_id;
		$data->enrolled_on_time = JFactory::getDate()->toSql(true);
		...
		if (!empty($post))
		{
			...
			{
				$users          = $post['cid'];
				$data->enrolled_by = JFactory::getUser()->id;
			}
			else
			{
		...
		foreach ($users as $userToEnroll)
		{
			...
			{
				JTable::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjlms/tables');
				$enrolledTable = JTable::getInstance('Enrolledusers', 'TjlmsTable', array('dbo', $db));
	...
	public function assignCourseToUser($post = array())
	{
		...
		foreach ($users as $userToAssign)
		{
			...
			// Trigger all "invitex" plugins method that renders the button/image
			$dispatcher->trigger('onAfterCourseAssign', array(
																$data->assign_to,
																$data->course_id,
																$data->assign_by
															)
	...
	public function getgroupinfo($group_id)
	{
		...
		elseif ($integration == 'easysocial')
		{
			if ( !JFile::exists(JPATH_ROOT . '/administrator/components/com_easysocial/includes/foundry.php') )
			{
				return false;
			}
	...
	public function updateEndTimeForCourse($planId, $enrollmentId = '', $add = 1)
	{
		...
		$res->end_time      = JFactory::getDate($endTime)->toSql(true);
		$res->modified_time = JFactory::getDate()->toSql(true);

		// Insert entry if no enrolllment ID.
		if ($enrollmentId == '')
		{
			$res->enrolled_on_time = JFactory::getDate('now', 'UTC', true);
	 ...
	 *
	 * @param   Object  $course     An optional LIMIT field.
	 *
	...
	public function getCourseImage($course, $imageSize)
	{
		...
		$courseDefaultImg = JUri::root() . '/media/com_tjlms/images/default/course.png';
		...
		if (!empty($course['image']) && $course['storage'] != 'invalid')
		{
			...
			{
				if (!JFile::exists(JPATH_SITE . '/' . $courseImgPath . $imageSize . $course['image']))
				{
	...
	public function addEnrolmentEntry($userid, $course_id, $orderStatus = 'P')
	{
		...
		if ($orderStatus == 'C')
		{
			...
			{
				$status = 0;
				$dispatcher->trigger('addCourseTrackEntry', array(
																$userid,
																$course_id,
															)
								);
	...
	public function addEnrolmentHistory($orderId, $enrollmentId)
	{
		...
		if ($enrollmentId)
		{
			...
			$res->created_date  = JFactory::getDate()->toSql(true);
			...
			if ($enrollmentHistoryCount)
			{
				...
				if (new DateTime($res->created_date) < new DateTime($enrollmentHistory['end_date']))
				{
					...
					$res->start_date = JFactory::getDate($startTime)->toSql(true);
					$endTime  		 = strtotime($res->start_date . ' + ' . $endTime);
					$res->end_date   = JFactory::getDate($endTime)->toSql(true);
				}
				else
				{
	...
	public function deleteEnrolmentHistory($order_id, $enrollment_id)
	{
		...
		$enrollmentHistoryTbl = JTable::getInstance('enrollmentHistory', 'TjlmsTable', array('dbo', $db));
