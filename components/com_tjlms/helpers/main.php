...
class ComtjlmsHelper
{
	...
	public function tagreplace($cer_data)
	{
		...
		if (isset($cer_data['msg_body']))
		{
			...
			// Handle {other.xxx} format tags
			preg_match_all("/\{other\.[^\}]+\}/", $message_body, $otherMatches);
			
			// Debug: Log tag replacement
			if (!empty($otherMatches[0]))
			{
				error_log('=== Tag Replacement START ===');
				error_log('Found other tags: ' . implode(', ', $otherMatches[0]));
				error_log('Cer_data keys: ' . implode(', ', array_keys($cer_data)));
			}
			
			if (!empty($otherMatches[0]))
			{
				foreach ($otherMatches[0] as $otherTag)
				{
					// Extract the key from {other.key}
					$key = str_replace('{other.', '', $otherTag);
					$key = str_replace('}', '', $key);
					
					error_log("Processing tag: {$otherTag}, key: {$key}");
					
					// Try to get value from array first (for object with array notation)
					if (is_object($cer_data) && isset($cer_data->{'other.' . $key}))
					{
						$value = $cer_data->{'other.' . $key};
						error_log("Found in cer_data->{'other.{$key}'}: {$value}");
					}
					elseif (is_array($cer_data) && isset($cer_data['other.' . $key]))
					{
						$value = $cer_data['other.' . $key];
						error_log("Found in cer_data['other.{$key}']: {$value}");
					}
					// Try to get from other object
					elseif (is_object($cer_data) && isset($cer_data->other) && is_object($cer_data->other) && isset($cer_data->other->$key))
					{
						$value = $cer_data->other->$key;
						error_log("Found in cer_data->other->{$key}: {$value}");
					}
					elseif (is_array($cer_data) && isset($cer_data['other']) && is_object($cer_data['other']) && isset($cer_data['other']->$key))
					{
						$value = $cer_data['other']->$key;
						error_log("Found in cer_data['other']->{$key}: {$value}");
					}
					...
					{
						$value = '';
						error_log("NOT FOUND for key: {$key}");
					}
					...
					$message_body = str_replace($otherTag, $value, $message_body);
					error_log("Replaced {$otherTag} with: {$value}");
				}
				error_log('=== Tag Replacement END ===');
			}
		}
