...
class ComtjlmstrackingHelper
{
	 ...
	 * @param   INT  $fromTools flag from tools
	 *
	 * @return  void
	 *
	 * @since  1.0.0
	 */
	public function addCourseTrackEntry($courseId , $actorId = '', $lessonId = 0, $fromTools = '')
	{
		$db = JFactory::getDbo();
		...
		try
		{
			if (!empty($actorId))
			{
				...
				// If certificate generation date from tools.
				$issuedCertData = '';

				if (!empty($fromTools))
				{
					$issuedCertData = HTMLHelper::date($result->timeend, $dateFormat);
				}
				...
				else
				{
					...
					if ($courseData->certificate_term == 2)
					{
						...
						if ($passableLessonsPassed && $lessonObj->consider_marks)
						{
							...
							if ($tjlmsModelcourse->addCertEntry($courseId, $actorId, $issuedCertData))
							{
					...
					*/
					...
					{
						//APSG 220616
						if (!empty($result->timeend) && $result->first_completion_date == Factory::getDbo()->getNullDate())
						{
							$courseTrack->first_completion_date = $result->timeend;
						}
						//ASPG
