...
class jMail_acy extends PHPMailer
{
	...
	public function Send()
	{
		...
		{
			return acymailing_raiseError(500, acymailing_translation('JLIB_MAIL_FUNCTION_DISABLED'));
		}
	...
	public function setSender($from)
	{
		...
		else
		{
			...
			acymailing_raiseError(500, acymailing_translation_sprintf('JLIB_MAIL_INVALID_EMAIL_SENDER', $from));
		}
	...
	{
	    // Can be called from other component with only simple text
	    if (!is_array($replyto)) {
            $replyto0 = JMailHelper::cleanLine($replyto);
            $replyto1 = '';
            if(!empty($name)) {
                $replyto1 = JMailHelper::cleanLine($name);
            }
		...
		$replyTo = null, $replyToName = null)
	{
        ...
        {
            $replyToEmail = acymailing_getCMSConfig('replyto');
            if (!empty($replyToEmail)) {
                $replyToName = acymailing_getCMSConfig('replytoname', '');
                $this->addReplyTo(array($replyToEmail, $replyToName));
            }
        }
