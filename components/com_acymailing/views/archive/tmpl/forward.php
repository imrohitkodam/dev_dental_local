...
<form action="<?php echo acymailing_route('index.php?option=com_acymailing&ctrl=archive'); ?>"
	  onsubmit="return submitacymailingforward('adminForm')"
	  method="post"
	  name="adminForm"
	  id="adminForm" >
		...
		<?php if($this->config->get('forward') == 2 AND acymailing_level(1)){ ?>
			<br /><div id="captcha_forward">
				<?php
					...
					$captchaClass->display('adminForm');
				?>
			</div>
		<?php } ?>
