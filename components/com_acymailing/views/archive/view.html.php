...
';
        ...
        acymailing_addScript(false, ACYMAILING_JS.'acymailing_forward.js?v='.str_replace('.', '', $config->get('version')));
		return $this->view();
	}
	...
	function listing(){
		...
		if(is_object($menu)){
			$menuparams = new acyParameter(ACYMAILING_J40 ? $menu->getParams() : $menu->params);
		}
		...
		if(empty($listid)){
			...
			if(empty($oneList->listid)) return acymailing_raiseError(404, 'Mailing List not found : '.$listid);
		...
		if (!empty($menuparams)) {
            $selectedYear = $menuparams->get('year');
        }
        if(!empty($selectedYear)){
            $filters[] = '(a.senddate > '.strtotime('01 January '.intval($selectedYear).' 00:00:00').' AND a.senddate < '.strtotime('31 December '.$selectedYear.' 23:59:59').')';
        }
		...
		acymailing_addScript(true, $js);
		if(!empty($menuparams)) {
            $metaDescription = $menuparams->get('menu-meta_description', '');
            if(!empty($metaDescription)) acymailing_addMetadata('description', $metaDescription);
            $metaKeywords = $menuparams->get('menu-meta_keywords', '');
            if(!empty($metaKeywords)) acymailing_addMetadata('keywords', $metaKeywords);
		}
	...
	function view(){
		...
		if(empty($mailid)) return acymailing_raiseError(404, 'Newsletter not found');
		...
		if(empty($oneMail->mailid)){
			return acymailing_raiseError(404, 'Newsletter not found : '.$mailid);
		}
