...
class SurveyforceHelper
{
	...
	public function SF_GetQuestData($q_data, $start_id = 0, $survey)
	{
		...
		if (preg_match_all($regex, $return, $matches))
		{
			...
			foreach ($matches[1] as $matchIndex => $match)
			{
				...
				$data = SurveyforceHelper::getfieldvalue($cid, $title);

				// Replace the string
				$return = str_replace($tag, $data, $return);
			}
		}
...
//	$sf_hid_rule = mosGetParam( $_REQUEST, 'sf_hid_rule', array() );
	 ...
	 * @since  1.0.0
	 */
	public function getfieldvalue($cid, $title)
	{
