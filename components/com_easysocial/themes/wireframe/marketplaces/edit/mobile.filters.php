...
<div class="es-mobile-filter" data-es-mobile-filters>
	...
	<div class="es-mobile-filter__bd" data-es-marketplaces-filters>
				...
				<div class="es-mobile-filter-slider__content swiper-container" data-es-swiper-container>
					<div class="swiper-wrapper">
						...
						<?php foreach ($steps as $step) { ?>
							<?php echo $this->html('mobile.filterTab', $step->get('title'), 'javascript:void(0)', $i == 0 ? true : false, array('data-step-nav', 'data-for="' . $step->id . '"', 'data-actions="1"')); ?>
							<?php $i++; ?>
						<?php } ?>

						<?php if ($listing->isDraft()) { ?>
							<?php echo $this->html('mobile.filterTab', JText::_('COM_ES_APPROVAL_HISTORY'), 'javascript:void(0)', false, array('data-step-nav', 'data-for="history"', 'data-actions="1"')); ?>
						<?php } ?>
					</div>
				</div>
			</div>
