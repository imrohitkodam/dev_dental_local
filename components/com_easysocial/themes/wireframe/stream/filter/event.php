		...
		<div class="es-stream-filter-bar__cell">
			<div class="o-media">
				<div class="o-media__image">
					<div>
						<?php echo JText::sprintf('COM_EASYSOCIAL_STREAM_HASHTAG_CURRENTLY_FILTERING', '<a href="' . ESR::events(array('layout' => 'item', 'id' => $cluster->getAlias(), 'tag' => $hashtagAlias)) . '">#' . $hashtag . '</a>'); ?>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="o-btn-group" data-filter-wrapper>
					...
					<ul class="dropdown-menu dropdown-menu-left es-timeline-filter-dropdown">
						...
						<li class="<?php echo $active == 'timeline' || !$active ? ' active' : '';?>" data-filter-item data-type="feeds" data-id="<?php echo $cluster->id;?>">
							<a href="<?php echo ESR::events(array('layout' => 'item', 'id' => $cluster->getAlias(), 'type' => 'timeline')); ?>">
								<span data-filter-text><?php echo JText::_('COM_EASYSOCIAL_EVENTS_SIDEBAR_MOST_RECENT_ITEMS');?></span>
							</a>
							<div class="es-timeline-filter-dropdown__bubble" data-counter>0</div>
						</li>

						<?php if (ES::user()->canPostScheduledStory(SOCIAL_TYPE_EVENT) && ($cluster->isAdmin() || $cluster->isOwner())) { ?>
						<li class="<?php echo $active == 'scheduled' ? ' active' : '';?>" data-filter-item data-type="scheduled" data-id="<?php echo $cluster->id;?>">
							<a href="<?php echo ESR::events(array('layout' => 'item', 'id' => $cluster->getAlias(), 'type' => 'scheduled')); ?>">
