		...
		<div class="es-stream-filter-bar__cell">
			<div class="o-media">
				<div class="o-media__image">
					<div>
						<?php echo JText::sprintf('COM_EASYSOCIAL_STREAM_HASHTAG_CURRENTLY_FILTERING', '<a href="' . ESR::groups(array('layout' => 'item', 'id' => $cluster->getAlias(), 'tag' => $hashtagAlias)) . '">#' . $hashtag . '</a>'); ?>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="o-btn-group" data-filter-wrapper>
					...
					<ul class="dropdown-menu dropdown-menu-left es-timeline-filter-dropdown">
						...
						<li class="<?php echo $active == 'timeline' || !$active ? ' active' : '';?>" data-filter-item data-type="feeds" data-id="<?php echo $cluster->id;?>">
							<a href="<?php echo ESR::groups(array('layout' => 'item', 'id' => $cluster->getAlias(), 'type' => 'timeline')); ?>">
								<span data-filter-text><?php echo JText::_('COM_EASYSOCIAL_GROUP_TIMELINE');?></span>
							</a>
							<div class="es-timeline-filter-dropdown__bubble" data-counter>0</div>
						</li>

						<?php if (ES::user()->canPostScheduledStory(SOCIAL_TYPE_GROUP) && ($cluster->isAdmin() || $cluster->isOwner())) { ?>
						<li class="<?php echo $active == 'scheduled' ? ' active' : '';?>" data-filter-item data-type="scheduled" data-id="<?php echo $cluster->id;?>">
							<a href="<?php echo ESR::groups(array('layout' => 'item', 'id' => $cluster->getAlias(), 'type' => 'scheduled')); ?>">
								<span data-filter-text><?php echo JText::_('COM_ES_STORY_SCHEDULED'); ?></span>
							</a>
						</li>
						<?php } ?>
						...
						<li class="divider"></li>
						<li>
							<span class="es-timeline-filter-dropdown__title"><?php echo JText::_('COM_ES_GROUP_SIDEBAR_MODERATE_POSTS'); ?></span>
						</li>
						<li class="<?php echo $active == 'moderation' ? ' active' : '';?> <?php echo $cluster->getTotalPendingPosts() ? 'has-bubble' : '' ?>" data-filter-item data-type="moderation">
							<a href="<?php echo ESR::groups(array('layout' => 'item', 'id' => $cluster->getAlias(), 'type' => 'moderation'));?>">
								<span data-filter-text><?php echo JText::_('COM_EASYSOCIAL_GROUP_SIDEBAR_PENDING_POSTS'); ?></span>
							</a>
						...
						<li class="divider"></li>
						<li>
							<a href="javascript:void(0);" data-filter-create data-type="<?php echo SOCIAL_TYPE_GROUP;?>">
								<?php echo JText::_('COM_ES_CREATE_NEW_FILTER');?>
							</a>
						</li>
						<?php } ?>
					...
					<?php foreach ($appFilters as $appFilter) { ?>
						<li class="es-stream-filter-dropdown__item">
							<div class="o-checkbox">
								<input id="post-type-<?php echo $appFilter->alias;?>" name="postTypes[]" value="<?php echo $appFilter->alias;?>" autocomplete="off" type="checkbox" data-filter-post-type />
