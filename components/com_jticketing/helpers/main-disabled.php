...
class Jticketingmainhelper
{
	...
	public function getticketHTML($data, $usesession = 0)
	{
		...
		$qr_url                   = "https://chart.apis.google.com/chart?cht=qr&chs=";
		...
		$msg['event_image']       = '<img src="' . $eventpicture . '" width="150px" height="150px" border="0">';
		...
		$msg['ticket_buyer_name'] = htmlspecialchars($data->name, ENT_COMPAT, 'UTF-8');
	...
	public function getticketDetails($eventid = '', $ticketid = '')
	{
		...
		if ($eventdata->booking_start_date == '0000-00-00 00:00:00')
		{
			$result->booking_start_date = $eventdata->created;
		}
		...
		if ($eventdata->booking_end_date == '0000-00-00 00:00:00')
		{
			$result->booking_end_date = $eventdata->enddate;
		}
		...
		if (isset($result->id))
		{
			...
			if (!$collect_attendee_info_checkout and $result->id)
			{
				...
				$db->setQuery($query);
				$attname   = $db->loadObject();
				$buyerDetails->first_name = $attname->firstname;
				$buyerDetails->last_name = $attname->lastname;
		...
		if ($integration == "com_jticketing")
		{
			...
			$result->venueDescription = $location_data->description;
	...
	public function afterordermail($row, $ticketid, $link)
	{
		...
		$qrstring                 = urlencode($qrstring);
		$qr_url                   = "//chart.apis.google.com/chart?cht=qr&chs=";
		$qrimage                  = $qr_url . $qr_code_width . "x" . $qr_code_height . "&chl=" . $qrstring . "&chld=H|0";
		$qr_path_url              = $qrimage;
	...
	public function getTimezoneString($eventid)
	{
		...
		if ($integration == 1 || $integration == 3 || $integration == 4)
		{
	...
	public function getJSheader()
	{
		...
		if (file_exists(JPATH_SITE . '/components/com_community'))
		{
			...
			$xml       = Factory::getXML(JPATH_ROOT . '/administrator/components/com_community/community.xml');
	...
	public function getSalesDataAdmin($creator = '', $eventid = '', $where = '')
	{
		...
		elseif ($integration == 3)
		{
			...
			WHERE a.status =  'C' AND i.eventid=b.ev_id AND i.source='com_jevents'" . $where;
		}
		elseif ($integration == 4)
		{
