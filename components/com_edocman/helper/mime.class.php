...
class EDocman_MIME_Magic 
{
    ...
    */
    public static function &_getMimeExtensionMap()
    {
    ...
    */
    public static function &_getMimeMagicFile()
    {
        static $mime_magic;
    ...
    */
    public static function extToMIME($ext)
    {
        if (empty($ext)) {
            ...
            $ext = strtolower($ext);
            $map = &EDocman_MIME_Magic::_getMimeExtensionMap();
            $pos = 0;
    ...
    */
    public static function filenameToMIME($id, $filename, $unknown = true)
    {
        if($filename == ""){
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
            $query->clear();
            $query->select('document_url')->from('#__edocman_documents')->where("id = '$id'");
            $db->setQuery($query);
        ...
        $type = EDocman_MIME_Magic::extToMIME(substr($filename, $pos));
    ...
    */
    public static function MIMEToExt($type)
    {
        ...
        $key = array_search($type, EDocman_MIME_Magic::_getMimeExtensionMap());
        if ($key === false) {
    ...
    */
    public static function analyzeFile($path)
    {
        ...
        if (Util::extensionExists('fileinfo')) {
            ...
            $mime_magic = &EDocman_MIME_Magic::_getMimeMagicFile();
            if (!($fp = @fopen($path, 'rb'))) {
                return false;
            }
