 ...
 * JLike factory class.
 *
 * This class perform the helpful operation required to JLike package
 *
...
{
	/**
	 * Holds the record of the loaded Jlike classes
	 *
	 * @var    array
	 * @since  __DEPLOY_VERSION__
	 */
	...
	public static function table($name, $config = array())
	{
		...
		$table = Table::getInstance($name, 'JlikeTable', $config);
	...
	public static function model($name, $config = array())
	{
		...
		$model = BaseDatabaseModel::getInstance($name, 'JlikeModel', $config);

		return $model;
	}

	/**
	 * Magic method to create instance of JLike library
	 *
