...
class EasyDiscussViewPost extends EasyDiscussView
{
	...
	public function showVideoDialog()
	{
		...
		$editorUuid = $this->input->get('editorUuid', 0, 'int');
		...
		$theme->set('editorUuid', $editorUuid);
	...
	{
		$id = $this->input->get('id', 0, 'int');
		$postAlias = $this->input->get('alias', '', 'string');

		if (!$id) {
			return $this->ajax->reject(JText::_('COM_EASYDISCUSS_SYSTEM_INVALID_ID'));
		}

		// Ensure that user have enter the alias before branch reply
		if (empty($postAlias)) {
			return $this->ajax->reject(JText::_('COM_ED_BRANCH_EMPTY_ALIAS'));
		}
		...
		$postAlias = ED::badwords()->filter($postAlias);
		$postAlias = ED::getAlias($postAlias, 'post', $parentId);

		//TODO: if the process required long time only can find a unique alias name then we need to do something here
		return $this->ajax->resolve($postAlias);
	}	
}