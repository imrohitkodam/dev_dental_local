...
class EasyDiscussViewPost extends EasyDiscussView
{
	...
	public function display($tpl = null)
	{
		...
		// check if the post is associated with the correct menu item or not.
		...
		if ($post->isCluster()) {
			...
			if (!$easysocial->isClusterAppExists($post->getClusterType())) {
			...
			if ($cluster) {
				...
				$header = ED::easysocial()->renderMiniHeader($post->cluster_id, $post->getClusterType(), false);
			}
		}
	...
	public function updates()
	{
		...
		if ($mode != 'SSE' || !$this->config->get('layout_post_liveupdates') || !$this->config->get('system_single_polling') || !$this->my->id) {
			return;
		}
		...
		while (true) {
			$lu = ED::liveupdates();
			$updates = $lu->updates($ids, $rExclusion, $cExclusion, $from);

			$data = $updates['data'];
			$ids = $updates['ids'];
			$rExclusion = $updates['rExclusion'];
			$cExclusion = $updates['cExclusion'];
