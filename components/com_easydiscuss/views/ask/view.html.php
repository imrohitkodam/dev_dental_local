...
class EasyDiscussViewAsk extends EasyDiscussView
{
	...
	public function display($tpl = null)
	{
		...
		$groupId = $this->input->get('group_id', 0, 'int'); // Legacy support
		...
		if ($post->isCluster() || $clusterId || $groupId) {
			$clusterId = $clusterId ? $clusterId : $post->cluster_id;
			$clusterType = $clusterType ? $clusterType : $post->cluster_type;
			...
			if (!$clusterType) {
				$cluster = ED::easysocial()->loadCluster($clusterId);
				$clusterType = $cluster->getType();
			}

			$header = ED::easysocial()->renderMiniHeader($clusterId, $clusterType, false);
		}
