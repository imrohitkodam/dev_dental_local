	...
	public function add()
	{
		...
		if ($post->user_id != $this->my->id) {
			...
			if ($post->isReply()) {
				...
				if ($typeValue == '1') {
					if ($voted) {
						ED::history()->log('easydiscuss.unvote.reply.reversed', $this->my->id, 'COM_ED_POINTS_HISTORY_DOWNVOTE_REPLY_REVERSED', $post->id, false);
					}

					// Add logging for user.
					ED::history()->log('easydiscuss.vote.reply', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_VOTE_REPLY', $post->id);
					ED::badges()->assign('easydiscuss.vote.reply', $this->my->id);

					// Assign badge for EasySocial
					ED::easysocial()->assignBadge('vote.reply', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_VOTE_REPLY');
					...
					if ($voted) {
						ED::history()->log('easydiscuss.vote.reply.reversed', $this->my->id, 'COM_ED_POINTS_HISTORY_UPVOTE_REPLY_REVERSED', $post->id, false);
					}

					ED::history()->log('easydiscuss.unvote.reply', $this->my->id, 'COM_EASYDISCUSS_BADGES_HISTORY_UNVOTE_REPLY', $post->id);
				...
				if ($typeValue == '1') {
					if ($voted) {
						ED::history()->log('easydiscuss.unvote.question.reversed', $this->my->id, 'COM_ED_POINTS_HISTORY_DOWNVOTE_QUESTION_REVERSED', $post->id, false);
					}

					ED::history()->log('easydiscuss.vote.question', $this->my->id, 'COM_ED_BADGES_HISTORY_UPVOTE_QUESTION', $post->id);
					...
					if ($voted) {
						ED::history()->log('easydiscuss.vote.question.reversed', $this->my->id, 'COM_ED_POINTS_HISTORY_UPVOTE_QUESTION_REVERSED', $post->id, false);
					}

					// Voted -1
					ED::history()->log('easydiscuss.unvote.question', $this->my->id, 'COM_ED_BADGES_HISTORY_DOWNVOTE_QUESTION', $post->id);
