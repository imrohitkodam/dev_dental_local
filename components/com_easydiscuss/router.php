...
class EasyDiscussRouterBase extends EasyDiscuss
{
	...
	public static function build(&$query)
	{
		...
		// Clusters view
		if (isset($query['view']) && $query['view'] == 'clusters') {

			$clusterType = (isset($query['cluster_type']) && $query['cluster_type']) ? $query['cluster_type'] : 'group';
			...
			unset($query['view']);
			unset($query['cluster_type']);

			if (isset($query['cluster_id']) && $query['cluster_id']) {
				$alias = ED::easysocial()->loadCluster($query['cluster_id'], $clusterType)->getAlias();

				$segments[] = $alias;

				unset($query['cluster_id']);
			}
		...
		if (isset($query['view']) && $query['view'] == 'ask') {
			...
			if (isset($query['cluster_id']) && $query['cluster_id']) {
				...
				if (isset($query['cluster_type'])) {
					$clusterType = $query['cluster_type'];
					unset($query['cluster_type']);
				}

				$easysocial = ED::easysocial();
				$cluster = $easysocial->loadCluster($query['cluster_id'], $clusterType);

				if ($cluster) {
					$alias = $cluster->getAlias();
					$segments[] = $cluster->getType();
					$segments[] = $alias;
	...
	public static function parse(&$segments)
	{
		...
		$views = array('attachments', 'categories', 'index', 'forums','conversation',
						...
						'points','dashboard', 'mypost', 'group', 'page', 'event', 'login', 'auth', 'download');
			...
			if (!in_array($segments[0], $views)) {
				...
				if (in_array($testItem, $catAliases) && $xView == 'forums') {
					...
					if (count($segments) == 1) {
						...
						if ($postId === null) {
							// lets try again to see if the testFirstItem has valid post id or not because now
							// the reply sef always has the id. #1300
							$tmpId = (int) $testFirstItem;

							if ($tmpId) {
								$table = ED::table('Post');
								$table->load($tmpId);

								if ($table->id && $table->parent_id) {
									// okay so this is a reply post. let take it
									$postId = $table->id;
								}
							}
						}
		...
		if (isset($segments[0]) && ($segments[0] == 'group' || $segments[0] == 'page' || $segments[0] == 'event')) {
			$vars['view'] = 'clusters';
			$vars['cluster_type'] = $segments[0];

			$easysocial = ED::easysocial();

			$count = count($segments);

			// dump($count, $segments);exit;

			if ($count > 1) {
				if (isset($segments[$count - 1])) {

					$alias = $segments[$count - 1];
					$vars['cluster_id'] = $easysocial->decodeAlias($alias);
					$vars['layout'] = 'listings';
				}
			}
		}
		...
		*/
		...
		$count = count($segments);
		if (!isset($vars['view']) && $count == 1 && in_array($segments[0], $views)) {
			$vars['view'] = $segments[0];
		}
