...
class EasyDiscussControllerPosts extends EasyDiscussController
{
	...
	public function save()
	{
		...
		if (isset($data['cluster_id']) && $data['cluster_id']) {
			...
			$lib = ED::easysocial();
			$cluster = $lib->loadCluster($data['cluster_id'], $data['cluster_type']);

			// If cluster is not exists, we save it normally.
			if (!$cluster) {
				$data['cluster_id'] = 0;
				$data['cluster_type'] = '';
			}
	...
	public function branch()
	{
		...
		// Disallow to proceed this if the user don't fill in the alias field
		if (empty($postAlias)) {
			ED::setMessage(JText::_('COM_ED_BRANCH_EMPTY_ALIAS'), 'error');
			return ED::redirect(EDR::getPostRoute($post->parent_id, false));			
		}

		$state = $post->branch($postAlias);
