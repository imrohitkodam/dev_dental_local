...
class EasyDiscussControllerSubscription extends EasyDiscussController
{
	...
	/**
	 * Unsubscribe all the post subscriptions
	 *
	 * @since	5.0.9
	 * @access	public
	 */
	public function unsubscribeAllPost()
	{
		ED::checktoken();

		$userId = $this->input->get('userId', 0, 'int');
		$redirectLink = EDR::_('view=subscription&filter=post');

		// ensure the current logged-in user to perform their own subscription
		...
		$model = ED::model('Subscribe');
		$state = $model->removeAllPostSubscription($userId);

		ED::setMessage(JText::_('COM_EASYDISCUSS_SUBSCRIPTION_UNSUBSCRIBE_SUCCESS'), 'success');
		ED::redirect($redirectLink);
		return $this->app->close();				
	}
