...
$('[data-ed-toggle=tab]').on('click', function() {
	...
	tabContent.removeClass('is-empty');
...
$(document).on('click.subscription.delete', '[data-ed-unsubscribe-all]', function() {
	var element = $(this);
	var userId = element.data('id');

	EasyDiscuss.dialog({
		content: EasyDiscuss.ajax('site/views/subscription/unsubscribeAllPostDialog', {
			'userId' : userId
		})
	});
});
