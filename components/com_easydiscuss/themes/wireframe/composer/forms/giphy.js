ed.require(['edq', 'easydiscuss'], function($, EasyDiscuss) {
	...
	$(document).on('click', giphyItem.selector, function(){
		const textarea = editor.find('[data-ed-editor]');
		const currentPosition = textarea[0].selectionStart;
		const original = $(this).data('original');
		const value = textarea.val();
		const giphy = '[giphy]' + original + '[/giphy]';

		textarea.val(value.slice(0, currentPosition) + giphy + value.slice(currentPosition));
	});
