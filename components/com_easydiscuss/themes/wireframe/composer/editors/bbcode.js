ed.require(['edq', 'easydiscuss', 'markitup', 'jquery.expanding', 'jquery.atwho', 'site/vendors/prism', 'spectrum'], function($, EasyDiscuss) {
	...
	$.each(wrapper, function(i, item) {
		...
		// Implement colors
		<?php if ($this->config->get('layout_bbcode_color')) { ?>
		...
		<?php if ($this->config->get('main_mentions') && $this->my->id) { ?>
			editor
				.atwho({
					...
					limit: 20,
					searchKey: "title",
					displayTpl: '<li data-hash="${hash}">${avatar} ${title}</li>',

					insertTpl: function(val) {
						return '${atwho-at}${title}';
					},
						...
						remoteFilter: function(query, callback) {
							...
							EasyDiscuss.ajax('site/views/users/search', {
								...
								$.each(result, function(i, item) {
									...
									var avatar = '<img src="' + item.avatar + '" width="16px" height="16px" />';
									item.avatar = avatar;
