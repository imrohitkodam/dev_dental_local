ed.require(['edq', 'spectrum'], function($){
	...
	EasyDiscuss.bbcode = [
			...
			{
				name: "<?php echo JText::_('COM_ED_COLOR'); ?>",
			...
			<?php if ($giphy->isEnabled()) { ?>
			{
				...
				replaceWith: function(h) {
					var composer = $(h.textarea).closest('[data-ed-composer]');
					var button = composer.find('.markitup-giphy');
					var browser = composer.find('[data-giphy-browser]');

					$(document).trigger('initializeGiphy');

					if (browser.hasClass('is-open')) {
						button.removeClass('is-active');
						browser.removeClass('is-open');

						return;
					}

					button.addClass('is-active');
					browser.addClass('is-open');
				},
