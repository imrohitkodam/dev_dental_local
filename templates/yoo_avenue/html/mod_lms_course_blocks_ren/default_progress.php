...
<?php
$certificateTable = JTable::getInstance('Certificate', 'TjlmsTable');
$certificateTable->load(array('user_id' => (int) $mod_data->oluser_id, 'course_id' => (int) $course->id));

// If user is not guest then show enrol or Buy now buttons
if ($mod_data->oluser->guest != 1)
{
	...
	<div class="courseProgress panel panel-default br-0">
		...
		<div class="panel-body progressDiv center text-center p-15">
			...
			<?php if ($course->certificate_term != 0 && (!empty($certificateTable->cert_id) || $mod_data->progress_in_percent == 100)): ?>
					...
					<?php else:?>
					<a rel="{handler: 'iframe', size: {x: 800, y: 600}}" class="tjmodal" href="index.php?option=com_tjlms&view=certificate&tmpl=component&course_id=<?php echo $course->id; ?>&user_id=<?php echo $mod_data->oluser_id; ?>">
